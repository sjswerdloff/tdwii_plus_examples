...
def cmove_specific_input(retrieve_ae_title, dest_ae_title, patient_id,
    ...
    """
    print(f"Move Study {study_uid}, Series {series_uid}, Instance {instance_uid} for Patient ID {patient_id} from {retrieve_ae_title} to {dest_ae_title}")
...
def cmove_inputs(path, local_store_ae_title):
    ...
    for iis_index in range(len(iis)):
        ...
        for ref_instance_index in range(len(ref_instance_seq)):
            ...
            cmove_specific_input(retrieve_ae_title,local_store_ae_title,patient_id,study_uid,series_uid,instance_uid)
