...
from .advanced_blending_sequence_item import AdvancedBlendingSequenceItem
...
from .blending_display_sequence_item import BlendingDisplaySequenceItem
...
class AdvancedBlendingPresentationState:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._AdvancedBlendingSequence: List[AdvancedBlendingSequenceItem] = []
    ...
    @property
    def BlendingDisplaySequence(self) -> Optional[List[BlendingDisplaySequenceItem]]:
        if "BlendingDisplaySequence" in self._dataset:
            if len(self._BlendingDisplaySequence) == len(self._dataset.BlendingDisplaySequence):
                return self._BlendingDisplaySequence
            else:
                return [BlendingDisplaySequenceItem(x) for x in self._dataset.BlendingDisplaySequence]
        return None

    @BlendingDisplaySequence.setter
    def BlendingDisplaySequence(self, value: Optional[List[BlendingDisplaySequenceItem]]):
        if value is None:
            self._BlendingDisplaySequence = []
            if "BlendingDisplaySequence" in self._dataset:
                del self._dataset.BlendingDisplaySequence
        elif not isinstance(value, list) or not all(isinstance(item, BlendingDisplaySequenceItem) for item in value):
            raise ValueError(f"BlendingDisplaySequence must be a list of BlendingDisplaySequenceItem objects")
        else:
            self._BlendingDisplaySequence = value
            if "BlendingDisplaySequence" not in self._dataset:
                self._dataset.BlendingDisplaySequence = pydicom.Sequence()
            self._dataset.BlendingDisplaySequence.clear()
            self._dataset.BlendingDisplaySequence.extend([item.to_dataset() for item in value])

    def add_BlendingDisplay(self, item: BlendingDisplaySequenceItem):
        if not isinstance(item, BlendingDisplaySequenceItem):
            raise ValueError(f"Item must be an instance of BlendingDisplaySequenceItem")
        self._BlendingDisplaySequence.append(item)
        if "BlendingDisplaySequence" not in self._dataset:
            self._dataset.BlendingDisplaySequence = pydicom.Sequence()
        self._dataset.BlendingDisplaySequence.append(item.to_dataset())
    ...
    @property
    def AdvancedBlendingSequence(self) -> Optional[List[AdvancedBlendingSequenceItem]]:
        if "AdvancedBlendingSequence" in self._dataset:
            if len(self._AdvancedBlendingSequence) == len(self._dataset.AdvancedBlendingSequence):
                return self._AdvancedBlendingSequence
            else:
                return [AdvancedBlendingSequenceItem(x) for x in self._dataset.AdvancedBlendingSequence]
        return None

    @AdvancedBlendingSequence.setter
    def AdvancedBlendingSequence(self, value: Optional[List[AdvancedBlendingSequenceItem]]):
        if value is None:
            self._AdvancedBlendingSequence = []
            if "AdvancedBlendingSequence" in self._dataset:
                del self._dataset.AdvancedBlendingSequence
        elif not isinstance(value, list) or not all(isinstance(item, AdvancedBlendingSequenceItem) for item in value):
            raise ValueError(f"AdvancedBlendingSequence must be a list of AdvancedBlendingSequenceItem objects")
        else:
            self._AdvancedBlendingSequence = value
            if "AdvancedBlendingSequence" not in self._dataset:
                self._dataset.AdvancedBlendingSequence = pydicom.Sequence()
            self._dataset.AdvancedBlendingSequence.clear()
            self._dataset.AdvancedBlendingSequence.extend([item.to_dataset() for item in value])

    def add_AdvancedBlending(self, item: AdvancedBlendingSequenceItem):
        if not isinstance(item, AdvancedBlendingSequenceItem):
            raise ValueError(f"Item must be an instance of AdvancedBlendingSequenceItem")
        self._AdvancedBlendingSequence.append(item)
        if "AdvancedBlendingSequence" not in self._dataset:
            self._dataset.AdvancedBlendingSequence = pydicom.Sequence()
        self._dataset.AdvancedBlendingSequence.append(item.to_dataset())
