...
from .anatomic_region_modifier_sequence_item import AnatomicRegionModifierSequenceItem
from .code_sequence_item import CodeSequenceItem


class AnatomicRegionSequenceItem:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        self._dataset = dataset if dataset is not None else pydicom.Dataset()
        self._EquivalentCodeSequence: List[CodeSequenceItem] = []
        self._AnatomicRegionModifierSequence: List[AnatomicRegionModifierSequenceItem] = []
    ...
    @property
    def AnatomicRegionModifierSequence(self) -> Optional[List[AnatomicRegionModifierSequenceItem]]:
        if "AnatomicRegionModifierSequence" in self._dataset:
            if len(self._AnatomicRegionModifierSequence) == len(self._dataset.AnatomicRegionModifierSequence):
                return self._AnatomicRegionModifierSequence
            else:
                return [AnatomicRegionModifierSequenceItem(x) for x in self._dataset.AnatomicRegionModifierSequence]
        return None

    @AnatomicRegionModifierSequence.setter
    def AnatomicRegionModifierSequence(self, value: Optional[List[AnatomicRegionModifierSequenceItem]]):
        if value is None:
            self._AnatomicRegionModifierSequence = []
            if "AnatomicRegionModifierSequence" in self._dataset:
                del self._dataset.AnatomicRegionModifierSequence
        elif not isinstance(value, list) or not all(isinstance(item, AnatomicRegionModifierSequenceItem) for item in value):
            raise ValueError(f"AnatomicRegionModifierSequence must be a list of AnatomicRegionModifierSequenceItem objects")
        else:
            self._AnatomicRegionModifierSequence = value
            if "AnatomicRegionModifierSequence" not in self._dataset:
                self._dataset.AnatomicRegionModifierSequence = pydicom.Sequence()
            self._dataset.AnatomicRegionModifierSequence.clear()
            self._dataset.AnatomicRegionModifierSequence.extend([item.to_dataset() for item in value])

    def add_AnatomicRegionModifier(self, item: AnatomicRegionModifierSequenceItem):
        if not isinstance(item, AnatomicRegionModifierSequenceItem):
            raise ValueError(f"Item must be an instance of AnatomicRegionModifierSequenceItem")
        self._AnatomicRegionModifierSequence.append(item)
        if "AnatomicRegionModifierSequence" not in self._dataset:
            self._dataset.AnatomicRegionModifierSequence = pydicom.Sequence()
        self._dataset.AnatomicRegionModifierSequence.append(item.to_dataset())
