...
class AttributeToleranceValuesSequenceItem:
    ...
    @property
    def ToleranceValue(self) -> Optional[float]:
        if "ToleranceValue" in self._dataset:
            return self._dataset.ToleranceValue
        return None

    @ToleranceValue.setter
    def ToleranceValue(self, value: Optional[float]):
        if value is None:
            if "ToleranceValue" in self._dataset:
                del self._dataset.ToleranceValue
        else:
            self._dataset.ToleranceValue = value
