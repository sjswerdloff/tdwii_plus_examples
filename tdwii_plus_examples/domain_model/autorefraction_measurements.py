...
from .autorefraction_left_eye_sequence_item import AutorefractionLeftEyeSequenceItem
from .autorefraction_right_eye_sequence_item import AutorefractionRightEyeSequenceItem
...
class AutorefractionMeasurements:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._AutorefractionRightEyeSequence: List[AutorefractionRightEyeSequenceItem] = []
        self._AutorefractionLeftEyeSequence: List[AutorefractionLeftEyeSequenceItem] = []
    ...
    @property
    def AutorefractionRightEyeSequence(self) -> Optional[List[AutorefractionRightEyeSequenceItem]]:
        if "AutorefractionRightEyeSequence" in self._dataset:
            if len(self._AutorefractionRightEyeSequence) == len(self._dataset.AutorefractionRightEyeSequence):
                return self._AutorefractionRightEyeSequence
            else:
                return [AutorefractionRightEyeSequenceItem(x) for x in self._dataset.AutorefractionRightEyeSequence]
        return None

    @AutorefractionRightEyeSequence.setter
    def AutorefractionRightEyeSequence(self, value: Optional[List[AutorefractionRightEyeSequenceItem]]):
        if value is None:
            self._AutorefractionRightEyeSequence = []
            if "AutorefractionRightEyeSequence" in self._dataset:
                del self._dataset.AutorefractionRightEyeSequence
        elif not isinstance(value, list) or not all(isinstance(item, AutorefractionRightEyeSequenceItem) for item in value):
            raise ValueError(f"AutorefractionRightEyeSequence must be a list of AutorefractionRightEyeSequenceItem objects")
        else:
            self._AutorefractionRightEyeSequence = value
            if "AutorefractionRightEyeSequence" not in self._dataset:
                self._dataset.AutorefractionRightEyeSequence = pydicom.Sequence()
            self._dataset.AutorefractionRightEyeSequence.clear()
            self._dataset.AutorefractionRightEyeSequence.extend([item.to_dataset() for item in value])

    def add_AutorefractionRightEye(self, item: AutorefractionRightEyeSequenceItem):
        if not isinstance(item, AutorefractionRightEyeSequenceItem):
            raise ValueError(f"Item must be an instance of AutorefractionRightEyeSequenceItem")
        self._AutorefractionRightEyeSequence.append(item)
        if "AutorefractionRightEyeSequence" not in self._dataset:
            self._dataset.AutorefractionRightEyeSequence = pydicom.Sequence()
        self._dataset.AutorefractionRightEyeSequence.append(item.to_dataset())

    @property
    def AutorefractionLeftEyeSequence(self) -> Optional[List[AutorefractionLeftEyeSequenceItem]]:
        if "AutorefractionLeftEyeSequence" in self._dataset:
            if len(self._AutorefractionLeftEyeSequence) == len(self._dataset.AutorefractionLeftEyeSequence):
                return self._AutorefractionLeftEyeSequence
            else:
                return [AutorefractionLeftEyeSequenceItem(x) for x in self._dataset.AutorefractionLeftEyeSequence]
        return None

    @AutorefractionLeftEyeSequence.setter
    def AutorefractionLeftEyeSequence(self, value: Optional[List[AutorefractionLeftEyeSequenceItem]]):
        if value is None:
            self._AutorefractionLeftEyeSequence = []
            if "AutorefractionLeftEyeSequence" in self._dataset:
                del self._dataset.AutorefractionLeftEyeSequence
        elif not isinstance(value, list) or not all(isinstance(item, AutorefractionLeftEyeSequenceItem) for item in value):
            raise ValueError(f"AutorefractionLeftEyeSequence must be a list of AutorefractionLeftEyeSequenceItem objects")
        else:
            self._AutorefractionLeftEyeSequence = value
            if "AutorefractionLeftEyeSequence" not in self._dataset:
                self._dataset.AutorefractionLeftEyeSequence = pydicom.Sequence()
            self._dataset.AutorefractionLeftEyeSequence.clear()
            self._dataset.AutorefractionLeftEyeSequence.extend([item.to_dataset() for item in value])

    def add_AutorefractionLeftEye(self, item: AutorefractionLeftEyeSequenceItem):
        if not isinstance(item, AutorefractionLeftEyeSequenceItem):
            raise ValueError(f"Item must be an instance of AutorefractionLeftEyeSequenceItem")
        self._AutorefractionLeftEyeSequence.append(item)
        if "AutorefractionLeftEyeSequence" not in self._dataset:
            self._dataset.AutorefractionLeftEyeSequence = pydicom.Sequence()
        self._dataset.AutorefractionLeftEyeSequence.append(item.to_dataset())
