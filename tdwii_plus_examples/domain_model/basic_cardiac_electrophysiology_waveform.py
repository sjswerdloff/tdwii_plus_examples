...
from .waveform_annotation_sequence_item import WaveformAnnotationSequenceItem
from .waveform_presentation_group_sequence_item import (
    WaveformPresentationGroupSequenceItem,
)
from .waveform_sequence_item import WaveformSequenceItem


class BasicCardiacElectrophysiologyWaveform:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._WaveformPresentationGroupSequence: List[WaveformPresentationGroupSequenceItem] = []
        self._WaveformSequence: List[WaveformSequenceItem] = []
        ...
        self._WaveformAnnotationSequence: List[WaveformAnnotationSequenceItem] = []
    ...
    @property
    def WaveformDataDisplayScale(self) -> Optional[float]:
        if "WaveformDataDisplayScale" in self._dataset:
            return self._dataset.WaveformDataDisplayScale
        return None

    @WaveformDataDisplayScale.setter
    def WaveformDataDisplayScale(self, value: Optional[float]):
        if value is None:
            if "WaveformDataDisplayScale" in self._dataset:
                del self._dataset.WaveformDataDisplayScale
        else:
            self._dataset.WaveformDataDisplayScale = value

    @property
    def WaveformDisplayBackgroundCIELabValue(self) -> Optional[List[int]]:
        if "WaveformDisplayBackgroundCIELabValue" in self._dataset:
            return self._dataset.WaveformDisplayBackgroundCIELabValue
        return None

    @WaveformDisplayBackgroundCIELabValue.setter
    def WaveformDisplayBackgroundCIELabValue(self, value: Optional[List[int]]):
        if value is None:
            if "WaveformDisplayBackgroundCIELabValue" in self._dataset:
                del self._dataset.WaveformDisplayBackgroundCIELabValue
        else:
            self._dataset.WaveformDisplayBackgroundCIELabValue = value

    @property
    def WaveformPresentationGroupSequence(self) -> Optional[List[WaveformPresentationGroupSequenceItem]]:
        if "WaveformPresentationGroupSequence" in self._dataset:
            if len(self._WaveformPresentationGroupSequence) == len(self._dataset.WaveformPresentationGroupSequence):
                return self._WaveformPresentationGroupSequence
            else:
                return [WaveformPresentationGroupSequenceItem(x) for x in self._dataset.WaveformPresentationGroupSequence]
        return None

    @WaveformPresentationGroupSequence.setter
    def WaveformPresentationGroupSequence(self, value: Optional[List[WaveformPresentationGroupSequenceItem]]):
        if value is None:
            self._WaveformPresentationGroupSequence = []
            if "WaveformPresentationGroupSequence" in self._dataset:
                del self._dataset.WaveformPresentationGroupSequence
        elif not isinstance(value, list) or not all(isinstance(item, WaveformPresentationGroupSequenceItem) for item in value):
            raise ValueError(
                f"WaveformPresentationGroupSequence must be a list of WaveformPresentationGroupSequenceItem objects"
            )
        else:
            self._WaveformPresentationGroupSequence = value
            if "WaveformPresentationGroupSequence" not in self._dataset:
                self._dataset.WaveformPresentationGroupSequence = pydicom.Sequence()
            self._dataset.WaveformPresentationGroupSequence.clear()
            self._dataset.WaveformPresentationGroupSequence.extend([item.to_dataset() for item in value])

    def add_WaveformPresentationGroup(self, item: WaveformPresentationGroupSequenceItem):
        if not isinstance(item, WaveformPresentationGroupSequenceItem):
            raise ValueError(f"Item must be an instance of WaveformPresentationGroupSequenceItem")
        self._WaveformPresentationGroupSequence.append(item)
        if "WaveformPresentationGroupSequence" not in self._dataset:
            self._dataset.WaveformPresentationGroupSequence = pydicom.Sequence()
        self._dataset.WaveformPresentationGroupSequence.append(item.to_dataset())

    @property
    def WaveformSequence(self) -> Optional[List[WaveformSequenceItem]]:
        if "WaveformSequence" in self._dataset:
            if len(self._WaveformSequence) == len(self._dataset.WaveformSequence):
                return self._WaveformSequence
            else:
                return [WaveformSequenceItem(x) for x in self._dataset.WaveformSequence]
        return None

    @WaveformSequence.setter
    def WaveformSequence(self, value: Optional[List[WaveformSequenceItem]]):
        if value is None:
            self._WaveformSequence = []
            if "WaveformSequence" in self._dataset:
                del self._dataset.WaveformSequence
        elif not isinstance(value, list) or not all(isinstance(item, WaveformSequenceItem) for item in value):
            raise ValueError(f"WaveformSequence must be a list of WaveformSequenceItem objects")
        else:
            self._WaveformSequence = value
            if "WaveformSequence" not in self._dataset:
                self._dataset.WaveformSequence = pydicom.Sequence()
            self._dataset.WaveformSequence.clear()
            self._dataset.WaveformSequence.extend([item.to_dataset() for item in value])

    def add_Waveform(self, item: WaveformSequenceItem):
        if not isinstance(item, WaveformSequenceItem):
            raise ValueError(f"Item must be an instance of WaveformSequenceItem")
        self._WaveformSequence.append(item)
        if "WaveformSequence" not in self._dataset:
            self._dataset.WaveformSequence = pydicom.Sequence()
        self._dataset.WaveformSequence.append(item.to_dataset())
    ...
    @property
    def WaveformAnnotationSequence(self) -> Optional[List[WaveformAnnotationSequenceItem]]:
        if "WaveformAnnotationSequence" in self._dataset:
            if len(self._WaveformAnnotationSequence) == len(self._dataset.WaveformAnnotationSequence):
                return self._WaveformAnnotationSequence
            else:
                return [WaveformAnnotationSequenceItem(x) for x in self._dataset.WaveformAnnotationSequence]
        return None

    @WaveformAnnotationSequence.setter
    def WaveformAnnotationSequence(self, value: Optional[List[WaveformAnnotationSequenceItem]]):
        if value is None:
            self._WaveformAnnotationSequence = []
            if "WaveformAnnotationSequence" in self._dataset:
                del self._dataset.WaveformAnnotationSequence
        elif not isinstance(value, list) or not all(isinstance(item, WaveformAnnotationSequenceItem) for item in value):
            raise ValueError(f"WaveformAnnotationSequence must be a list of WaveformAnnotationSequenceItem objects")
        else:
            self._WaveformAnnotationSequence = value
            if "WaveformAnnotationSequence" not in self._dataset:
                self._dataset.WaveformAnnotationSequence = pydicom.Sequence()
            self._dataset.WaveformAnnotationSequence.clear()
            self._dataset.WaveformAnnotationSequence.extend([item.to_dataset() for item in value])

    def add_WaveformAnnotation(self, item: WaveformAnnotationSequenceItem):
        if not isinstance(item, WaveformAnnotationSequenceItem):
            raise ValueError(f"Item must be an instance of WaveformAnnotationSequenceItem")
        self._WaveformAnnotationSequence.append(item)
        if "WaveformAnnotationSequence" not in self._dataset:
            self._dataset.WaveformAnnotationSequence = pydicom.Sequence()
        self._dataset.WaveformAnnotationSequence.append(item.to_dataset())
