...
from .image_box_synchronization_sequence_item import ImageBoxSynchronizationSequenceItem
...
from .nominal_screen_definition_sequence_item import NominalScreenDefinitionSequenceItem
...
from .structured_display_image_box_sequence_item import (
    StructuredDisplayImageBoxSequenceItem,
)
from .structured_display_text_box_sequence_item import (
...
class BasicStructuredDisplay:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._NominalScreenDefinitionSequence: List[NominalScreenDefinitionSequenceItem] = []
        ...
        self._StructuredDisplayTextBoxSequence: List[StructuredDisplayTextBoxSequenceItem] = []
        ...
        self._StructuredDisplayImageBoxSequence: List[StructuredDisplayImageBoxSequenceItem] = []
        self._ImageBoxSynchronizationSequence: List[ImageBoxSynchronizationSequenceItem] = []
    ...
    @property
    def HangingProtocolName(self) -> Optional[str]:
        if "HangingProtocolName" in self._dataset:
            return self._dataset.HangingProtocolName
        return None

    @HangingProtocolName.setter
    def HangingProtocolName(self, value: Optional[str]):
        if value is None:
            if "HangingProtocolName" in self._dataset:
                del self._dataset.HangingProtocolName
        else:
            self._dataset.HangingProtocolName = value

    @property
    def HangingProtocolCreator(self) -> Optional[str]:
        if "HangingProtocolCreator" in self._dataset:
            return self._dataset.HangingProtocolCreator
        return None

    @HangingProtocolCreator.setter
    def HangingProtocolCreator(self, value: Optional[str]):
        if value is None:
            if "HangingProtocolCreator" in self._dataset:
                del self._dataset.HangingProtocolCreator
        else:
            self._dataset.HangingProtocolCreator = value

    @property
    def NumberOfScreens(self) -> Optional[int]:
        if "NumberOfScreens" in self._dataset:
            return self._dataset.NumberOfScreens
        return None

    @NumberOfScreens.setter
    def NumberOfScreens(self, value: Optional[int]):
        if value is None:
            if "NumberOfScreens" in self._dataset:
                del self._dataset.NumberOfScreens
        else:
            self._dataset.NumberOfScreens = value

    @property
    def NominalScreenDefinitionSequence(self) -> Optional[List[NominalScreenDefinitionSequenceItem]]:
        if "NominalScreenDefinitionSequence" in self._dataset:
            if len(self._NominalScreenDefinitionSequence) == len(self._dataset.NominalScreenDefinitionSequence):
                return self._NominalScreenDefinitionSequence
            else:
                return [NominalScreenDefinitionSequenceItem(x) for x in self._dataset.NominalScreenDefinitionSequence]
        return None

    @NominalScreenDefinitionSequence.setter
    def NominalScreenDefinitionSequence(self, value: Optional[List[NominalScreenDefinitionSequenceItem]]):
        if value is None:
            self._NominalScreenDefinitionSequence = []
            if "NominalScreenDefinitionSequence" in self._dataset:
                del self._dataset.NominalScreenDefinitionSequence
        elif not isinstance(value, list) or not all(isinstance(item, NominalScreenDefinitionSequenceItem) for item in value):
            raise ValueError(f"NominalScreenDefinitionSequence must be a list of NominalScreenDefinitionSequenceItem objects")
        else:
            self._NominalScreenDefinitionSequence = value
            if "NominalScreenDefinitionSequence" not in self._dataset:
                self._dataset.NominalScreenDefinitionSequence = pydicom.Sequence()
            self._dataset.NominalScreenDefinitionSequence.clear()
            self._dataset.NominalScreenDefinitionSequence.extend([item.to_dataset() for item in value])

    def add_NominalScreenDefinition(self, item: NominalScreenDefinitionSequenceItem):
        if not isinstance(item, NominalScreenDefinitionSequenceItem):
            raise ValueError(f"Item must be an instance of NominalScreenDefinitionSequenceItem")
        self._NominalScreenDefinitionSequence.append(item)
        if "NominalScreenDefinitionSequence" not in self._dataset:
            self._dataset.NominalScreenDefinitionSequence = pydicom.Sequence()
        self._dataset.NominalScreenDefinitionSequence.append(item.to_dataset())

    @property
    def StructuredDisplayBackgroundCIELabValue(self) -> Optional[List[int]]:
        if "StructuredDisplayBackgroundCIELabValue" in self._dataset:
            return self._dataset.StructuredDisplayBackgroundCIELabValue
        return None

    @StructuredDisplayBackgroundCIELabValue.setter
    def StructuredDisplayBackgroundCIELabValue(self, value: Optional[List[int]]):
        if value is None:
            if "StructuredDisplayBackgroundCIELabValue" in self._dataset:
                del self._dataset.StructuredDisplayBackgroundCIELabValue
        else:
            self._dataset.StructuredDisplayBackgroundCIELabValue = value

    @property
    def EmptyImageBoxCIELabValue(self) -> Optional[List[int]]:
        if "EmptyImageBoxCIELabValue" in self._dataset:
            return self._dataset.EmptyImageBoxCIELabValue
        return None

    @EmptyImageBoxCIELabValue.setter
    def EmptyImageBoxCIELabValue(self, value: Optional[List[int]]):
        if value is None:
            if "EmptyImageBoxCIELabValue" in self._dataset:
                del self._dataset.EmptyImageBoxCIELabValue
        else:
            self._dataset.EmptyImageBoxCIELabValue = value
    ...
    @property
    def StructuredDisplayTextBoxSequence(self) -> Optional[List[StructuredDisplayTextBoxSequenceItem]]:
        if "StructuredDisplayTextBoxSequence" in self._dataset:
            if len(self._StructuredDisplayTextBoxSequence) == len(self._dataset.StructuredDisplayTextBoxSequence):
                return self._StructuredDisplayTextBoxSequence
            else:
                return [StructuredDisplayTextBoxSequenceItem(x) for x in self._dataset.StructuredDisplayTextBoxSequence]
        return None

    @StructuredDisplayTextBoxSequence.setter
    def StructuredDisplayTextBoxSequence(self, value: Optional[List[StructuredDisplayTextBoxSequenceItem]]):
        if value is None:
            self._StructuredDisplayTextBoxSequence = []
            if "StructuredDisplayTextBoxSequence" in self._dataset:
                del self._dataset.StructuredDisplayTextBoxSequence
        elif not isinstance(value, list) or not all(isinstance(item, StructuredDisplayTextBoxSequenceItem) for item in value):
            raise ValueError(
                f"StructuredDisplayTextBoxSequence must be a list of StructuredDisplayTextBoxSequenceItem objects"
            )
        else:
            self._StructuredDisplayTextBoxSequence = value
            if "StructuredDisplayTextBoxSequence" not in self._dataset:
                self._dataset.StructuredDisplayTextBoxSequence = pydicom.Sequence()
            self._dataset.StructuredDisplayTextBoxSequence.clear()
            self._dataset.StructuredDisplayTextBoxSequence.extend([item.to_dataset() for item in value])

    def add_StructuredDisplayTextBox(self, item: StructuredDisplayTextBoxSequenceItem):
        if not isinstance(item, StructuredDisplayTextBoxSequenceItem):
            raise ValueError(f"Item must be an instance of StructuredDisplayTextBoxSequenceItem")
        self._StructuredDisplayTextBoxSequence.append(item)
        if "StructuredDisplayTextBoxSequence" not in self._dataset:
            self._dataset.StructuredDisplayTextBoxSequence = pydicom.Sequence()
        self._dataset.StructuredDisplayTextBoxSequence.append(item.to_dataset())
    ...
    @property
    def StructuredDisplayImageBoxSequence(self) -> Optional[List[StructuredDisplayImageBoxSequenceItem]]:
        if "StructuredDisplayImageBoxSequence" in self._dataset:
            if len(self._StructuredDisplayImageBoxSequence) == len(self._dataset.StructuredDisplayImageBoxSequence):
                return self._StructuredDisplayImageBoxSequence
            else:
                return [StructuredDisplayImageBoxSequenceItem(x) for x in self._dataset.StructuredDisplayImageBoxSequence]
        return None

    @StructuredDisplayImageBoxSequence.setter
    def StructuredDisplayImageBoxSequence(self, value: Optional[List[StructuredDisplayImageBoxSequenceItem]]):
        if value is None:
            self._StructuredDisplayImageBoxSequence = []
            if "StructuredDisplayImageBoxSequence" in self._dataset:
                del self._dataset.StructuredDisplayImageBoxSequence
        elif not isinstance(value, list) or not all(isinstance(item, StructuredDisplayImageBoxSequenceItem) for item in value):
            raise ValueError(
                f"StructuredDisplayImageBoxSequence must be a list of StructuredDisplayImageBoxSequenceItem objects"
            )
        else:
            self._StructuredDisplayImageBoxSequence = value
            if "StructuredDisplayImageBoxSequence" not in self._dataset:
                self._dataset.StructuredDisplayImageBoxSequence = pydicom.Sequence()
            self._dataset.StructuredDisplayImageBoxSequence.clear()
            self._dataset.StructuredDisplayImageBoxSequence.extend([item.to_dataset() for item in value])

    def add_StructuredDisplayImageBox(self, item: StructuredDisplayImageBoxSequenceItem):
        if not isinstance(item, StructuredDisplayImageBoxSequenceItem):
            raise ValueError(f"Item must be an instance of StructuredDisplayImageBoxSequenceItem")
        self._StructuredDisplayImageBoxSequence.append(item)
        if "StructuredDisplayImageBoxSequence" not in self._dataset:
            self._dataset.StructuredDisplayImageBoxSequence = pydicom.Sequence()
        self._dataset.StructuredDisplayImageBoxSequence.append(item.to_dataset())

    @property
    def ImageBoxSynchronizationSequence(self) -> Optional[List[ImageBoxSynchronizationSequenceItem]]:
        if "ImageBoxSynchronizationSequence" in self._dataset:
            if len(self._ImageBoxSynchronizationSequence) == len(self._dataset.ImageBoxSynchronizationSequence):
                return self._ImageBoxSynchronizationSequence
            else:
                return [ImageBoxSynchronizationSequenceItem(x) for x in self._dataset.ImageBoxSynchronizationSequence]
        return None

    @ImageBoxSynchronizationSequence.setter
    def ImageBoxSynchronizationSequence(self, value: Optional[List[ImageBoxSynchronizationSequenceItem]]):
        if value is None:
            self._ImageBoxSynchronizationSequence = []
            if "ImageBoxSynchronizationSequence" in self._dataset:
                del self._dataset.ImageBoxSynchronizationSequence
        elif not isinstance(value, list) or not all(isinstance(item, ImageBoxSynchronizationSequenceItem) for item in value):
            raise ValueError(f"ImageBoxSynchronizationSequence must be a list of ImageBoxSynchronizationSequenceItem objects")
        else:
            self._ImageBoxSynchronizationSequence = value
            if "ImageBoxSynchronizationSequence" not in self._dataset:
                self._dataset.ImageBoxSynchronizationSequence = pydicom.Sequence()
            self._dataset.ImageBoxSynchronizationSequence.clear()
            self._dataset.ImageBoxSynchronizationSequence.extend([item.to_dataset() for item in value])

    def add_ImageBoxSynchronization(self, item: ImageBoxSynchronizationSequenceItem):
        if not isinstance(item, ImageBoxSynchronizationSequenceItem):
            raise ValueError(f"Item must be an instance of ImageBoxSynchronizationSequenceItem")
        self._ImageBoxSynchronizationSequence.append(item)
        if "ImageBoxSynchronizationSequence" not in self._dataset:
            self._dataset.ImageBoxSynchronizationSequence = pydicom.Sequence()
        self._dataset.ImageBoxSynchronizationSequence.append(item.to_dataset())
