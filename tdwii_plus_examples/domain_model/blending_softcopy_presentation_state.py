...
from .blending_sequence_item import BlendingSequenceItem
...
class BlendingSoftcopyPresentationState:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._BlendingSequence: List[BlendingSequenceItem] = []
    ...
    @property
    def BlendingSequence(self) -> Optional[List[BlendingSequenceItem]]:
        if "BlendingSequence" in self._dataset:
            if len(self._BlendingSequence) == len(self._dataset.BlendingSequence):
                return self._BlendingSequence
            else:
                return [BlendingSequenceItem(x) for x in self._dataset.BlendingSequence]
        return None

    @BlendingSequence.setter
    def BlendingSequence(self, value: Optional[List[BlendingSequenceItem]]):
        if value is None:
            self._BlendingSequence = []
            if "BlendingSequence" in self._dataset:
                del self._dataset.BlendingSequence
        elif not isinstance(value, list) or not all(isinstance(item, BlendingSequenceItem) for item in value):
            raise ValueError(f"BlendingSequence must be a list of BlendingSequenceItem objects")
        else:
            self._BlendingSequence = value
            if "BlendingSequence" not in self._dataset:
                self._dataset.BlendingSequence = pydicom.Sequence()
            self._dataset.BlendingSequence.clear()
            self._dataset.BlendingSequence.extend([item.to_dataset() for item in value])

    def add_Blending(self, item: BlendingSequenceItem):
        if not isinstance(item, BlendingSequenceItem):
            raise ValueError(f"Item must be an instance of BlendingSequenceItem")
        self._BlendingSequence.append(item)
        if "BlendingSequence" not in self._dataset:
            self._dataset.BlendingSequence = pydicom.Sequence()
        self._dataset.BlendingSequence.append(item.to_dataset())
