...
from .contributing_sources_sequence_item import ContributingSourcesSequenceItem
...
from .source_irradiation_event_sequence_item import SourceIrradiationEventSequenceItem
...
from .x_ray3_d_acquisition_sequence_item import XRay3DAcquisitionSequenceItem
from .x_ray3_d_reconstruction_sequence_item import XRay3DReconstructionSequenceItem


class BreastTomosynthesisImage:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._XRay3DReconstructionSequence: List[XRay3DReconstructionSequenceItem] = []
        ...
        self._ContributingSourcesSequence: List[ContributingSourcesSequenceItem] = []
        self._XRay3DAcquisitionSequence: List[XRay3DAcquisitionSequenceItem] = []
        ...
        self._SourceIrradiationEventSequence: List[SourceIrradiationEventSequenceItem] = []
    ...
    @property
    def XRay3DReconstructionSequence(self) -> Optional[List[XRay3DReconstructionSequenceItem]]:
        if "XRay3DReconstructionSequence" in self._dataset:
            if len(self._XRay3DReconstructionSequence) == len(self._dataset.XRay3DReconstructionSequence):
                return self._XRay3DReconstructionSequence
            else:
                return [XRay3DReconstructionSequenceItem(x) for x in self._dataset.XRay3DReconstructionSequence]
        return None

    @XRay3DReconstructionSequence.setter
    def XRay3DReconstructionSequence(self, value: Optional[List[XRay3DReconstructionSequenceItem]]):
        if value is None:
            self._XRay3DReconstructionSequence = []
            if "XRay3DReconstructionSequence" in self._dataset:
                del self._dataset.XRay3DReconstructionSequence
        elif not isinstance(value, list) or not all(isinstance(item, XRay3DReconstructionSequenceItem) for item in value):
            raise ValueError(f"XRay3DReconstructionSequence must be a list of XRay3DReconstructionSequenceItem objects")
        else:
            self._XRay3DReconstructionSequence = value
            if "XRay3DReconstructionSequence" not in self._dataset:
                self._dataset.XRay3DReconstructionSequence = pydicom.Sequence()
            self._dataset.XRay3DReconstructionSequence.clear()
            self._dataset.XRay3DReconstructionSequence.extend([item.to_dataset() for item in value])

    def add_XRay3DReconstruction(self, item: XRay3DReconstructionSequenceItem):
        if not isinstance(item, XRay3DReconstructionSequenceItem):
            raise ValueError(f"Item must be an instance of XRay3DReconstructionSequenceItem")
        self._XRay3DReconstructionSequence.append(item)
        if "XRay3DReconstructionSequence" not in self._dataset:
            self._dataset.XRay3DReconstructionSequence = pydicom.Sequence()
        self._dataset.XRay3DReconstructionSequence.append(item.to_dataset())
    ...
    @property
    def EquipmentCoordinateSystemIdentification(self) -> Optional[str]:
        if "EquipmentCoordinateSystemIdentification" in self._dataset:
            return self._dataset.EquipmentCoordinateSystemIdentification
        return None

    @EquipmentCoordinateSystemIdentification.setter
    def EquipmentCoordinateSystemIdentification(self, value: Optional[str]):
        if value is None:
            if "EquipmentCoordinateSystemIdentification" in self._dataset:
                del self._dataset.EquipmentCoordinateSystemIdentification
        else:
            self._dataset.EquipmentCoordinateSystemIdentification = value
    ...
    @property
    def ContributingSourcesSequence(self) -> Optional[List[ContributingSourcesSequenceItem]]:
        if "ContributingSourcesSequence" in self._dataset:
            if len(self._ContributingSourcesSequence) == len(self._dataset.ContributingSourcesSequence):
                return self._ContributingSourcesSequence
            else:
                return [ContributingSourcesSequenceItem(x) for x in self._dataset.ContributingSourcesSequence]
        return None

    @ContributingSourcesSequence.setter
    def ContributingSourcesSequence(self, value: Optional[List[ContributingSourcesSequenceItem]]):
        if value is None:
            self._ContributingSourcesSequence = []
            if "ContributingSourcesSequence" in self._dataset:
                del self._dataset.ContributingSourcesSequence
        elif not isinstance(value, list) or not all(isinstance(item, ContributingSourcesSequenceItem) for item in value):
            raise ValueError(f"ContributingSourcesSequence must be a list of ContributingSourcesSequenceItem objects")
        else:
            self._ContributingSourcesSequence = value
            if "ContributingSourcesSequence" not in self._dataset:
                self._dataset.ContributingSourcesSequence = pydicom.Sequence()
            self._dataset.ContributingSourcesSequence.clear()
            self._dataset.ContributingSourcesSequence.extend([item.to_dataset() for item in value])

    def add_ContributingSources(self, item: ContributingSourcesSequenceItem):
        if not isinstance(item, ContributingSourcesSequenceItem):
            raise ValueError(f"Item must be an instance of ContributingSourcesSequenceItem")
        self._ContributingSourcesSequence.append(item)
        if "ContributingSourcesSequence" not in self._dataset:
            self._dataset.ContributingSourcesSequence = pydicom.Sequence()
        self._dataset.ContributingSourcesSequence.append(item.to_dataset())

    @property
    def XRay3DAcquisitionSequence(self) -> Optional[List[XRay3DAcquisitionSequenceItem]]:
        if "XRay3DAcquisitionSequence" in self._dataset:
            if len(self._XRay3DAcquisitionSequence) == len(self._dataset.XRay3DAcquisitionSequence):
                return self._XRay3DAcquisitionSequence
            else:
                return [XRay3DAcquisitionSequenceItem(x) for x in self._dataset.XRay3DAcquisitionSequence]
        return None

    @XRay3DAcquisitionSequence.setter
    def XRay3DAcquisitionSequence(self, value: Optional[List[XRay3DAcquisitionSequenceItem]]):
        if value is None:
            self._XRay3DAcquisitionSequence = []
            if "XRay3DAcquisitionSequence" in self._dataset:
                del self._dataset.XRay3DAcquisitionSequence
        elif not isinstance(value, list) or not all(isinstance(item, XRay3DAcquisitionSequenceItem) for item in value):
            raise ValueError(f"XRay3DAcquisitionSequence must be a list of XRay3DAcquisitionSequenceItem objects")
        else:
            self._XRay3DAcquisitionSequence = value
            if "XRay3DAcquisitionSequence" not in self._dataset:
                self._dataset.XRay3DAcquisitionSequence = pydicom.Sequence()
            self._dataset.XRay3DAcquisitionSequence.clear()
            self._dataset.XRay3DAcquisitionSequence.extend([item.to_dataset() for item in value])

    def add_XRay3DAcquisition(self, item: XRay3DAcquisitionSequenceItem):
        if not isinstance(item, XRay3DAcquisitionSequenceItem):
            raise ValueError(f"Item must be an instance of XRay3DAcquisitionSequenceItem")
        self._XRay3DAcquisitionSequence.append(item)
        if "XRay3DAcquisitionSequence" not in self._dataset:
            self._dataset.XRay3DAcquisitionSequence = pydicom.Sequence()
        self._dataset.XRay3DAcquisitionSequence.append(item.to_dataset())
    ...
    @property
    def SourceIrradiationEventSequence(self) -> Optional[List[SourceIrradiationEventSequenceItem]]:
        if "SourceIrradiationEventSequence" in self._dataset:
            if len(self._SourceIrradiationEventSequence) == len(self._dataset.SourceIrradiationEventSequence):
                return self._SourceIrradiationEventSequence
            else:
                return [SourceIrradiationEventSequenceItem(x) for x in self._dataset.SourceIrradiationEventSequence]
        return None

    @SourceIrradiationEventSequence.setter
    def SourceIrradiationEventSequence(self, value: Optional[List[SourceIrradiationEventSequenceItem]]):
        if value is None:
            self._SourceIrradiationEventSequence = []
            if "SourceIrradiationEventSequence" in self._dataset:
                del self._dataset.SourceIrradiationEventSequence
        elif not isinstance(value, list) or not all(isinstance(item, SourceIrradiationEventSequenceItem) for item in value):
            raise ValueError(f"SourceIrradiationEventSequence must be a list of SourceIrradiationEventSequenceItem objects")
        else:
            self._SourceIrradiationEventSequence = value
            if "SourceIrradiationEventSequence" not in self._dataset:
                self._dataset.SourceIrradiationEventSequence = pydicom.Sequence()
            self._dataset.SourceIrradiationEventSequence.clear()
            self._dataset.SourceIrradiationEventSequence.extend([item.to_dataset() for item in value])

    def add_SourceIrradiationEvent(self, item: SourceIrradiationEventSequenceItem):
        if not isinstance(item, SourceIrradiationEventSequenceItem):
            raise ValueError(f"Item must be an instance of SourceIrradiationEventSequenceItem")
        self._SourceIrradiationEventSequence.append(item)
        if "SourceIrradiationEventSequence" not in self._dataset:
            self._dataset.SourceIrradiationEventSequence = pydicom.Sequence()
        self._dataset.SourceIrradiationEventSequence.append(item.to_dataset())
    ...
    @property
    def BreastImplantPresent(self) -> Optional[str]:
        if "BreastImplantPresent" in self._dataset:
            return self._dataset.BreastImplantPresent
        return None

    @BreastImplantPresent.setter
    def BreastImplantPresent(self, value: Optional[str]):
        if value is None:
            if "BreastImplantPresent" in self._dataset:
                del self._dataset.BreastImplantPresent
        else:
            self._dataset.BreastImplantPresent = value
    ...
    @property
    def PartialViewCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "PartialViewCodeSequence" in self._dataset:
            if len(self._PartialViewCodeSequence) == len(self._dataset.PartialViewCodeSequence):
                return self._PartialViewCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.PartialViewCodeSequence]
        return None

    @PartialViewCodeSequence.setter
    def PartialViewCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._PartialViewCodeSequence = []
            if "PartialViewCodeSequence" in self._dataset:
                del self._dataset.PartialViewCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"PartialViewCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._PartialViewCodeSequence = value
            if "PartialViewCodeSequence" not in self._dataset:
                self._dataset.PartialViewCodeSequence = pydicom.Sequence()
            self._dataset.PartialViewCodeSequence.clear()
            self._dataset.PartialViewCodeSequence.extend([item.to_dataset() for item in value])

    def add_PartialViewCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._PartialViewCodeSequence.append(item)
        if "PartialViewCodeSequence" not in self._dataset:
            self._dataset.PartialViewCodeSequence = pydicom.Sequence()
        self._dataset.PartialViewCodeSequence.append(item.to_dataset())
