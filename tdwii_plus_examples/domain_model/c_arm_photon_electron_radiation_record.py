...
from .additional_parameter_recording_instance_sequence_item import (
    AdditionalParameterRecordingInstanceSequenceItem,
)
from .author_identification_sequence_item import AuthorIdentificationSequenceItem
from .block_definition_sequence_item import BlockDefinitionSequenceItem
from .bolus_definition_sequence_item import BolusDefinitionSequenceItem
from .breed_registration_sequence_item import BreedRegistrationSequenceItem
from .c_arm_photon_electron_control_point_sequence_item import (
    CArmPhotonElectronControlPointSequenceItem,
)
from .code_sequence_item import CodeSequenceItem
from .coding_scheme_identification_sequence_item import (
    CodingSchemeIdentificationSequenceItem,
)
from .compensator_definition_sequence_item import CompensatorDefinitionSequenceItem
from .confirmation_sequence_item import ConfirmationSequenceItem
...
from .equipment_reference_point_coordinates_sequence_item import (
    EquipmentReferencePointCoordinatesSequenceItem,
)
from .general_accessory_definition_sequence_item import (
    GeneralAccessoryDefinitionSequenceItem,
)
...
from .instance_level_referenced_performed_procedure_step_sequence_item import (
    InstanceLevelReferencedPerformedProcedureStepSequenceItem,
)
...
from .patient_support_devices_sequence_item import PatientSupportDevicesSequenceItem
...
from .referenced_rt_instance_sequence_item import ReferencedRTInstanceSequenceItem
...
from .rt_accessory_holder_definition_sequence_item import (
    RTAccessoryHolderDefinitionSequenceItem,
)
from .rt_beam_limiting_device_definition_sequence_item import (
    RTBeamLimitingDeviceDefinitionSequenceItem,
)
from .rt_tolerance_set_sequence_item import RTToleranceSetSequenceItem
...
from .treatment_device_identification_sequence_item import (
    TreatmentDeviceIdentificationSequenceItem,
)
from .treatment_position_sequence_item import TreatmentPositionSequenceItem
from .treatment_tolerance_violation_sequence_item import (
    TreatmentToleranceViolationSequenceItem,
)
from .udi_sequence_item import UDISequenceItem
from .wedge_definition_sequence_item import WedgeDefinitionSequenceItem


class CArmPhotonElectronRadiationRecord:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._AuthorIdentificationSequence: List[AuthorIdentificationSequenceItem] = []
        self._InstanceLevelReferencedPerformedProcedureStepSequence: List[
            InstanceLevelReferencedPerformedProcedureStepSequenceItem
        ] = []
        ...
        self._RTAccessoryHolderDefinitionSequence: List[RTAccessoryHolderDefinitionSequenceItem] = []
        self._RTBeamLimitingDeviceDefinitionSequence: List[RTBeamLimitingDeviceDefinitionSequenceItem] = []
        self._WedgeDefinitionSequence: List[WedgeDefinitionSequenceItem] = []
        self._CompensatorDefinitionSequence: List[CompensatorDefinitionSequenceItem] = []
        self._BlockDefinitionSequence: List[BlockDefinitionSequenceItem] = []
        self._GeneralAccessoryDefinitionSequence: List[GeneralAccessoryDefinitionSequenceItem] = []
        self._BolusDefinitionSequence: List[BolusDefinitionSequenceItem] = []
        ...
        self._CArmPhotonElectronControlPointSequence: List[CArmPhotonElectronControlPointSequenceItem] = []
        ...
        self._TreatmentDeviceIdentificationSequence: List[TreatmentDeviceIdentificationSequenceItem] = []
        ...
        self._EquipmentReferencePointCoordinatesSequence: List[EquipmentReferencePointCoordinatesSequenceItem] = []
        self._PatientSupportDevicesSequence: List[PatientSupportDevicesSequenceItem] = []
        ...
        self._RTToleranceSetSequence: List[RTToleranceSetSequenceItem] = []
        self._ReferencedRTInstanceSequence: List[ReferencedRTInstanceSequenceItem] = []
        ...
        self._TreatmentToleranceViolationSequence: List[TreatmentToleranceViolationSequenceItem] = []
        self._ConfirmationSequence: List[ConfirmationSequenceItem] = []
        self._InterlockSequence: List[InterlockSequenceItem] = []
        self._AdditionalParameterRecordingInstanceSequence: List[AdditionalParameterRecordingInstanceSequenceItem] = []
    ...
    @property
    def AuthorIdentificationSequence(self) -> Optional[List[AuthorIdentificationSequenceItem]]:
        if "AuthorIdentificationSequence" in self._dataset:
            if len(self._AuthorIdentificationSequence) == len(self._dataset.AuthorIdentificationSequence):
                return self._AuthorIdentificationSequence
            else:
                return [AuthorIdentificationSequenceItem(x) for x in self._dataset.AuthorIdentificationSequence]
        return None

    @AuthorIdentificationSequence.setter
    def AuthorIdentificationSequence(self, value: Optional[List[AuthorIdentificationSequenceItem]]):
        if value is None:
            self._AuthorIdentificationSequence = []
            if "AuthorIdentificationSequence" in self._dataset:
                del self._dataset.AuthorIdentificationSequence
        elif not isinstance(value, list) or not all(isinstance(item, AuthorIdentificationSequenceItem) for item in value):
            raise ValueError(f"AuthorIdentificationSequence must be a list of AuthorIdentificationSequenceItem objects")
        else:
            self._AuthorIdentificationSequence = value
            if "AuthorIdentificationSequence" not in self._dataset:
                self._dataset.AuthorIdentificationSequence = pydicom.Sequence()
            self._dataset.AuthorIdentificationSequence.clear()
            self._dataset.AuthorIdentificationSequence.extend([item.to_dataset() for item in value])

    def add_AuthorIdentification(self, item: AuthorIdentificationSequenceItem):
        if not isinstance(item, AuthorIdentificationSequenceItem):
            raise ValueError(f"Item must be an instance of AuthorIdentificationSequenceItem")
        self._AuthorIdentificationSequence.append(item)
        if "AuthorIdentificationSequence" not in self._dataset:
            self._dataset.AuthorIdentificationSequence = pydicom.Sequence()
        self._dataset.AuthorIdentificationSequence.append(item.to_dataset())

    @property
    def InstanceLevelReferencedPerformedProcedureStepSequence(
        self,
    ) -> Optional[List[InstanceLevelReferencedPerformedProcedureStepSequenceItem]]:
        if "InstanceLevelReferencedPerformedProcedureStepSequence" in self._dataset:
            if len(self._InstanceLevelReferencedPerformedProcedureStepSequence) == len(
                self._dataset.InstanceLevelReferencedPerformedProcedureStepSequence
            ):
                return self._InstanceLevelReferencedPerformedProcedureStepSequence
            else:
                return [
                    InstanceLevelReferencedPerformedProcedureStepSequenceItem(x)
                    for x in self._dataset.InstanceLevelReferencedPerformedProcedureStepSequence
                ]
        return None

    @InstanceLevelReferencedPerformedProcedureStepSequence.setter
    def InstanceLevelReferencedPerformedProcedureStepSequence(
        self, value: Optional[List[InstanceLevelReferencedPerformedProcedureStepSequenceItem]]
    ):
        if value is None:
            self._InstanceLevelReferencedPerformedProcedureStepSequence = []
            if "InstanceLevelReferencedPerformedProcedureStepSequence" in self._dataset:
                del self._dataset.InstanceLevelReferencedPerformedProcedureStepSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, InstanceLevelReferencedPerformedProcedureStepSequenceItem) for item in value
        ):
            raise ValueError(
                f"InstanceLevelReferencedPerformedProcedureStepSequence must be a list of InstanceLevelReferencedPerformedProcedureStepSequenceItem objects"
            )
        else:
            self._InstanceLevelReferencedPerformedProcedureStepSequence = value
            if "InstanceLevelReferencedPerformedProcedureStepSequence" not in self._dataset:
                self._dataset.InstanceLevelReferencedPerformedProcedureStepSequence = pydicom.Sequence()
            self._dataset.InstanceLevelReferencedPerformedProcedureStepSequence.clear()
            self._dataset.InstanceLevelReferencedPerformedProcedureStepSequence.extend([item.to_dataset() for item in value])

    def add_InstanceLevelReferencedPerformedProcedureStep(
        self, item: InstanceLevelReferencedPerformedProcedureStepSequenceItem
    ):
        if not isinstance(item, InstanceLevelReferencedPerformedProcedureStepSequenceItem):
            raise ValueError(f"Item must be an instance of InstanceLevelReferencedPerformedProcedureStepSequenceItem")
        self._InstanceLevelReferencedPerformedProcedureStepSequence.append(item)
        if "InstanceLevelReferencedPerformedProcedureStepSequence" not in self._dataset:
            self._dataset.InstanceLevelReferencedPerformedProcedureStepSequence = pydicom.Sequence()
        self._dataset.InstanceLevelReferencedPerformedProcedureStepSequence.append(item.to_dataset())
    ...
    @property
    def RTAccessoryHolderDefinitionSequence(self) -> Optional[List[RTAccessoryHolderDefinitionSequenceItem]]:
        if "RTAccessoryHolderDefinitionSequence" in self._dataset:
            if len(self._RTAccessoryHolderDefinitionSequence) == len(self._dataset.RTAccessoryHolderDefinitionSequence):
                return self._RTAccessoryHolderDefinitionSequence
            else:
                return [RTAccessoryHolderDefinitionSequenceItem(x) for x in self._dataset.RTAccessoryHolderDefinitionSequence]
        return None

    @RTAccessoryHolderDefinitionSequence.setter
    def RTAccessoryHolderDefinitionSequence(self, value: Optional[List[RTAccessoryHolderDefinitionSequenceItem]]):
        if value is None:
            self._RTAccessoryHolderDefinitionSequence = []
            if "RTAccessoryHolderDefinitionSequence" in self._dataset:
                del self._dataset.RTAccessoryHolderDefinitionSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, RTAccessoryHolderDefinitionSequenceItem) for item in value
        ):
            raise ValueError(
                f"RTAccessoryHolderDefinitionSequence must be a list of RTAccessoryHolderDefinitionSequenceItem objects"
            )
        else:
            self._RTAccessoryHolderDefinitionSequence = value
            if "RTAccessoryHolderDefinitionSequence" not in self._dataset:
                self._dataset.RTAccessoryHolderDefinitionSequence = pydicom.Sequence()
            self._dataset.RTAccessoryHolderDefinitionSequence.clear()
            self._dataset.RTAccessoryHolderDefinitionSequence.extend([item.to_dataset() for item in value])

    def add_RTAccessoryHolderDefinition(self, item: RTAccessoryHolderDefinitionSequenceItem):
        if not isinstance(item, RTAccessoryHolderDefinitionSequenceItem):
            raise ValueError(f"Item must be an instance of RTAccessoryHolderDefinitionSequenceItem")
        self._RTAccessoryHolderDefinitionSequence.append(item)
        if "RTAccessoryHolderDefinitionSequence" not in self._dataset:
            self._dataset.RTAccessoryHolderDefinitionSequence = pydicom.Sequence()
        self._dataset.RTAccessoryHolderDefinitionSequence.append(item.to_dataset())

    @property
    def RadiationSourceAxisDistance(self) -> Optional[float]:
        if "RadiationSourceAxisDistance" in self._dataset:
            return self._dataset.RadiationSourceAxisDistance
        return None

    @RadiationSourceAxisDistance.setter
    def RadiationSourceAxisDistance(self, value: Optional[float]):
        if value is None:
            if "RadiationSourceAxisDistance" in self._dataset:
                del self._dataset.RadiationSourceAxisDistance
        else:
            self._dataset.RadiationSourceAxisDistance = value

    @property
    def NumberOfRTBeamLimitingDevices(self) -> Optional[int]:
        if "NumberOfRTBeamLimitingDevices" in self._dataset:
            return self._dataset.NumberOfRTBeamLimitingDevices
        return None

    @NumberOfRTBeamLimitingDevices.setter
    def NumberOfRTBeamLimitingDevices(self, value: Optional[int]):
        if value is None:
            if "NumberOfRTBeamLimitingDevices" in self._dataset:
                del self._dataset.NumberOfRTBeamLimitingDevices
        else:
            self._dataset.NumberOfRTBeamLimitingDevices = value

    @property
    def RTBeamLimitingDeviceDefinitionSequence(self) -> Optional[List[RTBeamLimitingDeviceDefinitionSequenceItem]]:
        if "RTBeamLimitingDeviceDefinitionSequence" in self._dataset:
            if len(self._RTBeamLimitingDeviceDefinitionSequence) == len(self._dataset.RTBeamLimitingDeviceDefinitionSequence):
                return self._RTBeamLimitingDeviceDefinitionSequence
            else:
                return [
                    RTBeamLimitingDeviceDefinitionSequenceItem(x) for x in self._dataset.RTBeamLimitingDeviceDefinitionSequence
                ]
        return None

    @RTBeamLimitingDeviceDefinitionSequence.setter
    def RTBeamLimitingDeviceDefinitionSequence(self, value: Optional[List[RTBeamLimitingDeviceDefinitionSequenceItem]]):
        if value is None:
            self._RTBeamLimitingDeviceDefinitionSequence = []
            if "RTBeamLimitingDeviceDefinitionSequence" in self._dataset:
                del self._dataset.RTBeamLimitingDeviceDefinitionSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, RTBeamLimitingDeviceDefinitionSequenceItem) for item in value
        ):
            raise ValueError(
                f"RTBeamLimitingDeviceDefinitionSequence must be a list of RTBeamLimitingDeviceDefinitionSequenceItem objects"
            )
        else:
            self._RTBeamLimitingDeviceDefinitionSequence = value
            if "RTBeamLimitingDeviceDefinitionSequence" not in self._dataset:
                self._dataset.RTBeamLimitingDeviceDefinitionSequence = pydicom.Sequence()
            self._dataset.RTBeamLimitingDeviceDefinitionSequence.clear()
            self._dataset.RTBeamLimitingDeviceDefinitionSequence.extend([item.to_dataset() for item in value])

    def add_RTBeamLimitingDeviceDefinition(self, item: RTBeamLimitingDeviceDefinitionSequenceItem):
        if not isinstance(item, RTBeamLimitingDeviceDefinitionSequenceItem):
            raise ValueError(f"Item must be an instance of RTBeamLimitingDeviceDefinitionSequenceItem")
        self._RTBeamLimitingDeviceDefinitionSequence.append(item)
        if "RTBeamLimitingDeviceDefinitionSequence" not in self._dataset:
            self._dataset.RTBeamLimitingDeviceDefinitionSequence = pydicom.Sequence()
        self._dataset.RTBeamLimitingDeviceDefinitionSequence.append(item.to_dataset())

    @property
    def WedgeDefinitionSequence(self) -> Optional[List[WedgeDefinitionSequenceItem]]:
        if "WedgeDefinitionSequence" in self._dataset:
            if len(self._WedgeDefinitionSequence) == len(self._dataset.WedgeDefinitionSequence):
                return self._WedgeDefinitionSequence
            else:
                return [WedgeDefinitionSequenceItem(x) for x in self._dataset.WedgeDefinitionSequence]
        return None

    @WedgeDefinitionSequence.setter
    def WedgeDefinitionSequence(self, value: Optional[List[WedgeDefinitionSequenceItem]]):
        if value is None:
            self._WedgeDefinitionSequence = []
            if "WedgeDefinitionSequence" in self._dataset:
                del self._dataset.WedgeDefinitionSequence
        elif not isinstance(value, list) or not all(isinstance(item, WedgeDefinitionSequenceItem) for item in value):
            raise ValueError(f"WedgeDefinitionSequence must be a list of WedgeDefinitionSequenceItem objects")
        else:
            self._WedgeDefinitionSequence = value
            if "WedgeDefinitionSequence" not in self._dataset:
                self._dataset.WedgeDefinitionSequence = pydicom.Sequence()
            self._dataset.WedgeDefinitionSequence.clear()
            self._dataset.WedgeDefinitionSequence.extend([item.to_dataset() for item in value])

    def add_WedgeDefinition(self, item: WedgeDefinitionSequenceItem):
        if not isinstance(item, WedgeDefinitionSequenceItem):
            raise ValueError(f"Item must be an instance of WedgeDefinitionSequenceItem")
        self._WedgeDefinitionSequence.append(item)
        if "WedgeDefinitionSequence" not in self._dataset:
            self._dataset.WedgeDefinitionSequence = pydicom.Sequence()
        self._dataset.WedgeDefinitionSequence.append(item.to_dataset())

    @property
    def CompensatorDefinitionSequence(self) -> Optional[List[CompensatorDefinitionSequenceItem]]:
        if "CompensatorDefinitionSequence" in self._dataset:
            if len(self._CompensatorDefinitionSequence) == len(self._dataset.CompensatorDefinitionSequence):
                return self._CompensatorDefinitionSequence
            else:
                return [CompensatorDefinitionSequenceItem(x) for x in self._dataset.CompensatorDefinitionSequence]
        return None

    @CompensatorDefinitionSequence.setter
    def CompensatorDefinitionSequence(self, value: Optional[List[CompensatorDefinitionSequenceItem]]):
        if value is None:
            self._CompensatorDefinitionSequence = []
            if "CompensatorDefinitionSequence" in self._dataset:
                del self._dataset.CompensatorDefinitionSequence
        elif not isinstance(value, list) or not all(isinstance(item, CompensatorDefinitionSequenceItem) for item in value):
            raise ValueError(f"CompensatorDefinitionSequence must be a list of CompensatorDefinitionSequenceItem objects")
        else:
            self._CompensatorDefinitionSequence = value
            if "CompensatorDefinitionSequence" not in self._dataset:
                self._dataset.CompensatorDefinitionSequence = pydicom.Sequence()
            self._dataset.CompensatorDefinitionSequence.clear()
            self._dataset.CompensatorDefinitionSequence.extend([item.to_dataset() for item in value])

    def add_CompensatorDefinition(self, item: CompensatorDefinitionSequenceItem):
        if not isinstance(item, CompensatorDefinitionSequenceItem):
            raise ValueError(f"Item must be an instance of CompensatorDefinitionSequenceItem")
        self._CompensatorDefinitionSequence.append(item)
        if "CompensatorDefinitionSequence" not in self._dataset:
            self._dataset.CompensatorDefinitionSequence = pydicom.Sequence()
        self._dataset.CompensatorDefinitionSequence.append(item.to_dataset())

    @property
    def BlockDefinitionSequence(self) -> Optional[List[BlockDefinitionSequenceItem]]:
        if "BlockDefinitionSequence" in self._dataset:
            if len(self._BlockDefinitionSequence) == len(self._dataset.BlockDefinitionSequence):
                return self._BlockDefinitionSequence
            else:
                return [BlockDefinitionSequenceItem(x) for x in self._dataset.BlockDefinitionSequence]
        return None

    @BlockDefinitionSequence.setter
    def BlockDefinitionSequence(self, value: Optional[List[BlockDefinitionSequenceItem]]):
        if value is None:
            self._BlockDefinitionSequence = []
            if "BlockDefinitionSequence" in self._dataset:
                del self._dataset.BlockDefinitionSequence
        elif not isinstance(value, list) or not all(isinstance(item, BlockDefinitionSequenceItem) for item in value):
            raise ValueError(f"BlockDefinitionSequence must be a list of BlockDefinitionSequenceItem objects")
        else:
            self._BlockDefinitionSequence = value
            if "BlockDefinitionSequence" not in self._dataset:
                self._dataset.BlockDefinitionSequence = pydicom.Sequence()
            self._dataset.BlockDefinitionSequence.clear()
            self._dataset.BlockDefinitionSequence.extend([item.to_dataset() for item in value])

    def add_BlockDefinition(self, item: BlockDefinitionSequenceItem):
        if not isinstance(item, BlockDefinitionSequenceItem):
            raise ValueError(f"Item must be an instance of BlockDefinitionSequenceItem")
        self._BlockDefinitionSequence.append(item)
        if "BlockDefinitionSequence" not in self._dataset:
            self._dataset.BlockDefinitionSequence = pydicom.Sequence()
        self._dataset.BlockDefinitionSequence.append(item.to_dataset())

    @property
    def NumberOfRTAccessoryHolders(self) -> Optional[int]:
        if "NumberOfRTAccessoryHolders" in self._dataset:
            return self._dataset.NumberOfRTAccessoryHolders
        return None

    @NumberOfRTAccessoryHolders.setter
    def NumberOfRTAccessoryHolders(self, value: Optional[int]):
        if value is None:
            if "NumberOfRTAccessoryHolders" in self._dataset:
                del self._dataset.NumberOfRTAccessoryHolders
        else:
            self._dataset.NumberOfRTAccessoryHolders = value

    @property
    def GeneralAccessoryDefinitionSequence(self) -> Optional[List[GeneralAccessoryDefinitionSequenceItem]]:
        if "GeneralAccessoryDefinitionSequence" in self._dataset:
            if len(self._GeneralAccessoryDefinitionSequence) == len(self._dataset.GeneralAccessoryDefinitionSequence):
                return self._GeneralAccessoryDefinitionSequence
            else:
                return [GeneralAccessoryDefinitionSequenceItem(x) for x in self._dataset.GeneralAccessoryDefinitionSequence]
        return None

    @GeneralAccessoryDefinitionSequence.setter
    def GeneralAccessoryDefinitionSequence(self, value: Optional[List[GeneralAccessoryDefinitionSequenceItem]]):
        if value is None:
            self._GeneralAccessoryDefinitionSequence = []
            if "GeneralAccessoryDefinitionSequence" in self._dataset:
                del self._dataset.GeneralAccessoryDefinitionSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, GeneralAccessoryDefinitionSequenceItem) for item in value
        ):
            raise ValueError(
                f"GeneralAccessoryDefinitionSequence must be a list of GeneralAccessoryDefinitionSequenceItem objects"
            )
        else:
            self._GeneralAccessoryDefinitionSequence = value
            if "GeneralAccessoryDefinitionSequence" not in self._dataset:
                self._dataset.GeneralAccessoryDefinitionSequence = pydicom.Sequence()
            self._dataset.GeneralAccessoryDefinitionSequence.clear()
            self._dataset.GeneralAccessoryDefinitionSequence.extend([item.to_dataset() for item in value])

    def add_GeneralAccessoryDefinition(self, item: GeneralAccessoryDefinitionSequenceItem):
        if not isinstance(item, GeneralAccessoryDefinitionSequenceItem):
            raise ValueError(f"Item must be an instance of GeneralAccessoryDefinitionSequenceItem")
        self._GeneralAccessoryDefinitionSequence.append(item)
        if "GeneralAccessoryDefinitionSequence" not in self._dataset:
            self._dataset.GeneralAccessoryDefinitionSequence = pydicom.Sequence()
        self._dataset.GeneralAccessoryDefinitionSequence.append(item.to_dataset())

    @property
    def NumberOfGeneralAccessories(self) -> Optional[int]:
        if "NumberOfGeneralAccessories" in self._dataset:
            return self._dataset.NumberOfGeneralAccessories
        return None

    @NumberOfGeneralAccessories.setter
    def NumberOfGeneralAccessories(self, value: Optional[int]):
        if value is None:
            if "NumberOfGeneralAccessories" in self._dataset:
                del self._dataset.NumberOfGeneralAccessories
        else:
            self._dataset.NumberOfGeneralAccessories = value

    @property
    def BolusDefinitionSequence(self) -> Optional[List[BolusDefinitionSequenceItem]]:
        if "BolusDefinitionSequence" in self._dataset:
            if len(self._BolusDefinitionSequence) == len(self._dataset.BolusDefinitionSequence):
                return self._BolusDefinitionSequence
            else:
                return [BolusDefinitionSequenceItem(x) for x in self._dataset.BolusDefinitionSequence]
        return None

    @BolusDefinitionSequence.setter
    def BolusDefinitionSequence(self, value: Optional[List[BolusDefinitionSequenceItem]]):
        if value is None:
            self._BolusDefinitionSequence = []
            if "BolusDefinitionSequence" in self._dataset:
                del self._dataset.BolusDefinitionSequence
        elif not isinstance(value, list) or not all(isinstance(item, BolusDefinitionSequenceItem) for item in value):
            raise ValueError(f"BolusDefinitionSequence must be a list of BolusDefinitionSequenceItem objects")
        else:
            self._BolusDefinitionSequence = value
            if "BolusDefinitionSequence" not in self._dataset:
                self._dataset.BolusDefinitionSequence = pydicom.Sequence()
            self._dataset.BolusDefinitionSequence.clear()
            self._dataset.BolusDefinitionSequence.extend([item.to_dataset() for item in value])

    def add_BolusDefinition(self, item: BolusDefinitionSequenceItem):
        if not isinstance(item, BolusDefinitionSequenceItem):
            raise ValueError(f"Item must be an instance of BolusDefinitionSequenceItem")
        self._BolusDefinitionSequence.append(item)
        if "BolusDefinitionSequence" not in self._dataset:
            self._dataset.BolusDefinitionSequence = pydicom.Sequence()
        self._dataset.BolusDefinitionSequence.append(item.to_dataset())

    @property
    def NumberOfBoluses(self) -> Optional[int]:
        if "NumberOfBoluses" in self._dataset:
            return self._dataset.NumberOfBoluses
        return None

    @NumberOfBoluses.setter
    def NumberOfBoluses(self, value: Optional[int]):
        if value is None:
            if "NumberOfBoluses" in self._dataset:
                del self._dataset.NumberOfBoluses
        else:
            self._dataset.NumberOfBoluses = value
    ...
    @property
    def NumberOfRadiationGenerationModes(self) -> Optional[int]:
        if "NumberOfRadiationGenerationModes" in self._dataset:
            return self._dataset.NumberOfRadiationGenerationModes
        return None

    @NumberOfRadiationGenerationModes.setter
    def NumberOfRadiationGenerationModes(self, value: Optional[int]):
        if value is None:
            if "NumberOfRadiationGenerationModes" in self._dataset:
                del self._dataset.NumberOfRadiationGenerationModes
        else:
            self._dataset.NumberOfRadiationGenerationModes = value
    ...
    @property
    def NumberOfRTControlPoints(self) -> Optional[int]:
        if "NumberOfRTControlPoints" in self._dataset:
            return self._dataset.NumberOfRTControlPoints
        return None

    @NumberOfRTControlPoints.setter
    def NumberOfRTControlPoints(self, value: Optional[int]):
        if value is None:
            if "NumberOfRTControlPoints" in self._dataset:
                del self._dataset.NumberOfRTControlPoints
        else:
            self._dataset.NumberOfRTControlPoints = value

    @property
    def CArmPhotonElectronControlPointSequence(self) -> Optional[List[CArmPhotonElectronControlPointSequenceItem]]:
        if "CArmPhotonElectronControlPointSequence" in self._dataset:
            if len(self._CArmPhotonElectronControlPointSequence) == len(self._dataset.CArmPhotonElectronControlPointSequence):
                return self._CArmPhotonElectronControlPointSequence
            else:
                return [
                    CArmPhotonElectronControlPointSequenceItem(x) for x in self._dataset.CArmPhotonElectronControlPointSequence
                ]
        return None

    @CArmPhotonElectronControlPointSequence.setter
    def CArmPhotonElectronControlPointSequence(self, value: Optional[List[CArmPhotonElectronControlPointSequenceItem]]):
        if value is None:
            self._CArmPhotonElectronControlPointSequence = []
            if "CArmPhotonElectronControlPointSequence" in self._dataset:
                del self._dataset.CArmPhotonElectronControlPointSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, CArmPhotonElectronControlPointSequenceItem) for item in value
        ):
            raise ValueError(
                f"CArmPhotonElectronControlPointSequence must be a list of CArmPhotonElectronControlPointSequenceItem objects"
            )
        else:
            self._CArmPhotonElectronControlPointSequence = value
            if "CArmPhotonElectronControlPointSequence" not in self._dataset:
                self._dataset.CArmPhotonElectronControlPointSequence = pydicom.Sequence()
            self._dataset.CArmPhotonElectronControlPointSequence.clear()
            self._dataset.CArmPhotonElectronControlPointSequence.extend([item.to_dataset() for item in value])

    def add_CArmPhotonElectronControlPoint(self, item: CArmPhotonElectronControlPointSequenceItem):
        if not isinstance(item, CArmPhotonElectronControlPointSequenceItem):
            raise ValueError(f"Item must be an instance of CArmPhotonElectronControlPointSequenceItem")
        self._CArmPhotonElectronControlPointSequence.append(item)
        if "CArmPhotonElectronControlPointSequence" not in self._dataset:
            self._dataset.CArmPhotonElectronControlPointSequence = pydicom.Sequence()
        self._dataset.CArmPhotonElectronControlPointSequence.append(item.to_dataset())
    ...
    @property
    def TreatmentDeviceIdentificationSequence(self) -> Optional[List[TreatmentDeviceIdentificationSequenceItem]]:
        if "TreatmentDeviceIdentificationSequence" in self._dataset:
            if len(self._TreatmentDeviceIdentificationSequence) == len(self._dataset.TreatmentDeviceIdentificationSequence):
                return self._TreatmentDeviceIdentificationSequence
            else:
                return [
                    TreatmentDeviceIdentificationSequenceItem(x) for x in self._dataset.TreatmentDeviceIdentificationSequence
                ]
        return None

    @TreatmentDeviceIdentificationSequence.setter
    def TreatmentDeviceIdentificationSequence(self, value: Optional[List[TreatmentDeviceIdentificationSequenceItem]]):
        if value is None:
            self._TreatmentDeviceIdentificationSequence = []
            if "TreatmentDeviceIdentificationSequence" in self._dataset:
                del self._dataset.TreatmentDeviceIdentificationSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, TreatmentDeviceIdentificationSequenceItem) for item in value
        ):
            raise ValueError(
                f"TreatmentDeviceIdentificationSequence must be a list of TreatmentDeviceIdentificationSequenceItem objects"
            )
        else:
            self._TreatmentDeviceIdentificationSequence = value
            if "TreatmentDeviceIdentificationSequence" not in self._dataset:
                self._dataset.TreatmentDeviceIdentificationSequence = pydicom.Sequence()
            self._dataset.TreatmentDeviceIdentificationSequence.clear()
            self._dataset.TreatmentDeviceIdentificationSequence.extend([item.to_dataset() for item in value])

    def add_TreatmentDeviceIdentification(self, item: TreatmentDeviceIdentificationSequenceItem):
        if not isinstance(item, TreatmentDeviceIdentificationSequenceItem):
            raise ValueError(f"Item must be an instance of TreatmentDeviceIdentificationSequenceItem")
        self._TreatmentDeviceIdentificationSequence.append(item)
        if "TreatmentDeviceIdentificationSequence" not in self._dataset:
            self._dataset.TreatmentDeviceIdentificationSequence = pydicom.Sequence()
        self._dataset.TreatmentDeviceIdentificationSequence.append(item.to_dataset())
    ...
    @property
    def EquipmentFrameOfReferenceUID(self) -> Optional[str]:
        if "EquipmentFrameOfReferenceUID" in self._dataset:
            return self._dataset.EquipmentFrameOfReferenceUID
        return None

    @EquipmentFrameOfReferenceUID.setter
    def EquipmentFrameOfReferenceUID(self, value: Optional[str]):
        if value is None:
            if "EquipmentFrameOfReferenceUID" in self._dataset:
                del self._dataset.EquipmentFrameOfReferenceUID
        else:
            self._dataset.EquipmentFrameOfReferenceUID = value

    @property
    def EquipmentFrameOfReferenceDescription(self) -> Optional[str]:
        if "EquipmentFrameOfReferenceDescription" in self._dataset:
            return self._dataset.EquipmentFrameOfReferenceDescription
        return None

    @EquipmentFrameOfReferenceDescription.setter
    def EquipmentFrameOfReferenceDescription(self, value: Optional[str]):
        if value is None:
            if "EquipmentFrameOfReferenceDescription" in self._dataset:
                del self._dataset.EquipmentFrameOfReferenceDescription
        else:
            self._dataset.EquipmentFrameOfReferenceDescription = value

    @property
    def EquipmentReferencePointCoordinatesSequence(self) -> Optional[List[EquipmentReferencePointCoordinatesSequenceItem]]:
        if "EquipmentReferencePointCoordinatesSequence" in self._dataset:
            if len(self._EquipmentReferencePointCoordinatesSequence) == len(
                self._dataset.EquipmentReferencePointCoordinatesSequence
            ):
                return self._EquipmentReferencePointCoordinatesSequence
            else:
                return [
                    EquipmentReferencePointCoordinatesSequenceItem(x)
                    for x in self._dataset.EquipmentReferencePointCoordinatesSequence
                ]
        return None

    @EquipmentReferencePointCoordinatesSequence.setter
    def EquipmentReferencePointCoordinatesSequence(
        self, value: Optional[List[EquipmentReferencePointCoordinatesSequenceItem]]
    ):
        if value is None:
            self._EquipmentReferencePointCoordinatesSequence = []
            if "EquipmentReferencePointCoordinatesSequence" in self._dataset:
                del self._dataset.EquipmentReferencePointCoordinatesSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, EquipmentReferencePointCoordinatesSequenceItem) for item in value
        ):
            raise ValueError(
                f"EquipmentReferencePointCoordinatesSequence must be a list of EquipmentReferencePointCoordinatesSequenceItem objects"
            )
        else:
            self._EquipmentReferencePointCoordinatesSequence = value
            if "EquipmentReferencePointCoordinatesSequence" not in self._dataset:
                self._dataset.EquipmentReferencePointCoordinatesSequence = pydicom.Sequence()
            self._dataset.EquipmentReferencePointCoordinatesSequence.clear()
            self._dataset.EquipmentReferencePointCoordinatesSequence.extend([item.to_dataset() for item in value])

    def add_EquipmentReferencePointCoordinates(self, item: EquipmentReferencePointCoordinatesSequenceItem):
        if not isinstance(item, EquipmentReferencePointCoordinatesSequenceItem):
            raise ValueError(f"Item must be an instance of EquipmentReferencePointCoordinatesSequenceItem")
        self._EquipmentReferencePointCoordinatesSequence.append(item)
        if "EquipmentReferencePointCoordinatesSequence" not in self._dataset:
            self._dataset.EquipmentReferencePointCoordinatesSequence = pydicom.Sequence()
        self._dataset.EquipmentReferencePointCoordinatesSequence.append(item.to_dataset())

    @property
    def PatientSupportDevicesSequence(self) -> Optional[List[PatientSupportDevicesSequenceItem]]:
        if "PatientSupportDevicesSequence" in self._dataset:
            if len(self._PatientSupportDevicesSequence) == len(self._dataset.PatientSupportDevicesSequence):
                return self._PatientSupportDevicesSequence
            else:
                return [PatientSupportDevicesSequenceItem(x) for x in self._dataset.PatientSupportDevicesSequence]
        return None

    @PatientSupportDevicesSequence.setter
    def PatientSupportDevicesSequence(self, value: Optional[List[PatientSupportDevicesSequenceItem]]):
        if value is None:
            self._PatientSupportDevicesSequence = []
            if "PatientSupportDevicesSequence" in self._dataset:
                del self._dataset.PatientSupportDevicesSequence
        elif not isinstance(value, list) or not all(isinstance(item, PatientSupportDevicesSequenceItem) for item in value):
            raise ValueError(f"PatientSupportDevicesSequence must be a list of PatientSupportDevicesSequenceItem objects")
        else:
            self._PatientSupportDevicesSequence = value
            if "PatientSupportDevicesSequence" not in self._dataset:
                self._dataset.PatientSupportDevicesSequence = pydicom.Sequence()
            self._dataset.PatientSupportDevicesSequence.clear()
            self._dataset.PatientSupportDevicesSequence.extend([item.to_dataset() for item in value])

    def add_PatientSupportDevices(self, item: PatientSupportDevicesSequenceItem):
        if not isinstance(item, PatientSupportDevicesSequenceItem):
            raise ValueError(f"Item must be an instance of PatientSupportDevicesSequenceItem")
        self._PatientSupportDevicesSequence.append(item)
        if "PatientSupportDevicesSequence" not in self._dataset:
            self._dataset.PatientSupportDevicesSequence = pydicom.Sequence()
        self._dataset.PatientSupportDevicesSequence.append(item.to_dataset())

    @property
    def NumberOfPatientSupportDevices(self) -> Optional[int]:
        if "NumberOfPatientSupportDevices" in self._dataset:
            return self._dataset.NumberOfPatientSupportDevices
        return None

    @NumberOfPatientSupportDevices.setter
    def NumberOfPatientSupportDevices(self, value: Optional[int]):
        if value is None:
            if "NumberOfPatientSupportDevices" in self._dataset:
                del self._dataset.NumberOfPatientSupportDevices
        else:
            self._dataset.NumberOfPatientSupportDevices = value
    ...
    @property
    def RTToleranceSetSequence(self) -> Optional[List[RTToleranceSetSequenceItem]]:
        if "RTToleranceSetSequence" in self._dataset:
            if len(self._RTToleranceSetSequence) == len(self._dataset.RTToleranceSetSequence):
                return self._RTToleranceSetSequence
            else:
                return [RTToleranceSetSequenceItem(x) for x in self._dataset.RTToleranceSetSequence]
        return None

    @RTToleranceSetSequence.setter
    def RTToleranceSetSequence(self, value: Optional[List[RTToleranceSetSequenceItem]]):
        if value is None:
            self._RTToleranceSetSequence = []
            if "RTToleranceSetSequence" in self._dataset:
                del self._dataset.RTToleranceSetSequence
        elif not isinstance(value, list) or not all(isinstance(item, RTToleranceSetSequenceItem) for item in value):
            raise ValueError(f"RTToleranceSetSequence must be a list of RTToleranceSetSequenceItem objects")
        else:
            self._RTToleranceSetSequence = value
            if "RTToleranceSetSequence" not in self._dataset:
                self._dataset.RTToleranceSetSequence = pydicom.Sequence()
            self._dataset.RTToleranceSetSequence.clear()
            self._dataset.RTToleranceSetSequence.extend([item.to_dataset() for item in value])

    def add_RTToleranceSet(self, item: RTToleranceSetSequenceItem):
        if not isinstance(item, RTToleranceSetSequenceItem):
            raise ValueError(f"Item must be an instance of RTToleranceSetSequenceItem")
        self._RTToleranceSetSequence.append(item)
        if "RTToleranceSetSequence" not in self._dataset:
            self._dataset.RTToleranceSetSequence = pydicom.Sequence()
        self._dataset.RTToleranceSetSequence.append(item.to_dataset())

    @property
    def ReferencedRTInstanceSequence(self) -> Optional[List[ReferencedRTInstanceSequenceItem]]:
        if "ReferencedRTInstanceSequence" in self._dataset:
            if len(self._ReferencedRTInstanceSequence) == len(self._dataset.ReferencedRTInstanceSequence):
                return self._ReferencedRTInstanceSequence
            else:
                return [ReferencedRTInstanceSequenceItem(x) for x in self._dataset.ReferencedRTInstanceSequence]
        return None

    @ReferencedRTInstanceSequence.setter
    def ReferencedRTInstanceSequence(self, value: Optional[List[ReferencedRTInstanceSequenceItem]]):
        if value is None:
            self._ReferencedRTInstanceSequence = []
            if "ReferencedRTInstanceSequence" in self._dataset:
                del self._dataset.ReferencedRTInstanceSequence
        elif not isinstance(value, list) or not all(isinstance(item, ReferencedRTInstanceSequenceItem) for item in value):
            raise ValueError(f"ReferencedRTInstanceSequence must be a list of ReferencedRTInstanceSequenceItem objects")
        else:
            self._ReferencedRTInstanceSequence = value
            if "ReferencedRTInstanceSequence" not in self._dataset:
                self._dataset.ReferencedRTInstanceSequence = pydicom.Sequence()
            self._dataset.ReferencedRTInstanceSequence.clear()
            self._dataset.ReferencedRTInstanceSequence.extend([item.to_dataset() for item in value])

    def add_ReferencedRTInstance(self, item: ReferencedRTInstanceSequenceItem):
        if not isinstance(item, ReferencedRTInstanceSequenceItem):
            raise ValueError(f"Item must be an instance of ReferencedRTInstanceSequenceItem")
        self._ReferencedRTInstanceSequence.append(item)
        if "ReferencedRTInstanceSequence" not in self._dataset:
            self._dataset.ReferencedRTInstanceSequence = pydicom.Sequence()
        self._dataset.ReferencedRTInstanceSequence.append(item.to_dataset())

    @property
    def TreatmentMachineSpecialModeCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "TreatmentMachineSpecialModeCodeSequence" in self._dataset:
            if len(self._TreatmentMachineSpecialModeCodeSequence) == len(
                self._dataset.TreatmentMachineSpecialModeCodeSequence
            ):
                return self._TreatmentMachineSpecialModeCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.TreatmentMachineSpecialModeCodeSequence]
        return None

    @TreatmentMachineSpecialModeCodeSequence.setter
    def TreatmentMachineSpecialModeCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._TreatmentMachineSpecialModeCodeSequence = []
            if "TreatmentMachineSpecialModeCodeSequence" in self._dataset:
                del self._dataset.TreatmentMachineSpecialModeCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"TreatmentMachineSpecialModeCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._TreatmentMachineSpecialModeCodeSequence = value
            if "TreatmentMachineSpecialModeCodeSequence" not in self._dataset:
                self._dataset.TreatmentMachineSpecialModeCodeSequence = pydicom.Sequence()
            self._dataset.TreatmentMachineSpecialModeCodeSequence.clear()
            self._dataset.TreatmentMachineSpecialModeCodeSequence.extend([item.to_dataset() for item in value])

    def add_TreatmentMachineSpecialModeCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._TreatmentMachineSpecialModeCodeSequence.append(item)
        if "TreatmentMachineSpecialModeCodeSequence" not in self._dataset:
            self._dataset.TreatmentMachineSpecialModeCodeSequence = pydicom.Sequence()
        self._dataset.TreatmentMachineSpecialModeCodeSequence.append(item.to_dataset())

    @property
    def RTRadiationPhysicalAndGeometricContentDetailFlag(self) -> Optional[str]:
        if "RTRadiationPhysicalAndGeometricContentDetailFlag" in self._dataset:
            return self._dataset.RTRadiationPhysicalAndGeometricContentDetailFlag
        return None

    @RTRadiationPhysicalAndGeometricContentDetailFlag.setter
    def RTRadiationPhysicalAndGeometricContentDetailFlag(self, value: Optional[str]):
        if value is None:
            if "RTRadiationPhysicalAndGeometricContentDetailFlag" in self._dataset:
                del self._dataset.RTRadiationPhysicalAndGeometricContentDetailFlag
        else:
            self._dataset.RTRadiationPhysicalAndGeometricContentDetailFlag = value

    @property
    def RTRecordFlag(self) -> Optional[str]:
        if "RTRecordFlag" in self._dataset:
            return self._dataset.RTRecordFlag
        return None

    @RTRecordFlag.setter
    def RTRecordFlag(self, value: Optional[str]):
        if value is None:
            if "RTRecordFlag" in self._dataset:
                del self._dataset.RTRecordFlag
        else:
            self._dataset.RTRecordFlag = value

    @property
    def TreatmentPositionSequence(self) -> Optional[List[TreatmentPositionSequenceItem]]:
        if "TreatmentPositionSequence" in self._dataset:
            if len(self._TreatmentPositionSequence) == len(self._dataset.TreatmentPositionSequence):
                return self._TreatmentPositionSequence
            else:
                return [TreatmentPositionSequenceItem(x) for x in self._dataset.TreatmentPositionSequence]
        return None

    @TreatmentPositionSequence.setter
    def TreatmentPositionSequence(self, value: Optional[List[TreatmentPositionSequenceItem]]):
        if value is None:
            self._TreatmentPositionSequence = []
            if "TreatmentPositionSequence" in self._dataset:
                del self._dataset.TreatmentPositionSequence
        elif not isinstance(value, list) or not all(isinstance(item, TreatmentPositionSequenceItem) for item in value):
            raise ValueError(f"TreatmentPositionSequence must be a list of TreatmentPositionSequenceItem objects")
        else:
            self._TreatmentPositionSequence = value
            if "TreatmentPositionSequence" not in self._dataset:
                self._dataset.TreatmentPositionSequence = pydicom.Sequence()
            self._dataset.TreatmentPositionSequence.clear()
            self._dataset.TreatmentPositionSequence.extend([item.to_dataset() for item in value])

    def add_TreatmentPosition(self, item: TreatmentPositionSequenceItem):
        if not isinstance(item, TreatmentPositionSequenceItem):
            raise ValueError(f"Item must be an instance of TreatmentPositionSequenceItem")
        self._TreatmentPositionSequence.append(item)
        if "TreatmentPositionSequence" not in self._dataset:
            self._dataset.TreatmentPositionSequence = pydicom.Sequence()
        self._dataset.TreatmentPositionSequence.append(item.to_dataset())
    ...
    @property
    def RTRadiationUsage(self) -> Optional[str]:
        if "RTRadiationUsage" in self._dataset:
            return self._dataset.RTRadiationUsage
        return None

    @RTRadiationUsage.setter
    def RTRadiationUsage(self, value: Optional[str]):
        if value is None:
            if "RTRadiationUsage" in self._dataset:
                del self._dataset.RTRadiationUsage
        else:
            self._dataset.RTRadiationUsage = value
    ...
    @property
    def TreatmentRecordContentOrigin(self) -> Optional[str]:
        if "TreatmentRecordContentOrigin" in self._dataset:
            return self._dataset.TreatmentRecordContentOrigin
        return None

    @TreatmentRecordContentOrigin.setter
    def TreatmentRecordContentOrigin(self, value: Optional[str]):
        if value is None:
            if "TreatmentRecordContentOrigin" in self._dataset:
                del self._dataset.TreatmentRecordContentOrigin
        else:
            self._dataset.TreatmentRecordContentOrigin = value

    @property
    def RTTreatmentTerminationStatus(self) -> Optional[str]:
        if "RTTreatmentTerminationStatus" in self._dataset:
            return self._dataset.RTTreatmentTerminationStatus
        return None

    @RTTreatmentTerminationStatus.setter
    def RTTreatmentTerminationStatus(self, value: Optional[str]):
        if value is None:
            if "RTTreatmentTerminationStatus" in self._dataset:
                del self._dataset.RTTreatmentTerminationStatus
        else:
            self._dataset.RTTreatmentTerminationStatus = value
    ...
    @property
    def TreatmentToleranceViolationSequence(self) -> Optional[List[TreatmentToleranceViolationSequenceItem]]:
        if "TreatmentToleranceViolationSequence" in self._dataset:
            if len(self._TreatmentToleranceViolationSequence) == len(self._dataset.TreatmentToleranceViolationSequence):
                return self._TreatmentToleranceViolationSequence
            else:
                return [TreatmentToleranceViolationSequenceItem(x) for x in self._dataset.TreatmentToleranceViolationSequence]
        return None

    @TreatmentToleranceViolationSequence.setter
    def TreatmentToleranceViolationSequence(self, value: Optional[List[TreatmentToleranceViolationSequenceItem]]):
        if value is None:
            self._TreatmentToleranceViolationSequence = []
            if "TreatmentToleranceViolationSequence" in self._dataset:
                del self._dataset.TreatmentToleranceViolationSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, TreatmentToleranceViolationSequenceItem) for item in value
        ):
            raise ValueError(
                f"TreatmentToleranceViolationSequence must be a list of TreatmentToleranceViolationSequenceItem objects"
            )
        else:
            self._TreatmentToleranceViolationSequence = value
            if "TreatmentToleranceViolationSequence" not in self._dataset:
                self._dataset.TreatmentToleranceViolationSequence = pydicom.Sequence()
            self._dataset.TreatmentToleranceViolationSequence.clear()
            self._dataset.TreatmentToleranceViolationSequence.extend([item.to_dataset() for item in value])

    def add_TreatmentToleranceViolation(self, item: TreatmentToleranceViolationSequenceItem):
        if not isinstance(item, TreatmentToleranceViolationSequenceItem):
            raise ValueError(f"Item must be an instance of TreatmentToleranceViolationSequenceItem")
        self._TreatmentToleranceViolationSequence.append(item)
        if "TreatmentToleranceViolationSequence" not in self._dataset:
            self._dataset.TreatmentToleranceViolationSequence = pydicom.Sequence()
        self._dataset.TreatmentToleranceViolationSequence.append(item.to_dataset())

    @property
    def ConfirmationSequence(self) -> Optional[List[ConfirmationSequenceItem]]:
        if "ConfirmationSequence" in self._dataset:
            if len(self._ConfirmationSequence) == len(self._dataset.ConfirmationSequence):
                return self._ConfirmationSequence
            else:
                return [ConfirmationSequenceItem(x) for x in self._dataset.ConfirmationSequence]
        return None

    @ConfirmationSequence.setter
    def ConfirmationSequence(self, value: Optional[List[ConfirmationSequenceItem]]):
        if value is None:
            self._ConfirmationSequence = []
            if "ConfirmationSequence" in self._dataset:
                del self._dataset.ConfirmationSequence
        elif not isinstance(value, list) or not all(isinstance(item, ConfirmationSequenceItem) for item in value):
            raise ValueError(f"ConfirmationSequence must be a list of ConfirmationSequenceItem objects")
        else:
            self._ConfirmationSequence = value
            if "ConfirmationSequence" not in self._dataset:
                self._dataset.ConfirmationSequence = pydicom.Sequence()
            self._dataset.ConfirmationSequence.clear()
            self._dataset.ConfirmationSequence.extend([item.to_dataset() for item in value])

    def add_Confirmation(self, item: ConfirmationSequenceItem):
        if not isinstance(item, ConfirmationSequenceItem):
            raise ValueError(f"Item must be an instance of ConfirmationSequenceItem")
        self._ConfirmationSequence.append(item)
        if "ConfirmationSequence" not in self._dataset:
            self._dataset.ConfirmationSequence = pydicom.Sequence()
        self._dataset.ConfirmationSequence.append(item.to_dataset())
    ...
    def InterlockSequence(self, value: Optional[List[InterlockSequenceItem]]):
        if value is None:
            ...
            raise ValueError(f"InterlockSequence must be a list of InterlockSequenceItem objects")
        else:
    ...
    def add_Interlock(self, item: InterlockSequenceItem):
        if not isinstance(item, InterlockSequenceItem):
            raise ValueError(f"Item must be an instance of InterlockSequenceItem")
    ...
    @property
    def AdditionalParameterRecordingInstanceSequence(self) -> Optional[List[AdditionalParameterRecordingInstanceSequenceItem]]:
        if "AdditionalParameterRecordingInstanceSequence" in self._dataset:
            if len(self._AdditionalParameterRecordingInstanceSequence) == len(
                self._dataset.AdditionalParameterRecordingInstanceSequence
            ):
                return self._AdditionalParameterRecordingInstanceSequence
            else:
                return [
                    AdditionalParameterRecordingInstanceSequenceItem(x)
                    for x in self._dataset.AdditionalParameterRecordingInstanceSequence
                ]
        return None

    @AdditionalParameterRecordingInstanceSequence.setter
    def AdditionalParameterRecordingInstanceSequence(
        self, value: Optional[List[AdditionalParameterRecordingInstanceSequenceItem]]
    ):
        if value is None:
            self._AdditionalParameterRecordingInstanceSequence = []
            if "AdditionalParameterRecordingInstanceSequence" in self._dataset:
                del self._dataset.AdditionalParameterRecordingInstanceSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, AdditionalParameterRecordingInstanceSequenceItem) for item in value
        ):
            raise ValueError(
                f"AdditionalParameterRecordingInstanceSequence must be a list of AdditionalParameterRecordingInstanceSequenceItem objects"
            )
        else:
            self._AdditionalParameterRecordingInstanceSequence = value
            if "AdditionalParameterRecordingInstanceSequence" not in self._dataset:
                self._dataset.AdditionalParameterRecordingInstanceSequence = pydicom.Sequence()
            self._dataset.AdditionalParameterRecordingInstanceSequence.clear()
            self._dataset.AdditionalParameterRecordingInstanceSequence.extend([item.to_dataset() for item in value])

    def add_AdditionalParameterRecordingInstance(self, item: AdditionalParameterRecordingInstanceSequenceItem):
        if not isinstance(item, AdditionalParameterRecordingInstanceSequenceItem):
            raise ValueError(f"Item must be an instance of AdditionalParameterRecordingInstanceSequenceItem")
        self._AdditionalParameterRecordingInstanceSequence.append(item)
        if "AdditionalParameterRecordingInstanceSequence" not in self._dataset:
            self._dataset.AdditionalParameterRecordingInstanceSequence = pydicom.Sequence()
        self._dataset.AdditionalParameterRecordingInstanceSequence.append(item.to_dataset())
    ...
    @property
    def UserContentLongLabel(self) -> Optional[str]:
        if "UserContentLongLabel" in self._dataset:
            return self._dataset.UserContentLongLabel
        return None

    @UserContentLongLabel.setter
    def UserContentLongLabel(self, value: Optional[str]):
        if value is None:
            if "UserContentLongLabel" in self._dataset:
                del self._dataset.UserContentLongLabel
        else:
            self._dataset.UserContentLongLabel = value
    ...
    def RTTreatmentTechniqueCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            ...
            raise ValueError(f"RTTreatmentTechniqueCodeSequence must be a list of CodeSequenceItem objects")
        else:
