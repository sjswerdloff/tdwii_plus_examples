...
from .compensator_shape_sequence_item import CompensatorShapeSequenceItem
from .udi_sequence_item import UDISequenceItem


class CompensatorDefinitionSequenceItem:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        self._dataset = dataset if dataset is not None else pydicom.Dataset()
        self._UDISequence: List[UDISequenceItem] = []
        self._CompensatorShapeSequence: List[CompensatorShapeSequenceItem] = []
    ...
    @property
    def CompensatorMapOrientation(self) -> Optional[str]:
        if "CompensatorMapOrientation" in self._dataset:
            return self._dataset.CompensatorMapOrientation
        return None

    @CompensatorMapOrientation.setter
    def CompensatorMapOrientation(self, value: Optional[str]):
        if value is None:
            if "CompensatorMapOrientation" in self._dataset:
                del self._dataset.CompensatorMapOrientation
        else:
            self._dataset.CompensatorMapOrientation = value

    @property
    def CompensatorBasePlaneOffset(self) -> Optional[float]:
        if "CompensatorBasePlaneOffset" in self._dataset:
            return self._dataset.CompensatorBasePlaneOffset
        return None

    @CompensatorBasePlaneOffset.setter
    def CompensatorBasePlaneOffset(self, value: Optional[float]):
        if value is None:
            if "CompensatorBasePlaneOffset" in self._dataset:
                del self._dataset.CompensatorBasePlaneOffset
        else:
            self._dataset.CompensatorBasePlaneOffset = value

    @property
    def CompensatorShapeSequence(self) -> Optional[List[CompensatorShapeSequenceItem]]:
        if "CompensatorShapeSequence" in self._dataset:
            if len(self._CompensatorShapeSequence) == len(self._dataset.CompensatorShapeSequence):
                return self._CompensatorShapeSequence
            else:
                return [CompensatorShapeSequenceItem(x) for x in self._dataset.CompensatorShapeSequence]
        return None

    @CompensatorShapeSequence.setter
    def CompensatorShapeSequence(self, value: Optional[List[CompensatorShapeSequenceItem]]):
        if value is None:
            self._CompensatorShapeSequence = []
            if "CompensatorShapeSequence" in self._dataset:
                del self._dataset.CompensatorShapeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CompensatorShapeSequenceItem) for item in value):
            raise ValueError(f"CompensatorShapeSequence must be a list of CompensatorShapeSequenceItem objects")
        else:
            self._CompensatorShapeSequence = value
            if "CompensatorShapeSequence" not in self._dataset:
                self._dataset.CompensatorShapeSequence = pydicom.Sequence()
            self._dataset.CompensatorShapeSequence.clear()
            self._dataset.CompensatorShapeSequence.extend([item.to_dataset() for item in value])

    def add_CompensatorShape(self, item: CompensatorShapeSequenceItem):
        if not isinstance(item, CompensatorShapeSequenceItem):
            raise ValueError(f"Item must be an instance of CompensatorShapeSequenceItem")
        self._CompensatorShapeSequence.append(item)
        if "CompensatorShapeSequence" not in self._dataset:
            self._dataset.CompensatorShapeSequence = pydicom.Sequence()
        self._dataset.CompensatorShapeSequence.append(item.to_dataset())
