...
class ComputedRadiographyImage:
    ...
    @property
    def CassetteOrientation(self) -> Optional[str]:
        if "CassetteOrientation" in self._dataset:
            return self._dataset.CassetteOrientation
        return None

    @CassetteOrientation.setter
    def CassetteOrientation(self, value: Optional[str]):
        if value is None:
            if "CassetteOrientation" in self._dataset:
                del self._dataset.CassetteOrientation
        else:
            self._dataset.CassetteOrientation = value

    @property
    def CassetteSize(self) -> Optional[str]:
        if "CassetteSize" in self._dataset:
            return self._dataset.CassetteSize
        return None

    @CassetteSize.setter
    def CassetteSize(self, value: Optional[str]):
        if value is None:
            if "CassetteSize" in self._dataset:
                del self._dataset.CassetteSize
        else:
            self._dataset.CassetteSize = value

    @property
    def ExposuresOnPlate(self) -> Optional[int]:
        if "ExposuresOnPlate" in self._dataset:
            return self._dataset.ExposuresOnPlate
        return None

    @ExposuresOnPlate.setter
    def ExposuresOnPlate(self, value: Optional[int]):
        if value is None:
            if "ExposuresOnPlate" in self._dataset:
                del self._dataset.ExposuresOnPlate
        else:
            self._dataset.ExposuresOnPlate = value
    ...
    @property
    def PlateType(self) -> Optional[str]:
        if "PlateType" in self._dataset:
            return self._dataset.PlateType
        return None

    @PlateType.setter
    def PlateType(self, value: Optional[str]):
        if value is None:
            if "PlateType" in self._dataset:
                del self._dataset.PlateType
        else:
            self._dataset.PlateType = value

    @property
    def PhosphorType(self) -> Optional[str]:
        if "PhosphorType" in self._dataset:
            return self._dataset.PhosphorType
        return None

    @PhosphorType.setter
    def PhosphorType(self, value: Optional[str]):
        if value is None:
            if "PhosphorType" in self._dataset:
                del self._dataset.PhosphorType
        else:
            self._dataset.PhosphorType = value
