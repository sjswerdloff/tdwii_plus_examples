...
class ConfocalMicroscopyTiledPyramidalImage:
    ...
    @property
    def ImageAcquisitionDepth(self) -> Optional[float]:
        if "ImageAcquisitionDepth" in self._dataset:
            return self._dataset.ImageAcquisitionDepth
        return None

    @ImageAcquisitionDepth.setter
    def ImageAcquisitionDepth(self, value: Optional[float]):
        if value is None:
            if "ImageAcquisitionDepth" in self._dataset:
                del self._dataset.ImageAcquisitionDepth
        else:
            self._dataset.ImageAcquisitionDepth = value
    ...
    @property
    def ConfocalMode(self) -> Optional[str]:
        if "ConfocalMode" in self._dataset:
            return self._dataset.ConfocalMode
        return None

    @ConfocalMode.setter
    def ConfocalMode(self, value: Optional[str]):
        if value is None:
            if "ConfocalMode" in self._dataset:
                del self._dataset.ConfocalMode
        else:
            self._dataset.ConfocalMode = value

    @property
    def TissueLocation(self) -> Optional[str]:
        if "TissueLocation" in self._dataset:
            return self._dataset.TissueLocation
        return None

    @TissueLocation.setter
    def TissueLocation(self, value: Optional[str]):
        if value is None:
            if "TissueLocation" in self._dataset:
                del self._dataset.TissueLocation
        else:
            self._dataset.TissueLocation = value
