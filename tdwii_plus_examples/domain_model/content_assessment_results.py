...
from .assessed_sop_instance_sequence_item import AssessedSOPInstanceSequenceItem
from .assessment_observations_sequence_item import AssessmentObservationsSequenceItem
from .assessment_requester_sequence_item import AssessmentRequesterSequenceItem
...
from .pertinent_resources_sequence_item import PertinentResourcesSequenceItem
...
class ContentAssessmentResults:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._PertinentResourcesSequence: List[PertinentResourcesSequenceItem] = []
        self._AssessedSOPInstanceSequence: List[AssessedSOPInstanceSequenceItem] = []
        self._AssessmentObservationsSequence: List[AssessmentObservationsSequenceItem] = []
        self._AssessmentRequesterSequence: List[AssessmentRequesterSequenceItem] = []
        self._AssessmentTypeCodeSequence: List[CodeSequenceItem] = []
    ...
    @property
    def PertinentResourcesSequence(self) -> Optional[List[PertinentResourcesSequenceItem]]:
        if "PertinentResourcesSequence" in self._dataset:
            if len(self._PertinentResourcesSequence) == len(self._dataset.PertinentResourcesSequence):
                return self._PertinentResourcesSequence
            else:
                return [PertinentResourcesSequenceItem(x) for x in self._dataset.PertinentResourcesSequence]
        return None

    @PertinentResourcesSequence.setter
    def PertinentResourcesSequence(self, value: Optional[List[PertinentResourcesSequenceItem]]):
        if value is None:
            self._PertinentResourcesSequence = []
            if "PertinentResourcesSequence" in self._dataset:
                del self._dataset.PertinentResourcesSequence
        elif not isinstance(value, list) or not all(isinstance(item, PertinentResourcesSequenceItem) for item in value):
            raise ValueError(f"PertinentResourcesSequence must be a list of PertinentResourcesSequenceItem objects")
        else:
            self._PertinentResourcesSequence = value
            if "PertinentResourcesSequence" not in self._dataset:
                self._dataset.PertinentResourcesSequence = pydicom.Sequence()
            self._dataset.PertinentResourcesSequence.clear()
            self._dataset.PertinentResourcesSequence.extend([item.to_dataset() for item in value])

    def add_PertinentResources(self, item: PertinentResourcesSequenceItem):
        if not isinstance(item, PertinentResourcesSequenceItem):
            raise ValueError(f"Item must be an instance of PertinentResourcesSequenceItem")
        self._PertinentResourcesSequence.append(item)
        if "PertinentResourcesSequence" not in self._dataset:
            self._dataset.PertinentResourcesSequence = pydicom.Sequence()
        self._dataset.PertinentResourcesSequence.append(item.to_dataset())

    @property
    def AssessmentSummary(self) -> Optional[str]:
        if "AssessmentSummary" in self._dataset:
            return self._dataset.AssessmentSummary
        return None

    @AssessmentSummary.setter
    def AssessmentSummary(self, value: Optional[str]):
        if value is None:
            if "AssessmentSummary" in self._dataset:
                del self._dataset.AssessmentSummary
        else:
            self._dataset.AssessmentSummary = value

    @property
    def AssessmentSummaryDescription(self) -> Optional[str]:
        if "AssessmentSummaryDescription" in self._dataset:
            return self._dataset.AssessmentSummaryDescription
        return None

    @AssessmentSummaryDescription.setter
    def AssessmentSummaryDescription(self, value: Optional[str]):
        if value is None:
            if "AssessmentSummaryDescription" in self._dataset:
                del self._dataset.AssessmentSummaryDescription
        else:
            self._dataset.AssessmentSummaryDescription = value

    @property
    def AssessedSOPInstanceSequence(self) -> Optional[List[AssessedSOPInstanceSequenceItem]]:
        if "AssessedSOPInstanceSequence" in self._dataset:
            if len(self._AssessedSOPInstanceSequence) == len(self._dataset.AssessedSOPInstanceSequence):
                return self._AssessedSOPInstanceSequence
            else:
                return [AssessedSOPInstanceSequenceItem(x) for x in self._dataset.AssessedSOPInstanceSequence]
        return None

    @AssessedSOPInstanceSequence.setter
    def AssessedSOPInstanceSequence(self, value: Optional[List[AssessedSOPInstanceSequenceItem]]):
        if value is None:
            self._AssessedSOPInstanceSequence = []
            if "AssessedSOPInstanceSequence" in self._dataset:
                del self._dataset.AssessedSOPInstanceSequence
        elif not isinstance(value, list) or not all(isinstance(item, AssessedSOPInstanceSequenceItem) for item in value):
            raise ValueError(f"AssessedSOPInstanceSequence must be a list of AssessedSOPInstanceSequenceItem objects")
        else:
            self._AssessedSOPInstanceSequence = value
            if "AssessedSOPInstanceSequence" not in self._dataset:
                self._dataset.AssessedSOPInstanceSequence = pydicom.Sequence()
            self._dataset.AssessedSOPInstanceSequence.clear()
            self._dataset.AssessedSOPInstanceSequence.extend([item.to_dataset() for item in value])

    def add_AssessedSOPInstance(self, item: AssessedSOPInstanceSequenceItem):
        if not isinstance(item, AssessedSOPInstanceSequenceItem):
            raise ValueError(f"Item must be an instance of AssessedSOPInstanceSequenceItem")
        self._AssessedSOPInstanceSequence.append(item)
        if "AssessedSOPInstanceSequence" not in self._dataset:
            self._dataset.AssessedSOPInstanceSequence = pydicom.Sequence()
        self._dataset.AssessedSOPInstanceSequence.append(item.to_dataset())

    @property
    def NumberOfAssessmentObservations(self) -> Optional[int]:
        if "NumberOfAssessmentObservations" in self._dataset:
            return self._dataset.NumberOfAssessmentObservations
        return None

    @NumberOfAssessmentObservations.setter
    def NumberOfAssessmentObservations(self, value: Optional[int]):
        if value is None:
            if "NumberOfAssessmentObservations" in self._dataset:
                del self._dataset.NumberOfAssessmentObservations
        else:
            self._dataset.NumberOfAssessmentObservations = value

    @property
    def AssessmentObservationsSequence(self) -> Optional[List[AssessmentObservationsSequenceItem]]:
        if "AssessmentObservationsSequence" in self._dataset:
            if len(self._AssessmentObservationsSequence) == len(self._dataset.AssessmentObservationsSequence):
                return self._AssessmentObservationsSequence
            else:
                return [AssessmentObservationsSequenceItem(x) for x in self._dataset.AssessmentObservationsSequence]
        return None

    @AssessmentObservationsSequence.setter
    def AssessmentObservationsSequence(self, value: Optional[List[AssessmentObservationsSequenceItem]]):
        if value is None:
            self._AssessmentObservationsSequence = []
            if "AssessmentObservationsSequence" in self._dataset:
                del self._dataset.AssessmentObservationsSequence
        elif not isinstance(value, list) or not all(isinstance(item, AssessmentObservationsSequenceItem) for item in value):
            raise ValueError(f"AssessmentObservationsSequence must be a list of AssessmentObservationsSequenceItem objects")
        else:
            self._AssessmentObservationsSequence = value
            if "AssessmentObservationsSequence" not in self._dataset:
                self._dataset.AssessmentObservationsSequence = pydicom.Sequence()
            self._dataset.AssessmentObservationsSequence.clear()
            self._dataset.AssessmentObservationsSequence.extend([item.to_dataset() for item in value])

    def add_AssessmentObservations(self, item: AssessmentObservationsSequenceItem):
        if not isinstance(item, AssessmentObservationsSequenceItem):
            raise ValueError(f"Item must be an instance of AssessmentObservationsSequenceItem")
        self._AssessmentObservationsSequence.append(item)
        if "AssessmentObservationsSequence" not in self._dataset:
            self._dataset.AssessmentObservationsSequence = pydicom.Sequence()
        self._dataset.AssessmentObservationsSequence.append(item.to_dataset())

    @property
    def AssessmentSetID(self) -> Optional[str]:
        if "AssessmentSetID" in self._dataset:
            return self._dataset.AssessmentSetID
        return None

    @AssessmentSetID.setter
    def AssessmentSetID(self, value: Optional[str]):
        if value is None:
            if "AssessmentSetID" in self._dataset:
                del self._dataset.AssessmentSetID
        else:
            self._dataset.AssessmentSetID = value

    @property
    def AssessmentRequesterSequence(self) -> Optional[List[AssessmentRequesterSequenceItem]]:
        if "AssessmentRequesterSequence" in self._dataset:
            if len(self._AssessmentRequesterSequence) == len(self._dataset.AssessmentRequesterSequence):
                return self._AssessmentRequesterSequence
            else:
                return [AssessmentRequesterSequenceItem(x) for x in self._dataset.AssessmentRequesterSequence]
        return None

    @AssessmentRequesterSequence.setter
    def AssessmentRequesterSequence(self, value: Optional[List[AssessmentRequesterSequenceItem]]):
        if value is None:
            self._AssessmentRequesterSequence = []
            if "AssessmentRequesterSequence" in self._dataset:
                del self._dataset.AssessmentRequesterSequence
        elif not isinstance(value, list) or not all(isinstance(item, AssessmentRequesterSequenceItem) for item in value):
            raise ValueError(f"AssessmentRequesterSequence must be a list of AssessmentRequesterSequenceItem objects")
        else:
            self._AssessmentRequesterSequence = value
            if "AssessmentRequesterSequence" not in self._dataset:
                self._dataset.AssessmentRequesterSequence = pydicom.Sequence()
            self._dataset.AssessmentRequesterSequence.clear()
            self._dataset.AssessmentRequesterSequence.extend([item.to_dataset() for item in value])

    def add_AssessmentRequester(self, item: AssessmentRequesterSequenceItem):
        if not isinstance(item, AssessmentRequesterSequenceItem):
            raise ValueError(f"Item must be an instance of AssessmentRequesterSequenceItem")
        self._AssessmentRequesterSequence.append(item)
        if "AssessmentRequesterSequence" not in self._dataset:
            self._dataset.AssessmentRequesterSequence = pydicom.Sequence()
        self._dataset.AssessmentRequesterSequence.append(item.to_dataset())

    @property
    def AssessmentTypeCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "AssessmentTypeCodeSequence" in self._dataset:
            if len(self._AssessmentTypeCodeSequence) == len(self._dataset.AssessmentTypeCodeSequence):
                return self._AssessmentTypeCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.AssessmentTypeCodeSequence]
        return None

    @AssessmentTypeCodeSequence.setter
    def AssessmentTypeCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._AssessmentTypeCodeSequence = []
            if "AssessmentTypeCodeSequence" in self._dataset:
                del self._dataset.AssessmentTypeCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"AssessmentTypeCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._AssessmentTypeCodeSequence = value
            if "AssessmentTypeCodeSequence" not in self._dataset:
                self._dataset.AssessmentTypeCodeSequence = pydicom.Sequence()
            self._dataset.AssessmentTypeCodeSequence.clear()
            self._dataset.AssessmentTypeCodeSequence.extend([item.to_dataset() for item in value])

    def add_AssessmentTypeCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._AssessmentTypeCodeSequence.append(item)
        if "AssessmentTypeCodeSequence" not in self._dataset:
            self._dataset.AssessmentTypeCodeSequence = pydicom.Sequence()
        self._dataset.AssessmentTypeCodeSequence.append(item.to_dataset())

    @property
    def AssessmentLabel(self) -> Optional[str]:
        if "AssessmentLabel" in self._dataset:
            return self._dataset.AssessmentLabel
        return None

    @AssessmentLabel.setter
    def AssessmentLabel(self, value: Optional[str]):
        if value is None:
            if "AssessmentLabel" in self._dataset:
                del self._dataset.AssessmentLabel
        else:
            self._dataset.AssessmentLabel = value
