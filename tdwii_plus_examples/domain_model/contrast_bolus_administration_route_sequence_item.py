...
from .additional_drug_sequence_item import AdditionalDrugSequenceItem
from .code_sequence_item import CodeSequenceItem


class ContrastBolusAdministrationRouteSequenceItem:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        self._dataset = dataset if dataset is not None else pydicom.Dataset()
        self._EquivalentCodeSequence: List[CodeSequenceItem] = []
        self._AdditionalDrugSequence: List[AdditionalDrugSequenceItem] = []
    ...
    @property
    def AdditionalDrugSequence(self) -> Optional[List[AdditionalDrugSequenceItem]]:
        if "AdditionalDrugSequence" in self._dataset:
            if len(self._AdditionalDrugSequence) == len(self._dataset.AdditionalDrugSequence):
                return self._AdditionalDrugSequence
            else:
                return [AdditionalDrugSequenceItem(x) for x in self._dataset.AdditionalDrugSequence]
        return None

    @AdditionalDrugSequence.setter
    def AdditionalDrugSequence(self, value: Optional[List[AdditionalDrugSequenceItem]]):
        if value is None:
            self._AdditionalDrugSequence = []
            if "AdditionalDrugSequence" in self._dataset:
            ...
            self._dataset.AdditionalDrugSequence.clear()
            self._dataset.AdditionalDrugSequence.extend([item.to_dataset() for item in value])
    ...
    def add_AdditionalDrug(self, item: AdditionalDrugSequenceItem):
        ...
        if "AdditionalDrugSequence" not in self._dataset:
            self._dataset.AdditionalDrugSequence = pydicom.Sequence()
        self._dataset.AdditionalDrugSequence.append(item.to_dataset())
