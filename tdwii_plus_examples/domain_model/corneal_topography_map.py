...
from .corneal_topography_mapping_normals_sequence_item import (
    CornealTopographyMappingNormalsSequenceItem,
)
...
from .minimum_keratometric_sequence_item import MinimumKeratometricSequenceItem
...
from .simulated_keratometric_cylinder_sequence_item import (
    SimulatedKeratometricCylinderSequenceItem,
)
from .source_image_corneal_processed_data_sequence_item import (
...
class CornealTopographyMap:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._CornealTopographyMapTypeCodeSequence: List[CodeSequenceItem] = []
        self._CornealTopographyMappingNormalsSequence: List[CornealTopographyMappingNormalsSequenceItem] = []
        self._MaximumCornealCurvatureSequence: List[MaximumCornealCurvatureSequenceItem] = []
        self._MinimumKeratometricSequence: List[MinimumKeratometricSequenceItem] = []
        self._SimulatedKeratometricCylinderSequence: List[SimulatedKeratometricCylinderSequenceItem] = []
        self._SourceImageCornealProcessedDataSequence: List[SourceImageCornealProcessedDataSequenceItem] = []
    ...
    @property
    def CornealTopographySurface(self) -> Optional[str]:
        if "CornealTopographySurface" in self._dataset:
            return self._dataset.CornealTopographySurface
        return None

    @CornealTopographySurface.setter
    def CornealTopographySurface(self, value: Optional[str]):
        if value is None:
            if "CornealTopographySurface" in self._dataset:
                del self._dataset.CornealTopographySurface
        else:
            self._dataset.CornealTopographySurface = value

    @property
    def CornealVertexLocation(self) -> Optional[List[float]]:
        if "CornealVertexLocation" in self._dataset:
            return self._dataset.CornealVertexLocation
        return None

    @CornealVertexLocation.setter
    def CornealVertexLocation(self, value: Optional[List[float]]):
        if value is None:
            if "CornealVertexLocation" in self._dataset:
                del self._dataset.CornealVertexLocation
        else:
            self._dataset.CornealVertexLocation = value

    @property
    def PupilCentroidXCoordinate(self) -> Optional[float]:
        if "PupilCentroidXCoordinate" in self._dataset:
            return self._dataset.PupilCentroidXCoordinate
        return None

    @PupilCentroidXCoordinate.setter
    def PupilCentroidXCoordinate(self, value: Optional[float]):
        if value is None:
            if "PupilCentroidXCoordinate" in self._dataset:
                del self._dataset.PupilCentroidXCoordinate
        else:
            self._dataset.PupilCentroidXCoordinate = value

    @property
    def PupilCentroidYCoordinate(self) -> Optional[float]:
        if "PupilCentroidYCoordinate" in self._dataset:
            return self._dataset.PupilCentroidYCoordinate
        return None

    @PupilCentroidYCoordinate.setter
    def PupilCentroidYCoordinate(self, value: Optional[float]):
        if value is None:
            if "PupilCentroidYCoordinate" in self._dataset:
                del self._dataset.PupilCentroidYCoordinate
        else:
            self._dataset.PupilCentroidYCoordinate = value

    @property
    def EquivalentPupilRadius(self) -> Optional[float]:
        if "EquivalentPupilRadius" in self._dataset:
            return self._dataset.EquivalentPupilRadius
        return None

    @EquivalentPupilRadius.setter
    def EquivalentPupilRadius(self, value: Optional[float]):
        if value is None:
            if "EquivalentPupilRadius" in self._dataset:
                del self._dataset.EquivalentPupilRadius
        else:
            self._dataset.EquivalentPupilRadius = value

    @property
    def CornealTopographyMapTypeCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "CornealTopographyMapTypeCodeSequence" in self._dataset:
            if len(self._CornealTopographyMapTypeCodeSequence) == len(self._dataset.CornealTopographyMapTypeCodeSequence):
                return self._CornealTopographyMapTypeCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.CornealTopographyMapTypeCodeSequence]
        return None

    @CornealTopographyMapTypeCodeSequence.setter
    def CornealTopographyMapTypeCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._CornealTopographyMapTypeCodeSequence = []
            if "CornealTopographyMapTypeCodeSequence" in self._dataset:
                del self._dataset.CornealTopographyMapTypeCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"CornealTopographyMapTypeCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._CornealTopographyMapTypeCodeSequence = value
            if "CornealTopographyMapTypeCodeSequence" not in self._dataset:
                self._dataset.CornealTopographyMapTypeCodeSequence = pydicom.Sequence()
            self._dataset.CornealTopographyMapTypeCodeSequence.clear()
            self._dataset.CornealTopographyMapTypeCodeSequence.extend([item.to_dataset() for item in value])

    def add_CornealTopographyMapTypeCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._CornealTopographyMapTypeCodeSequence.append(item)
        if "CornealTopographyMapTypeCodeSequence" not in self._dataset:
            self._dataset.CornealTopographyMapTypeCodeSequence = pydicom.Sequence()
        self._dataset.CornealTopographyMapTypeCodeSequence.append(item.to_dataset())

    @property
    def VerticesOfTheOutlineOfPupil(self) -> Optional[List[int]]:
        if "VerticesOfTheOutlineOfPupil" in self._dataset:
            return self._dataset.VerticesOfTheOutlineOfPupil
        return None

    @VerticesOfTheOutlineOfPupil.setter
    def VerticesOfTheOutlineOfPupil(self, value: Optional[List[int]]):
        if value is None:
            if "VerticesOfTheOutlineOfPupil" in self._dataset:
                del self._dataset.VerticesOfTheOutlineOfPupil
        else:
            self._dataset.VerticesOfTheOutlineOfPupil = value

    @property
    def CornealTopographyMappingNormalsSequence(self) -> Optional[List[CornealTopographyMappingNormalsSequenceItem]]:
        if "CornealTopographyMappingNormalsSequence" in self._dataset:
            if len(self._CornealTopographyMappingNormalsSequence) == len(
                self._dataset.CornealTopographyMappingNormalsSequence
            ):
                return self._CornealTopographyMappingNormalsSequence
            else:
                return [
                    CornealTopographyMappingNormalsSequenceItem(x)
                    for x in self._dataset.CornealTopographyMappingNormalsSequence
                ]
        return None

    @CornealTopographyMappingNormalsSequence.setter
    def CornealTopographyMappingNormalsSequence(self, value: Optional[List[CornealTopographyMappingNormalsSequenceItem]]):
        if value is None:
            self._CornealTopographyMappingNormalsSequence = []
            if "CornealTopographyMappingNormalsSequence" in self._dataset:
                del self._dataset.CornealTopographyMappingNormalsSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, CornealTopographyMappingNormalsSequenceItem) for item in value
        ):
            raise ValueError(
                f"CornealTopographyMappingNormalsSequence must be a list of CornealTopographyMappingNormalsSequenceItem objects"
            )
        else:
            self._CornealTopographyMappingNormalsSequence = value
            if "CornealTopographyMappingNormalsSequence" not in self._dataset:
                self._dataset.CornealTopographyMappingNormalsSequence = pydicom.Sequence()
            self._dataset.CornealTopographyMappingNormalsSequence.clear()
            self._dataset.CornealTopographyMappingNormalsSequence.extend([item.to_dataset() for item in value])

    def add_CornealTopographyMappingNormals(self, item: CornealTopographyMappingNormalsSequenceItem):
        if not isinstance(item, CornealTopographyMappingNormalsSequenceItem):
            raise ValueError(f"Item must be an instance of CornealTopographyMappingNormalsSequenceItem")
        self._CornealTopographyMappingNormalsSequence.append(item)
        if "CornealTopographyMappingNormalsSequence" not in self._dataset:
            self._dataset.CornealTopographyMappingNormalsSequence = pydicom.Sequence()
        self._dataset.CornealTopographyMappingNormalsSequence.append(item.to_dataset())

    @property
    def MaximumCornealCurvatureSequence(self) -> Optional[List[MaximumCornealCurvatureSequenceItem]]:
        if "MaximumCornealCurvatureSequence" in self._dataset:
            if len(self._MaximumCornealCurvatureSequence) == len(self._dataset.MaximumCornealCurvatureSequence):
                return self._MaximumCornealCurvatureSequence
            else:
                return [MaximumCornealCurvatureSequenceItem(x) for x in self._dataset.MaximumCornealCurvatureSequence]
        return None

    @MaximumCornealCurvatureSequence.setter
    def MaximumCornealCurvatureSequence(self, value: Optional[List[MaximumCornealCurvatureSequenceItem]]):
        if value is None:
            self._MaximumCornealCurvatureSequence = []
            if "MaximumCornealCurvatureSequence" in self._dataset:
                del self._dataset.MaximumCornealCurvatureSequence
        elif not isinstance(value, list) or not all(isinstance(item, MaximumCornealCurvatureSequenceItem) for item in value):
            raise ValueError(f"MaximumCornealCurvatureSequence must be a list of MaximumCornealCurvatureSequenceItem objects")
        else:
            self._MaximumCornealCurvatureSequence = value
            if "MaximumCornealCurvatureSequence" not in self._dataset:
                self._dataset.MaximumCornealCurvatureSequence = pydicom.Sequence()
            self._dataset.MaximumCornealCurvatureSequence.clear()
            self._dataset.MaximumCornealCurvatureSequence.extend([item.to_dataset() for item in value])

    def add_MaximumCornealCurvature(self, item: MaximumCornealCurvatureSequenceItem):
        if not isinstance(item, MaximumCornealCurvatureSequenceItem):
            raise ValueError(f"Item must be an instance of MaximumCornealCurvatureSequenceItem")
        self._MaximumCornealCurvatureSequence.append(item)
        if "MaximumCornealCurvatureSequence" not in self._dataset:
            self._dataset.MaximumCornealCurvatureSequence = pydicom.Sequence()
        self._dataset.MaximumCornealCurvatureSequence.append(item.to_dataset())

    @property
    def MinimumKeratometricSequence(self) -> Optional[List[MinimumKeratometricSequenceItem]]:
        if "MinimumKeratometricSequence" in self._dataset:
            if len(self._MinimumKeratometricSequence) == len(self._dataset.MinimumKeratometricSequence):
                return self._MinimumKeratometricSequence
            else:
                return [MinimumKeratometricSequenceItem(x) for x in self._dataset.MinimumKeratometricSequence]
        return None

    @MinimumKeratometricSequence.setter
    def MinimumKeratometricSequence(self, value: Optional[List[MinimumKeratometricSequenceItem]]):
        if value is None:
            self._MinimumKeratometricSequence = []
            if "MinimumKeratometricSequence" in self._dataset:
                del self._dataset.MinimumKeratometricSequence
        elif not isinstance(value, list) or not all(isinstance(item, MinimumKeratometricSequenceItem) for item in value):
            raise ValueError(f"MinimumKeratometricSequence must be a list of MinimumKeratometricSequenceItem objects")
        else:
            self._MinimumKeratometricSequence = value
            if "MinimumKeratometricSequence" not in self._dataset:
                self._dataset.MinimumKeratometricSequence = pydicom.Sequence()
            self._dataset.MinimumKeratometricSequence.clear()
            self._dataset.MinimumKeratometricSequence.extend([item.to_dataset() for item in value])

    def add_MinimumKeratometric(self, item: MinimumKeratometricSequenceItem):
        if not isinstance(item, MinimumKeratometricSequenceItem):
            raise ValueError(f"Item must be an instance of MinimumKeratometricSequenceItem")
        self._MinimumKeratometricSequence.append(item)
        if "MinimumKeratometricSequence" not in self._dataset:
            self._dataset.MinimumKeratometricSequence = pydicom.Sequence()
        self._dataset.MinimumKeratometricSequence.append(item.to_dataset())

    @property
    def SimulatedKeratometricCylinderSequence(self) -> Optional[List[SimulatedKeratometricCylinderSequenceItem]]:
        if "SimulatedKeratometricCylinderSequence" in self._dataset:
            if len(self._SimulatedKeratometricCylinderSequence) == len(self._dataset.SimulatedKeratometricCylinderSequence):
                return self._SimulatedKeratometricCylinderSequence
            else:
                return [
                    SimulatedKeratometricCylinderSequenceItem(x) for x in self._dataset.SimulatedKeratometricCylinderSequence
                ]
        return None

    @SimulatedKeratometricCylinderSequence.setter
    def SimulatedKeratometricCylinderSequence(self, value: Optional[List[SimulatedKeratometricCylinderSequenceItem]]):
        if value is None:
            self._SimulatedKeratometricCylinderSequence = []
            if "SimulatedKeratometricCylinderSequence" in self._dataset:
                del self._dataset.SimulatedKeratometricCylinderSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, SimulatedKeratometricCylinderSequenceItem) for item in value
        ):
            raise ValueError(
                f"SimulatedKeratometricCylinderSequence must be a list of SimulatedKeratometricCylinderSequenceItem objects"
            )
        else:
            self._SimulatedKeratometricCylinderSequence = value
            if "SimulatedKeratometricCylinderSequence" not in self._dataset:
                self._dataset.SimulatedKeratometricCylinderSequence = pydicom.Sequence()
            self._dataset.SimulatedKeratometricCylinderSequence.clear()
            self._dataset.SimulatedKeratometricCylinderSequence.extend([item.to_dataset() for item in value])

    def add_SimulatedKeratometricCylinder(self, item: SimulatedKeratometricCylinderSequenceItem):
        if not isinstance(item, SimulatedKeratometricCylinderSequenceItem):
            raise ValueError(f"Item must be an instance of SimulatedKeratometricCylinderSequenceItem")
        self._SimulatedKeratometricCylinderSequence.append(item)
        if "SimulatedKeratometricCylinderSequence" not in self._dataset:
            self._dataset.SimulatedKeratometricCylinderSequence = pydicom.Sequence()
        self._dataset.SimulatedKeratometricCylinderSequence.append(item.to_dataset())

    @property
    def AverageCornealPower(self) -> Optional[float]:
        if "AverageCornealPower" in self._dataset:
            return self._dataset.AverageCornealPower
        return None

    @AverageCornealPower.setter
    def AverageCornealPower(self, value: Optional[float]):
        if value is None:
            if "AverageCornealPower" in self._dataset:
                del self._dataset.AverageCornealPower
        else:
            self._dataset.AverageCornealPower = value

    @property
    def CornealISValue(self) -> Optional[float]:
        if "CornealISValue" in self._dataset:
            return self._dataset.CornealISValue
        return None

    @CornealISValue.setter
    def CornealISValue(self, value: Optional[float]):
        if value is None:
            if "CornealISValue" in self._dataset:
                del self._dataset.CornealISValue
        else:
            self._dataset.CornealISValue = value

    @property
    def AnalyzedArea(self) -> Optional[float]:
        if "AnalyzedArea" in self._dataset:
            return self._dataset.AnalyzedArea
        return None

    @AnalyzedArea.setter
    def AnalyzedArea(self, value: Optional[float]):
        if value is None:
            if "AnalyzedArea" in self._dataset:
                del self._dataset.AnalyzedArea
        else:
            self._dataset.AnalyzedArea = value

    @property
    def SurfaceRegularityIndex(self) -> Optional[float]:
        if "SurfaceRegularityIndex" in self._dataset:
            return self._dataset.SurfaceRegularityIndex
        return None

    @SurfaceRegularityIndex.setter
    def SurfaceRegularityIndex(self, value: Optional[float]):
        if value is None:
            if "SurfaceRegularityIndex" in self._dataset:
                del self._dataset.SurfaceRegularityIndex
        else:
            self._dataset.SurfaceRegularityIndex = value

    @property
    def SurfaceAsymmetryIndex(self) -> Optional[float]:
        if "SurfaceAsymmetryIndex" in self._dataset:
            return self._dataset.SurfaceAsymmetryIndex
        return None

    @SurfaceAsymmetryIndex.setter
    def SurfaceAsymmetryIndex(self, value: Optional[float]):
        if value is None:
            if "SurfaceAsymmetryIndex" in self._dataset:
                del self._dataset.SurfaceAsymmetryIndex
        else:
            self._dataset.SurfaceAsymmetryIndex = value

    @property
    def CornealEccentricityIndex(self) -> Optional[float]:
        if "CornealEccentricityIndex" in self._dataset:
            return self._dataset.CornealEccentricityIndex
        return None

    @CornealEccentricityIndex.setter
    def CornealEccentricityIndex(self, value: Optional[float]):
        if value is None:
            if "CornealEccentricityIndex" in self._dataset:
                del self._dataset.CornealEccentricityIndex
        else:
            self._dataset.CornealEccentricityIndex = value

    @property
    def KeratoconusPredictionIndex(self) -> Optional[float]:
        if "KeratoconusPredictionIndex" in self._dataset:
            return self._dataset.KeratoconusPredictionIndex
        return None

    @KeratoconusPredictionIndex.setter
    def KeratoconusPredictionIndex(self, value: Optional[float]):
        if value is None:
            if "KeratoconusPredictionIndex" in self._dataset:
                del self._dataset.KeratoconusPredictionIndex
        else:
            self._dataset.KeratoconusPredictionIndex = value

    @property
    def DecimalPotentialVisualAcuity(self) -> Optional[float]:
        if "DecimalPotentialVisualAcuity" in self._dataset:
            return self._dataset.DecimalPotentialVisualAcuity
        return None

    @DecimalPotentialVisualAcuity.setter
    def DecimalPotentialVisualAcuity(self, value: Optional[float]):
        if value is None:
            if "DecimalPotentialVisualAcuity" in self._dataset:
                del self._dataset.DecimalPotentialVisualAcuity
        else:
            self._dataset.DecimalPotentialVisualAcuity = value

    @property
    def CornealTopographyMapQualityEvaluation(self) -> Optional[str]:
        if "CornealTopographyMapQualityEvaluation" in self._dataset:
            return self._dataset.CornealTopographyMapQualityEvaluation
        return None

    @CornealTopographyMapQualityEvaluation.setter
    def CornealTopographyMapQualityEvaluation(self, value: Optional[str]):
        if value is None:
            if "CornealTopographyMapQualityEvaluation" in self._dataset:
                del self._dataset.CornealTopographyMapQualityEvaluation
        else:
            self._dataset.CornealTopographyMapQualityEvaluation = value

    @property
    def SourceImageCornealProcessedDataSequence(self) -> Optional[List[SourceImageCornealProcessedDataSequenceItem]]:
        if "SourceImageCornealProcessedDataSequence" in self._dataset:
            if len(self._SourceImageCornealProcessedDataSequence) == len(
                self._dataset.SourceImageCornealProcessedDataSequence
            ):
                return self._SourceImageCornealProcessedDataSequence
            else:
                return [
                    SourceImageCornealProcessedDataSequenceItem(x)
                    for x in self._dataset.SourceImageCornealProcessedDataSequence
                ]
        return None

    @SourceImageCornealProcessedDataSequence.setter
    def SourceImageCornealProcessedDataSequence(self, value: Optional[List[SourceImageCornealProcessedDataSequenceItem]]):
        if value is None:
            self._SourceImageCornealProcessedDataSequence = []
            if "SourceImageCornealProcessedDataSequence" in self._dataset:
                del self._dataset.SourceImageCornealProcessedDataSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, SourceImageCornealProcessedDataSequenceItem) for item in value
        ):
            raise ValueError(
                f"SourceImageCornealProcessedDataSequence must be a list of SourceImageCornealProcessedDataSequenceItem objects"
            )
        else:
            self._SourceImageCornealProcessedDataSequence = value
            if "SourceImageCornealProcessedDataSequence" not in self._dataset:
                self._dataset.SourceImageCornealProcessedDataSequence = pydicom.Sequence()
            self._dataset.SourceImageCornealProcessedDataSequence.clear()
            self._dataset.SourceImageCornealProcessedDataSequence.extend([item.to_dataset() for item in value])

    def add_SourceImageCornealProcessedData(self, item: SourceImageCornealProcessedDataSequenceItem):
        if not isinstance(item, SourceImageCornealProcessedDataSequenceItem):
            raise ValueError(f"Item must be an instance of SourceImageCornealProcessedDataSequenceItem")
        self._SourceImageCornealProcessedDataSequence.append(item)
        if "SourceImageCornealProcessedDataSequence" not in self._dataset:
            self._dataset.SourceImageCornealProcessedDataSequence = pydicom.Sequence()
        self._dataset.SourceImageCornealProcessedDataSequence.append(item.to_dataset())
