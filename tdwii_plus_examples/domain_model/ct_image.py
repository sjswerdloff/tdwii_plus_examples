...
from .multienergy_ct_acquisition_sequence_item import (
    MultienergyCTAcquisitionSequenceItem,
)
...
class CtImage:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._MultienergyCTAcquisitionSequence: List[MultienergyCTAcquisitionSequenceItem] = []
    ...
    @property
    def CalciumScoringMassFactorPatient(self) -> Optional[float]:
        if "CalciumScoringMassFactorPatient" in self._dataset:
            return self._dataset.CalciumScoringMassFactorPatient
        return None

    @CalciumScoringMassFactorPatient.setter
    def CalciumScoringMassFactorPatient(self, value: Optional[float]):
        if value is None:
            if "CalciumScoringMassFactorPatient" in self._dataset:
                del self._dataset.CalciumScoringMassFactorPatient
        else:
            self._dataset.CalciumScoringMassFactorPatient = value

    @property
    def CalciumScoringMassFactorDevice(self) -> Optional[List[float]]:
        if "CalciumScoringMassFactorDevice" in self._dataset:
            return self._dataset.CalciumScoringMassFactorDevice
        return None

    @CalciumScoringMassFactorDevice.setter
    def CalciumScoringMassFactorDevice(self, value: Optional[List[float]]):
        if value is None:
            if "CalciumScoringMassFactorDevice" in self._dataset:
                del self._dataset.CalciumScoringMassFactorDevice
        else:
            self._dataset.CalciumScoringMassFactorDevice = value
    ...
    def CTAdditionalXRaySourceSequence(self, value: Optional[List[CTAdditionalXRaySourceSequenceItem]]):
        if value is None:
            ...
            raise ValueError("CTAdditionalXRaySourceSequence must be a list of CTAdditionalXRaySourceSequenceItem objects")
        else:
    ...
    @property
    def MultienergyCTAcquisitionSequence(self) -> Optional[List[MultienergyCTAcquisitionSequenceItem]]:
        if "MultienergyCTAcquisitionSequence" in self._dataset:
            if len(self._MultienergyCTAcquisitionSequence) == len(self._dataset.MultienergyCTAcquisitionSequence):
                return self._MultienergyCTAcquisitionSequence
            else:
                return [MultienergyCTAcquisitionSequenceItem(x) for x in self._dataset.MultienergyCTAcquisitionSequence]
        return None

    @MultienergyCTAcquisitionSequence.setter
    def MultienergyCTAcquisitionSequence(self, value: Optional[List[MultienergyCTAcquisitionSequenceItem]]):
        if value is None:
            self._MultienergyCTAcquisitionSequence = []
            if "MultienergyCTAcquisitionSequence" in self._dataset:
                del self._dataset.MultienergyCTAcquisitionSequence
        elif not isinstance(value, list) or not all(isinstance(item, MultienergyCTAcquisitionSequenceItem) for item in value):
            raise ValueError("MultienergyCTAcquisitionSequence must be a list of MultienergyCTAcquisitionSequenceItem objects")
        else:
            self._MultienergyCTAcquisitionSequence = value
            if "MultienergyCTAcquisitionSequence" not in self._dataset:
                self._dataset.MultienergyCTAcquisitionSequence = pydicom.Sequence()
            self._dataset.MultienergyCTAcquisitionSequence.clear()
            self._dataset.MultienergyCTAcquisitionSequence.extend([item.to_dataset() for item in value])

    def add_MultienergyCTAcquisition(self, item: MultienergyCTAcquisitionSequenceItem):
        if not isinstance(item, MultienergyCTAcquisitionSequenceItem):
            raise ValueError("Item must be an instance of MultienergyCTAcquisitionSequenceItem")
        self._MultienergyCTAcquisitionSequence.append(item)
        if "MultienergyCTAcquisitionSequence" not in self._dataset:
            self._dataset.MultienergyCTAcquisitionSequence = pydicom.Sequence()
        self._dataset.MultienergyCTAcquisitionSequence.append(item.to_dataset())
    ...
    def MultienergyCTProcessingSequence(self, value: Optional[List[MultienergyCTProcessingSequenceItem]]):
        if value is None:
            ...
            raise ValueError("MultienergyCTProcessingSequence must be a list of MultienergyCTProcessingSequenceItem objects")
        else:
    ...
    def add_MultienergyCTProcessing(self, item: MultienergyCTProcessingSequenceItem):
        if not isinstance(item, MultienergyCTProcessingSequenceItem):
            raise ValueError("Item must be an instance of MultienergyCTProcessingSequenceItem")
    ...
    def MultienergyCTCharacteristicsSequence(self, value: Optional[List[MultienergyCTCharacteristicsSequenceItem]]):
        if value is None:
            ...
            raise ValueError(
                "MultienergyCTCharacteristicsSequence must be a list of MultienergyCTCharacteristicsSequenceItem objects"
            )
        else:
    ...
    def add_MultienergyCTCharacteristics(self, item: MultienergyCTCharacteristicsSequenceItem):
        if not isinstance(item, MultienergyCTCharacteristicsSequenceItem):
            raise ValueError("Item must be an instance of MultienergyCTCharacteristicsSequenceItem")
