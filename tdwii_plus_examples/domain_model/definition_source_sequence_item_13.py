...
class DefinitionSourceSequenceItem:
    ...
    @property
    def ReferencedFiducialUID(self) -> Optional[str]:
        if "ReferencedFiducialUID" in self._dataset:
            return self._dataset.ReferencedFiducialUID
        return None

    @ReferencedFiducialUID.setter
    def ReferencedFiducialUID(self, value: Optional[str]):
        if value is None:
            if "ReferencedFiducialUID" in self._dataset:
                del self._dataset.ReferencedFiducialUID
        else:
            self._dataset.ReferencedFiducialUID = value
