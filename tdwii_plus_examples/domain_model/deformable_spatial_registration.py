...
from .deformable_registration_sequence_item import DeformableRegistrationSequenceItem
...
class DeformableSpatialRegistration:
    ...
    @property
    def DeformableRegistrationSequence(self) -> Optional[List[DeformableRegistrationSequenceItem]]:
        if "DeformableRegistrationSequence" in self._dataset:
            if len(self._DeformableRegistrationSequence) == len(self._dataset.DeformableRegistrationSequence):
                return self._DeformableRegistrationSequence
            else:
                return [DeformableRegistrationSequenceItem(x) for x in self._dataset.DeformableRegistrationSequence]
        return None

    @DeformableRegistrationSequence.setter
    def DeformableRegistrationSequence(self, value: Optional[List[DeformableRegistrationSequenceItem]]):
        if value is None:
            self._DeformableRegistrationSequence = []
            if "DeformableRegistrationSequence" in self._dataset:
                del self._dataset.DeformableRegistrationSequence
        elif not isinstance(value, list) or not all(isinstance(item, DeformableRegistrationSequenceItem) for item in value):
            raise ValueError(f"DeformableRegistrationSequence must be a list of DeformableRegistrationSequenceItem objects")
        else:
            self._DeformableRegistrationSequence = value
            if "DeformableRegistrationSequence" not in self._dataset:
                self._dataset.DeformableRegistrationSequence = pydicom.Sequence()
            self._dataset.DeformableRegistrationSequence.clear()
            self._dataset.DeformableRegistrationSequence.extend([item.to_dataset() for item in value])

    def add_DeformableRegistration(self, item: DeformableRegistrationSequenceItem):
        if not isinstance(item, DeformableRegistrationSequenceItem):
            raise ValueError(f"Item must be an instance of DeformableRegistrationSequenceItem")
        self._DeformableRegistrationSequence.append(item)
        if "DeformableRegistrationSequence" not in self._dataset:
            self._dataset.DeformableRegistrationSequence = pydicom.Sequence()
        self._dataset.DeformableRegistrationSequence.append(item.to_dataset())
