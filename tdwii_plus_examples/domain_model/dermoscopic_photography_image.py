...
class DermoscopicPhotographyImage:
    ...
    @property
    def LightSourcePolarization(self) -> Optional[str]:
        if "LightSourcePolarization" in self._dataset:
            return self._dataset.LightSourcePolarization
        return None

    @LightSourcePolarization.setter
    def LightSourcePolarization(self, value: Optional[str]):
        if value is None:
            if "LightSourcePolarization" in self._dataset:
                del self._dataset.LightSourcePolarization
        else:
            self._dataset.LightSourcePolarization = value

    @property
    def EmitterColorTemperature(self) -> Optional[Decimal]:
        if "EmitterColorTemperature" in self._dataset:
            return self._dataset.EmitterColorTemperature
        return None

    @EmitterColorTemperature.setter
    def EmitterColorTemperature(self, value: Optional[Decimal]):
        if value is None:
            if "EmitterColorTemperature" in self._dataset:
                del self._dataset.EmitterColorTemperature
        else:
            self._dataset.EmitterColorTemperature = value

    @property
    def ContactMethod(self) -> Optional[str]:
        if "ContactMethod" in self._dataset:
            return self._dataset.ContactMethod
        return None

    @ContactMethod.setter
    def ContactMethod(self, value: Optional[str]):
        if value is None:
            if "ContactMethod" in self._dataset:
                del self._dataset.ContactMethod
        else:
            self._dataset.ContactMethod = value

    @property
    def ImmersionMedia(self) -> Optional[List[str]]:
        if "ImmersionMedia" in self._dataset:
            return self._dataset.ImmersionMedia
        return None

    @ImmersionMedia.setter
    def ImmersionMedia(self, value: Optional[List[str]]):
        if value is None:
            if "ImmersionMedia" in self._dataset:
                del self._dataset.ImmersionMedia
        else:
            self._dataset.ImmersionMedia = value

    @property
    def OpticalMagnificationFactor(self) -> Optional[Decimal]:
        if "OpticalMagnificationFactor" in self._dataset:
            return self._dataset.OpticalMagnificationFactor
        return None

    @OpticalMagnificationFactor.setter
    def OpticalMagnificationFactor(self, value: Optional[Decimal]):
        if value is None:
            if "OpticalMagnificationFactor" in self._dataset:
                del self._dataset.OpticalMagnificationFactor
        else:
            self._dataset.OpticalMagnificationFactor = value
    ...
    @property
    def PartialView(self) -> Optional[str]:
        if "PartialView" in self._dataset:
            return self._dataset.PartialView
        return None

    @PartialView.setter
    def PartialView(self, value: Optional[str]):
        if value is None:
            if "PartialView" in self._dataset:
                del self._dataset.PartialView
        else:
            self._dataset.PartialView = value

    @property
    def PartialViewDescription(self) -> Optional[str]:
        if "PartialViewDescription" in self._dataset:
            return self._dataset.PartialViewDescription
        return None

    @PartialViewDescription.setter
    def PartialViewDescription(self, value: Optional[str]):
        if value is None:
            if "PartialViewDescription" in self._dataset:
                del self._dataset.PartialViewDescription
        else:
            self._dataset.PartialViewDescription = value
