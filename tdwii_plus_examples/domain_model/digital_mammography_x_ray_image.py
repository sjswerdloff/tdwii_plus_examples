...
from .biopsy_target_sequence_item import BiopsyTargetSequenceItem
...
class DigitalMammographyXRayImage:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._BiopsyTargetSequence: List[BiopsyTargetSequenceItem] = []
    ...
    @property
    def BiopsyTargetSequence(self) -> Optional[List[BiopsyTargetSequenceItem]]:
        if "BiopsyTargetSequence" in self._dataset:
            if len(self._BiopsyTargetSequence) == len(self._dataset.BiopsyTargetSequence):
                return self._BiopsyTargetSequence
            else:
                return [BiopsyTargetSequenceItem(x) for x in self._dataset.BiopsyTargetSequence]
        return None

    @BiopsyTargetSequence.setter
    def BiopsyTargetSequence(self, value: Optional[List[BiopsyTargetSequenceItem]]):
        if value is None:
            self._BiopsyTargetSequence = []
            if "BiopsyTargetSequence" in self._dataset:
                del self._dataset.BiopsyTargetSequence
        elif not isinstance(value, list) or not all(isinstance(item, BiopsyTargetSequenceItem) for item in value):
            raise ValueError(f"BiopsyTargetSequence must be a list of BiopsyTargetSequenceItem objects")
        else:
            self._BiopsyTargetSequence = value
            if "BiopsyTargetSequence" not in self._dataset:
                self._dataset.BiopsyTargetSequence = pydicom.Sequence()
            self._dataset.BiopsyTargetSequence.clear()
            self._dataset.BiopsyTargetSequence.extend([item.to_dataset() for item in value])

    def add_BiopsyTarget(self, item: BiopsyTargetSequenceItem):
        if not isinstance(item, BiopsyTargetSequenceItem):
            raise ValueError(f"Item must be an instance of BiopsyTargetSequenceItem")
        self._BiopsyTargetSequence.append(item)
        if "BiopsyTargetSequence" not in self._dataset:
            self._dataset.BiopsyTargetSequence = pydicom.Sequence()
        self._dataset.BiopsyTargetSequence.append(item.to_dataset())
    ...
    def HistogramSequence(self, value: Optional[List[HistogramSequenceItem]]):
        if value is None:
            ...
            raise ValueError(f"HistogramSequence must be a list of HistogramSequenceItem objects")
        else:
    ...
    def add_Histogram(self, item: HistogramSequenceItem):
        if not isinstance(item, HistogramSequenceItem):
            raise ValueError(f"Item must be an instance of HistogramSequenceItem")
    ...
    def ProjectionEponymousNameCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            ...
            raise ValueError(f"ProjectionEponymousNameCodeSequence must be a list of CodeSequenceItem objects")
        else:
