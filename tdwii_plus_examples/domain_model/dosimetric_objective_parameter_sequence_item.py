...
from .radiobiological_dose_effect_sequence_item import (
    RadiobiologicalDoseEffectSequenceItem,
)
from .referenced_sop_sequence_item import ReferencedSOPSequenceItem


class DosimetricObjectiveParameterSequenceItem:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._RadiobiologicalDoseEffectSequence: List[RadiobiologicalDoseEffectSequenceItem] = []
    ...
    @property
    def RadiobiologicalDoseEffectSequence(self) -> Optional[List[RadiobiologicalDoseEffectSequenceItem]]:
        if "RadiobiologicalDoseEffectSequence" in self._dataset:
            if len(self._RadiobiologicalDoseEffectSequence) == len(self._dataset.RadiobiologicalDoseEffectSequence):
                return self._RadiobiologicalDoseEffectSequence
            else:
                return [RadiobiologicalDoseEffectSequenceItem(x) for x in self._dataset.RadiobiologicalDoseEffectSequence]
        return None

    @RadiobiologicalDoseEffectSequence.setter
    def RadiobiologicalDoseEffectSequence(self, value: Optional[List[RadiobiologicalDoseEffectSequenceItem]]):
        if value is None:
            self._RadiobiologicalDoseEffectSequence = []
            if "RadiobiologicalDoseEffectSequence" in self._dataset:
            ...
            self._dataset.RadiobiologicalDoseEffectSequence.clear()
            self._dataset.RadiobiologicalDoseEffectSequence.extend([item.to_dataset() for item in value])
    ...
    def add_RadiobiologicalDoseEffect(self, item: RadiobiologicalDoseEffectSequenceItem):
        ...
        if "RadiobiologicalDoseEffectSequence" not in self._dataset:
            self._dataset.RadiobiologicalDoseEffectSequence = pydicom.Sequence()
        self._dataset.RadiobiologicalDoseEffectSequence.append(item.to_dataset())
