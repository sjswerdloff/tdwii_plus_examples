...
class EncapsulatedCda:
    ...
    def DocumentTitle(self) -> Optional[str]:
        if "DocumentTitle" in self._dataset:
            return self._dataset.DocumentTitle
        return None

    @DocumentTitle.setter
    def DocumentTitle(self, value: Optional[str]):
        if value is None:
            if "DocumentTitle" in self._dataset:
                del self._dataset.DocumentTitle
        else:
            self._dataset.DocumentTitle = value

    @property
    def EncapsulatedDocument(self) -> Optional[bytes]:
        if "EncapsulatedDocument" in self._dataset:
            return self._dataset.EncapsulatedDocument
        return None

    @EncapsulatedDocument.setter
    def EncapsulatedDocument(self, value: Optional[bytes]):
        if value is None:
            if "EncapsulatedDocument" in self._dataset:
                del self._dataset.EncapsulatedDocument
        else:
            self._dataset.EncapsulatedDocument = value

    @property
    def MIMETypeOfEncapsulatedDocument(self) -> Optional[str]:
        if "MIMETypeOfEncapsulatedDocument" in self._dataset:
            return self._dataset.MIMETypeOfEncapsulatedDocument
        return None

    @MIMETypeOfEncapsulatedDocument.setter
    def MIMETypeOfEncapsulatedDocument(self, value: Optional[str]):
        if value is None:
            if "MIMETypeOfEncapsulatedDocument" in self._dataset:
                del self._dataset.MIMETypeOfEncapsulatedDocument
        else:
            self._dataset.MIMETypeOfEncapsulatedDocument = value
    ...
    @property
    def ListOfMIMETypes(self) -> Optional[List[str]]:
        if "ListOfMIMETypes" in self._dataset:
            return self._dataset.ListOfMIMETypes
        return None

    @ListOfMIMETypes.setter
    def ListOfMIMETypes(self, value: Optional[List[str]]):
        if value is None:
            if "ListOfMIMETypes" in self._dataset:
                del self._dataset.ListOfMIMETypes
        else:
            self._dataset.ListOfMIMETypes = value

    @property
    def EncapsulatedDocumentLength(self) -> Optional[int]:
        if "EncapsulatedDocumentLength" in self._dataset:
            return self._dataset.EncapsulatedDocumentLength
        return None

    @EncapsulatedDocumentLength.setter
    def EncapsulatedDocumentLength(self, value: Optional[int]):
        if value is None:
            if "EncapsulatedDocumentLength" in self._dataset:
                del self._dataset.EncapsulatedDocumentLength
        else:
            self._dataset.EncapsulatedDocumentLength = value
