...
class EncapsulatedMtl:
    ...
    @property
    def ModelModification(self) -> Optional[str]:
        if "ModelModification" in self._dataset:
            return self._dataset.ModelModification
        return None

    @ModelModification.setter
    def ModelModification(self, value: Optional[str]):
        if value is None:
            if "ModelModification" in self._dataset:
                del self._dataset.ModelModification
        else:
            self._dataset.ModelModification = value

    @property
    def ModelMirroring(self) -> Optional[str]:
        if "ModelMirroring" in self._dataset:
            return self._dataset.ModelMirroring
        return None

    @ModelMirroring.setter
    def ModelMirroring(self, value: Optional[str]):
        if value is None:
            if "ModelMirroring" in self._dataset:
                del self._dataset.ModelMirroring
        else:
            self._dataset.ModelMirroring = value

    @property
    def ModelUsageCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "ModelUsageCodeSequence" in self._dataset:
            if len(self._ModelUsageCodeSequence) == len(self._dataset.ModelUsageCodeSequence):
                return self._ModelUsageCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.ModelUsageCodeSequence]
        return None

    @ModelUsageCodeSequence.setter
    def ModelUsageCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._ModelUsageCodeSequence = []
            if "ModelUsageCodeSequence" in self._dataset:
                del self._dataset.ModelUsageCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"ModelUsageCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._ModelUsageCodeSequence = value
            if "ModelUsageCodeSequence" not in self._dataset:
                self._dataset.ModelUsageCodeSequence = pydicom.Sequence()
            self._dataset.ModelUsageCodeSequence.clear()
            self._dataset.ModelUsageCodeSequence.extend([item.to_dataset() for item in value])

    def add_ModelUsageCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._ModelUsageCodeSequence.append(item)
        if "ModelUsageCodeSequence" not in self._dataset:
            self._dataset.ModelUsageCodeSequence = pydicom.Sequence()
        self._dataset.ModelUsageCodeSequence.append(item.to_dataset())

    @property
    def ModelGroupUID(self) -> Optional[str]:
        if "ModelGroupUID" in self._dataset:
            return self._dataset.ModelGroupUID
        return None

    @ModelGroupUID.setter
    def ModelGroupUID(self, value: Optional[str]):
        if value is None:
            if "ModelGroupUID" in self._dataset:
                del self._dataset.ModelGroupUID
        else:
            self._dataset.ModelGroupUID = value
