...
from .acquisition_device_sequence_item import AcquisitionDeviceSequenceItem
...
from .selected_frame_functional_groups_sequence_item import (
    SelectedFrameFunctionalGroupsSequenceItem,
)
...
class EnhancedContinuousRtImage:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._AcquisitionDeviceSequence: List[AcquisitionDeviceSequenceItem] = []
        ...
        self._SelectedFrameFunctionalGroupsSequence: List[SelectedFrameFunctionalGroupsSequenceItem] = []
    ...
    @property
    def BeamModifierCoordinatesPresenceFlag(self) -> Optional[str]:
        if "BeamModifierCoordinatesPresenceFlag" in self._dataset:
            return self._dataset.BeamModifierCoordinatesPresenceFlag
        return None

    @BeamModifierCoordinatesPresenceFlag.setter
    def BeamModifierCoordinatesPresenceFlag(self, value: Optional[str]):
        if value is None:
            if "BeamModifierCoordinatesPresenceFlag" in self._dataset:
                del self._dataset.BeamModifierCoordinatesPresenceFlag
        else:
            self._dataset.BeamModifierCoordinatesPresenceFlag = value

    @property
    def NumberOfAcquisitionDevices(self) -> Optional[int]:
        if "NumberOfAcquisitionDevices" in self._dataset:
            return self._dataset.NumberOfAcquisitionDevices
        return None

    @NumberOfAcquisitionDevices.setter
    def NumberOfAcquisitionDevices(self, value: Optional[int]):
        if value is None:
            if "NumberOfAcquisitionDevices" in self._dataset:
                del self._dataset.NumberOfAcquisitionDevices
        else:
            self._dataset.NumberOfAcquisitionDevices = value

    @property
    def AcquisitionDeviceSequence(self) -> Optional[List[AcquisitionDeviceSequenceItem]]:
        if "AcquisitionDeviceSequence" in self._dataset:
            if len(self._AcquisitionDeviceSequence) == len(self._dataset.AcquisitionDeviceSequence):
                return self._AcquisitionDeviceSequence
            else:
                return [AcquisitionDeviceSequenceItem(x) for x in self._dataset.AcquisitionDeviceSequence]
        return None

    @AcquisitionDeviceSequence.setter
    def AcquisitionDeviceSequence(self, value: Optional[List[AcquisitionDeviceSequenceItem]]):
        if value is None:
            self._AcquisitionDeviceSequence = []
            if "AcquisitionDeviceSequence" in self._dataset:
                del self._dataset.AcquisitionDeviceSequence
        elif not isinstance(value, list) or not all(isinstance(item, AcquisitionDeviceSequenceItem) for item in value):
            raise ValueError(f"AcquisitionDeviceSequence must be a list of AcquisitionDeviceSequenceItem objects")
        else:
            self._AcquisitionDeviceSequence = value
            if "AcquisitionDeviceSequence" not in self._dataset:
                self._dataset.AcquisitionDeviceSequence = pydicom.Sequence()
            self._dataset.AcquisitionDeviceSequence.clear()
            self._dataset.AcquisitionDeviceSequence.extend([item.to_dataset() for item in value])

    def add_AcquisitionDevice(self, item: AcquisitionDeviceSequenceItem):
        if not isinstance(item, AcquisitionDeviceSequenceItem):
            raise ValueError(f"Item must be an instance of AcquisitionDeviceSequenceItem")
        self._AcquisitionDeviceSequence.append(item)
        if "AcquisitionDeviceSequence" not in self._dataset:
            self._dataset.AcquisitionDeviceSequence = pydicom.Sequence()
        self._dataset.AcquisitionDeviceSequence.append(item.to_dataset())
    ...
    @property
    def SelectedFrameFunctionalGroupsSequence(self) -> Optional[List[SelectedFrameFunctionalGroupsSequenceItem]]:
        if "SelectedFrameFunctionalGroupsSequence" in self._dataset:
            if len(self._SelectedFrameFunctionalGroupsSequence) == len(self._dataset.SelectedFrameFunctionalGroupsSequence):
                return self._SelectedFrameFunctionalGroupsSequence
            else:
                return [
                    SelectedFrameFunctionalGroupsSequenceItem(x) for x in self._dataset.SelectedFrameFunctionalGroupsSequence
                ]
        return None

    @SelectedFrameFunctionalGroupsSequence.setter
    def SelectedFrameFunctionalGroupsSequence(self, value: Optional[List[SelectedFrameFunctionalGroupsSequenceItem]]):
        if value is None:
            self._SelectedFrameFunctionalGroupsSequence = []
            if "SelectedFrameFunctionalGroupsSequence" in self._dataset:
                del self._dataset.SelectedFrameFunctionalGroupsSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, SelectedFrameFunctionalGroupsSequenceItem) for item in value
        ):
            raise ValueError(
                f"SelectedFrameFunctionalGroupsSequence must be a list of SelectedFrameFunctionalGroupsSequenceItem objects"
            )
        else:
            self._SelectedFrameFunctionalGroupsSequence = value
            if "SelectedFrameFunctionalGroupsSequence" not in self._dataset:
                self._dataset.SelectedFrameFunctionalGroupsSequence = pydicom.Sequence()
            self._dataset.SelectedFrameFunctionalGroupsSequence.clear()
            self._dataset.SelectedFrameFunctionalGroupsSequence.extend([item.to_dataset() for item in value])

    def add_SelectedFrameFunctionalGroups(self, item: SelectedFrameFunctionalGroupsSequenceItem):
        if not isinstance(item, SelectedFrameFunctionalGroupsSequenceItem):
            raise ValueError(f"Item must be an instance of SelectedFrameFunctionalGroupsSequenceItem")
        self._SelectedFrameFunctionalGroupsSequence.append(item)
        if "SelectedFrameFunctionalGroupsSequence" not in self._dataset:
            self._dataset.SelectedFrameFunctionalGroupsSequence = pydicom.Sequence()
        self._dataset.SelectedFrameFunctionalGroupsSequence.append(item.to_dataset())
    ...
    @property
    def StopCumulativeMeterset(self) -> Optional[float]:
        if "StopCumulativeMeterset" in self._dataset:
            return self._dataset.StopCumulativeMeterset
        return None

    @StopCumulativeMeterset.setter
    def StopCumulativeMeterset(self, value: Optional[float]):
        if value is None:
            if "StopCumulativeMeterset" in self._dataset:
                del self._dataset.StopCumulativeMeterset
        else:
            self._dataset.StopCumulativeMeterset = value
