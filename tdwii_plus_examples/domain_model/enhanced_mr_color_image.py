...
from .velocity_encoding_acquisition_sequence_item import (
    VelocityEncodingAcquisitionSequenceItem,
)


class EnhancedMrColorImage:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._VelocityEncodingAcquisitionSequence: List[VelocityEncodingAcquisitionSequenceItem] = []
    ...
    def EchoPulseSequence(self, value: Optional[List[EchoPulseSequenceItem]]):
        if value is None:
            ...
            raise ValueError("EchoPulseSequence must be a list of EchoPulseSequenceItem objects")
        else:
    ...
    def add_EchoPulse(self, item: EchoPulseSequenceItem):
        if not isinstance(item, EchoPulseSequenceItem):
            raise ValueError("Item must be an instance of EchoPulseSequenceItem")
    ...
    @property
    def PhaseContrast(self) -> Optional[str]:
        if "PhaseContrast" in self._dataset:
            return self._dataset.PhaseContrast
        return None

    @PhaseContrast.setter
    def PhaseContrast(self, value: Optional[str]):
        if value is None:
            if "PhaseContrast" in self._dataset:
                del self._dataset.PhaseContrast
        else:
            self._dataset.PhaseContrast = value

    @property
    def TimeOfFlightContrast(self) -> Optional[str]:
        if "TimeOfFlightContrast" in self._dataset:
            return self._dataset.TimeOfFlightContrast
        return None

    @TimeOfFlightContrast.setter
    def TimeOfFlightContrast(self, value: Optional[str]):
        if value is None:
            if "TimeOfFlightContrast" in self._dataset:
                del self._dataset.TimeOfFlightContrast
        else:
            self._dataset.TimeOfFlightContrast = value
    ...
    def SteadyStatePulseSequence(self, value: Optional[List[SteadyStatePulseSequenceItem]]):
        if value is None:
            ...
            raise ValueError("SteadyStatePulseSequence must be a list of SteadyStatePulseSequenceItem objects")
        else:
    ...
    def add_SteadyStatePulse(self, item: SteadyStatePulseSequenceItem):
        if not isinstance(item, SteadyStatePulseSequenceItem):
            raise ValueError("Item must be an instance of SteadyStatePulseSequenceItem")
    ...
    def EchoPlanarPulseSequence(self, value: Optional[List[EchoPlanarPulseSequenceItem]]):
        if value is None:
            ...
            raise ValueError("EchoPlanarPulseSequence must be a list of EchoPlanarPulseSequenceItem objects")
        else:
    ...
    def add_EchoPlanarPulse(self, item: EchoPlanarPulseSequenceItem):
        if not isinstance(item, EchoPlanarPulseSequenceItem):
            raise ValueError("Item must be an instance of EchoPlanarPulseSequenceItem")
    ...
    @property
    def SaturationRecovery(self) -> Optional[str]:
        if "SaturationRecovery" in self._dataset:
            return self._dataset.SaturationRecovery
        return None

    @SaturationRecovery.setter
    def SaturationRecovery(self, value: Optional[str]):
        if value is None:
            if "SaturationRecovery" in self._dataset:
                del self._dataset.SaturationRecovery
        else:
            self._dataset.SaturationRecovery = value
    ...
    @property
    def OversamplingPhase(self) -> Optional[str]:
        if "OversamplingPhase" in self._dataset:
            return self._dataset.OversamplingPhase
        return None

    @OversamplingPhase.setter
    def OversamplingPhase(self, value: Optional[str]):
        if value is None:
            if "OversamplingPhase" in self._dataset:
                del self._dataset.OversamplingPhase
        else:
            self._dataset.OversamplingPhase = value
    ...
    @property
    def VelocityEncodingAcquisitionSequence(self) -> Optional[List[VelocityEncodingAcquisitionSequenceItem]]:
        if "VelocityEncodingAcquisitionSequence" in self._dataset:
            if len(self._VelocityEncodingAcquisitionSequence) == len(self._dataset.VelocityEncodingAcquisitionSequence):
                return self._VelocityEncodingAcquisitionSequence
            else:
                return [VelocityEncodingAcquisitionSequenceItem(x) for x in self._dataset.VelocityEncodingAcquisitionSequence]
        return None

    @VelocityEncodingAcquisitionSequence.setter
    def VelocityEncodingAcquisitionSequence(self, value: Optional[List[VelocityEncodingAcquisitionSequenceItem]]):
        if value is None:
            self._VelocityEncodingAcquisitionSequence = []
            if "VelocityEncodingAcquisitionSequence" in self._dataset:
                del self._dataset.VelocityEncodingAcquisitionSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, VelocityEncodingAcquisitionSequenceItem) for item in value
        ):
            raise ValueError(
                "VelocityEncodingAcquisitionSequence must be a list of VelocityEncodingAcquisitionSequenceItem objects"
            )
        else:
            self._VelocityEncodingAcquisitionSequence = value
            if "VelocityEncodingAcquisitionSequence" not in self._dataset:
                self._dataset.VelocityEncodingAcquisitionSequence = pydicom.Sequence()
            self._dataset.VelocityEncodingAcquisitionSequence.clear()
            self._dataset.VelocityEncodingAcquisitionSequence.extend([item.to_dataset() for item in value])

    def add_VelocityEncodingAcquisition(self, item: VelocityEncodingAcquisitionSequenceItem):
        if not isinstance(item, VelocityEncodingAcquisitionSequenceItem):
            raise ValueError("Item must be an instance of VelocityEncodingAcquisitionSequenceItem")
        self._VelocityEncodingAcquisitionSequence.append(item)
        if "VelocityEncodingAcquisitionSequence" not in self._dataset:
            self._dataset.VelocityEncodingAcquisitionSequence = pydicom.Sequence()
        self._dataset.VelocityEncodingAcquisitionSequence.append(item.to_dataset())
    ...
    @property
    def ArterialSpinLabelingContrast(self) -> Optional[str]:
        if "ArterialSpinLabelingContrast" in self._dataset:
            return self._dataset.ArterialSpinLabelingContrast
        return None

    @ArterialSpinLabelingContrast.setter
    def ArterialSpinLabelingContrast(self, value: Optional[str]):
        if value is None:
            if "ArterialSpinLabelingContrast" in self._dataset:
                del self._dataset.ArterialSpinLabelingContrast
        else:
            self._dataset.ArterialSpinLabelingContrast = value
