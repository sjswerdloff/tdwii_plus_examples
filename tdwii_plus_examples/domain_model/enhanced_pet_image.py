...
class EnhancedPetImage:
    ...
    @property
    def DecayCorrectionDateTime(self) -> Optional[str]:
        if "DecayCorrectionDateTime" in self._dataset:
            return self._dataset.DecayCorrectionDateTime
        return None

    @DecayCorrectionDateTime.setter
    def DecayCorrectionDateTime(self, value: Optional[str]):
        if value is None:
            if "DecayCorrectionDateTime" in self._dataset:
                del self._dataset.DecayCorrectionDateTime
        else:
            self._dataset.DecayCorrectionDateTime = value

    @property
    def AttenuationCorrectionSource(self) -> Optional[str]:
        if "AttenuationCorrectionSource" in self._dataset:
            return self._dataset.AttenuationCorrectionSource
        return None

    @AttenuationCorrectionSource.setter
    def AttenuationCorrectionSource(self, value: Optional[str]):
        if value is None:
            if "AttenuationCorrectionSource" in self._dataset:
                del self._dataset.AttenuationCorrectionSource
        else:
            self._dataset.AttenuationCorrectionSource = value

    @property
    def DecayCorrected(self) -> Optional[str]:
        if "DecayCorrected" in self._dataset:
            return self._dataset.DecayCorrected
        return None

    @DecayCorrected.setter
    def DecayCorrected(self, value: Optional[str]):
        if value is None:
            if "DecayCorrected" in self._dataset:
                del self._dataset.DecayCorrected
        else:
            self._dataset.DecayCorrected = value

    @property
    def AttenuationCorrected(self) -> Optional[str]:
        if "AttenuationCorrected" in self._dataset:
            return self._dataset.AttenuationCorrected
        return None

    @AttenuationCorrected.setter
    def AttenuationCorrected(self, value: Optional[str]):
        if value is None:
            if "AttenuationCorrected" in self._dataset:
                del self._dataset.AttenuationCorrected
        else:
            self._dataset.AttenuationCorrected = value

    @property
    def ScatterCorrected(self) -> Optional[str]:
        if "ScatterCorrected" in self._dataset:
            return self._dataset.ScatterCorrected
        return None

    @ScatterCorrected.setter
    def ScatterCorrected(self, value: Optional[str]):
        if value is None:
            if "ScatterCorrected" in self._dataset:
                del self._dataset.ScatterCorrected
        else:
            self._dataset.ScatterCorrected = value

    @property
    def DeadTimeCorrected(self) -> Optional[str]:
        if "DeadTimeCorrected" in self._dataset:
            return self._dataset.DeadTimeCorrected
        return None

    @DeadTimeCorrected.setter
    def DeadTimeCorrected(self, value: Optional[str]):
        if value is None:
            if "DeadTimeCorrected" in self._dataset:
                del self._dataset.DeadTimeCorrected
        else:
            self._dataset.DeadTimeCorrected = value

    @property
    def GantryMotionCorrected(self) -> Optional[str]:
        if "GantryMotionCorrected" in self._dataset:
            return self._dataset.GantryMotionCorrected
        return None

    @GantryMotionCorrected.setter
    def GantryMotionCorrected(self, value: Optional[str]):
        if value is None:
            if "GantryMotionCorrected" in self._dataset:
                del self._dataset.GantryMotionCorrected
        else:
            self._dataset.GantryMotionCorrected = value

    @property
    def PatientMotionCorrected(self) -> Optional[str]:
        if "PatientMotionCorrected" in self._dataset:
            return self._dataset.PatientMotionCorrected
        return None

    @PatientMotionCorrected.setter
    def PatientMotionCorrected(self, value: Optional[str]):
        if value is None:
            if "PatientMotionCorrected" in self._dataset:
                del self._dataset.PatientMotionCorrected
        else:
            self._dataset.PatientMotionCorrected = value

    @property
    def CountLossNormalizationCorrected(self) -> Optional[str]:
        if "CountLossNormalizationCorrected" in self._dataset:
            return self._dataset.CountLossNormalizationCorrected
        return None

    @CountLossNormalizationCorrected.setter
    def CountLossNormalizationCorrected(self, value: Optional[str]):
        if value is None:
            if "CountLossNormalizationCorrected" in self._dataset:
                del self._dataset.CountLossNormalizationCorrected
        else:
            self._dataset.CountLossNormalizationCorrected = value

    @property
    def RandomsCorrected(self) -> Optional[str]:
        if "RandomsCorrected" in self._dataset:
            return self._dataset.RandomsCorrected
        return None

    @RandomsCorrected.setter
    def RandomsCorrected(self, value: Optional[str]):
        if value is None:
            if "RandomsCorrected" in self._dataset:
                del self._dataset.RandomsCorrected
        else:
            self._dataset.RandomsCorrected = value

    @property
    def NonUniformRadialSamplingCorrected(self) -> Optional[str]:
        if "NonUniformRadialSamplingCorrected" in self._dataset:
            return self._dataset.NonUniformRadialSamplingCorrected
        return None

    @NonUniformRadialSamplingCorrected.setter
    def NonUniformRadialSamplingCorrected(self, value: Optional[str]):
        if value is None:
            if "NonUniformRadialSamplingCorrected" in self._dataset:
                del self._dataset.NonUniformRadialSamplingCorrected
        else:
            self._dataset.NonUniformRadialSamplingCorrected = value

    @property
    def SensitivityCalibrated(self) -> Optional[str]:
        if "SensitivityCalibrated" in self._dataset:
            return self._dataset.SensitivityCalibrated
        return None

    @SensitivityCalibrated.setter
    def SensitivityCalibrated(self, value: Optional[str]):
        if value is None:
            if "SensitivityCalibrated" in self._dataset:
                del self._dataset.SensitivityCalibrated
        else:
            self._dataset.SensitivityCalibrated = value

    @property
    def DetectorNormalizationCorrection(self) -> Optional[str]:
        if "DetectorNormalizationCorrection" in self._dataset:
            return self._dataset.DetectorNormalizationCorrection
        return None

    @DetectorNormalizationCorrection.setter
    def DetectorNormalizationCorrection(self, value: Optional[str]):
        if value is None:
            if "DetectorNormalizationCorrection" in self._dataset:
                del self._dataset.DetectorNormalizationCorrection
        else:
            self._dataset.DetectorNormalizationCorrection = value

    @property
    def AttenuationCorrectionTemporalRelationship(self) -> Optional[str]:
        if "AttenuationCorrectionTemporalRelationship" in self._dataset:
            return self._dataset.AttenuationCorrectionTemporalRelationship
        return None

    @AttenuationCorrectionTemporalRelationship.setter
    def AttenuationCorrectionTemporalRelationship(self, value: Optional[str]):
        if value is None:
            if "AttenuationCorrectionTemporalRelationship" in self._dataset:
                del self._dataset.AttenuationCorrectionTemporalRelationship
        else:
            self._dataset.AttenuationCorrectionTemporalRelationship = value

    @property
    def CountsSource(self) -> Optional[str]:
        if "CountsSource" in self._dataset:
            return self._dataset.CountsSource
        return None

    @CountsSource.setter
    def CountsSource(self, value: Optional[str]):
        if value is None:
            if "CountsSource" in self._dataset:
                del self._dataset.CountsSource
        else:
            self._dataset.CountsSource = value

    @property
    def RandomsCorrectionMethod(self) -> Optional[str]:
        if "RandomsCorrectionMethod" in self._dataset:
            return self._dataset.RandomsCorrectionMethod
        return None

    @RandomsCorrectionMethod.setter
    def RandomsCorrectionMethod(self, value: Optional[str]):
        if value is None:
            if "RandomsCorrectionMethod" in self._dataset:
                del self._dataset.RandomsCorrectionMethod
        else:
            self._dataset.RandomsCorrectionMethod = value

    @property
    def ScatterCorrectionMethod(self) -> Optional[str]:
        if "ScatterCorrectionMethod" in self._dataset:
            return self._dataset.ScatterCorrectionMethod
        return None

    @ScatterCorrectionMethod.setter
    def ScatterCorrectionMethod(self, value: Optional[str]):
        if value is None:
            if "ScatterCorrectionMethod" in self._dataset:
                del self._dataset.ScatterCorrectionMethod
        else:
            self._dataset.ScatterCorrectionMethod = value
    ...
    def InterventionSequence(self, value: Optional[List[InterventionSequenceItem]]):
        if value is None:
            ...
            raise ValueError(f"InterventionSequence must be a list of InterventionSequenceItem objects")
        else:
    ...
    def add_Intervention(self, item: InterventionSequenceItem):
        if not isinstance(item, InterventionSequenceItem):
            raise ValueError(f"Item must be an instance of InterventionSequenceItem")
    ...
    @property
    def AcquisitionStartCondition(self) -> Optional[str]:
        if "AcquisitionStartCondition" in self._dataset:
            return self._dataset.AcquisitionStartCondition
        return None

    @AcquisitionStartCondition.setter
    def AcquisitionStartCondition(self, value: Optional[str]):
        if value is None:
            if "AcquisitionStartCondition" in self._dataset:
                del self._dataset.AcquisitionStartCondition
        else:
            self._dataset.AcquisitionStartCondition = value

    @property
    def TableMotion(self) -> Optional[str]:
        if "TableMotion" in self._dataset:
            return self._dataset.TableMotion
        return None

    @TableMotion.setter
    def TableMotion(self, value: Optional[str]):
        if value is None:
            if "TableMotion" in self._dataset:
                del self._dataset.TableMotion
        else:
            self._dataset.TableMotion = value
    ...
    @property
    def StartDensityThreshold(self) -> Optional[float]:
        if "StartDensityThreshold" in self._dataset:
            return self._dataset.StartDensityThreshold
        return None

    @StartDensityThreshold.setter
    def StartDensityThreshold(self, value: Optional[float]):
        if value is None:
            if "StartDensityThreshold" in self._dataset:
                del self._dataset.StartDensityThreshold
        else:
            self._dataset.StartDensityThreshold = value

    @property
    def StartRelativeDensityDifferenceThreshold(self) -> Optional[float]:
        if "StartRelativeDensityDifferenceThreshold" in self._dataset:
            return self._dataset.StartRelativeDensityDifferenceThreshold
        return None

    @StartRelativeDensityDifferenceThreshold.setter
    def StartRelativeDensityDifferenceThreshold(self, value: Optional[float]):
        if value is None:
            if "StartRelativeDensityDifferenceThreshold" in self._dataset:
                del self._dataset.StartRelativeDensityDifferenceThreshold
        else:
            self._dataset.StartRelativeDensityDifferenceThreshold = value

    @property
    def StartCardiacTriggerCountThreshold(self) -> Optional[float]:
        if "StartCardiacTriggerCountThreshold" in self._dataset:
            return self._dataset.StartCardiacTriggerCountThreshold
        return None

    @StartCardiacTriggerCountThreshold.setter
    def StartCardiacTriggerCountThreshold(self, value: Optional[float]):
        if value is None:
            if "StartCardiacTriggerCountThreshold" in self._dataset:
                del self._dataset.StartCardiacTriggerCountThreshold
        else:
            self._dataset.StartCardiacTriggerCountThreshold = value

    @property
    def StartRespiratoryTriggerCountThreshold(self) -> Optional[float]:
        if "StartRespiratoryTriggerCountThreshold" in self._dataset:
            return self._dataset.StartRespiratoryTriggerCountThreshold
        return None

    @StartRespiratoryTriggerCountThreshold.setter
    def StartRespiratoryTriggerCountThreshold(self, value: Optional[float]):
        if value is None:
            if "StartRespiratoryTriggerCountThreshold" in self._dataset:
                del self._dataset.StartRespiratoryTriggerCountThreshold
        else:
            self._dataset.StartRespiratoryTriggerCountThreshold = value

    @property
    def TerminationCountsThreshold(self) -> Optional[float]:
        if "TerminationCountsThreshold" in self._dataset:
            return self._dataset.TerminationCountsThreshold
        return None

    @TerminationCountsThreshold.setter
    def TerminationCountsThreshold(self, value: Optional[float]):
        if value is None:
            if "TerminationCountsThreshold" in self._dataset:
                del self._dataset.TerminationCountsThreshold
        else:
            self._dataset.TerminationCountsThreshold = value

    @property
    def TerminationDensityThreshold(self) -> Optional[float]:
        if "TerminationDensityThreshold" in self._dataset:
            return self._dataset.TerminationDensityThreshold
        return None

    @TerminationDensityThreshold.setter
    def TerminationDensityThreshold(self, value: Optional[float]):
        if value is None:
            if "TerminationDensityThreshold" in self._dataset:
                del self._dataset.TerminationDensityThreshold
        else:
            self._dataset.TerminationDensityThreshold = value

    @property
    def TerminationRelativeDensityThreshold(self) -> Optional[float]:
        if "TerminationRelativeDensityThreshold" in self._dataset:
            return self._dataset.TerminationRelativeDensityThreshold
        return None

    @TerminationRelativeDensityThreshold.setter
    def TerminationRelativeDensityThreshold(self, value: Optional[float]):
        if value is None:
            if "TerminationRelativeDensityThreshold" in self._dataset:
                del self._dataset.TerminationRelativeDensityThreshold
        else:
            self._dataset.TerminationRelativeDensityThreshold = value

    @property
    def TerminationTimeThreshold(self) -> Optional[float]:
        if "TerminationTimeThreshold" in self._dataset:
            return self._dataset.TerminationTimeThreshold
        return None

    @TerminationTimeThreshold.setter
    def TerminationTimeThreshold(self, value: Optional[float]):
        if value is None:
            if "TerminationTimeThreshold" in self._dataset:
                del self._dataset.TerminationTimeThreshold
        else:
            self._dataset.TerminationTimeThreshold = value

    @property
    def TerminationCardiacTriggerCountThreshold(self) -> Optional[float]:
        if "TerminationCardiacTriggerCountThreshold" in self._dataset:
            return self._dataset.TerminationCardiacTriggerCountThreshold
        return None

    @TerminationCardiacTriggerCountThreshold.setter
    def TerminationCardiacTriggerCountThreshold(self, value: Optional[float]):
        if value is None:
            if "TerminationCardiacTriggerCountThreshold" in self._dataset:
                del self._dataset.TerminationCardiacTriggerCountThreshold
        else:
            self._dataset.TerminationCardiacTriggerCountThreshold = value

    @property
    def TerminationRespiratoryTriggerCountThreshold(self) -> Optional[float]:
        if "TerminationRespiratoryTriggerCountThreshold" in self._dataset:
            return self._dataset.TerminationRespiratoryTriggerCountThreshold
        return None

    @TerminationRespiratoryTriggerCountThreshold.setter
    def TerminationRespiratoryTriggerCountThreshold(self, value: Optional[float]):
        if value is None:
            if "TerminationRespiratoryTriggerCountThreshold" in self._dataset:
                del self._dataset.TerminationRespiratoryTriggerCountThreshold
        else:
            self._dataset.TerminationRespiratoryTriggerCountThreshold = value

    @property
    def DetectorGeometry(self) -> Optional[str]:
        if "DetectorGeometry" in self._dataset:
            return self._dataset.DetectorGeometry
        return None

    @DetectorGeometry.setter
    def DetectorGeometry(self, value: Optional[str]):
        if value is None:
            if "DetectorGeometry" in self._dataset:
                del self._dataset.DetectorGeometry
        else:
            self._dataset.DetectorGeometry = value

    @property
    def TransverseDetectorSeparation(self) -> Optional[float]:
        if "TransverseDetectorSeparation" in self._dataset:
            return self._dataset.TransverseDetectorSeparation
        return None

    @TransverseDetectorSeparation.setter
    def TransverseDetectorSeparation(self, value: Optional[float]):
        if value is None:
            if "TransverseDetectorSeparation" in self._dataset:
                del self._dataset.TransverseDetectorSeparation
        else:
            self._dataset.TransverseDetectorSeparation = value

    @property
    def AxialDetectorDimension(self) -> Optional[float]:
        if "AxialDetectorDimension" in self._dataset:
            return self._dataset.AxialDetectorDimension
        return None

    @AxialDetectorDimension.setter
    def AxialDetectorDimension(self, value: Optional[float]):
        if value is None:
            if "AxialDetectorDimension" in self._dataset:
                del self._dataset.AxialDetectorDimension
        else:
            self._dataset.AxialDetectorDimension = value

    @property
    def TimeOfFlightInformationUsed(self) -> Optional[str]:
        if "TimeOfFlightInformationUsed" in self._dataset:
            return self._dataset.TimeOfFlightInformationUsed
        return None

    @TimeOfFlightInformationUsed.setter
    def TimeOfFlightInformationUsed(self, value: Optional[str]):
        if value is None:
            if "TimeOfFlightInformationUsed" in self._dataset:
                del self._dataset.TimeOfFlightInformationUsed
        else:
            self._dataset.TimeOfFlightInformationUsed = value
    ...
    @property
    def CoincidenceWindowWidth(self) -> Optional[Decimal]:
        if "CoincidenceWindowWidth" in self._dataset:
            return self._dataset.CoincidenceWindowWidth
        return None

    @CoincidenceWindowWidth.setter
    def CoincidenceWindowWidth(self, value: Optional[Decimal]):
        if value is None:
            if "CoincidenceWindowWidth" in self._dataset:
                del self._dataset.CoincidenceWindowWidth
        else:
            self._dataset.CoincidenceWindowWidth = value
