...
from .blending_lut1_sequence_item import BlendingLUT1SequenceItem
from .blending_lut2_sequence_item import BlendingLUT2SequenceItem
...
from .data_frame_assignment_sequence_item import DataFrameAssignmentSequenceItem
...
from .enhanced_palette_color_lookup_table_sequence_item import (
    EnhancedPaletteColorLookupTableSequenceItem,
)
from .event_timer_sequence_item import EventTimerSequenceItem
from .excluded_intervals_sequence_item import ExcludedIntervalsSequenceItem
...
from .transducer_identification_sequence_item import (
    TransducerIdentificationSequenceItem,
)
from .udi_sequence_item import UDISequenceItem


class EnhancedUsVolume:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._ExcludedIntervalsSequence: List[ExcludedIntervalsSequenceItem] = []
        ...
        self._DataFrameAssignmentSequence: List[DataFrameAssignmentSequenceItem] = []
        self._BlendingLUT1Sequence: List[BlendingLUT1SequenceItem] = []
        self._EnhancedPaletteColorLookupTableSequence: List[EnhancedPaletteColorLookupTableSequenceItem] = []
        self._BlendingLUT2Sequence: List[BlendingLUT2SequenceItem] = []
        ...
        self._ReferencedInstanceSequence: List[ReferencedInstanceSequenceItem] = []
        self._SourceImageSequence: List[SourceImageSequenceItem] = []
        self._EventTimerSequence: List[EventTimerSequenceItem] = []
        ...
        self._TransducerIdentificationSequence: List[TransducerIdentificationSequenceItem] = []
        self._TransducerScanPatternCodeSequence: List[CodeSequenceItem] = []
        self._TransducerGeometryCodeSequence: List[CodeSequenceItem] = []
    ...
    @property
    def ExcludedIntervalsSequence(self) -> Optional[List[ExcludedIntervalsSequenceItem]]:
        if "ExcludedIntervalsSequence" in self._dataset:
            if len(self._ExcludedIntervalsSequence) == len(self._dataset.ExcludedIntervalsSequence):
                return self._ExcludedIntervalsSequence
            else:
                return [ExcludedIntervalsSequenceItem(x) for x in self._dataset.ExcludedIntervalsSequence]
        return None

    @ExcludedIntervalsSequence.setter
    def ExcludedIntervalsSequence(self, value: Optional[List[ExcludedIntervalsSequenceItem]]):
        if value is None:
            self._ExcludedIntervalsSequence = []
            if "ExcludedIntervalsSequence" in self._dataset:
                del self._dataset.ExcludedIntervalsSequence
        elif not isinstance(value, list) or not all(isinstance(item, ExcludedIntervalsSequenceItem) for item in value):
            raise ValueError(f"ExcludedIntervalsSequence must be a list of ExcludedIntervalsSequenceItem objects")
        else:
            self._ExcludedIntervalsSequence = value
            if "ExcludedIntervalsSequence" not in self._dataset:
                self._dataset.ExcludedIntervalsSequence = pydicom.Sequence()
            self._dataset.ExcludedIntervalsSequence.clear()
            self._dataset.ExcludedIntervalsSequence.extend([item.to_dataset() for item in value])

    def add_ExcludedIntervals(self, item: ExcludedIntervalsSequenceItem):
        if not isinstance(item, ExcludedIntervalsSequenceItem):
            raise ValueError(f"Item must be an instance of ExcludedIntervalsSequenceItem")
        self._ExcludedIntervalsSequence.append(item)
        if "ExcludedIntervalsSequence" not in self._dataset:
            self._dataset.ExcludedIntervalsSequence = pydicom.Sequence()
        self._dataset.ExcludedIntervalsSequence.append(item.to_dataset())
    ...
    @property
    def PerformedProtocolType(self) -> Optional[str]:
        if "PerformedProtocolType" in self._dataset:
            return self._dataset.PerformedProtocolType
        return None

    @PerformedProtocolType.setter
    def PerformedProtocolType(self, value: Optional[str]):
        if value is None:
            if "PerformedProtocolType" in self._dataset:
                del self._dataset.PerformedProtocolType
        else:
            self._dataset.PerformedProtocolType = value
    ...
    @property
    def DataFrameAssignmentSequence(self) -> Optional[List[DataFrameAssignmentSequenceItem]]:
        if "DataFrameAssignmentSequence" in self._dataset:
            if len(self._DataFrameAssignmentSequence) == len(self._dataset.DataFrameAssignmentSequence):
                return self._DataFrameAssignmentSequence
            else:
                return [DataFrameAssignmentSequenceItem(x) for x in self._dataset.DataFrameAssignmentSequence]
        return None

    @DataFrameAssignmentSequence.setter
    def DataFrameAssignmentSequence(self, value: Optional[List[DataFrameAssignmentSequenceItem]]):
        if value is None:
            self._DataFrameAssignmentSequence = []
            if "DataFrameAssignmentSequence" in self._dataset:
                del self._dataset.DataFrameAssignmentSequence
        elif not isinstance(value, list) or not all(isinstance(item, DataFrameAssignmentSequenceItem) for item in value):
            raise ValueError(f"DataFrameAssignmentSequence must be a list of DataFrameAssignmentSequenceItem objects")
        else:
            self._DataFrameAssignmentSequence = value
            if "DataFrameAssignmentSequence" not in self._dataset:
                self._dataset.DataFrameAssignmentSequence = pydicom.Sequence()
            self._dataset.DataFrameAssignmentSequence.clear()
            self._dataset.DataFrameAssignmentSequence.extend([item.to_dataset() for item in value])

    def add_DataFrameAssignment(self, item: DataFrameAssignmentSequenceItem):
        if not isinstance(item, DataFrameAssignmentSequenceItem):
            raise ValueError(f"Item must be an instance of DataFrameAssignmentSequenceItem")
        self._DataFrameAssignmentSequence.append(item)
        if "DataFrameAssignmentSequence" not in self._dataset:
            self._dataset.DataFrameAssignmentSequence = pydicom.Sequence()
        self._dataset.DataFrameAssignmentSequence.append(item.to_dataset())

    @property
    def BlendingLUT1Sequence(self) -> Optional[List[BlendingLUT1SequenceItem]]:
        if "BlendingLUT1Sequence" in self._dataset:
            if len(self._BlendingLUT1Sequence) == len(self._dataset.BlendingLUT1Sequence):
                return self._BlendingLUT1Sequence
            else:
                return [BlendingLUT1SequenceItem(x) for x in self._dataset.BlendingLUT1Sequence]
        return None

    @BlendingLUT1Sequence.setter
    def BlendingLUT1Sequence(self, value: Optional[List[BlendingLUT1SequenceItem]]):
        if value is None:
            self._BlendingLUT1Sequence = []
            if "BlendingLUT1Sequence" in self._dataset:
                del self._dataset.BlendingLUT1Sequence
        elif not isinstance(value, list) or not all(isinstance(item, BlendingLUT1SequenceItem) for item in value):
            raise ValueError(f"BlendingLUT1Sequence must be a list of BlendingLUT1SequenceItem objects")
        else:
            self._BlendingLUT1Sequence = value
            if "BlendingLUT1Sequence" not in self._dataset:
                self._dataset.BlendingLUT1Sequence = pydicom.Sequence()
            self._dataset.BlendingLUT1Sequence.clear()
            self._dataset.BlendingLUT1Sequence.extend([item.to_dataset() for item in value])

    def add_BlendingLUT1(self, item: BlendingLUT1SequenceItem):
        if not isinstance(item, BlendingLUT1SequenceItem):
            raise ValueError(f"Item must be an instance of BlendingLUT1SequenceItem")
        self._BlendingLUT1Sequence.append(item)
        if "BlendingLUT1Sequence" not in self._dataset:
            self._dataset.BlendingLUT1Sequence = pydicom.Sequence()
        self._dataset.BlendingLUT1Sequence.append(item.to_dataset())

    @property
    def EnhancedPaletteColorLookupTableSequence(self) -> Optional[List[EnhancedPaletteColorLookupTableSequenceItem]]:
        if "EnhancedPaletteColorLookupTableSequence" in self._dataset:
            if len(self._EnhancedPaletteColorLookupTableSequence) == len(
                self._dataset.EnhancedPaletteColorLookupTableSequence
            ):
                return self._EnhancedPaletteColorLookupTableSequence
            else:
                return [
                    EnhancedPaletteColorLookupTableSequenceItem(x)
                    for x in self._dataset.EnhancedPaletteColorLookupTableSequence
                ]
        return None

    @EnhancedPaletteColorLookupTableSequence.setter
    def EnhancedPaletteColorLookupTableSequence(self, value: Optional[List[EnhancedPaletteColorLookupTableSequenceItem]]):
        if value is None:
            self._EnhancedPaletteColorLookupTableSequence = []
            if "EnhancedPaletteColorLookupTableSequence" in self._dataset:
                del self._dataset.EnhancedPaletteColorLookupTableSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, EnhancedPaletteColorLookupTableSequenceItem) for item in value
        ):
            raise ValueError(
                f"EnhancedPaletteColorLookupTableSequence must be a list of EnhancedPaletteColorLookupTableSequenceItem objects"
            )
        else:
            self._EnhancedPaletteColorLookupTableSequence = value
            if "EnhancedPaletteColorLookupTableSequence" not in self._dataset:
                self._dataset.EnhancedPaletteColorLookupTableSequence = pydicom.Sequence()
            self._dataset.EnhancedPaletteColorLookupTableSequence.clear()
            self._dataset.EnhancedPaletteColorLookupTableSequence.extend([item.to_dataset() for item in value])

    def add_EnhancedPaletteColorLookupTable(self, item: EnhancedPaletteColorLookupTableSequenceItem):
        if not isinstance(item, EnhancedPaletteColorLookupTableSequenceItem):
            raise ValueError(f"Item must be an instance of EnhancedPaletteColorLookupTableSequenceItem")
        self._EnhancedPaletteColorLookupTableSequence.append(item)
        if "EnhancedPaletteColorLookupTableSequence" not in self._dataset:
            self._dataset.EnhancedPaletteColorLookupTableSequence = pydicom.Sequence()
        self._dataset.EnhancedPaletteColorLookupTableSequence.append(item.to_dataset())

    @property
    def BlendingLUT2Sequence(self) -> Optional[List[BlendingLUT2SequenceItem]]:
        if "BlendingLUT2Sequence" in self._dataset:
            if len(self._BlendingLUT2Sequence) == len(self._dataset.BlendingLUT2Sequence):
                return self._BlendingLUT2Sequence
            else:
                return [BlendingLUT2SequenceItem(x) for x in self._dataset.BlendingLUT2Sequence]
        return None

    @BlendingLUT2Sequence.setter
    def BlendingLUT2Sequence(self, value: Optional[List[BlendingLUT2SequenceItem]]):
        if value is None:
            self._BlendingLUT2Sequence = []
            if "BlendingLUT2Sequence" in self._dataset:
                del self._dataset.BlendingLUT2Sequence
        elif not isinstance(value, list) or not all(isinstance(item, BlendingLUT2SequenceItem) for item in value):
            raise ValueError(f"BlendingLUT2Sequence must be a list of BlendingLUT2SequenceItem objects")
        else:
            self._BlendingLUT2Sequence = value
            if "BlendingLUT2Sequence" not in self._dataset:
                self._dataset.BlendingLUT2Sequence = pydicom.Sequence()
            self._dataset.BlendingLUT2Sequence.clear()
            self._dataset.BlendingLUT2Sequence.extend([item.to_dataset() for item in value])

    def add_BlendingLUT2(self, item: BlendingLUT2SequenceItem):
        if not isinstance(item, BlendingLUT2SequenceItem):
            raise ValueError(f"Item must be an instance of BlendingLUT2SequenceItem")
        self._BlendingLUT2Sequence.append(item)
        if "BlendingLUT2Sequence" not in self._dataset:
            self._dataset.BlendingLUT2Sequence = pydicom.Sequence()
        self._dataset.BlendingLUT2Sequence.append(item.to_dataset())
    ...
    @property
    def UltrasoundAcquisitionGeometry(self) -> Optional[str]:
        if "UltrasoundAcquisitionGeometry" in self._dataset:
            return self._dataset.UltrasoundAcquisitionGeometry
        return None

    @UltrasoundAcquisitionGeometry.setter
    def UltrasoundAcquisitionGeometry(self, value: Optional[str]):
        if value is None:
            if "UltrasoundAcquisitionGeometry" in self._dataset:
                del self._dataset.UltrasoundAcquisitionGeometry
        else:
            self._dataset.UltrasoundAcquisitionGeometry = value

    @property
    def ApexPosition(self) -> Optional[List[float]]:
        if "ApexPosition" in self._dataset:
            return self._dataset.ApexPosition
        return None

    @ApexPosition.setter
    def ApexPosition(self, value: Optional[List[float]]):
        if value is None:
            if "ApexPosition" in self._dataset:
                del self._dataset.ApexPosition
        else:
            self._dataset.ApexPosition = value

    @property
    def VolumeToTransducerMappingMatrix(self) -> Optional[List[float]]:
        if "VolumeToTransducerMappingMatrix" in self._dataset:
            return self._dataset.VolumeToTransducerMappingMatrix
        return None

    @VolumeToTransducerMappingMatrix.setter
    def VolumeToTransducerMappingMatrix(self, value: Optional[List[float]]):
        if value is None:
            if "VolumeToTransducerMappingMatrix" in self._dataset:
                del self._dataset.VolumeToTransducerMappingMatrix
        else:
            self._dataset.VolumeToTransducerMappingMatrix = value

    @property
    def VolumeToTableMappingMatrix(self) -> Optional[List[float]]:
        if "VolumeToTableMappingMatrix" in self._dataset:
            return self._dataset.VolumeToTableMappingMatrix
        return None

    @VolumeToTableMappingMatrix.setter
    def VolumeToTableMappingMatrix(self, value: Optional[List[float]]):
        if value is None:
            if "VolumeToTableMappingMatrix" in self._dataset:
                del self._dataset.VolumeToTableMappingMatrix
        else:
            self._dataset.VolumeToTableMappingMatrix = value

    @property
    def VolumeToTransducerRelationship(self) -> Optional[str]:
        if "VolumeToTransducerRelationship" in self._dataset:
            return self._dataset.VolumeToTransducerRelationship
        return None

    @VolumeToTransducerRelationship.setter
    def VolumeToTransducerRelationship(self, value: Optional[str]):
        if value is None:
            if "VolumeToTransducerRelationship" in self._dataset:
                del self._dataset.VolumeToTransducerRelationship
        else:
            self._dataset.VolumeToTransducerRelationship = value

    @property
    def PatientFrameOfReferenceSource(self) -> Optional[str]:
        if "PatientFrameOfReferenceSource" in self._dataset:
            return self._dataset.PatientFrameOfReferenceSource
        return None

    @PatientFrameOfReferenceSource.setter
    def PatientFrameOfReferenceSource(self, value: Optional[str]):
        if value is None:
            if "PatientFrameOfReferenceSource" in self._dataset:
                del self._dataset.PatientFrameOfReferenceSource
        else:
            self._dataset.PatientFrameOfReferenceSource = value

    @property
    def VolumeFrameOfReferenceUID(self) -> Optional[str]:
        if "VolumeFrameOfReferenceUID" in self._dataset:
            return self._dataset.VolumeFrameOfReferenceUID
        return None

    @VolumeFrameOfReferenceUID.setter
    def VolumeFrameOfReferenceUID(self, value: Optional[str]):
        if value is None:
            if "VolumeFrameOfReferenceUID" in self._dataset:
                del self._dataset.VolumeFrameOfReferenceUID
        else:
            self._dataset.VolumeFrameOfReferenceUID = value

    @property
    def TableFrameOfReferenceUID(self) -> Optional[str]:
        if "TableFrameOfReferenceUID" in self._dataset:
            return self._dataset.TableFrameOfReferenceUID
        return None

    @TableFrameOfReferenceUID.setter
    def TableFrameOfReferenceUID(self, value: Optional[str]):
        if value is None:
            if "TableFrameOfReferenceUID" in self._dataset:
                del self._dataset.TableFrameOfReferenceUID
        else:
            self._dataset.TableFrameOfReferenceUID = value
    ...
    @property
    def StageNumber(self) -> Optional[int]:
        if "StageNumber" in self._dataset:
            return self._dataset.StageNumber
        return None

    @StageNumber.setter
    def StageNumber(self, value: Optional[int]):
        if value is None:
            if "StageNumber" in self._dataset:
                del self._dataset.StageNumber
        else:
            self._dataset.StageNumber = value

    @property
    def NumberOfStages(self) -> Optional[int]:
        if "NumberOfStages" in self._dataset:
            return self._dataset.NumberOfStages
        return None

    @NumberOfStages.setter
    def NumberOfStages(self, value: Optional[int]):
        if value is None:
            if "NumberOfStages" in self._dataset:
                del self._dataset.NumberOfStages
        else:
            self._dataset.NumberOfStages = value

    @property
    def EventTimerSequence(self) -> Optional[List[EventTimerSequenceItem]]:
        if "EventTimerSequence" in self._dataset:
            if len(self._EventTimerSequence) == len(self._dataset.EventTimerSequence):
                return self._EventTimerSequence
            else:
                return [EventTimerSequenceItem(x) for x in self._dataset.EventTimerSequence]
        return None

    @EventTimerSequence.setter
    def EventTimerSequence(self, value: Optional[List[EventTimerSequenceItem]]):
        if value is None:
            self._EventTimerSequence = []
            if "EventTimerSequence" in self._dataset:
                del self._dataset.EventTimerSequence
        elif not isinstance(value, list) or not all(isinstance(item, EventTimerSequenceItem) for item in value):
            raise ValueError(f"EventTimerSequence must be a list of EventTimerSequenceItem objects")
        else:
            self._EventTimerSequence = value
            if "EventTimerSequence" not in self._dataset:
                self._dataset.EventTimerSequence = pydicom.Sequence()
            self._dataset.EventTimerSequence.clear()
            self._dataset.EventTimerSequence.extend([item.to_dataset() for item in value])

    def add_EventTimer(self, item: EventTimerSequenceItem):
        if not isinstance(item, EventTimerSequenceItem):
            raise ValueError(f"Item must be an instance of EventTimerSequenceItem")
        self._EventTimerSequence.append(item)
        if "EventTimerSequence" not in self._dataset:
            self._dataset.EventTimerSequence = pydicom.Sequence()
        self._dataset.EventTimerSequence.append(item.to_dataset())
    ...
    @property
    def TransducerData(self) -> Optional[List[str]]:
        if "TransducerData" in self._dataset:
            return self._dataset.TransducerData
        return None

    @TransducerData.setter
    def TransducerData(self, value: Optional[List[str]]):
        if value is None:
            if "TransducerData" in self._dataset:
                del self._dataset.TransducerData
        else:
            self._dataset.TransducerData = value

    @property
    def TransducerIdentificationSequence(self) -> Optional[List[TransducerIdentificationSequenceItem]]:
        if "TransducerIdentificationSequence" in self._dataset:
            if len(self._TransducerIdentificationSequence) == len(self._dataset.TransducerIdentificationSequence):
                return self._TransducerIdentificationSequence
            else:
                return [TransducerIdentificationSequenceItem(x) for x in self._dataset.TransducerIdentificationSequence]
        return None

    @TransducerIdentificationSequence.setter
    def TransducerIdentificationSequence(self, value: Optional[List[TransducerIdentificationSequenceItem]]):
        if value is None:
            self._TransducerIdentificationSequence = []
            if "TransducerIdentificationSequence" in self._dataset:
                del self._dataset.TransducerIdentificationSequence
        elif not isinstance(value, list) or not all(isinstance(item, TransducerIdentificationSequenceItem) for item in value):
            raise ValueError(
                f"TransducerIdentificationSequence must be a list of TransducerIdentificationSequenceItem objects"
            )
        else:
            self._TransducerIdentificationSequence = value
            if "TransducerIdentificationSequence" not in self._dataset:
                self._dataset.TransducerIdentificationSequence = pydicom.Sequence()
            self._dataset.TransducerIdentificationSequence.clear()
            self._dataset.TransducerIdentificationSequence.extend([item.to_dataset() for item in value])

    def add_TransducerIdentification(self, item: TransducerIdentificationSequenceItem):
        if not isinstance(item, TransducerIdentificationSequenceItem):
            raise ValueError(f"Item must be an instance of TransducerIdentificationSequenceItem")
        self._TransducerIdentificationSequence.append(item)
        if "TransducerIdentificationSequence" not in self._dataset:
            self._dataset.TransducerIdentificationSequence = pydicom.Sequence()
        self._dataset.TransducerIdentificationSequence.append(item.to_dataset())

    @property
    def ProcessingFunction(self) -> Optional[str]:
        if "ProcessingFunction" in self._dataset:
            return self._dataset.ProcessingFunction
        return None

    @ProcessingFunction.setter
    def ProcessingFunction(self, value: Optional[str]):
        if value is None:
            if "ProcessingFunction" in self._dataset:
                del self._dataset.ProcessingFunction
        else:
            self._dataset.ProcessingFunction = value

    @property
    def MechanicalIndex(self) -> Optional[Decimal]:
        if "MechanicalIndex" in self._dataset:
            return self._dataset.MechanicalIndex
        return None

    @MechanicalIndex.setter
    def MechanicalIndex(self, value: Optional[Decimal]):
        if value is None:
            if "MechanicalIndex" in self._dataset:
                del self._dataset.MechanicalIndex
        else:
            self._dataset.MechanicalIndex = value

    @property
    def BoneThermalIndex(self) -> Optional[Decimal]:
        if "BoneThermalIndex" in self._dataset:
            return self._dataset.BoneThermalIndex
        return None

    @BoneThermalIndex.setter
    def BoneThermalIndex(self, value: Optional[Decimal]):
        if value is None:
            if "BoneThermalIndex" in self._dataset:
                del self._dataset.BoneThermalIndex
        else:
            self._dataset.BoneThermalIndex = value

    @property
    def CranialThermalIndex(self) -> Optional[Decimal]:
        if "CranialThermalIndex" in self._dataset:
            return self._dataset.CranialThermalIndex
        return None

    @CranialThermalIndex.setter
    def CranialThermalIndex(self, value: Optional[Decimal]):
        if value is None:
            if "CranialThermalIndex" in self._dataset:
                del self._dataset.CranialThermalIndex
        else:
            self._dataset.CranialThermalIndex = value

    @property
    def SoftTissueThermalIndex(self) -> Optional[Decimal]:
        if "SoftTissueThermalIndex" in self._dataset:
            return self._dataset.SoftTissueThermalIndex
        return None

    @SoftTissueThermalIndex.setter
    def SoftTissueThermalIndex(self, value: Optional[Decimal]):
        if value is None:
            if "SoftTissueThermalIndex" in self._dataset:
                del self._dataset.SoftTissueThermalIndex
        else:
            self._dataset.SoftTissueThermalIndex = value

    @property
    def DepthOfScanField(self) -> Optional[int]:
        if "DepthOfScanField" in self._dataset:
            return self._dataset.DepthOfScanField
        return None

    @DepthOfScanField.setter
    def DepthOfScanField(self, value: Optional[int]):
        if value is None:
            if "DepthOfScanField" in self._dataset:
                del self._dataset.DepthOfScanField
        else:
            self._dataset.DepthOfScanField = value
    ...
    @property
    def DepthsOfFocus(self) -> Optional[List[float]]:
        if "DepthsOfFocus" in self._dataset:
            return self._dataset.DepthsOfFocus
        return None

    @DepthsOfFocus.setter
    def DepthsOfFocus(self, value: Optional[List[float]]):
        if value is None:
            if "DepthsOfFocus" in self._dataset:
                del self._dataset.DepthsOfFocus
        else:
            self._dataset.DepthsOfFocus = value

    @property
    def TransducerScanPatternCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "TransducerScanPatternCodeSequence" in self._dataset:
            if len(self._TransducerScanPatternCodeSequence) == len(self._dataset.TransducerScanPatternCodeSequence):
                return self._TransducerScanPatternCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.TransducerScanPatternCodeSequence]
        return None

    @TransducerScanPatternCodeSequence.setter
    def TransducerScanPatternCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._TransducerScanPatternCodeSequence = []
            if "TransducerScanPatternCodeSequence" in self._dataset:
                del self._dataset.TransducerScanPatternCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"TransducerScanPatternCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._TransducerScanPatternCodeSequence = value
            if "TransducerScanPatternCodeSequence" not in self._dataset:
                self._dataset.TransducerScanPatternCodeSequence = pydicom.Sequence()
            self._dataset.TransducerScanPatternCodeSequence.clear()
            self._dataset.TransducerScanPatternCodeSequence.extend([item.to_dataset() for item in value])

    def add_TransducerScanPatternCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._TransducerScanPatternCodeSequence.append(item)
        if "TransducerScanPatternCodeSequence" not in self._dataset:
            self._dataset.TransducerScanPatternCodeSequence = pydicom.Sequence()
        self._dataset.TransducerScanPatternCodeSequence.append(item.to_dataset())

    @property
    def PositionMeasuringDeviceUsed(self) -> Optional[str]:
        if "PositionMeasuringDeviceUsed" in self._dataset:
            return self._dataset.PositionMeasuringDeviceUsed
        return None

    @PositionMeasuringDeviceUsed.setter
    def PositionMeasuringDeviceUsed(self, value: Optional[str]):
        if value is None:
            if "PositionMeasuringDeviceUsed" in self._dataset:
                del self._dataset.PositionMeasuringDeviceUsed
        else:
            self._dataset.PositionMeasuringDeviceUsed = value

    @property
    def TransducerGeometryCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "TransducerGeometryCodeSequence" in self._dataset:
            if len(self._TransducerGeometryCodeSequence) == len(self._dataset.TransducerGeometryCodeSequence):
                return self._TransducerGeometryCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.TransducerGeometryCodeSequence]
        return None

    @TransducerGeometryCodeSequence.setter
    def TransducerGeometryCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._TransducerGeometryCodeSequence = []
            if "TransducerGeometryCodeSequence" in self._dataset:
                del self._dataset.TransducerGeometryCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"TransducerGeometryCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._TransducerGeometryCodeSequence = value
            if "TransducerGeometryCodeSequence" not in self._dataset:
                self._dataset.TransducerGeometryCodeSequence = pydicom.Sequence()
            self._dataset.TransducerGeometryCodeSequence.clear()
            self._dataset.TransducerGeometryCodeSequence.extend([item.to_dataset() for item in value])

    def add_TransducerGeometryCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._TransducerGeometryCodeSequence.append(item)
        if "TransducerGeometryCodeSequence" not in self._dataset:
            self._dataset.TransducerGeometryCodeSequence = pydicom.Sequence()
        self._dataset.TransducerGeometryCodeSequence.append(item.to_dataset())

    @property
    def TransducerBeamSteeringCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "TransducerBeamSteeringCodeSequence" in self._dataset:
            if len(self._TransducerBeamSteeringCodeSequence) == len(self._dataset.TransducerBeamSteeringCodeSequence):
                return self._TransducerBeamSteeringCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.TransducerBeamSteeringCodeSequence]
        return None

    @TransducerBeamSteeringCodeSequence.setter
    def TransducerBeamSteeringCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._TransducerBeamSteeringCodeSequence = []
            if "TransducerBeamSteeringCodeSequence" in self._dataset:
                del self._dataset.TransducerBeamSteeringCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"TransducerBeamSteeringCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._TransducerBeamSteeringCodeSequence = value
            if "TransducerBeamSteeringCodeSequence" not in self._dataset:
                self._dataset.TransducerBeamSteeringCodeSequence = pydicom.Sequence()
            self._dataset.TransducerBeamSteeringCodeSequence.clear()
            self._dataset.TransducerBeamSteeringCodeSequence.extend([item.to_dataset() for item in value])

    def add_TransducerBeamSteeringCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._TransducerBeamSteeringCodeSequence.append(item)
        if "TransducerBeamSteeringCodeSequence" not in self._dataset:
            self._dataset.TransducerBeamSteeringCodeSequence = pydicom.Sequence()
        self._dataset.TransducerBeamSteeringCodeSequence.append(item.to_dataset())

    @property
    def TransducerApplicationCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "TransducerApplicationCodeSequence" in self._dataset:
            if len(self._TransducerApplicationCodeSequence) == len(self._dataset.TransducerApplicationCodeSequence):
                return self._TransducerApplicationCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.TransducerApplicationCodeSequence]
        return None

    @TransducerApplicationCodeSequence.setter
    def TransducerApplicationCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._TransducerApplicationCodeSequence = []
            if "TransducerApplicationCodeSequence" in self._dataset:
                del self._dataset.TransducerApplicationCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"TransducerApplicationCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._TransducerApplicationCodeSequence = value
            if "TransducerApplicationCodeSequence" not in self._dataset:
                self._dataset.TransducerApplicationCodeSequence = pydicom.Sequence()
            self._dataset.TransducerApplicationCodeSequence.clear()
            self._dataset.TransducerApplicationCodeSequence.extend([item.to_dataset() for item in value])

    def add_TransducerApplicationCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._TransducerApplicationCodeSequence.append(item)
        if "TransducerApplicationCodeSequence" not in self._dataset:
            self._dataset.TransducerApplicationCodeSequence = pydicom.Sequence()
        self._dataset.TransducerApplicationCodeSequence.append(item.to_dataset())
    ...
    @property
    def StageCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "StageCodeSequence" in self._dataset:
            if len(self._StageCodeSequence) == len(self._dataset.StageCodeSequence):
                return self._StageCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.StageCodeSequence]
        return None

    @StageCodeSequence.setter
    def StageCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._StageCodeSequence = []
            if "StageCodeSequence" in self._dataset:
                del self._dataset.StageCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"StageCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._StageCodeSequence = value
            if "StageCodeSequence" not in self._dataset:
                self._dataset.StageCodeSequence = pydicom.Sequence()
            self._dataset.StageCodeSequence.clear()
            self._dataset.StageCodeSequence.extend([item.to_dataset() for item in value])

    def add_StageCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._StageCodeSequence.append(item)
        if "StageCodeSequence" not in self._dataset:
            self._dataset.StageCodeSequence = pydicom.Sequence()
        self._dataset.StageCodeSequence.append(item.to_dataset())
    ...
    @property
    def IVUSAcquisition(self) -> Optional[str]:
        if "IVUSAcquisition" in self._dataset:
            return self._dataset.IVUSAcquisition
        return None

    @IVUSAcquisition.setter
    def IVUSAcquisition(self, value: Optional[str]):
        if value is None:
            if "IVUSAcquisition" in self._dataset:
                del self._dataset.IVUSAcquisition
        else:
            self._dataset.IVUSAcquisition = value

    @property
    def IVUSPullbackRate(self) -> Optional[Decimal]:
        if "IVUSPullbackRate" in self._dataset:
            return self._dataset.IVUSPullbackRate
        return None

    @IVUSPullbackRate.setter
    def IVUSPullbackRate(self, value: Optional[Decimal]):
        if value is None:
            if "IVUSPullbackRate" in self._dataset:
                del self._dataset.IVUSPullbackRate
        else:
            self._dataset.IVUSPullbackRate = value

    @property
    def IVUSGatedRate(self) -> Optional[Decimal]:
        if "IVUSGatedRate" in self._dataset:
            return self._dataset.IVUSGatedRate
        return None

    @IVUSGatedRate.setter
    def IVUSGatedRate(self, value: Optional[Decimal]):
        if value is None:
            if "IVUSGatedRate" in self._dataset:
                del self._dataset.IVUSGatedRate
        else:
            self._dataset.IVUSGatedRate = value

    @property
    def IVUSPullbackStartFrameNumber(self) -> Optional[int]:
        if "IVUSPullbackStartFrameNumber" in self._dataset:
            return self._dataset.IVUSPullbackStartFrameNumber
        return None

    @IVUSPullbackStartFrameNumber.setter
    def IVUSPullbackStartFrameNumber(self, value: Optional[int]):
        if value is None:
            if "IVUSPullbackStartFrameNumber" in self._dataset:
                del self._dataset.IVUSPullbackStartFrameNumber
        else:
            self._dataset.IVUSPullbackStartFrameNumber = value

    @property
    def IVUSPullbackStopFrameNumber(self) -> Optional[int]:
        if "IVUSPullbackStopFrameNumber" in self._dataset:
            return self._dataset.IVUSPullbackStopFrameNumber
        return None

    @IVUSPullbackStopFrameNumber.setter
    def IVUSPullbackStopFrameNumber(self, value: Optional[int]):
        if value is None:
            if "IVUSPullbackStopFrameNumber" in self._dataset:
                del self._dataset.IVUSPullbackStopFrameNumber
        else:
            self._dataset.IVUSPullbackStopFrameNumber = value
