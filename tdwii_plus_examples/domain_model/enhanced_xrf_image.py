...
from .referenced_other_plane_sequence_item import ReferencedOtherPlaneSequenceItem
...
class EnhancedXrfImage:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._ReferencedOtherPlaneSequence: List[ReferencedOtherPlaneSequenceItem] = []
    ...
    @property
    def ReferencedOtherPlaneSequence(self) -> Optional[List[ReferencedOtherPlaneSequenceItem]]:
        if "ReferencedOtherPlaneSequence" in self._dataset:
            if len(self._ReferencedOtherPlaneSequence) == len(self._dataset.ReferencedOtherPlaneSequence):
                return self._ReferencedOtherPlaneSequence
            else:
                return [ReferencedOtherPlaneSequenceItem(x) for x in self._dataset.ReferencedOtherPlaneSequence]
        return None
    ...
    def ReferencedOtherPlaneSequence(self, value: Optional[List[ReferencedOtherPlaneSequenceItem]]):
        if value is None:
            self._ReferencedOtherPlaneSequence = []
            if "ReferencedOtherPlaneSequence" in self._dataset:
                del self._dataset.ReferencedOtherPlaneSequence
        elif not isinstance(value, list) or not all(isinstance(item, ReferencedOtherPlaneSequenceItem) for item in value):
            raise ValueError(f"ReferencedOtherPlaneSequence must be a list of ReferencedOtherPlaneSequenceItem objects")
        else:
            self._ReferencedOtherPlaneSequence = value
            if "ReferencedOtherPlaneSequence" not in self._dataset:
                self._dataset.ReferencedOtherPlaneSequence = pydicom.Sequence()
            self._dataset.ReferencedOtherPlaneSequence.clear()
            self._dataset.ReferencedOtherPlaneSequence.extend([item.to_dataset() for item in value])

    def add_ReferencedOtherPlane(self, item: ReferencedOtherPlaneSequenceItem):
        if not isinstance(item, ReferencedOtherPlaneSequenceItem):
            raise ValueError(f"Item must be an instance of ReferencedOtherPlaneSequenceItem")
        self._ReferencedOtherPlaneSequence.append(item)
        if "ReferencedOtherPlaneSequence" not in self._dataset:
            self._dataset.ReferencedOtherPlaneSequence = pydicom.Sequence()
        self._dataset.ReferencedOtherPlaneSequence.append(item.to_dataset())

    @property
    def ExaminedBodyThickness(self) -> Optional[float]:
        if "ExaminedBodyThickness" in self._dataset:
            return self._dataset.ExaminedBodyThickness
        return None

    @ExaminedBodyThickness.setter
    def ExaminedBodyThickness(self, value: Optional[float]):
        if value is None:
            if "ExaminedBodyThickness" in self._dataset:
                del self._dataset.ExaminedBodyThickness
        else:
            self._dataset.ExaminedBodyThickness = value
    ...
    @property
    def AcquisitionProtocolDescription(self) -> Optional[str]:
        if "AcquisitionProtocolDescription" in self._dataset:
            return self._dataset.AcquisitionProtocolDescription
        return None

    @AcquisitionProtocolDescription.setter
    def AcquisitionProtocolDescription(self, value: Optional[str]):
        if value is None:
            if "AcquisitionProtocolDescription" in self._dataset:
                del self._dataset.AcquisitionProtocolDescription
        else:
            self._dataset.AcquisitionProtocolDescription = value
    ...
    @property
    def DistanceReceptorPlaneToDetectorHousing(self) -> Optional[float]:
        if "DistanceReceptorPlaneToDetectorHousing" in self._dataset:
            return self._dataset.DistanceReceptorPlaneToDetectorHousing
        return None

    @DistanceReceptorPlaneToDetectorHousing.setter
    def DistanceReceptorPlaneToDetectorHousing(self, value: Optional[float]):
        if value is None:
            if "DistanceReceptorPlaneToDetectorHousing" in self._dataset:
                del self._dataset.DistanceReceptorPlaneToDetectorHousing
        else:
            self._dataset.DistanceReceptorPlaneToDetectorHousing = value

    @property
    def AcquiredImageAreaDoseProduct(self) -> Optional[float]:
        if "AcquiredImageAreaDoseProduct" in self._dataset:
            return self._dataset.AcquiredImageAreaDoseProduct
        return None

    @AcquiredImageAreaDoseProduct.setter
    def AcquiredImageAreaDoseProduct(self, value: Optional[float]):
        if value is None:
            if "AcquiredImageAreaDoseProduct" in self._dataset:
                del self._dataset.AcquiredImageAreaDoseProduct
        else:
            self._dataset.AcquiredImageAreaDoseProduct = value

    @property
    def CArmPositionerTabletopRelationship(self) -> Optional[str]:
        if "CArmPositionerTabletopRelationship" in self._dataset:
            return self._dataset.CArmPositionerTabletopRelationship
        return None

    @CArmPositionerTabletopRelationship.setter
    def CArmPositionerTabletopRelationship(self, value: Optional[str]):
        if value is None:
            if "CArmPositionerTabletopRelationship" in self._dataset:
                del self._dataset.CArmPositionerTabletopRelationship
        else:
            self._dataset.CArmPositionerTabletopRelationship = value
    ...
    @property
    def IntensifierActiveShape(self) -> Optional[str]:
        if "IntensifierActiveShape" in self._dataset:
            return self._dataset.IntensifierActiveShape
        return None

    @IntensifierActiveShape.setter
    def IntensifierActiveShape(self, value: Optional[str]):
        if value is None:
            if "IntensifierActiveShape" in self._dataset:
                del self._dataset.IntensifierActiveShape
        else:
            self._dataset.IntensifierActiveShape = value

    @property
    def IntensifierActiveDimensions(self) -> Optional[List[float]]:
        if "IntensifierActiveDimensions" in self._dataset:
            return self._dataset.IntensifierActiveDimensions
        return None

    @IntensifierActiveDimensions.setter
    def IntensifierActiveDimensions(self, value: Optional[List[float]]):
        if value is None:
            if "IntensifierActiveDimensions" in self._dataset:
                del self._dataset.IntensifierActiveDimensions
        else:
            self._dataset.IntensifierActiveDimensions = value
