...
class IndexProbabilitySequenceItem:
    ...
    @property
    def IndexProbability(self) -> Optional[float]:
        if "IndexProbability" in self._dataset:
            return self._dataset.IndexProbability
        return None

    @IndexProbability.setter
    def IndexProbability(self, value: Optional[float]):
        if value is None:
            if "IndexProbability" in self._dataset:
                del self._dataset.IndexProbability
        else:
            self._dataset.IndexProbability = value
