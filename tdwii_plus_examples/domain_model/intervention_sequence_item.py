...
class InterventionSequenceItem:
    ...
    def InterventionDrugCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            ...
            raise ValueError(f"InterventionDrugCodeSequence must be a list of CodeSequenceItem objects")
        else:
    ...
    @property
    def InterventionStatus(self) -> Optional[str]:
        if "InterventionStatus" in self._dataset:
            return self._dataset.InterventionStatus
        return None

    @InterventionStatus.setter
    def InterventionStatus(self, value: Optional[str]):
        if value is None:
            if "InterventionStatus" in self._dataset:
                del self._dataset.InterventionStatus
        else:
            self._dataset.InterventionStatus = value

    @property
    def InterventionDescription(self) -> Optional[str]:
        if "InterventionDescription" in self._dataset:
            return self._dataset.InterventionDescription
        return None

    @InterventionDescription.setter
    def InterventionDescription(self, value: Optional[str]):
        if value is None:
            if "InterventionDescription" in self._dataset:
                del self._dataset.InterventionDescription
        else:
            self._dataset.InterventionDescription = value
