...
from .mode_of_percutaneous_access_sequence_item import (
    ModeOfPercutaneousAccessSequenceItem,
)
...
class IntravascularOpticalCoherenceTomographyImage:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._ModeOfPercutaneousAccessSequence: List[ModeOfPercutaneousAccessSequenceItem] = []
    ...
    @property
    def InterpolationType(self) -> Optional[str]:
        if "InterpolationType" in self._dataset:
            return self._dataset.InterpolationType
        return None

    @InterpolationType.setter
    def InterpolationType(self, value: Optional[str]):
        if value is None:
            if "InterpolationType" in self._dataset:
                del self._dataset.InterpolationType
        else:
            self._dataset.InterpolationType = value
    ...
    @property
    def ALinePixelSpacing(self) -> Optional[float]:
        if "ALinePixelSpacing" in self._dataset:
            return self._dataset.ALinePixelSpacing
        return None

    @ALinePixelSpacing.setter
    def ALinePixelSpacing(self, value: Optional[float]):
        if value is None:
            if "ALinePixelSpacing" in self._dataset:
                del self._dataset.ALinePixelSpacing
        else:
            self._dataset.ALinePixelSpacing = value

    @property
    def OCTZOffsetApplied(self) -> Optional[str]:
        if "OCTZOffsetApplied" in self._dataset:
            return self._dataset.OCTZOffsetApplied
        return None

    @OCTZOffsetApplied.setter
    def OCTZOffsetApplied(self, value: Optional[str]):
        if value is None:
            if "OCTZOffsetApplied" in self._dataset:
                del self._dataset.OCTZOffsetApplied
        else:
            self._dataset.OCTZOffsetApplied = value

    @property
    def FirstALineLocation(self) -> Optional[float]:
        if "FirstALineLocation" in self._dataset:
            return self._dataset.FirstALineLocation
        return None

    @FirstALineLocation.setter
    def FirstALineLocation(self, value: Optional[float]):
        if value is None:
            if "FirstALineLocation" in self._dataset:
                del self._dataset.FirstALineLocation
        else:
            self._dataset.FirstALineLocation = value

    @property
    def RefractiveIndexApplied(self) -> Optional[str]:
        if "RefractiveIndexApplied" in self._dataset:
            return self._dataset.RefractiveIndexApplied
        return None

    @RefractiveIndexApplied.setter
    def RefractiveIndexApplied(self, value: Optional[str]):
        if value is None:
            if "RefractiveIndexApplied" in self._dataset:
                del self._dataset.RefractiveIndexApplied
        else:
            self._dataset.RefractiveIndexApplied = value
    ...
    @property
    def CatheterRotationalRate(self) -> Optional[float]:
        if "CatheterRotationalRate" in self._dataset:
            return self._dataset.CatheterRotationalRate
        return None

    @CatheterRotationalRate.setter
    def CatheterRotationalRate(self, value: Optional[float]):
        if value is None:
            if "CatheterRotationalRate" in self._dataset:
                del self._dataset.CatheterRotationalRate
        else:
            self._dataset.CatheterRotationalRate = value

    @property
    def ModeOfPercutaneousAccessSequence(self) -> Optional[List[ModeOfPercutaneousAccessSequenceItem]]:
        if "ModeOfPercutaneousAccessSequence" in self._dataset:
            if len(self._ModeOfPercutaneousAccessSequence) == len(self._dataset.ModeOfPercutaneousAccessSequence):
                return self._ModeOfPercutaneousAccessSequence
            else:
                return [ModeOfPercutaneousAccessSequenceItem(x) for x in self._dataset.ModeOfPercutaneousAccessSequence]
        return None

    @ModeOfPercutaneousAccessSequence.setter
    def ModeOfPercutaneousAccessSequence(self, value: Optional[List[ModeOfPercutaneousAccessSequenceItem]]):
        if value is None:
            self._ModeOfPercutaneousAccessSequence = []
            if "ModeOfPercutaneousAccessSequence" in self._dataset:
                del self._dataset.ModeOfPercutaneousAccessSequence
        elif not isinstance(value, list) or not all(isinstance(item, ModeOfPercutaneousAccessSequenceItem) for item in value):
            raise ValueError("ModeOfPercutaneousAccessSequence must be a list of ModeOfPercutaneousAccessSequenceItem objects")
        else:
            self._ModeOfPercutaneousAccessSequence = value
            if "ModeOfPercutaneousAccessSequence" not in self._dataset:
                self._dataset.ModeOfPercutaneousAccessSequence = pydicom.Sequence()
            self._dataset.ModeOfPercutaneousAccessSequence.clear()
            self._dataset.ModeOfPercutaneousAccessSequence.extend([item.to_dataset() for item in value])

    def add_ModeOfPercutaneousAccess(self, item: ModeOfPercutaneousAccessSequenceItem):
        if not isinstance(item, ModeOfPercutaneousAccessSequenceItem):
            raise ValueError("Item must be an instance of ModeOfPercutaneousAccessSequenceItem")
        self._ModeOfPercutaneousAccessSequence.append(item)
        if "ModeOfPercutaneousAccessSequence" not in self._dataset:
            self._dataset.ModeOfPercutaneousAccessSequence = pydicom.Sequence()
        self._dataset.ModeOfPercutaneousAccessSequence.append(item.to_dataset())

    @property
    def CatheterDirectionOfRotation(self) -> Optional[str]:
        if "CatheterDirectionOfRotation" in self._dataset:
            return self._dataset.CatheterDirectionOfRotation
        return None

    @CatheterDirectionOfRotation.setter
    def CatheterDirectionOfRotation(self, value: Optional[str]):
        if value is None:
            if "CatheterDirectionOfRotation" in self._dataset:
                del self._dataset.CatheterDirectionOfRotation
        else:
            self._dataset.CatheterDirectionOfRotation = value

    @property
    def OCTFocalDistance(self) -> Optional[float]:
        if "OCTFocalDistance" in self._dataset:
            return self._dataset.OCTFocalDistance
        return None

    @OCTFocalDistance.setter
    def OCTFocalDistance(self, value: Optional[float]):
        if value is None:
            if "OCTFocalDistance" in self._dataset:
                del self._dataset.OCTFocalDistance
        else:
            self._dataset.OCTFocalDistance = value

    @property
    def BeamSpotSize(self) -> Optional[float]:
        if "BeamSpotSize" in self._dataset:
            return self._dataset.BeamSpotSize
        return None

    @BeamSpotSize.setter
    def BeamSpotSize(self, value: Optional[float]):
        if value is None:
            if "BeamSpotSize" in self._dataset:
                del self._dataset.BeamSpotSize
        else:
            self._dataset.BeamSpotSize = value

    @property
    def EffectiveRefractiveIndex(self) -> Optional[float]:
        if "EffectiveRefractiveIndex" in self._dataset:
            return self._dataset.EffectiveRefractiveIndex
        return None

    @EffectiveRefractiveIndex.setter
    def EffectiveRefractiveIndex(self, value: Optional[float]):
        if value is None:
            if "EffectiveRefractiveIndex" in self._dataset:
                del self._dataset.EffectiveRefractiveIndex
        else:
            self._dataset.EffectiveRefractiveIndex = value

    @property
    def OCTAcquisitionDomain(self) -> Optional[str]:
        if "OCTAcquisitionDomain" in self._dataset:
            return self._dataset.OCTAcquisitionDomain
        return None

    @OCTAcquisitionDomain.setter
    def OCTAcquisitionDomain(self, value: Optional[str]):
        if value is None:
            if "OCTAcquisitionDomain" in self._dataset:
                del self._dataset.OCTAcquisitionDomain
        else:
            self._dataset.OCTAcquisitionDomain = value

    @property
    def OCTOpticalCenterWavelength(self) -> Optional[float]:
        if "OCTOpticalCenterWavelength" in self._dataset:
            return self._dataset.OCTOpticalCenterWavelength
        return None

    @OCTOpticalCenterWavelength.setter
    def OCTOpticalCenterWavelength(self, value: Optional[float]):
        if value is None:
            if "OCTOpticalCenterWavelength" in self._dataset:
                del self._dataset.OCTOpticalCenterWavelength
        else:
            self._dataset.OCTOpticalCenterWavelength = value

    @property
    def AxialResolution(self) -> Optional[float]:
        if "AxialResolution" in self._dataset:
            return self._dataset.AxialResolution
        return None

    @AxialResolution.setter
    def AxialResolution(self, value: Optional[float]):
        if value is None:
            if "AxialResolution" in self._dataset:
                del self._dataset.AxialResolution
        else:
            self._dataset.AxialResolution = value

    @property
    def RangingDepth(self) -> Optional[float]:
        if "RangingDepth" in self._dataset:
            return self._dataset.RangingDepth
        return None

    @RangingDepth.setter
    def RangingDepth(self, value: Optional[float]):
        if value is None:
            if "RangingDepth" in self._dataset:
                del self._dataset.RangingDepth
        else:
            self._dataset.RangingDepth = value

    @property
    def ALineRate(self) -> Optional[float]:
        if "ALineRate" in self._dataset:
            return self._dataset.ALineRate
        return None

    @ALineRate.setter
    def ALineRate(self, value: Optional[float]):
        if value is None:
            if "ALineRate" in self._dataset:
                del self._dataset.ALineRate
        else:
            self._dataset.ALineRate = value

    @property
    def ALinesPerFrame(self) -> Optional[int]:
        if "ALinesPerFrame" in self._dataset:
            return self._dataset.ALinesPerFrame
        return None

    @ALinesPerFrame.setter
    def ALinesPerFrame(self, value: Optional[int]):
        if value is None:
            if "ALinesPerFrame" in self._dataset:
                del self._dataset.ALinesPerFrame
        else:
            self._dataset.ALinesPerFrame = value
