...
from .keratometry_left_eye_sequence_item import KeratometryLeftEyeSequenceItem
from .keratometry_right_eye_sequence_item import KeratometryRightEyeSequenceItem
...
class KeratometryMeasurements:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._KeratometryRightEyeSequence: List[KeratometryRightEyeSequenceItem] = []
        self._KeratometryLeftEyeSequence: List[KeratometryLeftEyeSequenceItem] = []
    ...
    @property
    def KeratometryRightEyeSequence(self) -> Optional[List[KeratometryRightEyeSequenceItem]]:
        if "KeratometryRightEyeSequence" in self._dataset:
            if len(self._KeratometryRightEyeSequence) == len(self._dataset.KeratometryRightEyeSequence):
                return self._KeratometryRightEyeSequence
            else:
                return [KeratometryRightEyeSequenceItem(x) for x in self._dataset.KeratometryRightEyeSequence]
        return None

    @KeratometryRightEyeSequence.setter
    def KeratometryRightEyeSequence(self, value: Optional[List[KeratometryRightEyeSequenceItem]]):
        if value is None:
            self._KeratometryRightEyeSequence = []
            if "KeratometryRightEyeSequence" in self._dataset:
                del self._dataset.KeratometryRightEyeSequence
        elif not isinstance(value, list) or not all(isinstance(item, KeratometryRightEyeSequenceItem) for item in value):
            raise ValueError(f"KeratometryRightEyeSequence must be a list of KeratometryRightEyeSequenceItem objects")
        else:
            self._KeratometryRightEyeSequence = value
            if "KeratometryRightEyeSequence" not in self._dataset:
                self._dataset.KeratometryRightEyeSequence = pydicom.Sequence()
            self._dataset.KeratometryRightEyeSequence.clear()
            self._dataset.KeratometryRightEyeSequence.extend([item.to_dataset() for item in value])

    def add_KeratometryRightEye(self, item: KeratometryRightEyeSequenceItem):
        if not isinstance(item, KeratometryRightEyeSequenceItem):
            raise ValueError(f"Item must be an instance of KeratometryRightEyeSequenceItem")
        self._KeratometryRightEyeSequence.append(item)
        if "KeratometryRightEyeSequence" not in self._dataset:
            self._dataset.KeratometryRightEyeSequence = pydicom.Sequence()
        self._dataset.KeratometryRightEyeSequence.append(item.to_dataset())

    @property
    def KeratometryLeftEyeSequence(self) -> Optional[List[KeratometryLeftEyeSequenceItem]]:
        if "KeratometryLeftEyeSequence" in self._dataset:
            if len(self._KeratometryLeftEyeSequence) == len(self._dataset.KeratometryLeftEyeSequence):
                return self._KeratometryLeftEyeSequence
            else:
                return [KeratometryLeftEyeSequenceItem(x) for x in self._dataset.KeratometryLeftEyeSequence]
        return None

    @KeratometryLeftEyeSequence.setter
    def KeratometryLeftEyeSequence(self, value: Optional[List[KeratometryLeftEyeSequenceItem]]):
        if value is None:
            self._KeratometryLeftEyeSequence = []
            if "KeratometryLeftEyeSequence" in self._dataset:
                del self._dataset.KeratometryLeftEyeSequence
        elif not isinstance(value, list) or not all(isinstance(item, KeratometryLeftEyeSequenceItem) for item in value):
            raise ValueError(f"KeratometryLeftEyeSequence must be a list of KeratometryLeftEyeSequenceItem objects")
        else:
            self._KeratometryLeftEyeSequence = value
            if "KeratometryLeftEyeSequence" not in self._dataset:
                self._dataset.KeratometryLeftEyeSequence = pydicom.Sequence()
            self._dataset.KeratometryLeftEyeSequence.clear()
            self._dataset.KeratometryLeftEyeSequence.extend([item.to_dataset() for item in value])

    def add_KeratometryLeftEye(self, item: KeratometryLeftEyeSequenceItem):
        if not isinstance(item, KeratometryLeftEyeSequenceItem):
            raise ValueError(f"Item must be an instance of KeratometryLeftEyeSequenceItem")
        self._KeratometryLeftEyeSequence.append(item)
        if "KeratometryLeftEyeSequence" not in self._dataset:
            self._dataset.KeratometryLeftEyeSequence = pydicom.Sequence()
        self._dataset.KeratometryLeftEyeSequence.append(item.to_dataset())
