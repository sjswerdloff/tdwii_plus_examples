...
from .left_lens_sequence_item import LeftLensSequenceItem
...
from .right_lens_sequence_item import RightLensSequenceItem
...
from .unspecified_laterality_lens_sequence_item import (
    UnspecifiedLateralityLensSequenceItem,
)


class LensometryMeasurements:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._RightLensSequence: List[RightLensSequenceItem] = []
        self._LeftLensSequence: List[LeftLensSequenceItem] = []
        self._UnspecifiedLateralityLensSequence: List[UnspecifiedLateralityLensSequenceItem] = []
    ...
    @property
    def LensDescription(self) -> Optional[str]:
        if "LensDescription" in self._dataset:
            return self._dataset.LensDescription
        return None

    @LensDescription.setter
    def LensDescription(self, value: Optional[str]):
        if value is None:
            if "LensDescription" in self._dataset:
                del self._dataset.LensDescription
        else:
            self._dataset.LensDescription = value

    @property
    def RightLensSequence(self) -> Optional[List[RightLensSequenceItem]]:
        if "RightLensSequence" in self._dataset:
            if len(self._RightLensSequence) == len(self._dataset.RightLensSequence):
                return self._RightLensSequence
            else:
                return [RightLensSequenceItem(x) for x in self._dataset.RightLensSequence]
        return None

    @RightLensSequence.setter
    def RightLensSequence(self, value: Optional[List[RightLensSequenceItem]]):
        if value is None:
            self._RightLensSequence = []
            if "RightLensSequence" in self._dataset:
                del self._dataset.RightLensSequence
        elif not isinstance(value, list) or not all(isinstance(item, RightLensSequenceItem) for item in value):
            raise ValueError(f"RightLensSequence must be a list of RightLensSequenceItem objects")
        else:
            self._RightLensSequence = value
            if "RightLensSequence" not in self._dataset:
                self._dataset.RightLensSequence = pydicom.Sequence()
            self._dataset.RightLensSequence.clear()
            self._dataset.RightLensSequence.extend([item.to_dataset() for item in value])

    def add_RightLens(self, item: RightLensSequenceItem):
        if not isinstance(item, RightLensSequenceItem):
            raise ValueError(f"Item must be an instance of RightLensSequenceItem")
        self._RightLensSequence.append(item)
        if "RightLensSequence" not in self._dataset:
            self._dataset.RightLensSequence = pydicom.Sequence()
        self._dataset.RightLensSequence.append(item.to_dataset())

    @property
    def LeftLensSequence(self) -> Optional[List[LeftLensSequenceItem]]:
        if "LeftLensSequence" in self._dataset:
            if len(self._LeftLensSequence) == len(self._dataset.LeftLensSequence):
                return self._LeftLensSequence
            else:
                return [LeftLensSequenceItem(x) for x in self._dataset.LeftLensSequence]
        return None

    @LeftLensSequence.setter
    def LeftLensSequence(self, value: Optional[List[LeftLensSequenceItem]]):
        if value is None:
            self._LeftLensSequence = []
            if "LeftLensSequence" in self._dataset:
                del self._dataset.LeftLensSequence
        elif not isinstance(value, list) or not all(isinstance(item, LeftLensSequenceItem) for item in value):
            raise ValueError(f"LeftLensSequence must be a list of LeftLensSequenceItem objects")
        else:
            self._LeftLensSequence = value
            if "LeftLensSequence" not in self._dataset:
                self._dataset.LeftLensSequence = pydicom.Sequence()
            self._dataset.LeftLensSequence.clear()
            self._dataset.LeftLensSequence.extend([item.to_dataset() for item in value])

    def add_LeftLens(self, item: LeftLensSequenceItem):
        if not isinstance(item, LeftLensSequenceItem):
            raise ValueError(f"Item must be an instance of LeftLensSequenceItem")
        self._LeftLensSequence.append(item)
        if "LeftLensSequence" not in self._dataset:
            self._dataset.LeftLensSequence = pydicom.Sequence()
        self._dataset.LeftLensSequence.append(item.to_dataset())

    @property
    def UnspecifiedLateralityLensSequence(self) -> Optional[List[UnspecifiedLateralityLensSequenceItem]]:
        if "UnspecifiedLateralityLensSequence" in self._dataset:
            if len(self._UnspecifiedLateralityLensSequence) == len(self._dataset.UnspecifiedLateralityLensSequence):
                return self._UnspecifiedLateralityLensSequence
            else:
                return [UnspecifiedLateralityLensSequenceItem(x) for x in self._dataset.UnspecifiedLateralityLensSequence]
        return None

    @UnspecifiedLateralityLensSequence.setter
    def UnspecifiedLateralityLensSequence(self, value: Optional[List[UnspecifiedLateralityLensSequenceItem]]):
        if value is None:
            self._UnspecifiedLateralityLensSequence = []
            if "UnspecifiedLateralityLensSequence" in self._dataset:
                del self._dataset.UnspecifiedLateralityLensSequence
        elif not isinstance(value, list) or not all(isinstance(item, UnspecifiedLateralityLensSequenceItem) for item in value):
            raise ValueError(
                f"UnspecifiedLateralityLensSequence must be a list of UnspecifiedLateralityLensSequenceItem objects"
            )
        else:
            self._UnspecifiedLateralityLensSequence = value
            if "UnspecifiedLateralityLensSequence" not in self._dataset:
                self._dataset.UnspecifiedLateralityLensSequence = pydicom.Sequence()
            self._dataset.UnspecifiedLateralityLensSequence.clear()
            self._dataset.UnspecifiedLateralityLensSequence.extend([item.to_dataset() for item in value])

    def add_UnspecifiedLateralityLens(self, item: UnspecifiedLateralityLensSequenceItem):
        if not isinstance(item, UnspecifiedLateralityLensSequenceItem):
            raise ValueError(f"Item must be an instance of UnspecifiedLateralityLensSequenceItem")
        self._UnspecifiedLateralityLensSequence.append(item)
        if "UnspecifiedLateralityLensSequence" not in self._dataset:
            self._dataset.UnspecifiedLateralityLensSequence = pydicom.Sequence()
        self._dataset.UnspecifiedLateralityLensSequence.append(item.to_dataset())
