...
from .annotation_group_sequence_item import AnnotationGroupSequenceItem
...
class MicroscopyBulkSimpleAnnotations:
    ...
    @property
    def AnnotationCoordinateType(self) -> Optional[str]:
        if "AnnotationCoordinateType" in self._dataset:
            return self._dataset.AnnotationCoordinateType
        return None

    @AnnotationCoordinateType.setter
    def AnnotationCoordinateType(self, value: Optional[str]):
        if value is None:
            if "AnnotationCoordinateType" in self._dataset:
                del self._dataset.AnnotationCoordinateType
        else:
            self._dataset.AnnotationCoordinateType = value

    @property
    def AnnotationGroupSequence(self) -> Optional[List[AnnotationGroupSequenceItem]]:
        if "AnnotationGroupSequence" in self._dataset:
            if len(self._AnnotationGroupSequence) == len(self._dataset.AnnotationGroupSequence):
                return self._AnnotationGroupSequence
            else:
                return [AnnotationGroupSequenceItem(x) for x in self._dataset.AnnotationGroupSequence]
        return None

    @AnnotationGroupSequence.setter
    def AnnotationGroupSequence(self, value: Optional[List[AnnotationGroupSequenceItem]]):
        if value is None:
            self._AnnotationGroupSequence = []
            if "AnnotationGroupSequence" in self._dataset:
                del self._dataset.AnnotationGroupSequence
        elif not isinstance(value, list) or not all(isinstance(item, AnnotationGroupSequenceItem) for item in value):
            raise ValueError(f"AnnotationGroupSequence must be a list of AnnotationGroupSequenceItem objects")
        else:
            self._AnnotationGroupSequence = value
            if "AnnotationGroupSequence" not in self._dataset:
                self._dataset.AnnotationGroupSequence = pydicom.Sequence()
            self._dataset.AnnotationGroupSequence.clear()
            self._dataset.AnnotationGroupSequence.extend([item.to_dataset() for item in value])

    def add_AnnotationGroup(self, item: AnnotationGroupSequenceItem):
        if not isinstance(item, AnnotationGroupSequenceItem):
            raise ValueError(f"Item must be an instance of AnnotationGroupSequenceItem")
        self._AnnotationGroupSequence.append(item)
        if "AnnotationGroupSequence" not in self._dataset:
            self._dataset.AnnotationGroupSequence = pydicom.Sequence()
        self._dataset.AnnotationGroupSequence.append(item.to_dataset())
