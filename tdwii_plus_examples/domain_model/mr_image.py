...
from .scanning_sequence_item import ScanningSequenceItem
...
class MrImage:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._AnatomicRegionSequence: List[AnatomicRegionSequenceItem] = []
        self._PrimaryAnatomicStructureSequence: List[PrimaryAnatomicStructureSequenceItem] = []
        self._ScanningSequence: List[ScanningSequenceItem] = []
    ...
    @property
    def ScanningSequence(self) -> Optional[List[ScanningSequenceItem]]:
        if "ScanningSequence" in self._dataset:
            if len(self._ScanningSequence) == len(self._dataset.ScanningSequence):
                return self._ScanningSequence
            else:
                return [ScanningSequenceItem(x) for x in self._dataset.ScanningSequence]
        return None

    @ScanningSequence.setter
    def ScanningSequence(self, value: Optional[List[ScanningSequenceItem]]):
        if value is None:
            self._ScanningSequence = []
            if "ScanningSequence" in self._dataset:
                del self._dataset.ScanningSequence
        elif not isinstance(value, list) or not all(isinstance(item, ScanningSequenceItem) for item in value):
            raise ValueError(f"ScanningSequence must be a list of ScanningSequenceItem objects")
        else:
            self._ScanningSequence = value
            if "ScanningSequence" not in self._dataset:
                self._dataset.ScanningSequence = pydicom.Sequence()
            self._dataset.ScanningSequence.clear()
            self._dataset.ScanningSequence.extend([item.to_dataset() for item in value])

    def add_Scanning(self, item: ScanningSequenceItem):
        if not isinstance(item, ScanningSequenceItem):
            raise ValueError(f"Item must be an instance of ScanningSequenceItem")
        self._ScanningSequence.append(item)
        if "ScanningSequence" not in self._dataset:
            self._dataset.ScanningSequence = pydicom.Sequence()
        self._dataset.ScanningSequence.append(item.to_dataset())

    @property
    def SequenceVariant(self) -> Optional[List[str]]:
        if "SequenceVariant" in self._dataset:
            return self._dataset.SequenceVariant
        return None

    @SequenceVariant.setter
    def SequenceVariant(self, value: Optional[List[str]]):
        if value is None:
            if "SequenceVariant" in self._dataset:
                del self._dataset.SequenceVariant
        else:
            self._dataset.SequenceVariant = value
    ...
    @property
    def MRAcquisitionType(self) -> Optional[str]:
        if "MRAcquisitionType" in self._dataset:
            return self._dataset.MRAcquisitionType
        return None

    @MRAcquisitionType.setter
    def MRAcquisitionType(self, value: Optional[str]):
        if value is None:
            if "MRAcquisitionType" in self._dataset:
                del self._dataset.MRAcquisitionType
        else:
            self._dataset.MRAcquisitionType = value

    @property
    def SequenceName(self) -> Optional[str]:
        if "SequenceName" in self._dataset:
            return self._dataset.SequenceName
        return None

    @SequenceName.setter
    def SequenceName(self, value: Optional[str]):
        if value is None:
            if "SequenceName" in self._dataset:
                del self._dataset.SequenceName
        else:
            self._dataset.SequenceName = value

    @property
    def AngioFlag(self) -> Optional[str]:
        if "AngioFlag" in self._dataset:
            return self._dataset.AngioFlag
        return None

    @AngioFlag.setter
    def AngioFlag(self, value: Optional[str]):
        if value is None:
            if "AngioFlag" in self._dataset:
                del self._dataset.AngioFlag
        else:
            self._dataset.AngioFlag = value
    ...
    @property
    def EchoTime(self) -> Optional[Decimal]:
        if "EchoTime" in self._dataset:
            return self._dataset.EchoTime
        return None

    @EchoTime.setter
    def EchoTime(self, value: Optional[Decimal]):
        if value is None:
            if "EchoTime" in self._dataset:
                del self._dataset.EchoTime
        else:
            self._dataset.EchoTime = value

    @property
    def InversionTime(self) -> Optional[Decimal]:
        if "InversionTime" in self._dataset:
            return self._dataset.InversionTime
        return None

    @InversionTime.setter
    def InversionTime(self, value: Optional[Decimal]):
        if value is None:
            if "InversionTime" in self._dataset:
                del self._dataset.InversionTime
        else:
            self._dataset.InversionTime = value
    ...
    @property
    def ImagingFrequency(self) -> Optional[Decimal]:
        if "ImagingFrequency" in self._dataset:
            return self._dataset.ImagingFrequency
        return None

    @ImagingFrequency.setter
    def ImagingFrequency(self, value: Optional[Decimal]):
        if value is None:
            if "ImagingFrequency" in self._dataset:
                del self._dataset.ImagingFrequency
        else:
            self._dataset.ImagingFrequency = value

    @property
    def ImagedNucleus(self) -> Optional[str]:
        if "ImagedNucleus" in self._dataset:
            return self._dataset.ImagedNucleus
        return None

    @ImagedNucleus.setter
    def ImagedNucleus(self, value: Optional[str]):
        if value is None:
            if "ImagedNucleus" in self._dataset:
                del self._dataset.ImagedNucleus
        else:
            self._dataset.ImagedNucleus = value

    @property
    def EchoNumbers(self) -> Optional[List[int]]:
        if "EchoNumbers" in self._dataset:
            return self._dataset.EchoNumbers
        return None

    @EchoNumbers.setter
    def EchoNumbers(self, value: Optional[List[int]]):
        if value is None:
            if "EchoNumbers" in self._dataset:
                del self._dataset.EchoNumbers
        else:
            self._dataset.EchoNumbers = value
    ...
    @property
    def NumberOfPhaseEncodingSteps(self) -> Optional[int]:
        if "NumberOfPhaseEncodingSteps" in self._dataset:
            return self._dataset.NumberOfPhaseEncodingSteps
        return None

    @NumberOfPhaseEncodingSteps.setter
    def NumberOfPhaseEncodingSteps(self, value: Optional[int]):
        if value is None:
            if "NumberOfPhaseEncodingSteps" in self._dataset:
                del self._dataset.NumberOfPhaseEncodingSteps
        else:
            self._dataset.NumberOfPhaseEncodingSteps = value
    ...
    @property
    def CardiacNumberOfImages(self) -> Optional[int]:
        if "CardiacNumberOfImages" in self._dataset:
            return self._dataset.CardiacNumberOfImages
        return None

    @CardiacNumberOfImages.setter
    def CardiacNumberOfImages(self, value: Optional[int]):
        if value is None:
            if "CardiacNumberOfImages" in self._dataset:
                del self._dataset.CardiacNumberOfImages
        else:
            self._dataset.CardiacNumberOfImages = value

    @property
    def TriggerWindow(self) -> Optional[int]:
        if "TriggerWindow" in self._dataset:
            return self._dataset.TriggerWindow
        return None

    @TriggerWindow.setter
    def TriggerWindow(self, value: Optional[int]):
        if value is None:
            if "TriggerWindow" in self._dataset:
                del self._dataset.TriggerWindow
        else:
            self._dataset.TriggerWindow = value
    ...
    @property
    def AcquisitionMatrix(self) -> Optional[List[int]]:
        if "AcquisitionMatrix" in self._dataset:
            return self._dataset.AcquisitionMatrix
        return None

    @AcquisitionMatrix.setter
    def AcquisitionMatrix(self, value: Optional[List[int]]):
        if value is None:
            if "AcquisitionMatrix" in self._dataset:
                del self._dataset.AcquisitionMatrix
        else:
            self._dataset.AcquisitionMatrix = value
    ...
    @property
    def VariableFlipAngleFlag(self) -> Optional[str]:
        if "VariableFlipAngleFlag" in self._dataset:
            return self._dataset.VariableFlipAngleFlag
        return None

    @VariableFlipAngleFlag.setter
    def VariableFlipAngleFlag(self, value: Optional[str]):
        if value is None:
            if "VariableFlipAngleFlag" in self._dataset:
                del self._dataset.VariableFlipAngleFlag
        else:
            self._dataset.VariableFlipAngleFlag = value

    @property
    def SAR(self) -> Optional[Decimal]:
        if "SAR" in self._dataset:
            return self._dataset.SAR
        return None

    @SAR.setter
    def SAR(self, value: Optional[Decimal]):
        if value is None:
            if "SAR" in self._dataset:
                del self._dataset.SAR
        else:
            self._dataset.SAR = value

    @property
    def dBdt(self) -> Optional[Decimal]:
        if "dBdt" in self._dataset:
            return self._dataset.dBdt
        return None

    @dBdt.setter
    def dBdt(self, value: Optional[Decimal]):
        if value is None:
            if "dBdt" in self._dataset:
                del self._dataset.dBdt
        else:
            self._dataset.dBdt = value
    ...
    @property
    def TemporalPositionIdentifier(self) -> Optional[int]:
        if "TemporalPositionIdentifier" in self._dataset:
            return self._dataset.TemporalPositionIdentifier
        return None

    @TemporalPositionIdentifier.setter
    def TemporalPositionIdentifier(self, value: Optional[int]):
        if value is None:
            if "TemporalPositionIdentifier" in self._dataset:
                del self._dataset.TemporalPositionIdentifier
        else:
            self._dataset.TemporalPositionIdentifier = value

    @property
    def NumberOfTemporalPositions(self) -> Optional[int]:
        if "NumberOfTemporalPositions" in self._dataset:
            return self._dataset.NumberOfTemporalPositions
        return None

    @NumberOfTemporalPositions.setter
    def NumberOfTemporalPositions(self, value: Optional[int]):
        if value is None:
            if "NumberOfTemporalPositions" in self._dataset:
                del self._dataset.NumberOfTemporalPositions
        else:
            self._dataset.NumberOfTemporalPositions = value

    @property
    def TemporalResolution(self) -> Optional[Decimal]:
        if "TemporalResolution" in self._dataset:
            return self._dataset.TemporalResolution
        return None

    @TemporalResolution.setter
    def TemporalResolution(self, value: Optional[Decimal]):
        if value is None:
            if "TemporalResolution" in self._dataset:
                del self._dataset.TemporalResolution
        else:
            self._dataset.TemporalResolution = value
