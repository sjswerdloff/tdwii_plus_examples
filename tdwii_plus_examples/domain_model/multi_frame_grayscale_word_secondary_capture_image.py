...
class MultiFrameGrayscaleWordSecondaryCaptureImage:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._DocumentClassCodeSequence: List[CodeSequenceItem] = []
    ...
    @property
    def PageNumberVector(self) -> Optional[List[int]]:
        if "PageNumberVector" in self._dataset:
            return self._dataset.PageNumberVector
        return None

    @PageNumberVector.setter
    def PageNumberVector(self, value: Optional[List[int]]):
        if value is None:
            if "PageNumberVector" in self._dataset:
                del self._dataset.PageNumberVector
        else:
            self._dataset.PageNumberVector = value

    @property
    def FrameLabelVector(self) -> Optional[List[str]]:
        if "FrameLabelVector" in self._dataset:
            return self._dataset.FrameLabelVector
        return None

    @FrameLabelVector.setter
    def FrameLabelVector(self, value: Optional[List[str]]):
        if value is None:
            if "FrameLabelVector" in self._dataset:
                del self._dataset.FrameLabelVector
        else:
            self._dataset.FrameLabelVector = value

    @property
    def FramePrimaryAngleVector(self) -> Optional[List[Decimal]]:
        if "FramePrimaryAngleVector" in self._dataset:
            return self._dataset.FramePrimaryAngleVector
        return None

    @FramePrimaryAngleVector.setter
    def FramePrimaryAngleVector(self, value: Optional[List[Decimal]]):
        if value is None:
            if "FramePrimaryAngleVector" in self._dataset:
                del self._dataset.FramePrimaryAngleVector
        else:
            self._dataset.FramePrimaryAngleVector = value

    @property
    def FrameSecondaryAngleVector(self) -> Optional[List[Decimal]]:
        if "FrameSecondaryAngleVector" in self._dataset:
            return self._dataset.FrameSecondaryAngleVector
        return None

    @FrameSecondaryAngleVector.setter
    def FrameSecondaryAngleVector(self, value: Optional[List[Decimal]]):
        if value is None:
            if "FrameSecondaryAngleVector" in self._dataset:
                del self._dataset.FrameSecondaryAngleVector
        else:
            self._dataset.FrameSecondaryAngleVector = value

    @property
    def SliceLocationVector(self) -> Optional[List[Decimal]]:
        if "SliceLocationVector" in self._dataset:
            return self._dataset.SliceLocationVector
        return None

    @SliceLocationVector.setter
    def SliceLocationVector(self, value: Optional[List[Decimal]]):
        if value is None:
            if "SliceLocationVector" in self._dataset:
                del self._dataset.SliceLocationVector
        else:
            self._dataset.SliceLocationVector = value

    @property
    def DisplayWindowLabelVector(self) -> Optional[List[str]]:
        if "DisplayWindowLabelVector" in self._dataset:
            return self._dataset.DisplayWindowLabelVector
        return None

    @DisplayWindowLabelVector.setter
    def DisplayWindowLabelVector(self, value: Optional[List[str]]):
        if value is None:
            if "DisplayWindowLabelVector" in self._dataset:
                del self._dataset.DisplayWindowLabelVector
        else:
            self._dataset.DisplayWindowLabelVector = value
    ...
    @property
    def DateOfSecondaryCapture(self) -> Optional[str]:
        if "DateOfSecondaryCapture" in self._dataset:
            return self._dataset.DateOfSecondaryCapture
        return None

    @DateOfSecondaryCapture.setter
    def DateOfSecondaryCapture(self, value: Optional[str]):
        if value is None:
            if "DateOfSecondaryCapture" in self._dataset:
                del self._dataset.DateOfSecondaryCapture
        else:
            self._dataset.DateOfSecondaryCapture = value

    @property
    def TimeOfSecondaryCapture(self) -> Optional[str]:
        if "TimeOfSecondaryCapture" in self._dataset:
            return self._dataset.TimeOfSecondaryCapture
        return None

    @TimeOfSecondaryCapture.setter
    def TimeOfSecondaryCapture(self, value: Optional[str]):
        if value is None:
            if "TimeOfSecondaryCapture" in self._dataset:
                del self._dataset.TimeOfSecondaryCapture
        else:
            self._dataset.TimeOfSecondaryCapture = value

    @property
    def NominalScannedPixelSpacing(self) -> Optional[List[Decimal]]:
        if "NominalScannedPixelSpacing" in self._dataset:
            return self._dataset.NominalScannedPixelSpacing
        return None

    @NominalScannedPixelSpacing.setter
    def NominalScannedPixelSpacing(self, value: Optional[List[Decimal]]):
        if value is None:
            if "NominalScannedPixelSpacing" in self._dataset:
                del self._dataset.NominalScannedPixelSpacing
        else:
            self._dataset.NominalScannedPixelSpacing = value
    ...
    @property
    def DocumentClassCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "DocumentClassCodeSequence" in self._dataset:
            if len(self._DocumentClassCodeSequence) == len(self._dataset.DocumentClassCodeSequence):
                return self._DocumentClassCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.DocumentClassCodeSequence]
        return None

    @DocumentClassCodeSequence.setter
    def DocumentClassCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._DocumentClassCodeSequence = []
            if "DocumentClassCodeSequence" in self._dataset:
                del self._dataset.DocumentClassCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"DocumentClassCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._DocumentClassCodeSequence = value
            if "DocumentClassCodeSequence" not in self._dataset:
                self._dataset.DocumentClassCodeSequence = pydicom.Sequence()
            self._dataset.DocumentClassCodeSequence.clear()
            self._dataset.DocumentClassCodeSequence.extend([item.to_dataset() for item in value])

    def add_DocumentClassCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._DocumentClassCodeSequence.append(item)
        if "DocumentClassCodeSequence" not in self._dataset:
            self._dataset.DocumentClassCodeSequence = pydicom.Sequence()
        self._dataset.DocumentClassCodeSequence.append(item.to_dataset())
    ...
    @property
    def DigitizingDeviceTransportDirection(self) -> Optional[str]:
        if "DigitizingDeviceTransportDirection" in self._dataset:
            return self._dataset.DigitizingDeviceTransportDirection
        return None

    @DigitizingDeviceTransportDirection.setter
    def DigitizingDeviceTransportDirection(self, value: Optional[str]):
        if value is None:
            if "DigitizingDeviceTransportDirection" in self._dataset:
                del self._dataset.DigitizingDeviceTransportDirection
        else:
            self._dataset.DigitizingDeviceTransportDirection = value

    @property
    def RotationOfScannedFilm(self) -> Optional[Decimal]:
        if "RotationOfScannedFilm" in self._dataset:
            return self._dataset.RotationOfScannedFilm
        return None

    @RotationOfScannedFilm.setter
    def RotationOfScannedFilm(self, value: Optional[Decimal]):
        if value is None:
            if "RotationOfScannedFilm" in self._dataset:
                del self._dataset.RotationOfScannedFilm
        else:
            self._dataset.RotationOfScannedFilm = value
    ...
    @property
    def Illumination(self) -> Optional[int]:
        if "Illumination" in self._dataset:
            return self._dataset.Illumination
        return None

    @Illumination.setter
    def Illumination(self, value: Optional[int]):
        if value is None:
            if "Illumination" in self._dataset:
                del self._dataset.Illumination
        else:
            self._dataset.Illumination = value

    @property
    def ReflectedAmbientLight(self) -> Optional[int]:
        if "ReflectedAmbientLight" in self._dataset:
            return self._dataset.ReflectedAmbientLight
        return None

    @ReflectedAmbientLight.setter
    def ReflectedAmbientLight(self, value: Optional[int]):
        if value is None:
            if "ReflectedAmbientLight" in self._dataset:
                del self._dataset.ReflectedAmbientLight
        else:
            self._dataset.ReflectedAmbientLight = value
    ...
    @property
    def ConversionType(self) -> Optional[str]:
        if "ConversionType" in self._dataset:
            return self._dataset.ConversionType
        return None

    @ConversionType.setter
    def ConversionType(self, value: Optional[str]):
        if value is None:
            if "ConversionType" in self._dataset:
                del self._dataset.ConversionType
        else:
            self._dataset.ConversionType = value

    @property
    def SecondaryCaptureDeviceID(self) -> Optional[str]:
        if "SecondaryCaptureDeviceID" in self._dataset:
            return self._dataset.SecondaryCaptureDeviceID
        return None

    @SecondaryCaptureDeviceID.setter
    def SecondaryCaptureDeviceID(self, value: Optional[str]):
        if value is None:
            if "SecondaryCaptureDeviceID" in self._dataset:
                del self._dataset.SecondaryCaptureDeviceID
        else:
            self._dataset.SecondaryCaptureDeviceID = value

    @property
    def SecondaryCaptureDeviceManufacturer(self) -> Optional[str]:
        if "SecondaryCaptureDeviceManufacturer" in self._dataset:
            return self._dataset.SecondaryCaptureDeviceManufacturer
        return None

    @SecondaryCaptureDeviceManufacturer.setter
    def SecondaryCaptureDeviceManufacturer(self, value: Optional[str]):
        if value is None:
            if "SecondaryCaptureDeviceManufacturer" in self._dataset:
                del self._dataset.SecondaryCaptureDeviceManufacturer
        else:
            self._dataset.SecondaryCaptureDeviceManufacturer = value

    @property
    def SecondaryCaptureDeviceManufacturerModelName(self) -> Optional[str]:
        if "SecondaryCaptureDeviceManufacturerModelName" in self._dataset:
            return self._dataset.SecondaryCaptureDeviceManufacturerModelName
        return None

    @SecondaryCaptureDeviceManufacturerModelName.setter
    def SecondaryCaptureDeviceManufacturerModelName(self, value: Optional[str]):
        if value is None:
            if "SecondaryCaptureDeviceManufacturerModelName" in self._dataset:
                del self._dataset.SecondaryCaptureDeviceManufacturerModelName
        else:
            self._dataset.SecondaryCaptureDeviceManufacturerModelName = value

    @property
    def SecondaryCaptureDeviceSoftwareVersions(self) -> Optional[List[str]]:
        if "SecondaryCaptureDeviceSoftwareVersions" in self._dataset:
            return self._dataset.SecondaryCaptureDeviceSoftwareVersions
        return None

    @SecondaryCaptureDeviceSoftwareVersions.setter
    def SecondaryCaptureDeviceSoftwareVersions(self, value: Optional[List[str]]):
        if value is None:
            if "SecondaryCaptureDeviceSoftwareVersions" in self._dataset:
                del self._dataset.SecondaryCaptureDeviceSoftwareVersions
        else:
            self._dataset.SecondaryCaptureDeviceSoftwareVersions = value

    @property
    def VideoImageFormatAcquired(self) -> Optional[str]:
        if "VideoImageFormatAcquired" in self._dataset:
            return self._dataset.VideoImageFormatAcquired
        return None

    @VideoImageFormatAcquired.setter
    def VideoImageFormatAcquired(self, value: Optional[str]):
        if value is None:
            if "VideoImageFormatAcquired" in self._dataset:
                del self._dataset.VideoImageFormatAcquired
        else:
            self._dataset.VideoImageFormatAcquired = value

    @property
    def DigitalImageFormatAcquired(self) -> Optional[str]:
        if "DigitalImageFormatAcquired" in self._dataset:
            return self._dataset.DigitalImageFormatAcquired
        return None

    @DigitalImageFormatAcquired.setter
    def DigitalImageFormatAcquired(self, value: Optional[str]):
        if value is None:
            if "DigitalImageFormatAcquired" in self._dataset:
                del self._dataset.DigitalImageFormatAcquired
        else:
            self._dataset.DigitalImageFormatAcquired = value
