...
from .detector_information_sequence_item import DetectorInformationSequenceItem
...
from .energy_window_information_sequence_item import EnergyWindowInformationSequenceItem
from .frame_extraction_sequence_item import FrameExtractionSequenceItem
from .gated_information_sequence_item import GatedInformationSequenceItem
...
from .intervention_drug_information_sequence_item import (
...
from .phase_information_sequence_item import PhaseInformationSequenceItem
...
from .radiopharmaceutical_information_sequence_item import (
...
from .rotation_information_sequence_item import RotationInformationSequenceItem
...
class NuclearMedicineImage:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._DetectorInformationSequence: List[DetectorInformationSequenceItem] = []
        ...
        self._GatedInformationSequence: List[GatedInformationSequenceItem] = []
        ...
        self._RotationInformationSequence: List[RotationInformationSequenceItem] = []
        ...
        self._EnergyWindowInformationSequence: List[EnergyWindowInformationSequenceItem] = []
        self._RadiopharmaceuticalInformationSequence: List[RadiopharmaceuticalInformationSequenceItem] = []
    ...
    @property
    def DetectorInformationSequence(self) -> Optional[List[DetectorInformationSequenceItem]]:
        if "DetectorInformationSequence" in self._dataset:
            if len(self._DetectorInformationSequence) == len(self._dataset.DetectorInformationSequence):
                return self._DetectorInformationSequence
            else:
                return [DetectorInformationSequenceItem(x) for x in self._dataset.DetectorInformationSequence]
        return None

    @DetectorInformationSequence.setter
    def DetectorInformationSequence(self, value: Optional[List[DetectorInformationSequenceItem]]):
        if value is None:
            self._DetectorInformationSequence = []
            if "DetectorInformationSequence" in self._dataset:
                del self._dataset.DetectorInformationSequence
        elif not isinstance(value, list) or not all(isinstance(item, DetectorInformationSequenceItem) for item in value):
            raise ValueError(f"DetectorInformationSequence must be a list of DetectorInformationSequenceItem objects")
        else:
            self._DetectorInformationSequence = value
            if "DetectorInformationSequence" not in self._dataset:
                self._dataset.DetectorInformationSequence = pydicom.Sequence()
            self._dataset.DetectorInformationSequence.clear()
            self._dataset.DetectorInformationSequence.extend([item.to_dataset() for item in value])

    def add_DetectorInformation(self, item: DetectorInformationSequenceItem):
        if not isinstance(item, DetectorInformationSequenceItem):
            raise ValueError(f"Item must be an instance of DetectorInformationSequenceItem")
        self._DetectorInformationSequence.append(item)
        if "DetectorInformationSequence" not in self._dataset:
            self._dataset.DetectorInformationSequence = pydicom.Sequence()
        self._dataset.DetectorInformationSequence.append(item.to_dataset())
    ...
    def PatientGantryRelationshipCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            ...
            raise ValueError(f"PatientGantryRelationshipCodeSequence must be a list of CodeSequenceItem objects")
        else:
    ...
    @property
    def FrameIncrementPointer(self) -> Optional[List[int]]:
        if "FrameIncrementPointer" in self._dataset:
            return self._dataset.FrameIncrementPointer
        return None

    @FrameIncrementPointer.setter
    def FrameIncrementPointer(self, value: Optional[List[int]]):
        if value is None:
            if "FrameIncrementPointer" in self._dataset:
                del self._dataset.FrameIncrementPointer
        else:
            self._dataset.FrameIncrementPointer = value
    ...
    @property
    def PhaseInformationSequence(self) -> Optional[List[PhaseInformationSequenceItem]]:
        if "PhaseInformationSequence" in self._dataset:
            if len(self._PhaseInformationSequence) == len(self._dataset.PhaseInformationSequence):
                return self._PhaseInformationSequence
            else:
                return [PhaseInformationSequenceItem(x) for x in self._dataset.PhaseInformationSequence]
        return None

    @PhaseInformationSequence.setter
    def PhaseInformationSequence(self, value: Optional[List[PhaseInformationSequenceItem]]):
        if value is None:
            self._PhaseInformationSequence = []
            if "PhaseInformationSequence" in self._dataset:
                del self._dataset.PhaseInformationSequence
        elif not isinstance(value, list) or not all(isinstance(item, PhaseInformationSequenceItem) for item in value):
            raise ValueError(f"PhaseInformationSequence must be a list of PhaseInformationSequenceItem objects")
        else:
            self._PhaseInformationSequence = value
            if "PhaseInformationSequence" not in self._dataset:
                self._dataset.PhaseInformationSequence = pydicom.Sequence()
            self._dataset.PhaseInformationSequence.clear()
            self._dataset.PhaseInformationSequence.extend([item.to_dataset() for item in value])

    def add_PhaseInformation(self, item: PhaseInformationSequenceItem):
        if not isinstance(item, PhaseInformationSequenceItem):
            raise ValueError(f"Item must be an instance of PhaseInformationSequenceItem")
        self._PhaseInformationSequence.append(item)
        if "PhaseInformationSequence" not in self._dataset:
            self._dataset.PhaseInformationSequence = pydicom.Sequence()
        self._dataset.PhaseInformationSequence.append(item.to_dataset())
    ...
    @property
    def SliceLocation(self) -> Optional[Decimal]:
        if "SliceLocation" in self._dataset:
            return self._dataset.SliceLocation
        return None

    @SliceLocation.setter
    def SliceLocation(self, value: Optional[Decimal]):
        if value is None:
            if "SliceLocation" in self._dataset:
                del self._dataset.SliceLocation
        else:
            self._dataset.SliceLocation = value
    ...
    @property
    def EnergyWindowVector(self) -> Optional[List[int]]:
        if "EnergyWindowVector" in self._dataset:
            return self._dataset.EnergyWindowVector
        return None

    @EnergyWindowVector.setter
    def EnergyWindowVector(self, value: Optional[List[int]]):
        if value is None:
            if "EnergyWindowVector" in self._dataset:
                del self._dataset.EnergyWindowVector
        else:
            self._dataset.EnergyWindowVector = value

    @property
    def NumberOfEnergyWindows(self) -> Optional[int]:
        if "NumberOfEnergyWindows" in self._dataset:
            return self._dataset.NumberOfEnergyWindows
        return None

    @NumberOfEnergyWindows.setter
    def NumberOfEnergyWindows(self, value: Optional[int]):
        if value is None:
            if "NumberOfEnergyWindows" in self._dataset:
                del self._dataset.NumberOfEnergyWindows
        else:
            self._dataset.NumberOfEnergyWindows = value

    @property
    def DetectorVector(self) -> Optional[List[int]]:
        if "DetectorVector" in self._dataset:
            return self._dataset.DetectorVector
        return None

    @DetectorVector.setter
    def DetectorVector(self, value: Optional[List[int]]):
        if value is None:
            if "DetectorVector" in self._dataset:
                del self._dataset.DetectorVector
        else:
            self._dataset.DetectorVector = value

    @property
    def NumberOfDetectors(self) -> Optional[int]:
        if "NumberOfDetectors" in self._dataset:
            return self._dataset.NumberOfDetectors
        return None

    @NumberOfDetectors.setter
    def NumberOfDetectors(self, value: Optional[int]):
        if value is None:
            if "NumberOfDetectors" in self._dataset:
                del self._dataset.NumberOfDetectors
        else:
            self._dataset.NumberOfDetectors = value

    @property
    def PhaseVector(self) -> Optional[List[int]]:
        if "PhaseVector" in self._dataset:
            return self._dataset.PhaseVector
        return None

    @PhaseVector.setter
    def PhaseVector(self, value: Optional[List[int]]):
        if value is None:
            if "PhaseVector" in self._dataset:
                del self._dataset.PhaseVector
        else:
            self._dataset.PhaseVector = value

    @property
    def NumberOfPhases(self) -> Optional[int]:
        if "NumberOfPhases" in self._dataset:
            return self._dataset.NumberOfPhases
        return None

    @NumberOfPhases.setter
    def NumberOfPhases(self, value: Optional[int]):
        if value is None:
            if "NumberOfPhases" in self._dataset:
                del self._dataset.NumberOfPhases
        else:
            self._dataset.NumberOfPhases = value

    @property
    def RotationVector(self) -> Optional[List[int]]:
        if "RotationVector" in self._dataset:
            return self._dataset.RotationVector
        return None

    @RotationVector.setter
    def RotationVector(self, value: Optional[List[int]]):
        if value is None:
            if "RotationVector" in self._dataset:
                del self._dataset.RotationVector
        else:
            self._dataset.RotationVector = value

    @property
    def NumberOfRotations(self) -> Optional[int]:
        if "NumberOfRotations" in self._dataset:
            return self._dataset.NumberOfRotations
        return None

    @NumberOfRotations.setter
    def NumberOfRotations(self, value: Optional[int]):
        if value is None:
            if "NumberOfRotations" in self._dataset:
                del self._dataset.NumberOfRotations
        else:
            self._dataset.NumberOfRotations = value

    @property
    def RRIntervalVector(self) -> Optional[List[int]]:
        if "RRIntervalVector" in self._dataset:
            return self._dataset.RRIntervalVector
        return None

    @RRIntervalVector.setter
    def RRIntervalVector(self, value: Optional[List[int]]):
        if value is None:
            if "RRIntervalVector" in self._dataset:
                del self._dataset.RRIntervalVector
        else:
            self._dataset.RRIntervalVector = value

    @property
    def NumberOfRRIntervals(self) -> Optional[int]:
        if "NumberOfRRIntervals" in self._dataset:
            return self._dataset.NumberOfRRIntervals
        return None

    @NumberOfRRIntervals.setter
    def NumberOfRRIntervals(self, value: Optional[int]):
        if value is None:
            if "NumberOfRRIntervals" in self._dataset:
                del self._dataset.NumberOfRRIntervals
        else:
            self._dataset.NumberOfRRIntervals = value

    @property
    def TimeSlotVector(self) -> Optional[List[int]]:
        if "TimeSlotVector" in self._dataset:
            return self._dataset.TimeSlotVector
        return None

    @TimeSlotVector.setter
    def TimeSlotVector(self, value: Optional[List[int]]):
        if value is None:
            if "TimeSlotVector" in self._dataset:
                del self._dataset.TimeSlotVector
        else:
            self._dataset.TimeSlotVector = value

    @property
    def NumberOfTimeSlots(self) -> Optional[int]:
        if "NumberOfTimeSlots" in self._dataset:
            return self._dataset.NumberOfTimeSlots
        return None

    @NumberOfTimeSlots.setter
    def NumberOfTimeSlots(self, value: Optional[int]):
        if value is None:
            if "NumberOfTimeSlots" in self._dataset:
                del self._dataset.NumberOfTimeSlots
        else:
            self._dataset.NumberOfTimeSlots = value

    @property
    def SliceVector(self) -> Optional[List[int]]:
        if "SliceVector" in self._dataset:
            return self._dataset.SliceVector
        return None

    @SliceVector.setter
    def SliceVector(self, value: Optional[List[int]]):
        if value is None:
            if "SliceVector" in self._dataset:
                del self._dataset.SliceVector
        else:
            self._dataset.SliceVector = value
    ...
    @property
    def AngularViewVector(self) -> Optional[List[int]]:
        if "AngularViewVector" in self._dataset:
            return self._dataset.AngularViewVector
        return None

    @AngularViewVector.setter
    def AngularViewVector(self, value: Optional[List[int]]):
        if value is None:
            if "AngularViewVector" in self._dataset:
                del self._dataset.AngularViewVector
        else:
            self._dataset.AngularViewVector = value

    @property
    def TimeSliceVector(self) -> Optional[List[int]]:
        if "TimeSliceVector" in self._dataset:
            return self._dataset.TimeSliceVector
        return None

    @TimeSliceVector.setter
    def TimeSliceVector(self, value: Optional[List[int]]):
        if value is None:
            if "TimeSliceVector" in self._dataset:
                del self._dataset.TimeSliceVector
        else:
            self._dataset.TimeSliceVector = value
    ...
    @property
    def BeatRejectionFlag(self) -> Optional[str]:
        if "BeatRejectionFlag" in self._dataset:
            return self._dataset.BeatRejectionFlag
        return None

    @BeatRejectionFlag.setter
    def BeatRejectionFlag(self, value: Optional[str]):
        if value is None:
            if "BeatRejectionFlag" in self._dataset:
                del self._dataset.BeatRejectionFlag
        else:
            self._dataset.BeatRejectionFlag = value

    @property
    def PVCRejection(self) -> Optional[str]:
        if "PVCRejection" in self._dataset:
            return self._dataset.PVCRejection
        return None

    @PVCRejection.setter
    def PVCRejection(self, value: Optional[str]):
        if value is None:
            if "PVCRejection" in self._dataset:
                del self._dataset.PVCRejection
        else:
            self._dataset.PVCRejection = value
    ...
    @property
    def GatedInformationSequence(self) -> Optional[List[GatedInformationSequenceItem]]:
        if "GatedInformationSequence" in self._dataset:
            if len(self._GatedInformationSequence) == len(self._dataset.GatedInformationSequence):
                return self._GatedInformationSequence
            else:
                return [GatedInformationSequenceItem(x) for x in self._dataset.GatedInformationSequence]
        return None

    @GatedInformationSequence.setter
    def GatedInformationSequence(self, value: Optional[List[GatedInformationSequenceItem]]):
        if value is None:
            self._GatedInformationSequence = []
            if "GatedInformationSequence" in self._dataset:
                del self._dataset.GatedInformationSequence
        elif not isinstance(value, list) or not all(isinstance(item, GatedInformationSequenceItem) for item in value):
            raise ValueError(f"GatedInformationSequence must be a list of GatedInformationSequenceItem objects")
        else:
            self._GatedInformationSequence = value
            if "GatedInformationSequence" not in self._dataset:
                self._dataset.GatedInformationSequence = pydicom.Sequence()
            self._dataset.GatedInformationSequence.clear()
            self._dataset.GatedInformationSequence.extend([item.to_dataset() for item in value])

    def add_GatedInformation(self, item: GatedInformationSequenceItem):
        if not isinstance(item, GatedInformationSequenceItem):
            raise ValueError(f"Item must be an instance of GatedInformationSequenceItem")
        self._GatedInformationSequence.append(item)
        if "GatedInformationSequence" not in self._dataset:
            self._dataset.GatedInformationSequence = pydicom.Sequence()
        self._dataset.GatedInformationSequence.append(item.to_dataset())

    @property
    def NumberOfFramesInOverlay(self) -> Optional[int]:
        if "NumberOfFramesInOverlay" in self._dataset:
            return self._dataset.NumberOfFramesInOverlay
        return None

    @NumberOfFramesInOverlay.setter
    def NumberOfFramesInOverlay(self, value: Optional[int]):
        if value is None:
            if "NumberOfFramesInOverlay" in self._dataset:
                del self._dataset.NumberOfFramesInOverlay
        else:
            self._dataset.NumberOfFramesInOverlay = value

    @property
    def ImageFrameOrigin(self) -> Optional[int]:
        if "ImageFrameOrigin" in self._dataset:
            return self._dataset.ImageFrameOrigin
        return None

    @ImageFrameOrigin.setter
    def ImageFrameOrigin(self, value: Optional[int]):
        if value is None:
            if "ImageFrameOrigin" in self._dataset:
                del self._dataset.ImageFrameOrigin
        else:
            self._dataset.ImageFrameOrigin = value
    ...
    @property
    def RotationInformationSequence(self) -> Optional[List[RotationInformationSequenceItem]]:
        if "RotationInformationSequence" in self._dataset:
            if len(self._RotationInformationSequence) == len(self._dataset.RotationInformationSequence):
                return self._RotationInformationSequence
            else:
                return [RotationInformationSequenceItem(x) for x in self._dataset.RotationInformationSequence]
        return None

    @RotationInformationSequence.setter
    def RotationInformationSequence(self, value: Optional[List[RotationInformationSequenceItem]]):
        if value is None:
            self._RotationInformationSequence = []
            if "RotationInformationSequence" in self._dataset:
                del self._dataset.RotationInformationSequence
        elif not isinstance(value, list) or not all(isinstance(item, RotationInformationSequenceItem) for item in value):
            raise ValueError(f"RotationInformationSequence must be a list of RotationInformationSequenceItem objects")
        else:
            self._RotationInformationSequence = value
            if "RotationInformationSequence" not in self._dataset:
                self._dataset.RotationInformationSequence = pydicom.Sequence()
            self._dataset.RotationInformationSequence.clear()
            self._dataset.RotationInformationSequence.extend([item.to_dataset() for item in value])

    def add_RotationInformation(self, item: RotationInformationSequenceItem):
        if not isinstance(item, RotationInformationSequenceItem):
            raise ValueError(f"Item must be an instance of RotationInformationSequenceItem")
        self._RotationInformationSequence.append(item)
        if "RotationInformationSequence" not in self._dataset:
            self._dataset.RotationInformationSequence = pydicom.Sequence()
        self._dataset.RotationInformationSequence.append(item.to_dataset())

    @property
    def TypeOfDetectorMotion(self) -> Optional[str]:
        if "TypeOfDetectorMotion" in self._dataset:
            return self._dataset.TypeOfDetectorMotion
        return None

    @TypeOfDetectorMotion.setter
    def TypeOfDetectorMotion(self, value: Optional[str]):
        if value is None:
            if "TypeOfDetectorMotion" in self._dataset:
                del self._dataset.TypeOfDetectorMotion
        else:
            self._dataset.TypeOfDetectorMotion = value
    ...
    @property
    def InterventionDrugInformationSequence(self) -> Optional[List[InterventionDrugInformationSequenceItem]]:
        if "InterventionDrugInformationSequence" in self._dataset:
            if len(self._InterventionDrugInformationSequence) == len(self._dataset.InterventionDrugInformationSequence):
                return self._InterventionDrugInformationSequence
            else:
                return [InterventionDrugInformationSequenceItem(x) for x in self._dataset.InterventionDrugInformationSequence]
        return None

    @InterventionDrugInformationSequence.setter
    def InterventionDrugInformationSequence(self, value: Optional[List[InterventionDrugInformationSequenceItem]]):
        if value is None:
            self._InterventionDrugInformationSequence = []
            if "InterventionDrugInformationSequence" in self._dataset:
                del self._dataset.InterventionDrugInformationSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, InterventionDrugInformationSequenceItem) for item in value
        ):
            raise ValueError(
                f"InterventionDrugInformationSequence must be a list of InterventionDrugInformationSequenceItem objects"
            )
        else:
            self._InterventionDrugInformationSequence = value
            if "InterventionDrugInformationSequence" not in self._dataset:
                self._dataset.InterventionDrugInformationSequence = pydicom.Sequence()
            self._dataset.InterventionDrugInformationSequence.clear()
            self._dataset.InterventionDrugInformationSequence.extend([item.to_dataset() for item in value])

    def add_InterventionDrugInformation(self, item: InterventionDrugInformationSequenceItem):
        if not isinstance(item, InterventionDrugInformationSequenceItem):
            raise ValueError(f"Item must be an instance of InterventionDrugInformationSequenceItem")
        self._InterventionDrugInformationSequence.append(item)
        if "InterventionDrugInformationSequence" not in self._dataset:
            self._dataset.InterventionDrugInformationSequence = pydicom.Sequence()
        self._dataset.InterventionDrugInformationSequence.append(item.to_dataset())

    @property
    def EnergyWindowInformationSequence(self) -> Optional[List[EnergyWindowInformationSequenceItem]]:
        if "EnergyWindowInformationSequence" in self._dataset:
            if len(self._EnergyWindowInformationSequence) == len(self._dataset.EnergyWindowInformationSequence):
                return self._EnergyWindowInformationSequence
            else:
                return [EnergyWindowInformationSequenceItem(x) for x in self._dataset.EnergyWindowInformationSequence]
        return None

    @EnergyWindowInformationSequence.setter
    def EnergyWindowInformationSequence(self, value: Optional[List[EnergyWindowInformationSequenceItem]]):
        if value is None:
            self._EnergyWindowInformationSequence = []
            if "EnergyWindowInformationSequence" in self._dataset:
                del self._dataset.EnergyWindowInformationSequence
        elif not isinstance(value, list) or not all(isinstance(item, EnergyWindowInformationSequenceItem) for item in value):
            raise ValueError(f"EnergyWindowInformationSequence must be a list of EnergyWindowInformationSequenceItem objects")
        else:
            self._EnergyWindowInformationSequence = value
            if "EnergyWindowInformationSequence" not in self._dataset:
                self._dataset.EnergyWindowInformationSequence = pydicom.Sequence()
            self._dataset.EnergyWindowInformationSequence.clear()
            self._dataset.EnergyWindowInformationSequence.extend([item.to_dataset() for item in value])

    def add_EnergyWindowInformation(self, item: EnergyWindowInformationSequenceItem):
        if not isinstance(item, EnergyWindowInformationSequenceItem):
            raise ValueError(f"Item must be an instance of EnergyWindowInformationSequenceItem")
        self._EnergyWindowInformationSequence.append(item)
        if "EnergyWindowInformationSequence" not in self._dataset:
            self._dataset.EnergyWindowInformationSequence = pydicom.Sequence()
        self._dataset.EnergyWindowInformationSequence.append(item.to_dataset())

    @property
    def RadiopharmaceuticalInformationSequence(self) -> Optional[List[RadiopharmaceuticalInformationSequenceItem]]:
        if "RadiopharmaceuticalInformationSequence" in self._dataset:
            if len(self._RadiopharmaceuticalInformationSequence) == len(self._dataset.RadiopharmaceuticalInformationSequence):
                return self._RadiopharmaceuticalInformationSequence
            else:
                return [
                    RadiopharmaceuticalInformationSequenceItem(x) for x in self._dataset.RadiopharmaceuticalInformationSequence
                ]
        return None

    @RadiopharmaceuticalInformationSequence.setter
    def RadiopharmaceuticalInformationSequence(self, value: Optional[List[RadiopharmaceuticalInformationSequenceItem]]):
        if value is None:
            self._RadiopharmaceuticalInformationSequence = []
            if "RadiopharmaceuticalInformationSequence" in self._dataset:
                del self._dataset.RadiopharmaceuticalInformationSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, RadiopharmaceuticalInformationSequenceItem) for item in value
        ):
            raise ValueError(
                f"RadiopharmaceuticalInformationSequence must be a list of RadiopharmaceuticalInformationSequenceItem objects"
            )
        else:
            self._RadiopharmaceuticalInformationSequence = value
            if "RadiopharmaceuticalInformationSequence" not in self._dataset:
                self._dataset.RadiopharmaceuticalInformationSequence = pydicom.Sequence()
            self._dataset.RadiopharmaceuticalInformationSequence.clear()
            self._dataset.RadiopharmaceuticalInformationSequence.extend([item.to_dataset() for item in value])

    def add_RadiopharmaceuticalInformation(self, item: RadiopharmaceuticalInformationSequenceItem):
        if not isinstance(item, RadiopharmaceuticalInformationSequenceItem):
            raise ValueError(f"Item must be an instance of RadiopharmaceuticalInformationSequenceItem")
        self._RadiopharmaceuticalInformationSequence.append(item)
        if "RadiopharmaceuticalInformationSequence" not in self._dataset:
            self._dataset.RadiopharmaceuticalInformationSequence = pydicom.Sequence()
        self._dataset.RadiopharmaceuticalInformationSequence.append(item.to_dataset())
    ...
    @property
    def CountsAccumulated(self) -> Optional[int]:
        if "CountsAccumulated" in self._dataset:
            return self._dataset.CountsAccumulated
        return None

    @CountsAccumulated.setter
    def CountsAccumulated(self, value: Optional[int]):
        if value is None:
            if "CountsAccumulated" in self._dataset:
                del self._dataset.CountsAccumulated
        else:
            self._dataset.CountsAccumulated = value

    @property
    def AcquisitionTerminationCondition(self) -> Optional[str]:
        if "AcquisitionTerminationCondition" in self._dataset:
            return self._dataset.AcquisitionTerminationCondition
        return None

    @AcquisitionTerminationCondition.setter
    def AcquisitionTerminationCondition(self, value: Optional[str]):
        if value is None:
            if "AcquisitionTerminationCondition" in self._dataset:
                del self._dataset.AcquisitionTerminationCondition
        else:
            self._dataset.AcquisitionTerminationCondition = value
    ...
    @property
    def CountRate(self) -> Optional[int]:
        if "CountRate" in self._dataset:
            return self._dataset.CountRate
        return None

    @CountRate.setter
    def CountRate(self, value: Optional[int]):
        if value is None:
            if "CountRate" in self._dataset:
                del self._dataset.CountRate
        else:
            self._dataset.CountRate = value

    @property
    def ScanVelocity(self) -> Optional[Decimal]:
        if "ScanVelocity" in self._dataset:
            return self._dataset.ScanVelocity
        return None

    @ScanVelocity.setter
    def ScanVelocity(self, value: Optional[Decimal]):
        if value is None:
            if "ScanVelocity" in self._dataset:
                del self._dataset.ScanVelocity
        else:
            self._dataset.ScanVelocity = value

    @property
    def WholeBodyTechnique(self) -> Optional[List[str]]:
        if "WholeBodyTechnique" in self._dataset:
            return self._dataset.WholeBodyTechnique
        return None

    @WholeBodyTechnique.setter
    def WholeBodyTechnique(self, value: Optional[List[str]]):
        if value is None:
            if "WholeBodyTechnique" in self._dataset:
                del self._dataset.WholeBodyTechnique
        else:
            self._dataset.WholeBodyTechnique = value

    @property
    def ScanLength(self) -> Optional[int]:
        if "ScanLength" in self._dataset:
            return self._dataset.ScanLength
        return None

    @ScanLength.setter
    def ScanLength(self, value: Optional[int]):
        if value is None:
            if "ScanLength" in self._dataset:
                del self._dataset.ScanLength
        else:
            self._dataset.ScanLength = value
    ...
    @property
    def CorrectedImage(self) -> Optional[List[str]]:
        if "CorrectedImage" in self._dataset:
            return self._dataset.CorrectedImage
        return None

    @CorrectedImage.setter
    def CorrectedImage(self, value: Optional[List[str]]):
        if value is None:
            if "CorrectedImage" in self._dataset:
                del self._dataset.CorrectedImage
        else:
            self._dataset.CorrectedImage = value
    ...
    def ImageID(self) -> Optional[str]:
        if "ImageID" in self._dataset:
            return self._dataset.ImageID
        return None

    @ImageID.setter
    def ImageID(self, value: Optional[str]):
        if value is None:
            if "ImageID" in self._dataset:
                del self._dataset.ImageID
        else:
            self._dataset.ImageID = value

    @property
    def ScanProgressionDirection(self) -> Optional[str]:
        if "ScanProgressionDirection" in self._dataset:
            return self._dataset.ScanProgressionDirection
        return None

    @ScanProgressionDirection.setter
    def ScanProgressionDirection(self, value: Optional[str]):
        if value is None:
            if "ScanProgressionDirection" in self._dataset:
                del self._dataset.ScanProgressionDirection
        else:
            self._dataset.ScanProgressionDirection = value
