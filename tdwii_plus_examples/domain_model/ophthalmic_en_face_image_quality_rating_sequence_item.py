...
class OphthalmicEnFaceImageQualityRatingSequenceItem:
    ...
    @property
    def QualityThreshold(self) -> Optional[Decimal]:
        if "QualityThreshold" in self._dataset:
            return self._dataset.QualityThreshold
        return None

    @QualityThreshold.setter
    def QualityThreshold(self, value: Optional[Decimal]):
        if value is None:
            if "QualityThreshold" in self._dataset:
                del self._dataset.QualityThreshold
        else:
            self._dataset.QualityThreshold = value
