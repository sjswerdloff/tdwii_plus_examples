...
from .ophthalmic_en_face_image_quality_rating_sequence_item import (
...
from .referenced_surface_mesh_identification_sequence_item import (
    ReferencedSurfaceMeshIdentificationSequenceItem,
)
...
class OphthalmicOpticalCoherenceTomographyEnFaceImage:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        self._dataset = dataset if dataset is not None else pydicom.Dataset()
        self._OphthalmicEnFaceImageQualityRatingSequence: List[OphthalmicEnFaceImageQualityRatingSequenceItem] = []
        ...
        self._OphthalmicImageTypeCodeSequence: List[CodeSequenceItem] = []
    ...
    @property
    def OphthalmicEnFaceImageQualityRatingSequence(self) -> Optional[List[OphthalmicEnFaceImageQualityRatingSequenceItem]]:
        if "OphthalmicEnFaceImageQualityRatingSequence" in self._dataset:
            if len(self._OphthalmicEnFaceImageQualityRatingSequence) == len(
                self._dataset.OphthalmicEnFaceImageQualityRatingSequence
            ):
                return self._OphthalmicEnFaceImageQualityRatingSequence
            else:
                return [
                    OphthalmicEnFaceImageQualityRatingSequenceItem(x)
                    for x in self._dataset.OphthalmicEnFaceImageQualityRatingSequence
                ]
        return None

    @OphthalmicEnFaceImageQualityRatingSequence.setter
    def OphthalmicEnFaceImageQualityRatingSequence(
        self, value: Optional[List[OphthalmicEnFaceImageQualityRatingSequenceItem]]
    ):
        if value is None:
            self._OphthalmicEnFaceImageQualityRatingSequence = []
            if "OphthalmicEnFaceImageQualityRatingSequence" in self._dataset:
                del self._dataset.OphthalmicEnFaceImageQualityRatingSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, OphthalmicEnFaceImageQualityRatingSequenceItem) for item in value
        ):
            raise ValueError(
                f"OphthalmicEnFaceImageQualityRatingSequence must be a list of OphthalmicEnFaceImageQualityRatingSequenceItem objects"
            )
        else:
            self._OphthalmicEnFaceImageQualityRatingSequence = value
            if "OphthalmicEnFaceImageQualityRatingSequence" not in self._dataset:
                self._dataset.OphthalmicEnFaceImageQualityRatingSequence = pydicom.Sequence()
            self._dataset.OphthalmicEnFaceImageQualityRatingSequence.clear()
            self._dataset.OphthalmicEnFaceImageQualityRatingSequence.extend([item.to_dataset() for item in value])

    def add_OphthalmicEnFaceImageQualityRating(self, item: OphthalmicEnFaceImageQualityRatingSequenceItem):
        if not isinstance(item, OphthalmicEnFaceImageQualityRatingSequenceItem):
            raise ValueError(f"Item must be an instance of OphthalmicEnFaceImageQualityRatingSequenceItem")
        self._OphthalmicEnFaceImageQualityRatingSequence.append(item)
        if "OphthalmicEnFaceImageQualityRatingSequence" not in self._dataset:
            self._dataset.OphthalmicEnFaceImageQualityRatingSequence = pydicom.Sequence()
        self._dataset.OphthalmicEnFaceImageQualityRatingSequence.append(item.to_dataset())
    ...
    @property
    def OphthalmicImageTypeCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "OphthalmicImageTypeCodeSequence" in self._dataset:
            if len(self._OphthalmicImageTypeCodeSequence) == len(self._dataset.OphthalmicImageTypeCodeSequence):
                return self._OphthalmicImageTypeCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.OphthalmicImageTypeCodeSequence]
        return None

    @OphthalmicImageTypeCodeSequence.setter
    def OphthalmicImageTypeCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._OphthalmicImageTypeCodeSequence = []
            if "OphthalmicImageTypeCodeSequence" in self._dataset:
                del self._dataset.OphthalmicImageTypeCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"OphthalmicImageTypeCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._OphthalmicImageTypeCodeSequence = value
            if "OphthalmicImageTypeCodeSequence" not in self._dataset:
                self._dataset.OphthalmicImageTypeCodeSequence = pydicom.Sequence()
            self._dataset.OphthalmicImageTypeCodeSequence.clear()
            self._dataset.OphthalmicImageTypeCodeSequence.extend([item.to_dataset() for item in value])

    def add_OphthalmicImageTypeCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._OphthalmicImageTypeCodeSequence.append(item)
        if "OphthalmicImageTypeCodeSequence" not in self._dataset:
            self._dataset.OphthalmicImageTypeCodeSequence = pydicom.Sequence()
        self._dataset.OphthalmicImageTypeCodeSequence.append(item.to_dataset())

    @property
    def OphthalmicImageTypeDescription(self) -> Optional[str]:
        if "OphthalmicImageTypeDescription" in self._dataset:
            return self._dataset.OphthalmicImageTypeDescription
        return None

    @OphthalmicImageTypeDescription.setter
    def OphthalmicImageTypeDescription(self, value: Optional[str]):
        if value is None:
            if "OphthalmicImageTypeDescription" in self._dataset:
                del self._dataset.OphthalmicImageTypeDescription
        else:
            self._dataset.OphthalmicImageTypeDescription = value

    @property
    def ReferencedSurfaceMeshIdentificationSequence(self) -> Optional[List[ReferencedSurfaceMeshIdentificationSequenceItem]]:
        if "ReferencedSurfaceMeshIdentificationSequence" in self._dataset:
            if len(self._ReferencedSurfaceMeshIdentificationSequence) == len(
                self._dataset.ReferencedSurfaceMeshIdentificationSequence
            ):
                return self._ReferencedSurfaceMeshIdentificationSequence
            else:
                return [
                    ReferencedSurfaceMeshIdentificationSequenceItem(x)
                    for x in self._dataset.ReferencedSurfaceMeshIdentificationSequence
                ]
        return None

    @ReferencedSurfaceMeshIdentificationSequence.setter
    def ReferencedSurfaceMeshIdentificationSequence(
        self, value: Optional[List[ReferencedSurfaceMeshIdentificationSequenceItem]]
    ):
        if value is None:
            self._ReferencedSurfaceMeshIdentificationSequence = []
            if "ReferencedSurfaceMeshIdentificationSequence" in self._dataset:
                del self._dataset.ReferencedSurfaceMeshIdentificationSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, ReferencedSurfaceMeshIdentificationSequenceItem) for item in value
        ):
            raise ValueError(
                f"ReferencedSurfaceMeshIdentificationSequence must be a list of ReferencedSurfaceMeshIdentificationSequenceItem objects"
            )
        else:
            self._ReferencedSurfaceMeshIdentificationSequence = value
            if "ReferencedSurfaceMeshIdentificationSequence" not in self._dataset:
                self._dataset.ReferencedSurfaceMeshIdentificationSequence = pydicom.Sequence()
            self._dataset.ReferencedSurfaceMeshIdentificationSequence.clear()
            self._dataset.ReferencedSurfaceMeshIdentificationSequence.extend([item.to_dataset() for item in value])

    def add_ReferencedSurfaceMeshIdentification(self, item: ReferencedSurfaceMeshIdentificationSequenceItem):
        if not isinstance(item, ReferencedSurfaceMeshIdentificationSequenceItem):
            raise ValueError(f"Item must be an instance of ReferencedSurfaceMeshIdentificationSequenceItem")
        self._ReferencedSurfaceMeshIdentificationSequence.append(item)
        if "ReferencedSurfaceMeshIdentificationSequence" not in self._dataset:
            self._dataset.ReferencedSurfaceMeshIdentificationSequence = pydicom.Sequence()
        self._dataset.ReferencedSurfaceMeshIdentificationSequence.append(item.to_dataset())
