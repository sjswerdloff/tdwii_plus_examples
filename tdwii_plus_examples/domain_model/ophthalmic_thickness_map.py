...
from .acquisition_method_algorithm_sequence_item import (
    AcquisitionMethodAlgorithmSequenceItem,
)
...
from .ophthalmic_thickness_map_quality_rating_sequence_item import (
    OphthalmicThicknessMapQualityRatingSequenceItem,
)
from .ophthalmic_thickness_mapping_normals_sequence_item import (
    OphthalmicThicknessMappingNormalsSequenceItem,
)
...
from .pixel_value_mapping_to_coded_concept_sequence_item import (
...
from .registration_to_localizer_sequence_item import RegistrationToLocalizerSequenceItem
from .related_series_sequence_item import RelatedSeriesSequenceItem
from .relevant_opt_attributes_sequence_item import RelevantOPTAttributesSequenceItem
...
class OphthalmicThicknessMap:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._AnatomicRegionSequence: List[AnatomicRegionSequenceItem] = []
        self._PrimaryAnatomicStructureSequence: List[PrimaryAnatomicStructureSequenceItem] = []
        self._RelativeImagePositionCodeSequence: List[CodeSequenceItem] = []
        self._AcquisitionMethodCodeSequence: List[CodeSequenceItem] = []
        self._AcquisitionMethodAlgorithmSequence: List[AcquisitionMethodAlgorithmSequenceItem] = []
        self._OphthalmicThicknessMapTypeCodeSequence: List[CodeSequenceItem] = []
        self._OphthalmicThicknessMappingNormalsSequence: List[OphthalmicThicknessMappingNormalsSequenceItem] = []
        self._RetinalThicknessDefinitionCodeSequence: List[CodeSequenceItem] = []
        self._PixelValueMappingToCodedConceptSequence: List[PixelValueMappingToCodedConceptSequenceItem] = []
        self._RegistrationToLocalizerSequence: List[RegistrationToLocalizerSequenceItem] = []
        self._RelevantOPTAttributesSequence: List[RelevantOPTAttributesSequenceItem] = []
        self._RealWorldValueMappingSequence: List[RealWorldValueMappingSequenceItem] = []
        ...
        self._OphthalmicThicknessMapQualityRatingSequence: List[OphthalmicThicknessMapQualityRatingSequenceItem] = []
    ...
    @property
    def PatientEyeMovementCommanded(self) -> Optional[str]:
        if "PatientEyeMovementCommanded" in self._dataset:
            return self._dataset.PatientEyeMovementCommanded
        return None

    @PatientEyeMovementCommanded.setter
    def PatientEyeMovementCommanded(self, value: Optional[str]):
        if value is None:
            if "PatientEyeMovementCommanded" in self._dataset:
                del self._dataset.PatientEyeMovementCommanded
        else:
            self._dataset.PatientEyeMovementCommanded = value

    @property
    def PatientEyeMovementCommandCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "PatientEyeMovementCommandCodeSequence" in self._dataset:
            if len(self._PatientEyeMovementCommandCodeSequence) == len(self._dataset.PatientEyeMovementCommandCodeSequence):
                return self._PatientEyeMovementCommandCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.PatientEyeMovementCommandCodeSequence]
        return None

    @PatientEyeMovementCommandCodeSequence.setter
    def PatientEyeMovementCommandCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._PatientEyeMovementCommandCodeSequence = []
            if "PatientEyeMovementCommandCodeSequence" in self._dataset:
            ...
            self._dataset.PatientEyeMovementCommandCodeSequence.clear()
            self._dataset.PatientEyeMovementCommandCodeSequence.extend([item.to_dataset() for item in value])
    ...
    def add_PatientEyeMovementCommandCode(self, item: CodeSequenceItem):
        ...
        if "PatientEyeMovementCommandCodeSequence" not in self._dataset:
            self._dataset.PatientEyeMovementCommandCodeSequence = pydicom.Sequence()
        self._dataset.PatientEyeMovementCommandCodeSequence.append(item.to_dataset())

    @property
    def EmmetropicMagnification(self) -> Optional[float]:
        if "EmmetropicMagnification" in self._dataset:
            return self._dataset.EmmetropicMagnification
        return None

    @EmmetropicMagnification.setter
    def EmmetropicMagnification(self, value: Optional[float]):
        if value is None:
            if "EmmetropicMagnification" in self._dataset:
                del self._dataset.EmmetropicMagnification
        else:
            self._dataset.EmmetropicMagnification = value
    ...
    @property
    def HorizontalFieldOfView(self) -> Optional[float]:
        if "HorizontalFieldOfView" in self._dataset:
            return self._dataset.HorizontalFieldOfView
        return None

    @HorizontalFieldOfView.setter
    def HorizontalFieldOfView(self, value: Optional[float]):
        if value is None:
            if "HorizontalFieldOfView" in self._dataset:
                del self._dataset.HorizontalFieldOfView
        else:
            self._dataset.HorizontalFieldOfView = value
    ...
    @property
    def ShutterOverlayGroup(self) -> Optional[int]:
        if "ShutterOverlayGroup" in self._dataset:
            return self._dataset.ShutterOverlayGroup
        return None

    @ShutterOverlayGroup.setter
    def ShutterOverlayGroup(self, value: Optional[int]):
        if value is None:
            if "ShutterOverlayGroup" in self._dataset:
                del self._dataset.ShutterOverlayGroup
        else:
            self._dataset.ShutterOverlayGroup = value
    ...
    @property
    def RelativeImagePositionCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "RelativeImagePositionCodeSequence" in self._dataset:
            if len(self._RelativeImagePositionCodeSequence) == len(self._dataset.RelativeImagePositionCodeSequence):
                return self._RelativeImagePositionCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.RelativeImagePositionCodeSequence]
        return None

    @RelativeImagePositionCodeSequence.setter
    def RelativeImagePositionCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._RelativeImagePositionCodeSequence = []
            if "RelativeImagePositionCodeSequence" in self._dataset:
            ...
            self._dataset.RelativeImagePositionCodeSequence.clear()
            self._dataset.RelativeImagePositionCodeSequence.extend([item.to_dataset() for item in value])
    ...
    def add_RelativeImagePositionCode(self, item: CodeSequenceItem):
        ...
        if "RelativeImagePositionCodeSequence" not in self._dataset:
            self._dataset.RelativeImagePositionCodeSequence = pydicom.Sequence()
        self._dataset.RelativeImagePositionCodeSequence.append(item.to_dataset())

    @property
    def OphthalmicMappingDeviceType(self) -> Optional[str]:
        if "OphthalmicMappingDeviceType" in self._dataset:
            return self._dataset.OphthalmicMappingDeviceType
        return None

    @OphthalmicMappingDeviceType.setter
    def OphthalmicMappingDeviceType(self, value: Optional[str]):
        if value is None:
            if "OphthalmicMappingDeviceType" in self._dataset:
                del self._dataset.OphthalmicMappingDeviceType
        else:
            self._dataset.OphthalmicMappingDeviceType = value

    @property
    def AcquisitionMethodCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "AcquisitionMethodCodeSequence" in self._dataset:
            if len(self._AcquisitionMethodCodeSequence) == len(self._dataset.AcquisitionMethodCodeSequence):
                return self._AcquisitionMethodCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.AcquisitionMethodCodeSequence]
        return None

    @AcquisitionMethodCodeSequence.setter
    def AcquisitionMethodCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._AcquisitionMethodCodeSequence = []
            if "AcquisitionMethodCodeSequence" in self._dataset:
                del self._dataset.AcquisitionMethodCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"AcquisitionMethodCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._AcquisitionMethodCodeSequence = value
            if "AcquisitionMethodCodeSequence" not in self._dataset:
                self._dataset.AcquisitionMethodCodeSequence = pydicom.Sequence()
            self._dataset.AcquisitionMethodCodeSequence.clear()
            self._dataset.AcquisitionMethodCodeSequence.extend([item.to_dataset() for item in value])

    def add_AcquisitionMethodCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._AcquisitionMethodCodeSequence.append(item)
        if "AcquisitionMethodCodeSequence" not in self._dataset:
            self._dataset.AcquisitionMethodCodeSequence = pydicom.Sequence()
        self._dataset.AcquisitionMethodCodeSequence.append(item.to_dataset())

    @property
    def AcquisitionMethodAlgorithmSequence(self) -> Optional[List[AcquisitionMethodAlgorithmSequenceItem]]:
        if "AcquisitionMethodAlgorithmSequence" in self._dataset:
            if len(self._AcquisitionMethodAlgorithmSequence) == len(self._dataset.AcquisitionMethodAlgorithmSequence):
                return self._AcquisitionMethodAlgorithmSequence
            else:
                return [AcquisitionMethodAlgorithmSequenceItem(x) for x in self._dataset.AcquisitionMethodAlgorithmSequence]
        return None

    @AcquisitionMethodAlgorithmSequence.setter
    def AcquisitionMethodAlgorithmSequence(self, value: Optional[List[AcquisitionMethodAlgorithmSequenceItem]]):
        if value is None:
            self._AcquisitionMethodAlgorithmSequence = []
            if "AcquisitionMethodAlgorithmSequence" in self._dataset:
                del self._dataset.AcquisitionMethodAlgorithmSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, AcquisitionMethodAlgorithmSequenceItem) for item in value
        ):
            raise ValueError(
                f"AcquisitionMethodAlgorithmSequence must be a list of AcquisitionMethodAlgorithmSequenceItem objects"
            )
        else:
            self._AcquisitionMethodAlgorithmSequence = value
            if "AcquisitionMethodAlgorithmSequence" not in self._dataset:
                self._dataset.AcquisitionMethodAlgorithmSequence = pydicom.Sequence()
            self._dataset.AcquisitionMethodAlgorithmSequence.clear()
            self._dataset.AcquisitionMethodAlgorithmSequence.extend([item.to_dataset() for item in value])

    def add_AcquisitionMethodAlgorithm(self, item: AcquisitionMethodAlgorithmSequenceItem):
        if not isinstance(item, AcquisitionMethodAlgorithmSequenceItem):
            raise ValueError(f"Item must be an instance of AcquisitionMethodAlgorithmSequenceItem")
        self._AcquisitionMethodAlgorithmSequence.append(item)
        if "AcquisitionMethodAlgorithmSequence" not in self._dataset:
            self._dataset.AcquisitionMethodAlgorithmSequence = pydicom.Sequence()
        self._dataset.AcquisitionMethodAlgorithmSequence.append(item.to_dataset())

    @property
    def OphthalmicThicknessMapTypeCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "OphthalmicThicknessMapTypeCodeSequence" in self._dataset:
            if len(self._OphthalmicThicknessMapTypeCodeSequence) == len(self._dataset.OphthalmicThicknessMapTypeCodeSequence):
                return self._OphthalmicThicknessMapTypeCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.OphthalmicThicknessMapTypeCodeSequence]
        return None

    @OphthalmicThicknessMapTypeCodeSequence.setter
    def OphthalmicThicknessMapTypeCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._OphthalmicThicknessMapTypeCodeSequence = []
            if "OphthalmicThicknessMapTypeCodeSequence" in self._dataset:
                del self._dataset.OphthalmicThicknessMapTypeCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"OphthalmicThicknessMapTypeCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._OphthalmicThicknessMapTypeCodeSequence = value
            if "OphthalmicThicknessMapTypeCodeSequence" not in self._dataset:
                self._dataset.OphthalmicThicknessMapTypeCodeSequence = pydicom.Sequence()
            self._dataset.OphthalmicThicknessMapTypeCodeSequence.clear()
            self._dataset.OphthalmicThicknessMapTypeCodeSequence.extend([item.to_dataset() for item in value])

    def add_OphthalmicThicknessMapTypeCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._OphthalmicThicknessMapTypeCodeSequence.append(item)
        if "OphthalmicThicknessMapTypeCodeSequence" not in self._dataset:
            self._dataset.OphthalmicThicknessMapTypeCodeSequence = pydicom.Sequence()
        self._dataset.OphthalmicThicknessMapTypeCodeSequence.append(item.to_dataset())

    @property
    def OphthalmicThicknessMappingNormalsSequence(self) -> Optional[List[OphthalmicThicknessMappingNormalsSequenceItem]]:
        if "OphthalmicThicknessMappingNormalsSequence" in self._dataset:
            if len(self._OphthalmicThicknessMappingNormalsSequence) == len(
                self._dataset.OphthalmicThicknessMappingNormalsSequence
            ):
                return self._OphthalmicThicknessMappingNormalsSequence
            else:
                return [
                    OphthalmicThicknessMappingNormalsSequenceItem(x)
                    for x in self._dataset.OphthalmicThicknessMappingNormalsSequence
                ]
        return None

    @OphthalmicThicknessMappingNormalsSequence.setter
    def OphthalmicThicknessMappingNormalsSequence(self, value: Optional[List[OphthalmicThicknessMappingNormalsSequenceItem]]):
        if value is None:
            self._OphthalmicThicknessMappingNormalsSequence = []
            if "OphthalmicThicknessMappingNormalsSequence" in self._dataset:
                del self._dataset.OphthalmicThicknessMappingNormalsSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, OphthalmicThicknessMappingNormalsSequenceItem) for item in value
        ):
            raise ValueError(
                f"OphthalmicThicknessMappingNormalsSequence must be a list of OphthalmicThicknessMappingNormalsSequenceItem objects"
            )
        else:
            self._OphthalmicThicknessMappingNormalsSequence = value
            if "OphthalmicThicknessMappingNormalsSequence" not in self._dataset:
                self._dataset.OphthalmicThicknessMappingNormalsSequence = pydicom.Sequence()
            self._dataset.OphthalmicThicknessMappingNormalsSequence.clear()
            self._dataset.OphthalmicThicknessMappingNormalsSequence.extend([item.to_dataset() for item in value])

    def add_OphthalmicThicknessMappingNormals(self, item: OphthalmicThicknessMappingNormalsSequenceItem):
        if not isinstance(item, OphthalmicThicknessMappingNormalsSequenceItem):
            raise ValueError(f"Item must be an instance of OphthalmicThicknessMappingNormalsSequenceItem")
        self._OphthalmicThicknessMappingNormalsSequence.append(item)
        if "OphthalmicThicknessMappingNormalsSequence" not in self._dataset:
            self._dataset.OphthalmicThicknessMappingNormalsSequence = pydicom.Sequence()
        self._dataset.OphthalmicThicknessMappingNormalsSequence.append(item.to_dataset())

    @property
    def RetinalThicknessDefinitionCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "RetinalThicknessDefinitionCodeSequence" in self._dataset:
            if len(self._RetinalThicknessDefinitionCodeSequence) == len(self._dataset.RetinalThicknessDefinitionCodeSequence):
                return self._RetinalThicknessDefinitionCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.RetinalThicknessDefinitionCodeSequence]
        return None

    @RetinalThicknessDefinitionCodeSequence.setter
    def RetinalThicknessDefinitionCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._RetinalThicknessDefinitionCodeSequence = []
            if "RetinalThicknessDefinitionCodeSequence" in self._dataset:
                del self._dataset.RetinalThicknessDefinitionCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"RetinalThicknessDefinitionCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._RetinalThicknessDefinitionCodeSequence = value
            if "RetinalThicknessDefinitionCodeSequence" not in self._dataset:
                self._dataset.RetinalThicknessDefinitionCodeSequence = pydicom.Sequence()
            self._dataset.RetinalThicknessDefinitionCodeSequence.clear()
            self._dataset.RetinalThicknessDefinitionCodeSequence.extend([item.to_dataset() for item in value])

    def add_RetinalThicknessDefinitionCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._RetinalThicknessDefinitionCodeSequence.append(item)
        if "RetinalThicknessDefinitionCodeSequence" not in self._dataset:
            self._dataset.RetinalThicknessDefinitionCodeSequence = pydicom.Sequence()
        self._dataset.RetinalThicknessDefinitionCodeSequence.append(item.to_dataset())

    @property
    def PixelValueMappingToCodedConceptSequence(self) -> Optional[List[PixelValueMappingToCodedConceptSequenceItem]]:
        if "PixelValueMappingToCodedConceptSequence" in self._dataset:
            if len(self._PixelValueMappingToCodedConceptSequence) == len(
                self._dataset.PixelValueMappingToCodedConceptSequence
            ):
                return self._PixelValueMappingToCodedConceptSequence
            else:
                return [
                    PixelValueMappingToCodedConceptSequenceItem(x)
                    for x in self._dataset.PixelValueMappingToCodedConceptSequence
                ]
        return None

    @PixelValueMappingToCodedConceptSequence.setter
    def PixelValueMappingToCodedConceptSequence(self, value: Optional[List[PixelValueMappingToCodedConceptSequenceItem]]):
        if value is None:
            self._PixelValueMappingToCodedConceptSequence = []
            if "PixelValueMappingToCodedConceptSequence" in self._dataset:
                del self._dataset.PixelValueMappingToCodedConceptSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, PixelValueMappingToCodedConceptSequenceItem) for item in value
        ):
            raise ValueError(
                f"PixelValueMappingToCodedConceptSequence must be a list of PixelValueMappingToCodedConceptSequenceItem objects"
            )
        else:
            self._PixelValueMappingToCodedConceptSequence = value
            if "PixelValueMappingToCodedConceptSequence" not in self._dataset:
                self._dataset.PixelValueMappingToCodedConceptSequence = pydicom.Sequence()
            self._dataset.PixelValueMappingToCodedConceptSequence.clear()
            self._dataset.PixelValueMappingToCodedConceptSequence.extend([item.to_dataset() for item in value])

    def add_PixelValueMappingToCodedConcept(self, item: PixelValueMappingToCodedConceptSequenceItem):
        if not isinstance(item, PixelValueMappingToCodedConceptSequenceItem):
            raise ValueError(f"Item must be an instance of PixelValueMappingToCodedConceptSequenceItem")
        self._PixelValueMappingToCodedConceptSequence.append(item)
        if "PixelValueMappingToCodedConceptSequence" not in self._dataset:
            self._dataset.PixelValueMappingToCodedConceptSequence = pydicom.Sequence()
        self._dataset.PixelValueMappingToCodedConceptSequence.append(item.to_dataset())

    @property
    def AnatomicStructureReferencePoint(self) -> Optional[List[float]]:
        if "AnatomicStructureReferencePoint" in self._dataset:
            return self._dataset.AnatomicStructureReferencePoint
        return None

    @AnatomicStructureReferencePoint.setter
    def AnatomicStructureReferencePoint(self, value: Optional[List[float]]):
        if value is None:
            if "AnatomicStructureReferencePoint" in self._dataset:
                del self._dataset.AnatomicStructureReferencePoint
        else:
            self._dataset.AnatomicStructureReferencePoint = value

    @property
    def RegistrationToLocalizerSequence(self) -> Optional[List[RegistrationToLocalizerSequenceItem]]:
        if "RegistrationToLocalizerSequence" in self._dataset:
            if len(self._RegistrationToLocalizerSequence) == len(self._dataset.RegistrationToLocalizerSequence):
                return self._RegistrationToLocalizerSequence
            else:
                return [RegistrationToLocalizerSequenceItem(x) for x in self._dataset.RegistrationToLocalizerSequence]
        return None

    @RegistrationToLocalizerSequence.setter
    def RegistrationToLocalizerSequence(self, value: Optional[List[RegistrationToLocalizerSequenceItem]]):
        if value is None:
            self._RegistrationToLocalizerSequence = []
            if "RegistrationToLocalizerSequence" in self._dataset:
                del self._dataset.RegistrationToLocalizerSequence
        elif not isinstance(value, list) or not all(isinstance(item, RegistrationToLocalizerSequenceItem) for item in value):
            raise ValueError(f"RegistrationToLocalizerSequence must be a list of RegistrationToLocalizerSequenceItem objects")
        else:
            self._RegistrationToLocalizerSequence = value
            if "RegistrationToLocalizerSequence" not in self._dataset:
                self._dataset.RegistrationToLocalizerSequence = pydicom.Sequence()
            self._dataset.RegistrationToLocalizerSequence.clear()
            self._dataset.RegistrationToLocalizerSequence.extend([item.to_dataset() for item in value])

    def add_RegistrationToLocalizer(self, item: RegistrationToLocalizerSequenceItem):
        if not isinstance(item, RegistrationToLocalizerSequenceItem):
            raise ValueError(f"Item must be an instance of RegistrationToLocalizerSequenceItem")
        self._RegistrationToLocalizerSequence.append(item)
        if "RegistrationToLocalizerSequence" not in self._dataset:
            self._dataset.RegistrationToLocalizerSequence = pydicom.Sequence()
        self._dataset.RegistrationToLocalizerSequence.append(item.to_dataset())

    @property
    def RelevantOPTAttributesSequence(self) -> Optional[List[RelevantOPTAttributesSequenceItem]]:
        if "RelevantOPTAttributesSequence" in self._dataset:
            if len(self._RelevantOPTAttributesSequence) == len(self._dataset.RelevantOPTAttributesSequence):
                return self._RelevantOPTAttributesSequence
            else:
                return [RelevantOPTAttributesSequenceItem(x) for x in self._dataset.RelevantOPTAttributesSequence]
        return None

    @RelevantOPTAttributesSequence.setter
    def RelevantOPTAttributesSequence(self, value: Optional[List[RelevantOPTAttributesSequenceItem]]):
        if value is None:
            self._RelevantOPTAttributesSequence = []
            if "RelevantOPTAttributesSequence" in self._dataset:
                del self._dataset.RelevantOPTAttributesSequence
        elif not isinstance(value, list) or not all(isinstance(item, RelevantOPTAttributesSequenceItem) for item in value):
            raise ValueError(f"RelevantOPTAttributesSequence must be a list of RelevantOPTAttributesSequenceItem objects")
        else:
            self._RelevantOPTAttributesSequence = value
            if "RelevantOPTAttributesSequence" not in self._dataset:
                self._dataset.RelevantOPTAttributesSequence = pydicom.Sequence()
            self._dataset.RelevantOPTAttributesSequence.clear()
            self._dataset.RelevantOPTAttributesSequence.extend([item.to_dataset() for item in value])

    def add_RelevantOPTAttributes(self, item: RelevantOPTAttributesSequenceItem):
        if not isinstance(item, RelevantOPTAttributesSequenceItem):
            raise ValueError(f"Item must be an instance of RelevantOPTAttributesSequenceItem")
        self._RelevantOPTAttributesSequence.append(item)
        if "RelevantOPTAttributesSequence" not in self._dataset:
            self._dataset.RelevantOPTAttributesSequence = pydicom.Sequence()
        self._dataset.RelevantOPTAttributesSequence.append(item.to_dataset())
    ...
    @property
    def ReferencedColorPaletteInstanceUID(self) -> Optional[str]:
        if "ReferencedColorPaletteInstanceUID" in self._dataset:
            return self._dataset.ReferencedColorPaletteInstanceUID
        return None

    @ReferencedColorPaletteInstanceUID.setter
    def ReferencedColorPaletteInstanceUID(self, value: Optional[str]):
        if value is None:
            if "ReferencedColorPaletteInstanceUID" in self._dataset:
                del self._dataset.ReferencedColorPaletteInstanceUID
        else:
            self._dataset.ReferencedColorPaletteInstanceUID = value
    ...
    @property
    def OphthalmicThicknessMapQualityRatingSequence(self) -> Optional[List[OphthalmicThicknessMapQualityRatingSequenceItem]]:
        if "OphthalmicThicknessMapQualityRatingSequence" in self._dataset:
            if len(self._OphthalmicThicknessMapQualityRatingSequence) == len(
                self._dataset.OphthalmicThicknessMapQualityRatingSequence
            ):
                return self._OphthalmicThicknessMapQualityRatingSequence
            else:
                return [
                    OphthalmicThicknessMapQualityRatingSequenceItem(x)
                    for x in self._dataset.OphthalmicThicknessMapQualityRatingSequence
                ]
        return None

    @OphthalmicThicknessMapQualityRatingSequence.setter
    def OphthalmicThicknessMapQualityRatingSequence(
        self, value: Optional[List[OphthalmicThicknessMapQualityRatingSequenceItem]]
    ):
        if value is None:
            self._OphthalmicThicknessMapQualityRatingSequence = []
            if "OphthalmicThicknessMapQualityRatingSequence" in self._dataset:
                del self._dataset.OphthalmicThicknessMapQualityRatingSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, OphthalmicThicknessMapQualityRatingSequenceItem) for item in value
        ):
            raise ValueError(
                f"OphthalmicThicknessMapQualityRatingSequence must be a list of OphthalmicThicknessMapQualityRatingSequenceItem objects"
            )
        else:
            self._OphthalmicThicknessMapQualityRatingSequence = value
            if "OphthalmicThicknessMapQualityRatingSequence" not in self._dataset:
                self._dataset.OphthalmicThicknessMapQualityRatingSequence = pydicom.Sequence()
            self._dataset.OphthalmicThicknessMapQualityRatingSequence.clear()
            self._dataset.OphthalmicThicknessMapQualityRatingSequence.extend([item.to_dataset() for item in value])

    def add_OphthalmicThicknessMapQualityRating(self, item: OphthalmicThicknessMapQualityRatingSequenceItem):
        if not isinstance(item, OphthalmicThicknessMapQualityRatingSequenceItem):
            raise ValueError(f"Item must be an instance of OphthalmicThicknessMapQualityRatingSequenceItem")
        self._OphthalmicThicknessMapQualityRatingSequence.append(item)
        if "OphthalmicThicknessMapQualityRatingSequence" not in self._dataset:
            self._dataset.OphthalmicThicknessMapQualityRatingSequence = pydicom.Sequence()
        self._dataset.OphthalmicThicknessMapQualityRatingSequence.append(item.to_dataset())
