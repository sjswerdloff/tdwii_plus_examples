...
class OphthalmicTomographyImage:
    ...
    @property
    def AlongScanSpatialResolution(self) -> Optional[float]:
        if "AlongScanSpatialResolution" in self._dataset:
            return self._dataset.AlongScanSpatialResolution
        return None

    @AlongScanSpatialResolution.setter
    def AlongScanSpatialResolution(self, value: Optional[float]):
        if value is None:
            if "AlongScanSpatialResolution" in self._dataset:
                del self._dataset.AlongScanSpatialResolution
        else:
            self._dataset.AlongScanSpatialResolution = value

    @property
    def MaximumAlongScanDistortion(self) -> Optional[float]:
        if "MaximumAlongScanDistortion" in self._dataset:
            return self._dataset.MaximumAlongScanDistortion
        return None

    @MaximumAlongScanDistortion.setter
    def MaximumAlongScanDistortion(self, value: Optional[float]):
        if value is None:
            if "MaximumAlongScanDistortion" in self._dataset:
                del self._dataset.MaximumAlongScanDistortion
        else:
            self._dataset.MaximumAlongScanDistortion = value

    @property
    def AcrossScanSpatialResolution(self) -> Optional[float]:
        if "AcrossScanSpatialResolution" in self._dataset:
            return self._dataset.AcrossScanSpatialResolution
        return None

    @AcrossScanSpatialResolution.setter
    def AcrossScanSpatialResolution(self, value: Optional[float]):
        if value is None:
            if "AcrossScanSpatialResolution" in self._dataset:
                del self._dataset.AcrossScanSpatialResolution
        else:
            self._dataset.AcrossScanSpatialResolution = value

    @property
    def MaximumAcrossScanDistortion(self) -> Optional[float]:
        if "MaximumAcrossScanDistortion" in self._dataset:
            return self._dataset.MaximumAcrossScanDistortion
        return None

    @MaximumAcrossScanDistortion.setter
    def MaximumAcrossScanDistortion(self, value: Optional[float]):
        if value is None:
            if "MaximumAcrossScanDistortion" in self._dataset:
                del self._dataset.MaximumAcrossScanDistortion
        else:
            self._dataset.MaximumAcrossScanDistortion = value
    ...
    @property
    def IlluminationPower(self) -> Optional[float]:
        if "IlluminationPower" in self._dataset:
            return self._dataset.IlluminationPower
        return None

    @IlluminationPower.setter
    def IlluminationPower(self, value: Optional[float]):
        if value is None:
            if "IlluminationPower" in self._dataset:
                del self._dataset.IlluminationPower
        else:
            self._dataset.IlluminationPower = value

    @property
    def IlluminationBandwidth(self) -> Optional[float]:
        if "IlluminationBandwidth" in self._dataset:
            return self._dataset.IlluminationBandwidth
        return None

    @IlluminationBandwidth.setter
    def IlluminationBandwidth(self, value: Optional[float]):
        if value is None:
            if "IlluminationBandwidth" in self._dataset:
                del self._dataset.IlluminationBandwidth
        else:
            self._dataset.IlluminationBandwidth = value
    ...
    def ScanPatternTypeCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            ...
            raise ValueError("ScanPatternTypeCodeSequence must be a list of CodeSequenceItem objects")
        else:
    ...
    @property
    def OphthalmicVolumetricPropertiesFlag(self) -> Optional[str]:
        if "OphthalmicVolumetricPropertiesFlag" in self._dataset:
            return self._dataset.OphthalmicVolumetricPropertiesFlag
        return None

    @OphthalmicVolumetricPropertiesFlag.setter
    def OphthalmicVolumetricPropertiesFlag(self, value: Optional[str]):
        if value is None:
            if "OphthalmicVolumetricPropertiesFlag" in self._dataset:
                del self._dataset.OphthalmicVolumetricPropertiesFlag
        else:
            self._dataset.OphthalmicVolumetricPropertiesFlag = value
    ...
    @property
    def AxialLengthOfTheEye(self) -> Optional[float]:
        if "AxialLengthOfTheEye" in self._dataset:
            return self._dataset.AxialLengthOfTheEye
        return None

    @AxialLengthOfTheEye.setter
    def AxialLengthOfTheEye(self, value: Optional[float]):
        if value is None:
            if "AxialLengthOfTheEye" in self._dataset:
                del self._dataset.AxialLengthOfTheEye
        else:
            self._dataset.AxialLengthOfTheEye = value
