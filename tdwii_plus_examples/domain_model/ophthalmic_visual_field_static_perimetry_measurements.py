...
from .age_corrected_sensitivity_deviation_algorithm_sequence_item import (
    AgeCorrectedSensitivityDeviationAlgorithmSequenceItem,
)
...
from .fixation_sequence_item import FixationSequenceItem
from .generalized_defect_sensitivity_deviation_algorithm_sequence_item import (
    GeneralizedDefectSensitivityDeviationAlgorithmSequenceItem,
)
...
from .ophthalmic_patient_clinical_information_left_eye_sequence_item import (
    OphthalmicPatientClinicalInformationLeftEyeSequenceItem,
)
from .ophthalmic_patient_clinical_information_right_eye_sequence_item import (
    OphthalmicPatientClinicalInformationRightEyeSequenceItem,
)
...
from .results_normals_sequence_item import ResultsNormalsSequenceItem
from .screening_baseline_measured_sequence_item import (
...
from .test_point_normals_sequence_item import TestPointNormalsSequenceItem
from .udi_sequence_item import UDISequenceItem
from .visual_field_catch_trial_sequence_item import VisualFieldCatchTrialSequenceItem
from .visual_field_global_results_index_sequence_item import (
    VisualFieldGlobalResultsIndexSequenceItem,
)
from .visual_field_test_point_sequence_item import VisualFieldTestPointSequenceItem
from .visual_field_test_reliability_global_index_sequence_item import (
    VisualFieldTestReliabilityGlobalIndexSequenceItem,
)
...
class OphthalmicVisualFieldStaticPerimetryMeasurements:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._TestPointNormalsSequence: List[TestPointNormalsSequenceItem] = []
        self._AgeCorrectedSensitivityDeviationAlgorithmSequence: List[
            AgeCorrectedSensitivityDeviationAlgorithmSequenceItem
        ] = []
        self._GeneralizedDefectSensitivityDeviationAlgorithmSequence: List[
            GeneralizedDefectSensitivityDeviationAlgorithmSequenceItem
        ] = []
        self._VisualFieldTestPointSequence: List[VisualFieldTestPointSequenceItem] = []
        self._ScreeningBaselineMeasuredSequence: List[ScreeningBaselineMeasuredSequenceItem] = []
        self._ResultsNormalsSequence: List[ResultsNormalsSequenceItem] = []
        self._VisualFieldGlobalResultsIndexSequence: List[VisualFieldGlobalResultsIndexSequenceItem] = []
        self._OphthalmicPatientClinicalInformationLeftEyeSequence: List[
            OphthalmicPatientClinicalInformationLeftEyeSequenceItem
        ] = []
        self._OphthalmicPatientClinicalInformationRightEyeSequence: List[
            OphthalmicPatientClinicalInformationRightEyeSequenceItem
        ] = []
        ...
        self._ScreeningTestModeCodeSequence: List[CodeSequenceItem] = []
        self._StimulusColorCodeSequence: List[CodeSequenceItem] = []
        ...
        self._FixationSequence: List[FixationSequenceItem] = []
        self._VisualFieldCatchTrialSequence: List[VisualFieldCatchTrialSequenceItem] = []
    ...
    @property
    def PresentedVisualStimuliDataFlag(self) -> Optional[str]:
        if "PresentedVisualStimuliDataFlag" in self._dataset:
            return self._dataset.PresentedVisualStimuliDataFlag
        return None

    @PresentedVisualStimuliDataFlag.setter
    def PresentedVisualStimuliDataFlag(self, value: Optional[str]):
        if value is None:
            if "PresentedVisualStimuliDataFlag" in self._dataset:
                del self._dataset.PresentedVisualStimuliDataFlag
        else:
            self._dataset.PresentedVisualStimuliDataFlag = value

    @property
    def NumberOfVisualStimuli(self) -> Optional[int]:
        if "NumberOfVisualStimuli" in self._dataset:
            return self._dataset.NumberOfVisualStimuli
        return None

    @NumberOfVisualStimuli.setter
    def NumberOfVisualStimuli(self, value: Optional[int]):
        if value is None:
            if "NumberOfVisualStimuli" in self._dataset:
                del self._dataset.NumberOfVisualStimuli
        else:
            self._dataset.NumberOfVisualStimuli = value

    @property
    def TestPointNormalsDataFlag(self) -> Optional[str]:
        if "TestPointNormalsDataFlag" in self._dataset:
            return self._dataset.TestPointNormalsDataFlag
        return None

    @TestPointNormalsDataFlag.setter
    def TestPointNormalsDataFlag(self, value: Optional[str]):
        if value is None:
            if "TestPointNormalsDataFlag" in self._dataset:
                del self._dataset.TestPointNormalsDataFlag
        else:
            self._dataset.TestPointNormalsDataFlag = value

    @property
    def TestPointNormalsSequence(self) -> Optional[List[TestPointNormalsSequenceItem]]:
        if "TestPointNormalsSequence" in self._dataset:
            if len(self._TestPointNormalsSequence) == len(self._dataset.TestPointNormalsSequence):
                return self._TestPointNormalsSequence
            else:
                return [TestPointNormalsSequenceItem(x) for x in self._dataset.TestPointNormalsSequence]
        return None

    @TestPointNormalsSequence.setter
    def TestPointNormalsSequence(self, value: Optional[List[TestPointNormalsSequenceItem]]):
        if value is None:
            self._TestPointNormalsSequence = []
            if "TestPointNormalsSequence" in self._dataset:
                del self._dataset.TestPointNormalsSequence
        elif not isinstance(value, list) or not all(isinstance(item, TestPointNormalsSequenceItem) for item in value):
            raise ValueError(f"TestPointNormalsSequence must be a list of TestPointNormalsSequenceItem objects")
        else:
            self._TestPointNormalsSequence = value
            if "TestPointNormalsSequence" not in self._dataset:
                self._dataset.TestPointNormalsSequence = pydicom.Sequence()
            self._dataset.TestPointNormalsSequence.clear()
            self._dataset.TestPointNormalsSequence.extend([item.to_dataset() for item in value])

    def add_TestPointNormals(self, item: TestPointNormalsSequenceItem):
        if not isinstance(item, TestPointNormalsSequenceItem):
            raise ValueError(f"Item must be an instance of TestPointNormalsSequenceItem")
        self._TestPointNormalsSequence.append(item)
        if "TestPointNormalsSequence" not in self._dataset:
            self._dataset.TestPointNormalsSequence = pydicom.Sequence()
        self._dataset.TestPointNormalsSequence.append(item.to_dataset())

    @property
    def AgeCorrectedSensitivityDeviationAlgorithmSequence(
        self,
    ) -> Optional[List[AgeCorrectedSensitivityDeviationAlgorithmSequenceItem]]:
        if "AgeCorrectedSensitivityDeviationAlgorithmSequence" in self._dataset:
            if len(self._AgeCorrectedSensitivityDeviationAlgorithmSequence) == len(
                self._dataset.AgeCorrectedSensitivityDeviationAlgorithmSequence
            ):
                return self._AgeCorrectedSensitivityDeviationAlgorithmSequence
            else:
                return [
                    AgeCorrectedSensitivityDeviationAlgorithmSequenceItem(x)
                    for x in self._dataset.AgeCorrectedSensitivityDeviationAlgorithmSequence
                ]
        return None

    @AgeCorrectedSensitivityDeviationAlgorithmSequence.setter
    def AgeCorrectedSensitivityDeviationAlgorithmSequence(
        self, value: Optional[List[AgeCorrectedSensitivityDeviationAlgorithmSequenceItem]]
    ):
        if value is None:
            self._AgeCorrectedSensitivityDeviationAlgorithmSequence = []
            if "AgeCorrectedSensitivityDeviationAlgorithmSequence" in self._dataset:
                del self._dataset.AgeCorrectedSensitivityDeviationAlgorithmSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, AgeCorrectedSensitivityDeviationAlgorithmSequenceItem) for item in value
        ):
            raise ValueError(
                f"AgeCorrectedSensitivityDeviationAlgorithmSequence must be a list of AgeCorrectedSensitivityDeviationAlgorithmSequenceItem objects"
            )
        else:
            self._AgeCorrectedSensitivityDeviationAlgorithmSequence = value
            if "AgeCorrectedSensitivityDeviationAlgorithmSequence" not in self._dataset:
                self._dataset.AgeCorrectedSensitivityDeviationAlgorithmSequence = pydicom.Sequence()
            self._dataset.AgeCorrectedSensitivityDeviationAlgorithmSequence.clear()
            self._dataset.AgeCorrectedSensitivityDeviationAlgorithmSequence.extend([item.to_dataset() for item in value])

    def add_AgeCorrectedSensitivityDeviationAlgorithm(self, item: AgeCorrectedSensitivityDeviationAlgorithmSequenceItem):
        if not isinstance(item, AgeCorrectedSensitivityDeviationAlgorithmSequenceItem):
            raise ValueError(f"Item must be an instance of AgeCorrectedSensitivityDeviationAlgorithmSequenceItem")
        self._AgeCorrectedSensitivityDeviationAlgorithmSequence.append(item)
        if "AgeCorrectedSensitivityDeviationAlgorithmSequence" not in self._dataset:
            self._dataset.AgeCorrectedSensitivityDeviationAlgorithmSequence = pydicom.Sequence()
        self._dataset.AgeCorrectedSensitivityDeviationAlgorithmSequence.append(item.to_dataset())

    @property
    def GeneralizedDefectSensitivityDeviationAlgorithmSequence(
        self,
    ) -> Optional[List[GeneralizedDefectSensitivityDeviationAlgorithmSequenceItem]]:
        if "GeneralizedDefectSensitivityDeviationAlgorithmSequence" in self._dataset:
            if len(self._GeneralizedDefectSensitivityDeviationAlgorithmSequence) == len(
                self._dataset.GeneralizedDefectSensitivityDeviationAlgorithmSequence
            ):
                return self._GeneralizedDefectSensitivityDeviationAlgorithmSequence
            else:
                return [
                    GeneralizedDefectSensitivityDeviationAlgorithmSequenceItem(x)
                    for x in self._dataset.GeneralizedDefectSensitivityDeviationAlgorithmSequence
                ]
        return None

    @GeneralizedDefectSensitivityDeviationAlgorithmSequence.setter
    def GeneralizedDefectSensitivityDeviationAlgorithmSequence(
        self, value: Optional[List[GeneralizedDefectSensitivityDeviationAlgorithmSequenceItem]]
    ):
        if value is None:
            self._GeneralizedDefectSensitivityDeviationAlgorithmSequence = []
            if "GeneralizedDefectSensitivityDeviationAlgorithmSequence" in self._dataset:
                del self._dataset.GeneralizedDefectSensitivityDeviationAlgorithmSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, GeneralizedDefectSensitivityDeviationAlgorithmSequenceItem) for item in value
        ):
            raise ValueError(
                f"GeneralizedDefectSensitivityDeviationAlgorithmSequence must be a list of GeneralizedDefectSensitivityDeviationAlgorithmSequenceItem objects"
            )
        else:
            self._GeneralizedDefectSensitivityDeviationAlgorithmSequence = value
            if "GeneralizedDefectSensitivityDeviationAlgorithmSequence" not in self._dataset:
                self._dataset.GeneralizedDefectSensitivityDeviationAlgorithmSequence = pydicom.Sequence()
            self._dataset.GeneralizedDefectSensitivityDeviationAlgorithmSequence.clear()
            self._dataset.GeneralizedDefectSensitivityDeviationAlgorithmSequence.extend([item.to_dataset() for item in value])

    def add_GeneralizedDefectSensitivityDeviationAlgorithm(
        ...
        if not isinstance(item, GeneralizedDefectSensitivityDeviationAlgorithmSequenceItem):
            raise ValueError(f"Item must be an instance of GeneralizedDefectSensitivityDeviationAlgorithmSequenceItem")
        self._GeneralizedDefectSensitivityDeviationAlgorithmSequence.append(item)
        if "GeneralizedDefectSensitivityDeviationAlgorithmSequence" not in self._dataset:
            self._dataset.GeneralizedDefectSensitivityDeviationAlgorithmSequence = pydicom.Sequence()
        self._dataset.GeneralizedDefectSensitivityDeviationAlgorithmSequence.append(item.to_dataset())

    @property
    def FovealSensitivityMeasured(self) -> Optional[str]:
        if "FovealSensitivityMeasured" in self._dataset:
            return self._dataset.FovealSensitivityMeasured
        return None

    @FovealSensitivityMeasured.setter
    def FovealSensitivityMeasured(self, value: Optional[str]):
        if value is None:
            if "FovealSensitivityMeasured" in self._dataset:
                del self._dataset.FovealSensitivityMeasured
        else:
            self._dataset.FovealSensitivityMeasured = value

    @property
    def FovealSensitivity(self) -> Optional[float]:
        if "FovealSensitivity" in self._dataset:
            return self._dataset.FovealSensitivity
        return None

    @FovealSensitivity.setter
    def FovealSensitivity(self, value: Optional[float]):
        if value is None:
            if "FovealSensitivity" in self._dataset:
                del self._dataset.FovealSensitivity
        else:
            self._dataset.FovealSensitivity = value

    @property
    def VisualFieldTestDuration(self) -> Optional[float]:
        if "VisualFieldTestDuration" in self._dataset:
            return self._dataset.VisualFieldTestDuration
        return None

    @VisualFieldTestDuration.setter
    def VisualFieldTestDuration(self, value: Optional[float]):
        if value is None:
            if "VisualFieldTestDuration" in self._dataset:
                del self._dataset.VisualFieldTestDuration
        else:
            self._dataset.VisualFieldTestDuration = value

    @property
    def VisualFieldTestPointSequence(self) -> Optional[List[VisualFieldTestPointSequenceItem]]:
        if "VisualFieldTestPointSequence" in self._dataset:
            if len(self._VisualFieldTestPointSequence) == len(self._dataset.VisualFieldTestPointSequence):
                return self._VisualFieldTestPointSequence
            else:
                return [VisualFieldTestPointSequenceItem(x) for x in self._dataset.VisualFieldTestPointSequence]
        return None

    @VisualFieldTestPointSequence.setter
    def VisualFieldTestPointSequence(self, value: Optional[List[VisualFieldTestPointSequenceItem]]):
        if value is None:
            self._VisualFieldTestPointSequence = []
            if "VisualFieldTestPointSequence" in self._dataset:
                del self._dataset.VisualFieldTestPointSequence
        elif not isinstance(value, list) or not all(isinstance(item, VisualFieldTestPointSequenceItem) for item in value):
            raise ValueError(f"VisualFieldTestPointSequence must be a list of VisualFieldTestPointSequenceItem objects")
        else:
            self._VisualFieldTestPointSequence = value
            if "VisualFieldTestPointSequence" not in self._dataset:
                self._dataset.VisualFieldTestPointSequence = pydicom.Sequence()
            self._dataset.VisualFieldTestPointSequence.clear()
            self._dataset.VisualFieldTestPointSequence.extend([item.to_dataset() for item in value])

    def add_VisualFieldTestPoint(self, item: VisualFieldTestPointSequenceItem):
        if not isinstance(item, VisualFieldTestPointSequenceItem):
            raise ValueError(f"Item must be an instance of VisualFieldTestPointSequenceItem")
        self._VisualFieldTestPointSequence.append(item)
        if "VisualFieldTestPointSequence" not in self._dataset:
            self._dataset.VisualFieldTestPointSequence = pydicom.Sequence()
        self._dataset.VisualFieldTestPointSequence.append(item.to_dataset())

    @property
    def MinimumSensitivityValue(self) -> Optional[float]:
        if "MinimumSensitivityValue" in self._dataset:
            return self._dataset.MinimumSensitivityValue
        return None

    @MinimumSensitivityValue.setter
    def MinimumSensitivityValue(self, value: Optional[float]):
        if value is None:
            if "MinimumSensitivityValue" in self._dataset:
                del self._dataset.MinimumSensitivityValue
        else:
            self._dataset.MinimumSensitivityValue = value

    @property
    def BlindSpotLocalized(self) -> Optional[str]:
        if "BlindSpotLocalized" in self._dataset:
            return self._dataset.BlindSpotLocalized
        return None

    @BlindSpotLocalized.setter
    def BlindSpotLocalized(self, value: Optional[str]):
        if value is None:
            if "BlindSpotLocalized" in self._dataset:
                del self._dataset.BlindSpotLocalized
        else:
            self._dataset.BlindSpotLocalized = value

    @property
    def BlindSpotXCoordinate(self) -> Optional[float]:
        if "BlindSpotXCoordinate" in self._dataset:
            return self._dataset.BlindSpotXCoordinate
        return None

    @BlindSpotXCoordinate.setter
    def BlindSpotXCoordinate(self, value: Optional[float]):
        if value is None:
            if "BlindSpotXCoordinate" in self._dataset:
                del self._dataset.BlindSpotXCoordinate
        else:
            self._dataset.BlindSpotXCoordinate = value

    @property
    def BlindSpotYCoordinate(self) -> Optional[float]:
        if "BlindSpotYCoordinate" in self._dataset:
            return self._dataset.BlindSpotYCoordinate
        return None

    @BlindSpotYCoordinate.setter
    def BlindSpotYCoordinate(self, value: Optional[float]):
        if value is None:
            if "BlindSpotYCoordinate" in self._dataset:
                del self._dataset.BlindSpotYCoordinate
        else:
            self._dataset.BlindSpotYCoordinate = value

    @property
    def MeasurementLaterality(self) -> Optional[str]:
        if "MeasurementLaterality" in self._dataset:
            return self._dataset.MeasurementLaterality
        return None

    @MeasurementLaterality.setter
    def MeasurementLaterality(self, value: Optional[str]):
        if value is None:
            if "MeasurementLaterality" in self._dataset:
                del self._dataset.MeasurementLaterality
        else:
            self._dataset.MeasurementLaterality = value

    @property
    def FovealPointNormativeDataFlag(self) -> Optional[str]:
        if "FovealPointNormativeDataFlag" in self._dataset:
            return self._dataset.FovealPointNormativeDataFlag
        return None

    @FovealPointNormativeDataFlag.setter
    def FovealPointNormativeDataFlag(self, value: Optional[str]):
        if value is None:
            if "FovealPointNormativeDataFlag" in self._dataset:
                del self._dataset.FovealPointNormativeDataFlag
        else:
            self._dataset.FovealPointNormativeDataFlag = value

    @property
    def FovealPointProbabilityValue(self) -> Optional[float]:
        if "FovealPointProbabilityValue" in self._dataset:
            return self._dataset.FovealPointProbabilityValue
        return None

    @FovealPointProbabilityValue.setter
    def FovealPointProbabilityValue(self, value: Optional[float]):
        if value is None:
            if "FovealPointProbabilityValue" in self._dataset:
                del self._dataset.FovealPointProbabilityValue
        else:
            self._dataset.FovealPointProbabilityValue = value

    @property
    def ScreeningBaselineMeasured(self) -> Optional[str]:
        if "ScreeningBaselineMeasured" in self._dataset:
            return self._dataset.ScreeningBaselineMeasured
        return None

    @ScreeningBaselineMeasured.setter
    def ScreeningBaselineMeasured(self, value: Optional[str]):
        if value is None:
            if "ScreeningBaselineMeasured" in self._dataset:
                del self._dataset.ScreeningBaselineMeasured
        else:
            self._dataset.ScreeningBaselineMeasured = value

    @property
    def ScreeningBaselineMeasuredSequence(self) -> Optional[List[ScreeningBaselineMeasuredSequenceItem]]:
        if "ScreeningBaselineMeasuredSequence" in self._dataset:
            if len(self._ScreeningBaselineMeasuredSequence) == len(self._dataset.ScreeningBaselineMeasuredSequence):
                return self._ScreeningBaselineMeasuredSequence
            else:
                return [ScreeningBaselineMeasuredSequenceItem(x) for x in self._dataset.ScreeningBaselineMeasuredSequence]
        return None

    @ScreeningBaselineMeasuredSequence.setter
    def ScreeningBaselineMeasuredSequence(self, value: Optional[List[ScreeningBaselineMeasuredSequenceItem]]):
        if value is None:
            self._ScreeningBaselineMeasuredSequence = []
            if "ScreeningBaselineMeasuredSequence" in self._dataset:
                del self._dataset.ScreeningBaselineMeasuredSequence
        elif not isinstance(value, list) or not all(isinstance(item, ScreeningBaselineMeasuredSequenceItem) for item in value):
            raise ValueError(
                f"ScreeningBaselineMeasuredSequence must be a list of ScreeningBaselineMeasuredSequenceItem objects"
            )
        else:
            self._ScreeningBaselineMeasuredSequence = value
            if "ScreeningBaselineMeasuredSequence" not in self._dataset:
                self._dataset.ScreeningBaselineMeasuredSequence = pydicom.Sequence()
            self._dataset.ScreeningBaselineMeasuredSequence.clear()
            self._dataset.ScreeningBaselineMeasuredSequence.extend([item.to_dataset() for item in value])

    def add_ScreeningBaselineMeasured(self, item: ScreeningBaselineMeasuredSequenceItem):
        if not isinstance(item, ScreeningBaselineMeasuredSequenceItem):
            raise ValueError(f"Item must be an instance of ScreeningBaselineMeasuredSequenceItem")
        self._ScreeningBaselineMeasuredSequence.append(item)
        if "ScreeningBaselineMeasuredSequence" not in self._dataset:
            self._dataset.ScreeningBaselineMeasuredSequence = pydicom.Sequence()
        self._dataset.ScreeningBaselineMeasuredSequence.append(item.to_dataset())
    ...
    @property
    def VisualFieldTestNormalsFlag(self) -> Optional[str]:
        if "VisualFieldTestNormalsFlag" in self._dataset:
            return self._dataset.VisualFieldTestNormalsFlag
        return None

    @VisualFieldTestNormalsFlag.setter
    def VisualFieldTestNormalsFlag(self, value: Optional[str]):
        if value is None:
            if "VisualFieldTestNormalsFlag" in self._dataset:
                del self._dataset.VisualFieldTestNormalsFlag
        else:
            self._dataset.VisualFieldTestNormalsFlag = value

    @property
    def ResultsNormalsSequence(self) -> Optional[List[ResultsNormalsSequenceItem]]:
        if "ResultsNormalsSequence" in self._dataset:
            if len(self._ResultsNormalsSequence) == len(self._dataset.ResultsNormalsSequence):
                return self._ResultsNormalsSequence
            else:
                return [ResultsNormalsSequenceItem(x) for x in self._dataset.ResultsNormalsSequence]
        return None

    @ResultsNormalsSequence.setter
    def ResultsNormalsSequence(self, value: Optional[List[ResultsNormalsSequenceItem]]):
        if value is None:
            self._ResultsNormalsSequence = []
            if "ResultsNormalsSequence" in self._dataset:
                del self._dataset.ResultsNormalsSequence
        elif not isinstance(value, list) or not all(isinstance(item, ResultsNormalsSequenceItem) for item in value):
            raise ValueError(f"ResultsNormalsSequence must be a list of ResultsNormalsSequenceItem objects")
        else:
            self._ResultsNormalsSequence = value
            if "ResultsNormalsSequence" not in self._dataset:
                self._dataset.ResultsNormalsSequence = pydicom.Sequence()
            self._dataset.ResultsNormalsSequence.clear()
            self._dataset.ResultsNormalsSequence.extend([item.to_dataset() for item in value])

    def add_ResultsNormals(self, item: ResultsNormalsSequenceItem):
        if not isinstance(item, ResultsNormalsSequenceItem):
            raise ValueError(f"Item must be an instance of ResultsNormalsSequenceItem")
        self._ResultsNormalsSequence.append(item)
        if "ResultsNormalsSequence" not in self._dataset:
            self._dataset.ResultsNormalsSequence = pydicom.Sequence()
        self._dataset.ResultsNormalsSequence.append(item.to_dataset())

    @property
    def VisualFieldMeanSensitivity(self) -> Optional[float]:
        if "VisualFieldMeanSensitivity" in self._dataset:
            return self._dataset.VisualFieldMeanSensitivity
        return None

    @VisualFieldMeanSensitivity.setter
    def VisualFieldMeanSensitivity(self, value: Optional[float]):
        if value is None:
            if "VisualFieldMeanSensitivity" in self._dataset:
                del self._dataset.VisualFieldMeanSensitivity
        else:
            self._dataset.VisualFieldMeanSensitivity = value

    @property
    def ShortTermFluctuationCalculated(self) -> Optional[str]:
        if "ShortTermFluctuationCalculated" in self._dataset:
            return self._dataset.ShortTermFluctuationCalculated
        return None

    @ShortTermFluctuationCalculated.setter
    def ShortTermFluctuationCalculated(self, value: Optional[str]):
        if value is None:
            if "ShortTermFluctuationCalculated" in self._dataset:
                del self._dataset.ShortTermFluctuationCalculated
        else:
            self._dataset.ShortTermFluctuationCalculated = value

    @property
    def ShortTermFluctuation(self) -> Optional[float]:
        if "ShortTermFluctuation" in self._dataset:
            return self._dataset.ShortTermFluctuation
        return None

    @ShortTermFluctuation.setter
    def ShortTermFluctuation(self, value: Optional[float]):
        if value is None:
            if "ShortTermFluctuation" in self._dataset:
                del self._dataset.ShortTermFluctuation
        else:
            self._dataset.ShortTermFluctuation = value

    @property
    def ShortTermFluctuationProbabilityCalculated(self) -> Optional[str]:
        if "ShortTermFluctuationProbabilityCalculated" in self._dataset:
            return self._dataset.ShortTermFluctuationProbabilityCalculated
        return None

    @ShortTermFluctuationProbabilityCalculated.setter
    def ShortTermFluctuationProbabilityCalculated(self, value: Optional[str]):
        if value is None:
            if "ShortTermFluctuationProbabilityCalculated" in self._dataset:
                del self._dataset.ShortTermFluctuationProbabilityCalculated
        else:
            self._dataset.ShortTermFluctuationProbabilityCalculated = value

    @property
    def ShortTermFluctuationProbability(self) -> Optional[float]:
        if "ShortTermFluctuationProbability" in self._dataset:
            return self._dataset.ShortTermFluctuationProbability
        return None

    @ShortTermFluctuationProbability.setter
    def ShortTermFluctuationProbability(self, value: Optional[float]):
        if value is None:
            if "ShortTermFluctuationProbability" in self._dataset:
                del self._dataset.ShortTermFluctuationProbability
        else:
            self._dataset.ShortTermFluctuationProbability = value

    @property
    def CorrectedLocalizedDeviationFromNormalCalculated(self) -> Optional[str]:
        if "CorrectedLocalizedDeviationFromNormalCalculated" in self._dataset:
            return self._dataset.CorrectedLocalizedDeviationFromNormalCalculated
        return None

    @CorrectedLocalizedDeviationFromNormalCalculated.setter
    def CorrectedLocalizedDeviationFromNormalCalculated(self, value: Optional[str]):
        if value is None:
            if "CorrectedLocalizedDeviationFromNormalCalculated" in self._dataset:
                del self._dataset.CorrectedLocalizedDeviationFromNormalCalculated
        else:
            self._dataset.CorrectedLocalizedDeviationFromNormalCalculated = value

    @property
    def CorrectedLocalizedDeviationFromNormal(self) -> Optional[float]:
        if "CorrectedLocalizedDeviationFromNormal" in self._dataset:
            return self._dataset.CorrectedLocalizedDeviationFromNormal
        return None

    @CorrectedLocalizedDeviationFromNormal.setter
    def CorrectedLocalizedDeviationFromNormal(self, value: Optional[float]):
        if value is None:
            if "CorrectedLocalizedDeviationFromNormal" in self._dataset:
                del self._dataset.CorrectedLocalizedDeviationFromNormal
        else:
            self._dataset.CorrectedLocalizedDeviationFromNormal = value

    @property
    def CorrectedLocalizedDeviationFromNormalProbabilityCalculated(self) -> Optional[str]:
        if "CorrectedLocalizedDeviationFromNormalProbabilityCalculated" in self._dataset:
            return self._dataset.CorrectedLocalizedDeviationFromNormalProbabilityCalculated
        return None

    @CorrectedLocalizedDeviationFromNormalProbabilityCalculated.setter
    def CorrectedLocalizedDeviationFromNormalProbabilityCalculated(self, value: Optional[str]):
        if value is None:
            if "CorrectedLocalizedDeviationFromNormalProbabilityCalculated" in self._dataset:
                del self._dataset.CorrectedLocalizedDeviationFromNormalProbabilityCalculated
        else:
            self._dataset.CorrectedLocalizedDeviationFromNormalProbabilityCalculated = value

    @property
    def CorrectedLocalizedDeviationFromNormalProbability(self) -> Optional[float]:
        if "CorrectedLocalizedDeviationFromNormalProbability" in self._dataset:
            return self._dataset.CorrectedLocalizedDeviationFromNormalProbability
        return None

    @CorrectedLocalizedDeviationFromNormalProbability.setter
    def CorrectedLocalizedDeviationFromNormalProbability(self, value: Optional[float]):
        if value is None:
            if "CorrectedLocalizedDeviationFromNormalProbability" in self._dataset:
                del self._dataset.CorrectedLocalizedDeviationFromNormalProbability
        else:
            self._dataset.CorrectedLocalizedDeviationFromNormalProbability = value

    @property
    def VisualFieldGlobalResultsIndexSequence(self) -> Optional[List[VisualFieldGlobalResultsIndexSequenceItem]]:
        if "VisualFieldGlobalResultsIndexSequence" in self._dataset:
            if len(self._VisualFieldGlobalResultsIndexSequence) == len(self._dataset.VisualFieldGlobalResultsIndexSequence):
                return self._VisualFieldGlobalResultsIndexSequence
            else:
                return [
                    VisualFieldGlobalResultsIndexSequenceItem(x) for x in self._dataset.VisualFieldGlobalResultsIndexSequence
                ]
        return None

    @VisualFieldGlobalResultsIndexSequence.setter
    def VisualFieldGlobalResultsIndexSequence(self, value: Optional[List[VisualFieldGlobalResultsIndexSequenceItem]]):
        if value is None:
            self._VisualFieldGlobalResultsIndexSequence = []
            if "VisualFieldGlobalResultsIndexSequence" in self._dataset:
                del self._dataset.VisualFieldGlobalResultsIndexSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, VisualFieldGlobalResultsIndexSequenceItem) for item in value
        ):
            raise ValueError(
                f"VisualFieldGlobalResultsIndexSequence must be a list of VisualFieldGlobalResultsIndexSequenceItem objects"
            )
        else:
            self._VisualFieldGlobalResultsIndexSequence = value
            if "VisualFieldGlobalResultsIndexSequence" not in self._dataset:
                self._dataset.VisualFieldGlobalResultsIndexSequence = pydicom.Sequence()
            self._dataset.VisualFieldGlobalResultsIndexSequence.clear()
            self._dataset.VisualFieldGlobalResultsIndexSequence.extend([item.to_dataset() for item in value])

    def add_VisualFieldGlobalResultsIndex(self, item: VisualFieldGlobalResultsIndexSequenceItem):
        if not isinstance(item, VisualFieldGlobalResultsIndexSequenceItem):
            raise ValueError(f"Item must be an instance of VisualFieldGlobalResultsIndexSequenceItem")
        self._VisualFieldGlobalResultsIndexSequence.append(item)
        if "VisualFieldGlobalResultsIndexSequence" not in self._dataset:
            self._dataset.VisualFieldGlobalResultsIndexSequence = pydicom.Sequence()
        self._dataset.VisualFieldGlobalResultsIndexSequence.append(item.to_dataset())

    @property
    def OphthalmicPatientClinicalInformationLeftEyeSequence(
        self,
    ) -> Optional[List[OphthalmicPatientClinicalInformationLeftEyeSequenceItem]]:
        if "OphthalmicPatientClinicalInformationLeftEyeSequence" in self._dataset:
            if len(self._OphthalmicPatientClinicalInformationLeftEyeSequence) == len(
                self._dataset.OphthalmicPatientClinicalInformationLeftEyeSequence
            ):
                return self._OphthalmicPatientClinicalInformationLeftEyeSequence
            else:
                return [
                    OphthalmicPatientClinicalInformationLeftEyeSequenceItem(x)
                    for x in self._dataset.OphthalmicPatientClinicalInformationLeftEyeSequence
                ]
        return None

    @OphthalmicPatientClinicalInformationLeftEyeSequence.setter
    def OphthalmicPatientClinicalInformationLeftEyeSequence(
        self, value: Optional[List[OphthalmicPatientClinicalInformationLeftEyeSequenceItem]]
    ):
        if value is None:
            self._OphthalmicPatientClinicalInformationLeftEyeSequence = []
            if "OphthalmicPatientClinicalInformationLeftEyeSequence" in self._dataset:
                del self._dataset.OphthalmicPatientClinicalInformationLeftEyeSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, OphthalmicPatientClinicalInformationLeftEyeSequenceItem) for item in value
        ):
            raise ValueError(
                f"OphthalmicPatientClinicalInformationLeftEyeSequence must be a list of OphthalmicPatientClinicalInformationLeftEyeSequenceItem objects"
            )
        else:
            self._OphthalmicPatientClinicalInformationLeftEyeSequence = value
            if "OphthalmicPatientClinicalInformationLeftEyeSequence" not in self._dataset:
                self._dataset.OphthalmicPatientClinicalInformationLeftEyeSequence = pydicom.Sequence()
            self._dataset.OphthalmicPatientClinicalInformationLeftEyeSequence.clear()
            self._dataset.OphthalmicPatientClinicalInformationLeftEyeSequence.extend([item.to_dataset() for item in value])

    def add_OphthalmicPatientClinicalInformationLeftEye(self, item: OphthalmicPatientClinicalInformationLeftEyeSequenceItem):
        if not isinstance(item, OphthalmicPatientClinicalInformationLeftEyeSequenceItem):
            raise ValueError(f"Item must be an instance of OphthalmicPatientClinicalInformationLeftEyeSequenceItem")
        self._OphthalmicPatientClinicalInformationLeftEyeSequence.append(item)
        if "OphthalmicPatientClinicalInformationLeftEyeSequence" not in self._dataset:
            self._dataset.OphthalmicPatientClinicalInformationLeftEyeSequence = pydicom.Sequence()
        self._dataset.OphthalmicPatientClinicalInformationLeftEyeSequence.append(item.to_dataset())

    @property
    def OphthalmicPatientClinicalInformationRightEyeSequence(
        self,
    ) -> Optional[List[OphthalmicPatientClinicalInformationRightEyeSequenceItem]]:
        if "OphthalmicPatientClinicalInformationRightEyeSequence" in self._dataset:
            if len(self._OphthalmicPatientClinicalInformationRightEyeSequence) == len(
                self._dataset.OphthalmicPatientClinicalInformationRightEyeSequence
            ):
                return self._OphthalmicPatientClinicalInformationRightEyeSequence
            else:
                return [
                    OphthalmicPatientClinicalInformationRightEyeSequenceItem(x)
                    for x in self._dataset.OphthalmicPatientClinicalInformationRightEyeSequence
                ]
        return None

    @OphthalmicPatientClinicalInformationRightEyeSequence.setter
    def OphthalmicPatientClinicalInformationRightEyeSequence(
        self, value: Optional[List[OphthalmicPatientClinicalInformationRightEyeSequenceItem]]
    ):
        if value is None:
            self._OphthalmicPatientClinicalInformationRightEyeSequence = []
            if "OphthalmicPatientClinicalInformationRightEyeSequence" in self._dataset:
                del self._dataset.OphthalmicPatientClinicalInformationRightEyeSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, OphthalmicPatientClinicalInformationRightEyeSequenceItem) for item in value
        ):
            raise ValueError(
                f"OphthalmicPatientClinicalInformationRightEyeSequence must be a list of OphthalmicPatientClinicalInformationRightEyeSequenceItem objects"
            )
        else:
            self._OphthalmicPatientClinicalInformationRightEyeSequence = value
            if "OphthalmicPatientClinicalInformationRightEyeSequence" not in self._dataset:
                self._dataset.OphthalmicPatientClinicalInformationRightEyeSequence = pydicom.Sequence()
            self._dataset.OphthalmicPatientClinicalInformationRightEyeSequence.clear()
            self._dataset.OphthalmicPatientClinicalInformationRightEyeSequence.extend([item.to_dataset() for item in value])

    def add_OphthalmicPatientClinicalInformationRightEye(self, item: OphthalmicPatientClinicalInformationRightEyeSequenceItem):
        if not isinstance(item, OphthalmicPatientClinicalInformationRightEyeSequenceItem):
            raise ValueError(f"Item must be an instance of OphthalmicPatientClinicalInformationRightEyeSequenceItem")
        self._OphthalmicPatientClinicalInformationRightEyeSequence.append(item)
        if "OphthalmicPatientClinicalInformationRightEyeSequence" not in self._dataset:
            self._dataset.OphthalmicPatientClinicalInformationRightEyeSequence = pydicom.Sequence()
        self._dataset.OphthalmicPatientClinicalInformationRightEyeSequence.append(item.to_dataset())
    ...
    @property
    def VisualFieldHorizontalExtent(self) -> Optional[float]:
        if "VisualFieldHorizontalExtent" in self._dataset:
            return self._dataset.VisualFieldHorizontalExtent
        return None

    @VisualFieldHorizontalExtent.setter
    def VisualFieldHorizontalExtent(self, value: Optional[float]):
        if value is None:
            if "VisualFieldHorizontalExtent" in self._dataset:
                del self._dataset.VisualFieldHorizontalExtent
        else:
            self._dataset.VisualFieldHorizontalExtent = value

    @property
    def VisualFieldVerticalExtent(self) -> Optional[float]:
        if "VisualFieldVerticalExtent" in self._dataset:
            return self._dataset.VisualFieldVerticalExtent
        return None

    @VisualFieldVerticalExtent.setter
    def VisualFieldVerticalExtent(self, value: Optional[float]):
        if value is None:
            if "VisualFieldVerticalExtent" in self._dataset:
                del self._dataset.VisualFieldVerticalExtent
        else:
            self._dataset.VisualFieldVerticalExtent = value

    @property
    def VisualFieldShape(self) -> Optional[str]:
        if "VisualFieldShape" in self._dataset:
            return self._dataset.VisualFieldShape
        return None

    @VisualFieldShape.setter
    def VisualFieldShape(self, value: Optional[str]):
        if value is None:
            if "VisualFieldShape" in self._dataset:
                del self._dataset.VisualFieldShape
        else:
            self._dataset.VisualFieldShape = value

    @property
    def ScreeningTestModeCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "ScreeningTestModeCodeSequence" in self._dataset:
            if len(self._ScreeningTestModeCodeSequence) == len(self._dataset.ScreeningTestModeCodeSequence):
                return self._ScreeningTestModeCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.ScreeningTestModeCodeSequence]
        return None

    @ScreeningTestModeCodeSequence.setter
    def ScreeningTestModeCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._ScreeningTestModeCodeSequence = []
            if "ScreeningTestModeCodeSequence" in self._dataset:
                del self._dataset.ScreeningTestModeCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"ScreeningTestModeCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._ScreeningTestModeCodeSequence = value
            if "ScreeningTestModeCodeSequence" not in self._dataset:
                self._dataset.ScreeningTestModeCodeSequence = pydicom.Sequence()
            self._dataset.ScreeningTestModeCodeSequence.clear()
            self._dataset.ScreeningTestModeCodeSequence.extend([item.to_dataset() for item in value])

    def add_ScreeningTestModeCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._ScreeningTestModeCodeSequence.append(item)
        if "ScreeningTestModeCodeSequence" not in self._dataset:
            self._dataset.ScreeningTestModeCodeSequence = pydicom.Sequence()
        self._dataset.ScreeningTestModeCodeSequence.append(item.to_dataset())

    @property
    def MaximumStimulusLuminance(self) -> Optional[float]:
        if "MaximumStimulusLuminance" in self._dataset:
            return self._dataset.MaximumStimulusLuminance
        return None

    @MaximumStimulusLuminance.setter
    def MaximumStimulusLuminance(self, value: Optional[float]):
        if value is None:
            if "MaximumStimulusLuminance" in self._dataset:
                del self._dataset.MaximumStimulusLuminance
        else:
            self._dataset.MaximumStimulusLuminance = value

    @property
    def BackgroundLuminance(self) -> Optional[float]:
        if "BackgroundLuminance" in self._dataset:
            return self._dataset.BackgroundLuminance
        return None

    @BackgroundLuminance.setter
    def BackgroundLuminance(self, value: Optional[float]):
        if value is None:
            if "BackgroundLuminance" in self._dataset:
                del self._dataset.BackgroundLuminance
        else:
            self._dataset.BackgroundLuminance = value

    @property
    def StimulusColorCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "StimulusColorCodeSequence" in self._dataset:
            if len(self._StimulusColorCodeSequence) == len(self._dataset.StimulusColorCodeSequence):
                return self._StimulusColorCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.StimulusColorCodeSequence]
        return None

    @StimulusColorCodeSequence.setter
    def StimulusColorCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._StimulusColorCodeSequence = []
            if "StimulusColorCodeSequence" in self._dataset:
                del self._dataset.StimulusColorCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"StimulusColorCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._StimulusColorCodeSequence = value
            if "StimulusColorCodeSequence" not in self._dataset:
                self._dataset.StimulusColorCodeSequence = pydicom.Sequence()
            self._dataset.StimulusColorCodeSequence.clear()
            self._dataset.StimulusColorCodeSequence.extend([item.to_dataset() for item in value])

    def add_StimulusColorCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._StimulusColorCodeSequence.append(item)
        if "StimulusColorCodeSequence" not in self._dataset:
            self._dataset.StimulusColorCodeSequence = pydicom.Sequence()
        self._dataset.StimulusColorCodeSequence.append(item.to_dataset())

    @property
    def BackgroundIlluminationColorCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "BackgroundIlluminationColorCodeSequence" in self._dataset:
            if len(self._BackgroundIlluminationColorCodeSequence) == len(
                self._dataset.BackgroundIlluminationColorCodeSequence
            ):
                return self._BackgroundIlluminationColorCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.BackgroundIlluminationColorCodeSequence]
        return None

    @BackgroundIlluminationColorCodeSequence.setter
    def BackgroundIlluminationColorCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._BackgroundIlluminationColorCodeSequence = []
            if "BackgroundIlluminationColorCodeSequence" in self._dataset:
                del self._dataset.BackgroundIlluminationColorCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"BackgroundIlluminationColorCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._BackgroundIlluminationColorCodeSequence = value
            if "BackgroundIlluminationColorCodeSequence" not in self._dataset:
                self._dataset.BackgroundIlluminationColorCodeSequence = pydicom.Sequence()
            self._dataset.BackgroundIlluminationColorCodeSequence.clear()
            self._dataset.BackgroundIlluminationColorCodeSequence.extend([item.to_dataset() for item in value])

    def add_BackgroundIlluminationColorCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._BackgroundIlluminationColorCodeSequence.append(item)
        if "BackgroundIlluminationColorCodeSequence" not in self._dataset:
            self._dataset.BackgroundIlluminationColorCodeSequence = pydicom.Sequence()
        self._dataset.BackgroundIlluminationColorCodeSequence.append(item.to_dataset())

    @property
    def StimulusArea(self) -> Optional[float]:
        if "StimulusArea" in self._dataset:
            return self._dataset.StimulusArea
        return None

    @StimulusArea.setter
    def StimulusArea(self, value: Optional[float]):
        if value is None:
            if "StimulusArea" in self._dataset:
                del self._dataset.StimulusArea
        else:
            self._dataset.StimulusArea = value

    @property
    def StimulusPresentationTime(self) -> Optional[float]:
        if "StimulusPresentationTime" in self._dataset:
            return self._dataset.StimulusPresentationTime
        return None

    @StimulusPresentationTime.setter
    def StimulusPresentationTime(self, value: Optional[float]):
        if value is None:
            if "StimulusPresentationTime" in self._dataset:
                del self._dataset.StimulusPresentationTime
        else:
            self._dataset.StimulusPresentationTime = value
    ...
    @property
    def FixationSequence(self) -> Optional[List[FixationSequenceItem]]:
        if "FixationSequence" in self._dataset:
            if len(self._FixationSequence) == len(self._dataset.FixationSequence):
                return self._FixationSequence
            else:
                return [FixationSequenceItem(x) for x in self._dataset.FixationSequence]
        return None

    @FixationSequence.setter
    def FixationSequence(self, value: Optional[List[FixationSequenceItem]]):
        if value is None:
            self._FixationSequence = []
            if "FixationSequence" in self._dataset:
                del self._dataset.FixationSequence
        elif not isinstance(value, list) or not all(isinstance(item, FixationSequenceItem) for item in value):
            raise ValueError(f"FixationSequence must be a list of FixationSequenceItem objects")
        else:
            self._FixationSequence = value
            if "FixationSequence" not in self._dataset:
                self._dataset.FixationSequence = pydicom.Sequence()
            self._dataset.FixationSequence.clear()
            self._dataset.FixationSequence.extend([item.to_dataset() for item in value])

    def add_Fixation(self, item: FixationSequenceItem):
        if not isinstance(item, FixationSequenceItem):
            raise ValueError(f"Item must be an instance of FixationSequenceItem")
        self._FixationSequence.append(item)
        if "FixationSequence" not in self._dataset:
            self._dataset.FixationSequence = pydicom.Sequence()
        self._dataset.FixationSequence.append(item.to_dataset())

    @property
    def VisualFieldCatchTrialSequence(self) -> Optional[List[VisualFieldCatchTrialSequenceItem]]:
        if "VisualFieldCatchTrialSequence" in self._dataset:
            if len(self._VisualFieldCatchTrialSequence) == len(self._dataset.VisualFieldCatchTrialSequence):
                return self._VisualFieldCatchTrialSequence
            else:
                return [VisualFieldCatchTrialSequenceItem(x) for x in self._dataset.VisualFieldCatchTrialSequence]
        return None

    @VisualFieldCatchTrialSequence.setter
    def VisualFieldCatchTrialSequence(self, value: Optional[List[VisualFieldCatchTrialSequenceItem]]):
        if value is None:
            self._VisualFieldCatchTrialSequence = []
            if "VisualFieldCatchTrialSequence" in self._dataset:
                del self._dataset.VisualFieldCatchTrialSequence
        elif not isinstance(value, list) or not all(isinstance(item, VisualFieldCatchTrialSequenceItem) for item in value):
            raise ValueError(f"VisualFieldCatchTrialSequence must be a list of VisualFieldCatchTrialSequenceItem objects")
        else:
            self._VisualFieldCatchTrialSequence = value
            if "VisualFieldCatchTrialSequence" not in self._dataset:
                self._dataset.VisualFieldCatchTrialSequence = pydicom.Sequence()
            self._dataset.VisualFieldCatchTrialSequence.clear()
            self._dataset.VisualFieldCatchTrialSequence.extend([item.to_dataset() for item in value])

    def add_VisualFieldCatchTrial(self, item: VisualFieldCatchTrialSequenceItem):
        if not isinstance(item, VisualFieldCatchTrialSequenceItem):
            raise ValueError(f"Item must be an instance of VisualFieldCatchTrialSequenceItem")
        self._VisualFieldCatchTrialSequence.append(item)
        if "VisualFieldCatchTrialSequence" not in self._dataset:
            self._dataset.VisualFieldCatchTrialSequence = pydicom.Sequence()
        self._dataset.VisualFieldCatchTrialSequence.append(item.to_dataset())

    @property
    def StimuliRetestingQuantity(self) -> Optional[int]:
        if "StimuliRetestingQuantity" in self._dataset:
            return self._dataset.StimuliRetestingQuantity
        return None

    @StimuliRetestingQuantity.setter
    def StimuliRetestingQuantity(self, value: Optional[int]):
        if value is None:
            if "StimuliRetestingQuantity" in self._dataset:
                del self._dataset.StimuliRetestingQuantity
        else:
            self._dataset.StimuliRetestingQuantity = value

    @property
    def CommentsOnPatientPerformanceOfVisualField(self) -> Optional[str]:
        if "CommentsOnPatientPerformanceOfVisualField" in self._dataset:
            return self._dataset.CommentsOnPatientPerformanceOfVisualField
        return None

    @CommentsOnPatientPerformanceOfVisualField.setter
    def CommentsOnPatientPerformanceOfVisualField(self, value: Optional[str]):
        if value is None:
            if "CommentsOnPatientPerformanceOfVisualField" in self._dataset:
                del self._dataset.CommentsOnPatientPerformanceOfVisualField
        else:
            self._dataset.CommentsOnPatientPerformanceOfVisualField = value

    @property
    def PatientReliabilityIndicator(self) -> Optional[str]:
        if "PatientReliabilityIndicator" in self._dataset:
            return self._dataset.PatientReliabilityIndicator
        return None

    @PatientReliabilityIndicator.setter
    def PatientReliabilityIndicator(self, value: Optional[str]):
        if value is None:
            if "PatientReliabilityIndicator" in self._dataset:
                del self._dataset.PatientReliabilityIndicator
        else:
            self._dataset.PatientReliabilityIndicator = value

    @property
    def VisualFieldTestReliabilityGlobalIndexSequence(
        self,
    ) -> Optional[List[VisualFieldTestReliabilityGlobalIndexSequenceItem]]:
        if "VisualFieldTestReliabilityGlobalIndexSequence" in self._dataset:
            if len(self._VisualFieldTestReliabilityGlobalIndexSequence) == len(
                self._dataset.VisualFieldTestReliabilityGlobalIndexSequence
            ):
                return self._VisualFieldTestReliabilityGlobalIndexSequence
            else:
                return [
                    VisualFieldTestReliabilityGlobalIndexSequenceItem(x)
                    for x in self._dataset.VisualFieldTestReliabilityGlobalIndexSequence
                ]
        return None

    @VisualFieldTestReliabilityGlobalIndexSequence.setter
    def VisualFieldTestReliabilityGlobalIndexSequence(
        self, value: Optional[List[VisualFieldTestReliabilityGlobalIndexSequenceItem]]
    ):
        if value is None:
            self._VisualFieldTestReliabilityGlobalIndexSequence = []
            if "VisualFieldTestReliabilityGlobalIndexSequence" in self._dataset:
                del self._dataset.VisualFieldTestReliabilityGlobalIndexSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, VisualFieldTestReliabilityGlobalIndexSequenceItem) for item in value
        ):
            raise ValueError(
                f"VisualFieldTestReliabilityGlobalIndexSequence must be a list of VisualFieldTestReliabilityGlobalIndexSequenceItem objects"
            )
        else:
            self._VisualFieldTestReliabilityGlobalIndexSequence = value
            if "VisualFieldTestReliabilityGlobalIndexSequence" not in self._dataset:
                self._dataset.VisualFieldTestReliabilityGlobalIndexSequence = pydicom.Sequence()
            self._dataset.VisualFieldTestReliabilityGlobalIndexSequence.clear()
            self._dataset.VisualFieldTestReliabilityGlobalIndexSequence.extend([item.to_dataset() for item in value])

    def add_VisualFieldTestReliabilityGlobalIndex(self, item: VisualFieldTestReliabilityGlobalIndexSequenceItem):
        if not isinstance(item, VisualFieldTestReliabilityGlobalIndexSequenceItem):
            raise ValueError(f"Item must be an instance of VisualFieldTestReliabilityGlobalIndexSequenceItem")
        self._VisualFieldTestReliabilityGlobalIndexSequence.append(item)
        if "VisualFieldTestReliabilityGlobalIndexSequence" not in self._dataset:
            self._dataset.VisualFieldTestReliabilityGlobalIndexSequence = pydicom.Sequence()
        self._dataset.VisualFieldTestReliabilityGlobalIndexSequence.append(item.to_dataset())
