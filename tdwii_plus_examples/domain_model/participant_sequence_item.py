...
class ParticipantSequenceItem:
    ...
    @property
    def ParticipationType(self) -> Optional[str]:
        if "ParticipationType" in self._dataset:
            return self._dataset.ParticipationType
        return None

    @ParticipationType.setter
    def ParticipationType(self, value: Optional[str]):
        if value is None:
            if "ParticipationType" in self._dataset:
                del self._dataset.ParticipationType
        else:
            self._dataset.ParticipationType = value

    @property
    def ParticipationDateTime(self) -> Optional[str]:
        if "ParticipationDateTime" in self._dataset:
            return self._dataset.ParticipationDateTime
        return None

    @ParticipationDateTime.setter
    def ParticipationDateTime(self, value: Optional[str]):
        if value is None:
            if "ParticipationDateTime" in self._dataset:
                del self._dataset.ParticipationDateTime
        else:
            self._dataset.ParticipationDateTime = value
