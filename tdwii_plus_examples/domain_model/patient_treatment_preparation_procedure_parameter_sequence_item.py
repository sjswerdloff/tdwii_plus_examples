...
from .content_item_modifier_sequence_item import ContentItemModifierSequenceItem
from .referenced_sop_sequence_item import ReferencedSOPSequenceItem


class PatientTreatmentPreparationProcedureParameterSequenceItem:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        self._dataset = dataset if dataset is not None else pydicom.Dataset()
        self._ReferencedSOPSequence: List[ReferencedSOPSequenceItem] = []
        self._ContentItemModifierSequence: List[ContentItemModifierSequenceItem] = []
    ...
    @property
    def ContentItemModifierSequence(self) -> Optional[List[ContentItemModifierSequenceItem]]:
        if "ContentItemModifierSequence" in self._dataset:
            if len(self._ContentItemModifierSequence) == len(self._dataset.ContentItemModifierSequence):
                return self._ContentItemModifierSequence
            else:
                return [ContentItemModifierSequenceItem(x) for x in self._dataset.ContentItemModifierSequence]
        return None

    @ContentItemModifierSequence.setter
    def ContentItemModifierSequence(self, value: Optional[List[ContentItemModifierSequenceItem]]):
        if value is None:
            self._ContentItemModifierSequence = []
            if "ContentItemModifierSequence" in self._dataset:
            ...
            self._dataset.ContentItemModifierSequence.clear()
            self._dataset.ContentItemModifierSequence.extend([item.to_dataset() for item in value])
    ...
    def add_ContentItemModifier(self, item: ContentItemModifierSequenceItem):
        ...
        if "ContentItemModifierSequence" not in self._dataset:
            self._dataset.ContentItemModifierSequence = pydicom.Sequence()
        self._dataset.ContentItemModifierSequence.append(item.to_dataset())
