...
from .author_observer_sequence_item import AuthorObserverSequenceItem
...
from .participant_sequence_item import ParticipantSequenceItem
from .pertinent_other_evidence_sequence_item import PertinentOtherEvidenceSequenceItem
...
from .predecessor_documents_sequence_item import PredecessorDocumentsSequenceItem
...
from .verifying_observer_sequence_item import VerifyingObserverSequenceItem


class PerformedImagingAgentAdministrationSr:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._VerifyingObserverSequence: List[VerifyingObserverSequenceItem] = []
        self._AuthorObserverSequence: List[AuthorObserverSequenceItem] = []
        self._ParticipantSequence: List[ParticipantSequenceItem] = []
        self._CustodialOrganizationSequence: List[CustodialOrganizationSequenceItem] = []
        self._PredecessorDocumentsSequence: List[PredecessorDocumentsSequenceItem] = []
        self._ReferencedRequestSequence: List[ReferencedRequestSequenceItem] = []
        self._PerformedProcedureCodeSequence: List[CodeSequenceItem] = []
        self._CurrentRequestedProcedureEvidenceSequence: List[CurrentRequestedProcedureEvidenceSequenceItem] = []
        self._PertinentOtherEvidenceSequence: List[PertinentOtherEvidenceSequenceItem] = []
    ...
    @property
    def VerifyingObserverSequence(self) -> Optional[List[VerifyingObserverSequenceItem]]:
        if "VerifyingObserverSequence" in self._dataset:
            if len(self._VerifyingObserverSequence) == len(self._dataset.VerifyingObserverSequence):
                return self._VerifyingObserverSequence
            else:
                return [VerifyingObserverSequenceItem(x) for x in self._dataset.VerifyingObserverSequence]
        return None

    @VerifyingObserverSequence.setter
    def VerifyingObserverSequence(self, value: Optional[List[VerifyingObserverSequenceItem]]):
        if value is None:
            self._VerifyingObserverSequence = []
            if "VerifyingObserverSequence" in self._dataset:
                del self._dataset.VerifyingObserverSequence
        elif not isinstance(value, list) or not all(isinstance(item, VerifyingObserverSequenceItem) for item in value):
            raise ValueError(f"VerifyingObserverSequence must be a list of VerifyingObserverSequenceItem objects")
        else:
            self._VerifyingObserverSequence = value
            if "VerifyingObserverSequence" not in self._dataset:
                self._dataset.VerifyingObserverSequence = pydicom.Sequence()
            self._dataset.VerifyingObserverSequence.clear()
            self._dataset.VerifyingObserverSequence.extend([item.to_dataset() for item in value])

    def add_VerifyingObserver(self, item: VerifyingObserverSequenceItem):
        if not isinstance(item, VerifyingObserverSequenceItem):
            raise ValueError(f"Item must be an instance of VerifyingObserverSequenceItem")
        self._VerifyingObserverSequence.append(item)
        if "VerifyingObserverSequence" not in self._dataset:
            self._dataset.VerifyingObserverSequence = pydicom.Sequence()
        self._dataset.VerifyingObserverSequence.append(item.to_dataset())

    @property
    def AuthorObserverSequence(self) -> Optional[List[AuthorObserverSequenceItem]]:
        if "AuthorObserverSequence" in self._dataset:
            if len(self._AuthorObserverSequence) == len(self._dataset.AuthorObserverSequence):
                return self._AuthorObserverSequence
            else:
                return [AuthorObserverSequenceItem(x) for x in self._dataset.AuthorObserverSequence]
        return None

    @AuthorObserverSequence.setter
    def AuthorObserverSequence(self, value: Optional[List[AuthorObserverSequenceItem]]):
        if value is None:
            self._AuthorObserverSequence = []
            if "AuthorObserverSequence" in self._dataset:
                del self._dataset.AuthorObserverSequence
        elif not isinstance(value, list) or not all(isinstance(item, AuthorObserverSequenceItem) for item in value):
            raise ValueError(f"AuthorObserverSequence must be a list of AuthorObserverSequenceItem objects")
        else:
            self._AuthorObserverSequence = value
            if "AuthorObserverSequence" not in self._dataset:
                self._dataset.AuthorObserverSequence = pydicom.Sequence()
            self._dataset.AuthorObserverSequence.clear()
            self._dataset.AuthorObserverSequence.extend([item.to_dataset() for item in value])

    def add_AuthorObserver(self, item: AuthorObserverSequenceItem):
        if not isinstance(item, AuthorObserverSequenceItem):
            raise ValueError(f"Item must be an instance of AuthorObserverSequenceItem")
        self._AuthorObserverSequence.append(item)
        if "AuthorObserverSequence" not in self._dataset:
            self._dataset.AuthorObserverSequence = pydicom.Sequence()
        self._dataset.AuthorObserverSequence.append(item.to_dataset())

    @property
    def ParticipantSequence(self) -> Optional[List[ParticipantSequenceItem]]:
        if "ParticipantSequence" in self._dataset:
            if len(self._ParticipantSequence) == len(self._dataset.ParticipantSequence):
                return self._ParticipantSequence
            else:
                return [ParticipantSequenceItem(x) for x in self._dataset.ParticipantSequence]
        return None

    @ParticipantSequence.setter
    def ParticipantSequence(self, value: Optional[List[ParticipantSequenceItem]]):
        if value is None:
            self._ParticipantSequence = []
            if "ParticipantSequence" in self._dataset:
                del self._dataset.ParticipantSequence
        elif not isinstance(value, list) or not all(isinstance(item, ParticipantSequenceItem) for item in value):
            raise ValueError(f"ParticipantSequence must be a list of ParticipantSequenceItem objects")
        else:
            self._ParticipantSequence = value
            if "ParticipantSequence" not in self._dataset:
                self._dataset.ParticipantSequence = pydicom.Sequence()
            self._dataset.ParticipantSequence.clear()
            self._dataset.ParticipantSequence.extend([item.to_dataset() for item in value])

    def add_Participant(self, item: ParticipantSequenceItem):
        if not isinstance(item, ParticipantSequenceItem):
            raise ValueError(f"Item must be an instance of ParticipantSequenceItem")
        self._ParticipantSequence.append(item)
        if "ParticipantSequence" not in self._dataset:
            self._dataset.ParticipantSequence = pydicom.Sequence()
        self._dataset.ParticipantSequence.append(item.to_dataset())
    ...
    def CustodialOrganizationSequence(self, value: Optional[List[CustodialOrganizationSequenceItem]]):
        if value is None:
            ...
            raise ValueError(f"CustodialOrganizationSequence must be a list of CustodialOrganizationSequenceItem objects")
        else:
    ...
    def add_CustodialOrganization(self, item: CustodialOrganizationSequenceItem):
        if not isinstance(item, CustodialOrganizationSequenceItem):
            raise ValueError(f"Item must be an instance of CustodialOrganizationSequenceItem")
    ...
    @property
    def PredecessorDocumentsSequence(self) -> Optional[List[PredecessorDocumentsSequenceItem]]:
        if "PredecessorDocumentsSequence" in self._dataset:
            if len(self._PredecessorDocumentsSequence) == len(self._dataset.PredecessorDocumentsSequence):
                return self._PredecessorDocumentsSequence
            else:
                return [PredecessorDocumentsSequenceItem(x) for x in self._dataset.PredecessorDocumentsSequence]
        return None

    @PredecessorDocumentsSequence.setter
    def PredecessorDocumentsSequence(self, value: Optional[List[PredecessorDocumentsSequenceItem]]):
        if value is None:
            self._PredecessorDocumentsSequence = []
            if "PredecessorDocumentsSequence" in self._dataset:
                del self._dataset.PredecessorDocumentsSequence
        elif not isinstance(value, list) or not all(isinstance(item, PredecessorDocumentsSequenceItem) for item in value):
            raise ValueError(f"PredecessorDocumentsSequence must be a list of PredecessorDocumentsSequenceItem objects")
        else:
            self._PredecessorDocumentsSequence = value
            if "PredecessorDocumentsSequence" not in self._dataset:
                self._dataset.PredecessorDocumentsSequence = pydicom.Sequence()
            self._dataset.PredecessorDocumentsSequence.clear()
            self._dataset.PredecessorDocumentsSequence.extend([item.to_dataset() for item in value])

    def add_PredecessorDocuments(self, item: PredecessorDocumentsSequenceItem):
        if not isinstance(item, PredecessorDocumentsSequenceItem):
            raise ValueError(f"Item must be an instance of PredecessorDocumentsSequenceItem")
        self._PredecessorDocumentsSequence.append(item)
        if "PredecessorDocumentsSequence" not in self._dataset:
            self._dataset.PredecessorDocumentsSequence = pydicom.Sequence()
        self._dataset.PredecessorDocumentsSequence.append(item.to_dataset())
    ...
    def ReferencedRequestSequence(self, value: Optional[List[ReferencedRequestSequenceItem]]):
        if value is None:
            ...
            raise ValueError(f"ReferencedRequestSequence must be a list of ReferencedRequestSequenceItem objects")
        else:
    ...
    def add_ReferencedRequest(self, item: ReferencedRequestSequenceItem):
        if not isinstance(item, ReferencedRequestSequenceItem):
            raise ValueError(f"Item must be an instance of ReferencedRequestSequenceItem")
    ...
    @property
    def PerformedProcedureCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "PerformedProcedureCodeSequence" in self._dataset:
            if len(self._PerformedProcedureCodeSequence) == len(self._dataset.PerformedProcedureCodeSequence):
                return self._PerformedProcedureCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.PerformedProcedureCodeSequence]
        return None

    @PerformedProcedureCodeSequence.setter
    def PerformedProcedureCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._PerformedProcedureCodeSequence = []
            if "PerformedProcedureCodeSequence" in self._dataset:
                del self._dataset.PerformedProcedureCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"PerformedProcedureCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._PerformedProcedureCodeSequence = value
            if "PerformedProcedureCodeSequence" not in self._dataset:
                self._dataset.PerformedProcedureCodeSequence = pydicom.Sequence()
            self._dataset.PerformedProcedureCodeSequence.clear()
            self._dataset.PerformedProcedureCodeSequence.extend([item.to_dataset() for item in value])

    def add_PerformedProcedureCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._PerformedProcedureCodeSequence.append(item)
        if "PerformedProcedureCodeSequence" not in self._dataset:
            self._dataset.PerformedProcedureCodeSequence = pydicom.Sequence()
        self._dataset.PerformedProcedureCodeSequence.append(item.to_dataset())
    ...
    def CurrentRequestedProcedureEvidenceSequence(self, value: Optional[List[CurrentRequestedProcedureEvidenceSequenceItem]]):
        if value is None:
            ...
            raise ValueError(
                f"CurrentRequestedProcedureEvidenceSequence must be a list of CurrentRequestedProcedureEvidenceSequenceItem objects"
            )
        else:
    ...
    def add_CurrentRequestedProcedureEvidence(self, item: CurrentRequestedProcedureEvidenceSequenceItem):
        if not isinstance(item, CurrentRequestedProcedureEvidenceSequenceItem):
            raise ValueError(f"Item must be an instance of CurrentRequestedProcedureEvidenceSequenceItem")
    ...
    @property
    def PertinentOtherEvidenceSequence(self) -> Optional[List[PertinentOtherEvidenceSequenceItem]]:
        if "PertinentOtherEvidenceSequence" in self._dataset:
            if len(self._PertinentOtherEvidenceSequence) == len(self._dataset.PertinentOtherEvidenceSequence):
                return self._PertinentOtherEvidenceSequence
            else:
                return [PertinentOtherEvidenceSequenceItem(x) for x in self._dataset.PertinentOtherEvidenceSequence]
        return None

    @PertinentOtherEvidenceSequence.setter
    def PertinentOtherEvidenceSequence(self, value: Optional[List[PertinentOtherEvidenceSequenceItem]]):
        if value is None:
            self._PertinentOtherEvidenceSequence = []
            if "PertinentOtherEvidenceSequence" in self._dataset:
                del self._dataset.PertinentOtherEvidenceSequence
        elif not isinstance(value, list) or not all(isinstance(item, PertinentOtherEvidenceSequenceItem) for item in value):
            raise ValueError(f"PertinentOtherEvidenceSequence must be a list of PertinentOtherEvidenceSequenceItem objects")
        else:
            self._PertinentOtherEvidenceSequence = value
            if "PertinentOtherEvidenceSequence" not in self._dataset:
                self._dataset.PertinentOtherEvidenceSequence = pydicom.Sequence()
            self._dataset.PertinentOtherEvidenceSequence.clear()
            self._dataset.PertinentOtherEvidenceSequence.extend([item.to_dataset() for item in value])

    def add_PertinentOtherEvidence(self, item: PertinentOtherEvidenceSequenceItem):
        if not isinstance(item, PertinentOtherEvidenceSequenceItem):
            raise ValueError(f"Item must be an instance of PertinentOtherEvidenceSequenceItem")
        self._PertinentOtherEvidenceSequence.append(item)
        if "PertinentOtherEvidenceSequence" not in self._dataset:
            self._dataset.PertinentOtherEvidenceSequence = pydicom.Sequence()
        self._dataset.PertinentOtherEvidenceSequence.append(item.to_dataset())

    @property
    def CompletionFlag(self) -> Optional[str]:
        if "CompletionFlag" in self._dataset:
            return self._dataset.CompletionFlag
        return None

    @CompletionFlag.setter
    def CompletionFlag(self, value: Optional[str]):
        if value is None:
            if "CompletionFlag" in self._dataset:
                del self._dataset.CompletionFlag
        else:
            self._dataset.CompletionFlag = value

    @property
    def CompletionFlagDescription(self) -> Optional[str]:
        if "CompletionFlagDescription" in self._dataset:
            return self._dataset.CompletionFlagDescription
        return None

    @CompletionFlagDescription.setter
    def CompletionFlagDescription(self, value: Optional[str]):
        if value is None:
            if "CompletionFlagDescription" in self._dataset:
                del self._dataset.CompletionFlagDescription
        else:
            self._dataset.CompletionFlagDescription = value

    @property
    def VerificationFlag(self) -> Optional[str]:
        if "VerificationFlag" in self._dataset:
            return self._dataset.VerificationFlag
        return None

    @VerificationFlag.setter
    def VerificationFlag(self, value: Optional[str]):
        if value is None:
            if "VerificationFlag" in self._dataset:
                del self._dataset.VerificationFlag
        else:
            self._dataset.VerificationFlag = value

    @property
    def PreliminaryFlag(self) -> Optional[str]:
        if "PreliminaryFlag" in self._dataset:
            return self._dataset.PreliminaryFlag
        return None

    @PreliminaryFlag.setter
    def PreliminaryFlag(self, value: Optional[str]):
        if value is None:
            if "PreliminaryFlag" in self._dataset:
                del self._dataset.PreliminaryFlag
        else:
            self._dataset.PreliminaryFlag = value
    ...
    def IdenticalDocumentsSequence(self, value: Optional[List[IdenticalDocumentsSequenceItem]]):
        if value is None:
            ...
            raise ValueError(f"IdenticalDocumentsSequence must be a list of IdenticalDocumentsSequenceItem objects")
        else:
    ...
    def add_IdenticalDocuments(self, item: IdenticalDocumentsSequenceItem):
        if not isinstance(item, IdenticalDocumentsSequenceItem):
            raise ValueError(f"Item must be an instance of IdenticalDocumentsSequenceItem")
    ...
    def ContentSequence(self, value: Optional[List[ContentSequenceItem]]):
        if value is None:
            ...
            raise ValueError(f"ContentSequence must be a list of ContentSequenceItem objects")
        else:
    ...
    def add_Content(self, item: ContentSequenceItem):
        if not isinstance(item, ContentSequenceItem):
            raise ValueError(f"Item must be an instance of ContentSequenceItem")
