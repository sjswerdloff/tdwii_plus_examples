...
from .excitation_wavelength_sequence_item import ExcitationWavelengthSequenceItem
...
from .transducer_response_sequence_item import TransducerResponseSequenceItem
from .transducer_technology_sequence_item import TransducerTechnologySequenceItem
from .udi_sequence_item import UDISequenceItem


class PhotoacousticImage:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._TransducerResponseSequence: List[TransducerResponseSequenceItem] = []
        self._TransducerTechnologySequence: List[TransducerTechnologySequenceItem] = []
        self._ExcitationWavelengthSequence: List[ExcitationWavelengthSequenceItem] = []
        self._AcousticCouplingMediumCodeSequence: List[CodeSequenceItem] = []
        ...
        self._SoundSpeedCorrectionMechanismCodeSequence: List[CodeSequenceItem] = []
    ...
    @property
    def TransducerResponseSequence(self) -> Optional[List[TransducerResponseSequenceItem]]:
        if "TransducerResponseSequence" in self._dataset:
            if len(self._TransducerResponseSequence) == len(self._dataset.TransducerResponseSequence):
                return self._TransducerResponseSequence
            else:
                return [TransducerResponseSequenceItem(x) for x in self._dataset.TransducerResponseSequence]
        return None

    @TransducerResponseSequence.setter
    def TransducerResponseSequence(self, value: Optional[List[TransducerResponseSequenceItem]]):
        if value is None:
            self._TransducerResponseSequence = []
            if "TransducerResponseSequence" in self._dataset:
                del self._dataset.TransducerResponseSequence
        elif not isinstance(value, list) or not all(isinstance(item, TransducerResponseSequenceItem) for item in value):
            raise ValueError(f"TransducerResponseSequence must be a list of TransducerResponseSequenceItem objects")
        else:
            self._TransducerResponseSequence = value
            if "TransducerResponseSequence" not in self._dataset:
                self._dataset.TransducerResponseSequence = pydicom.Sequence()
            self._dataset.TransducerResponseSequence.clear()
            self._dataset.TransducerResponseSequence.extend([item.to_dataset() for item in value])

    def add_TransducerResponse(self, item: TransducerResponseSequenceItem):
        if not isinstance(item, TransducerResponseSequenceItem):
            raise ValueError(f"Item must be an instance of TransducerResponseSequenceItem")
        self._TransducerResponseSequence.append(item)
        if "TransducerResponseSequence" not in self._dataset:
            self._dataset.TransducerResponseSequence = pydicom.Sequence()
        self._dataset.TransducerResponseSequence.append(item.to_dataset())

    @property
    def TransducerTechnologySequence(self) -> Optional[List[TransducerTechnologySequenceItem]]:
        if "TransducerTechnologySequence" in self._dataset:
            if len(self._TransducerTechnologySequence) == len(self._dataset.TransducerTechnologySequence):
                return self._TransducerTechnologySequence
            else:
                return [TransducerTechnologySequenceItem(x) for x in self._dataset.TransducerTechnologySequence]
        return None

    @TransducerTechnologySequence.setter
    def TransducerTechnologySequence(self, value: Optional[List[TransducerTechnologySequenceItem]]):
        if value is None:
            self._TransducerTechnologySequence = []
            if "TransducerTechnologySequence" in self._dataset:
                del self._dataset.TransducerTechnologySequence
        elif not isinstance(value, list) or not all(isinstance(item, TransducerTechnologySequenceItem) for item in value):
            raise ValueError(f"TransducerTechnologySequence must be a list of TransducerTechnologySequenceItem objects")
        else:
            self._TransducerTechnologySequence = value
            if "TransducerTechnologySequence" not in self._dataset:
                self._dataset.TransducerTechnologySequence = pydicom.Sequence()
            self._dataset.TransducerTechnologySequence.clear()
            self._dataset.TransducerTechnologySequence.extend([item.to_dataset() for item in value])

    def add_TransducerTechnology(self, item: TransducerTechnologySequenceItem):
        if not isinstance(item, TransducerTechnologySequenceItem):
            raise ValueError(f"Item must be an instance of TransducerTechnologySequenceItem")
        self._TransducerTechnologySequence.append(item)
        if "TransducerTechnologySequence" not in self._dataset:
            self._dataset.TransducerTechnologySequence = pydicom.Sequence()
        self._dataset.TransducerTechnologySequence.append(item.to_dataset())

    @property
    def ExcitationWavelengthSequence(self) -> Optional[List[ExcitationWavelengthSequenceItem]]:
        if "ExcitationWavelengthSequence" in self._dataset:
            if len(self._ExcitationWavelengthSequence) == len(self._dataset.ExcitationWavelengthSequence):
                return self._ExcitationWavelengthSequence
            else:
                return [ExcitationWavelengthSequenceItem(x) for x in self._dataset.ExcitationWavelengthSequence]
        return None

    @ExcitationWavelengthSequence.setter
    def ExcitationWavelengthSequence(self, value: Optional[List[ExcitationWavelengthSequenceItem]]):
        if value is None:
            self._ExcitationWavelengthSequence = []
            if "ExcitationWavelengthSequence" in self._dataset:
                del self._dataset.ExcitationWavelengthSequence
        elif not isinstance(value, list) or not all(isinstance(item, ExcitationWavelengthSequenceItem) for item in value):
            raise ValueError(f"ExcitationWavelengthSequence must be a list of ExcitationWavelengthSequenceItem objects")
        else:
            self._ExcitationWavelengthSequence = value
            if "ExcitationWavelengthSequence" not in self._dataset:
                self._dataset.ExcitationWavelengthSequence = pydicom.Sequence()
            self._dataset.ExcitationWavelengthSequence.clear()
            self._dataset.ExcitationWavelengthSequence.extend([item.to_dataset() for item in value])

    def add_ExcitationWavelength(self, item: ExcitationWavelengthSequenceItem):
        if not isinstance(item, ExcitationWavelengthSequenceItem):
            raise ValueError(f"Item must be an instance of ExcitationWavelengthSequenceItem")
        self._ExcitationWavelengthSequence.append(item)
        if "ExcitationWavelengthSequence" not in self._dataset:
            self._dataset.ExcitationWavelengthSequence = pydicom.Sequence()
        self._dataset.ExcitationWavelengthSequence.append(item.to_dataset())

    @property
    def IlluminationTranslationFlag(self) -> Optional[str]:
        if "IlluminationTranslationFlag" in self._dataset:
            return self._dataset.IlluminationTranslationFlag
        return None

    @IlluminationTranslationFlag.setter
    def IlluminationTranslationFlag(self, value: Optional[str]):
        if value is None:
            if "IlluminationTranslationFlag" in self._dataset:
                del self._dataset.IlluminationTranslationFlag
        else:
            self._dataset.IlluminationTranslationFlag = value

    @property
    def AcousticCouplingMediumFlag(self) -> Optional[str]:
        if "AcousticCouplingMediumFlag" in self._dataset:
            return self._dataset.AcousticCouplingMediumFlag
        return None

    @AcousticCouplingMediumFlag.setter
    def AcousticCouplingMediumFlag(self, value: Optional[str]):
        if value is None:
            if "AcousticCouplingMediumFlag" in self._dataset:
                del self._dataset.AcousticCouplingMediumFlag
        else:
            self._dataset.AcousticCouplingMediumFlag = value

    @property
    def AcousticCouplingMediumCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "AcousticCouplingMediumCodeSequence" in self._dataset:
            if len(self._AcousticCouplingMediumCodeSequence) == len(self._dataset.AcousticCouplingMediumCodeSequence):
                return self._AcousticCouplingMediumCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.AcousticCouplingMediumCodeSequence]
        return None

    @AcousticCouplingMediumCodeSequence.setter
    def AcousticCouplingMediumCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._AcousticCouplingMediumCodeSequence = []
            if "AcousticCouplingMediumCodeSequence" in self._dataset:
                del self._dataset.AcousticCouplingMediumCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"AcousticCouplingMediumCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._AcousticCouplingMediumCodeSequence = value
            if "AcousticCouplingMediumCodeSequence" not in self._dataset:
                self._dataset.AcousticCouplingMediumCodeSequence = pydicom.Sequence()
            self._dataset.AcousticCouplingMediumCodeSequence.clear()
            self._dataset.AcousticCouplingMediumCodeSequence.extend([item.to_dataset() for item in value])

    def add_AcousticCouplingMediumCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._AcousticCouplingMediumCodeSequence.append(item)
        if "AcousticCouplingMediumCodeSequence" not in self._dataset:
            self._dataset.AcousticCouplingMediumCodeSequence = pydicom.Sequence()
        self._dataset.AcousticCouplingMediumCodeSequence.append(item.to_dataset())

    @property
    def AcousticCouplingMediumTemperature(self) -> Optional[float]:
        if "AcousticCouplingMediumTemperature" in self._dataset:
            return self._dataset.AcousticCouplingMediumTemperature
        return None

    @AcousticCouplingMediumTemperature.setter
    def AcousticCouplingMediumTemperature(self, value: Optional[float]):
        if value is None:
            if "AcousticCouplingMediumTemperature" in self._dataset:
                del self._dataset.AcousticCouplingMediumTemperature
        else:
            self._dataset.AcousticCouplingMediumTemperature = value
    ...
    @property
    def SoundSpeedCorrectionMechanismCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "SoundSpeedCorrectionMechanismCodeSequence" in self._dataset:
            if len(self._SoundSpeedCorrectionMechanismCodeSequence) == len(
                self._dataset.SoundSpeedCorrectionMechanismCodeSequence
            ):
                return self._SoundSpeedCorrectionMechanismCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.SoundSpeedCorrectionMechanismCodeSequence]
        return None

    @SoundSpeedCorrectionMechanismCodeSequence.setter
    def SoundSpeedCorrectionMechanismCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._SoundSpeedCorrectionMechanismCodeSequence = []
            if "SoundSpeedCorrectionMechanismCodeSequence" in self._dataset:
                del self._dataset.SoundSpeedCorrectionMechanismCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"SoundSpeedCorrectionMechanismCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._SoundSpeedCorrectionMechanismCodeSequence = value
            if "SoundSpeedCorrectionMechanismCodeSequence" not in self._dataset:
                self._dataset.SoundSpeedCorrectionMechanismCodeSequence = pydicom.Sequence()
            self._dataset.SoundSpeedCorrectionMechanismCodeSequence.clear()
            self._dataset.SoundSpeedCorrectionMechanismCodeSequence.extend([item.to_dataset() for item in value])

    def add_SoundSpeedCorrectionMechanismCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._SoundSpeedCorrectionMechanismCodeSequence.append(item)
        if "SoundSpeedCorrectionMechanismCodeSequence" not in self._dataset:
            self._dataset.SoundSpeedCorrectionMechanismCodeSequence = pydicom.Sequence()
        self._dataset.SoundSpeedCorrectionMechanismCodeSequence.append(item.to_dataset())
