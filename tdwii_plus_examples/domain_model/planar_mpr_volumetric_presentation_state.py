...
class PlanarMprVolumetricPresentationState:
    ...
    @property
    def MultiPlanarReconstructionStyle(self) -> Optional[str]:
        if "MultiPlanarReconstructionStyle" in self._dataset:
            return self._dataset.MultiPlanarReconstructionStyle
        return None

    @MultiPlanarReconstructionStyle.setter
    def MultiPlanarReconstructionStyle(self, value: Optional[str]):
        if value is None:
            if "MultiPlanarReconstructionStyle" in self._dataset:
                del self._dataset.MultiPlanarReconstructionStyle
        else:
            self._dataset.MultiPlanarReconstructionStyle = value

    @property
    def MPRThicknessType(self) -> Optional[str]:
        if "MPRThicknessType" in self._dataset:
            return self._dataset.MPRThicknessType
        return None

    @MPRThicknessType.setter
    def MPRThicknessType(self, value: Optional[str]):
        if value is None:
            if "MPRThicknessType" in self._dataset:
                del self._dataset.MPRThicknessType
        else:
            self._dataset.MPRThicknessType = value

    @property
    def MPRSlabThickness(self) -> Optional[float]:
        if "MPRSlabThickness" in self._dataset:
            return self._dataset.MPRSlabThickness
        return None

    @MPRSlabThickness.setter
    def MPRSlabThickness(self, value: Optional[float]):
        if value is None:
            if "MPRSlabThickness" in self._dataset:
                del self._dataset.MPRSlabThickness
        else:
            self._dataset.MPRSlabThickness = value

    @property
    def MPRTopLeftHandCorner(self) -> Optional[List[float]]:
        if "MPRTopLeftHandCorner" in self._dataset:
            return self._dataset.MPRTopLeftHandCorner
        return None

    @MPRTopLeftHandCorner.setter
    def MPRTopLeftHandCorner(self, value: Optional[List[float]]):
        if value is None:
            if "MPRTopLeftHandCorner" in self._dataset:
                del self._dataset.MPRTopLeftHandCorner
        else:
            self._dataset.MPRTopLeftHandCorner = value

    @property
    def MPRViewWidthDirection(self) -> Optional[List[float]]:
        if "MPRViewWidthDirection" in self._dataset:
            return self._dataset.MPRViewWidthDirection
        return None

    @MPRViewWidthDirection.setter
    def MPRViewWidthDirection(self, value: Optional[List[float]]):
        if value is None:
            if "MPRViewWidthDirection" in self._dataset:
                del self._dataset.MPRViewWidthDirection
        else:
            self._dataset.MPRViewWidthDirection = value

    @property
    def MPRViewWidth(self) -> Optional[float]:
        if "MPRViewWidth" in self._dataset:
            return self._dataset.MPRViewWidth
        return None

    @MPRViewWidth.setter
    def MPRViewWidth(self, value: Optional[float]):
        if value is None:
            if "MPRViewWidth" in self._dataset:
                del self._dataset.MPRViewWidth
        else:
            self._dataset.MPRViewWidth = value

    @property
    def MPRViewHeightDirection(self) -> Optional[List[float]]:
        if "MPRViewHeightDirection" in self._dataset:
            return self._dataset.MPRViewHeightDirection
        return None

    @MPRViewHeightDirection.setter
    def MPRViewHeightDirection(self, value: Optional[List[float]]):
        if value is None:
            if "MPRViewHeightDirection" in self._dataset:
                del self._dataset.MPRViewHeightDirection
        else:
            self._dataset.MPRViewHeightDirection = value

    @property
    def MPRViewHeight(self) -> Optional[float]:
        if "MPRViewHeight" in self._dataset:
            return self._dataset.MPRViewHeight
        return None

    @MPRViewHeight.setter
    def MPRViewHeight(self, value: Optional[float]):
        if value is None:
            if "MPRViewHeight" in self._dataset:
                del self._dataset.MPRViewHeight
        else:
            self._dataset.MPRViewHeight = value
