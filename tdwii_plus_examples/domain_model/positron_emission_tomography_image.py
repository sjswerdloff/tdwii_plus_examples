...
class PositronEmissionTomographyImage:
    ...
    @property
    def AcquisitionStartConditionData(self) -> Optional[int]:
        if "AcquisitionStartConditionData" in self._dataset:
            return self._dataset.AcquisitionStartConditionData
        return None

    @AcquisitionStartConditionData.setter
    def AcquisitionStartConditionData(self, value: Optional[int]):
        if value is None:
            if "AcquisitionStartConditionData" in self._dataset:
                del self._dataset.AcquisitionStartConditionData
        else:
            self._dataset.AcquisitionStartConditionData = value

    @property
    def AcquisitionTerminationConditionData(self) -> Optional[int]:
        if "AcquisitionTerminationConditionData" in self._dataset:
            return self._dataset.AcquisitionTerminationConditionData
        return None

    @AcquisitionTerminationConditionData.setter
    def AcquisitionTerminationConditionData(self, value: Optional[int]):
        if value is None:
            if "AcquisitionTerminationConditionData" in self._dataset:
                del self._dataset.AcquisitionTerminationConditionData
        else:
            self._dataset.AcquisitionTerminationConditionData = value
    ...
    @property
    def NumberOfTimeSlices(self) -> Optional[int]:
        if "NumberOfTimeSlices" in self._dataset:
            return self._dataset.NumberOfTimeSlices
        return None

    @NumberOfTimeSlices.setter
    def NumberOfTimeSlices(self, value: Optional[int]):
        if value is None:
            if "NumberOfTimeSlices" in self._dataset:
                del self._dataset.NumberOfTimeSlices
        else:
            self._dataset.NumberOfTimeSlices = value
    ...
    @property
    def SeriesType(self) -> Optional[List[str]]:
        if "SeriesType" in self._dataset:
            return self._dataset.SeriesType
        return None

    @SeriesType.setter
    def SeriesType(self, value: Optional[List[str]]):
        if value is None:
            if "SeriesType" in self._dataset:
                del self._dataset.SeriesType
        else:
            self._dataset.SeriesType = value

    @property
    def Units(self) -> Optional[str]:
        if "Units" in self._dataset:
    ...
    def Units(self, value: Optional[str]):
        if value is None:
            if "Units" in self._dataset:
                del self._dataset.Units
        else:
            self._dataset.Units = value
    ...
    @property
    def ReprojectionMethod(self) -> Optional[str]:
        if "ReprojectionMethod" in self._dataset:
            return self._dataset.ReprojectionMethod
        return None

    @ReprojectionMethod.setter
    def ReprojectionMethod(self, value: Optional[str]):
        if value is None:
            if "ReprojectionMethod" in self._dataset:
                del self._dataset.ReprojectionMethod
        else:
            self._dataset.ReprojectionMethod = value

    @property
    def SUVType(self) -> Optional[str]:
        if "SUVType" in self._dataset:
            return self._dataset.SUVType
        return None

    @SUVType.setter
    def SUVType(self, value: Optional[str]):
        if value is None:
            if "SUVType" in self._dataset:
                del self._dataset.SUVType
        else:
            self._dataset.SUVType = value
    ...
    @property
    def AttenuationCorrectionMethod(self) -> Optional[str]:
        if "AttenuationCorrectionMethod" in self._dataset:
            return self._dataset.AttenuationCorrectionMethod
        return None

    @AttenuationCorrectionMethod.setter
    def AttenuationCorrectionMethod(self, value: Optional[str]):
        if value is None:
            if "AttenuationCorrectionMethod" in self._dataset:
                del self._dataset.AttenuationCorrectionMethod
        else:
            self._dataset.AttenuationCorrectionMethod = value

    @property
    def DecayCorrection(self) -> Optional[str]:
        if "DecayCorrection" in self._dataset:
            return self._dataset.DecayCorrection
        return None

    @DecayCorrection.setter
    def DecayCorrection(self, value: Optional[str]):
        if value is None:
            if "DecayCorrection" in self._dataset:
                del self._dataset.DecayCorrection
        else:
            self._dataset.DecayCorrection = value

    @property
    def ReconstructionMethod(self) -> Optional[str]:
        if "ReconstructionMethod" in self._dataset:
            return self._dataset.ReconstructionMethod
        return None

    @ReconstructionMethod.setter
    def ReconstructionMethod(self, value: Optional[str]):
        if value is None:
            if "ReconstructionMethod" in self._dataset:
                del self._dataset.ReconstructionMethod
        else:
            self._dataset.ReconstructionMethod = value

    @property
    def DetectorLinesOfResponseUsed(self) -> Optional[str]:
        if "DetectorLinesOfResponseUsed" in self._dataset:
            return self._dataset.DetectorLinesOfResponseUsed
        return None

    @DetectorLinesOfResponseUsed.setter
    def DetectorLinesOfResponseUsed(self, value: Optional[str]):
        if value is None:
            if "DetectorLinesOfResponseUsed" in self._dataset:
                del self._dataset.DetectorLinesOfResponseUsed
        else:
            self._dataset.DetectorLinesOfResponseUsed = value
    ...
    @property
    def AxialAcceptance(self) -> Optional[Decimal]:
        if "AxialAcceptance" in self._dataset:
            return self._dataset.AxialAcceptance
        return None

    @AxialAcceptance.setter
    def AxialAcceptance(self, value: Optional[Decimal]):
        if value is None:
            if "AxialAcceptance" in self._dataset:
                del self._dataset.AxialAcceptance
        else:
            self._dataset.AxialAcceptance = value

    @property
    def AxialMash(self) -> Optional[List[int]]:
        if "AxialMash" in self._dataset:
            return self._dataset.AxialMash
        return None

    @AxialMash.setter
    def AxialMash(self, value: Optional[List[int]]):
        if value is None:
            if "AxialMash" in self._dataset:
                del self._dataset.AxialMash
        else:
            self._dataset.AxialMash = value

    @property
    def TransverseMash(self) -> Optional[int]:
        if "TransverseMash" in self._dataset:
            return self._dataset.TransverseMash
        return None

    @TransverseMash.setter
    def TransverseMash(self, value: Optional[int]):
        if value is None:
            if "TransverseMash" in self._dataset:
                del self._dataset.TransverseMash
        else:
            self._dataset.TransverseMash = value

    @property
    def DetectorElementSize(self) -> Optional[List[Decimal]]:
        if "DetectorElementSize" in self._dataset:
            return self._dataset.DetectorElementSize
        return None

    @DetectorElementSize.setter
    def DetectorElementSize(self, value: Optional[List[Decimal]]):
        if value is None:
            if "DetectorElementSize" in self._dataset:
                del self._dataset.DetectorElementSize
        else:
            self._dataset.DetectorElementSize = value
    ...
    @property
    def SecondaryCountsType(self) -> Optional[List[str]]:
        if "SecondaryCountsType" in self._dataset:
            return self._dataset.SecondaryCountsType
        return None

    @SecondaryCountsType.setter
    def SecondaryCountsType(self, value: Optional[List[str]]):
        if value is None:
            if "SecondaryCountsType" in self._dataset:
                del self._dataset.SecondaryCountsType
        else:
            self._dataset.SecondaryCountsType = value
    ...
    @property
    def FrameReferenceTime(self) -> Optional[Decimal]:
        if "FrameReferenceTime" in self._dataset:
            return self._dataset.FrameReferenceTime
        return None

    @FrameReferenceTime.setter
    def FrameReferenceTime(self, value: Optional[Decimal]):
        if value is None:
            if "FrameReferenceTime" in self._dataset:
                del self._dataset.FrameReferenceTime
        else:
            self._dataset.FrameReferenceTime = value
    ...
    @property
    def SecondaryCountsAccumulated(self) -> Optional[List[int]]:
        if "SecondaryCountsAccumulated" in self._dataset:
            return self._dataset.SecondaryCountsAccumulated
        return None

    @SecondaryCountsAccumulated.setter
    def SecondaryCountsAccumulated(self, value: Optional[List[int]]):
        if value is None:
            if "SecondaryCountsAccumulated" in self._dataset:
                del self._dataset.SecondaryCountsAccumulated
        else:
            self._dataset.SecondaryCountsAccumulated = value
    ...
    @property
    def DoseCalibrationFactor(self) -> Optional[Decimal]:
        if "DoseCalibrationFactor" in self._dataset:
            return self._dataset.DoseCalibrationFactor
        return None

    @DoseCalibrationFactor.setter
    def DoseCalibrationFactor(self, value: Optional[Decimal]):
        if value is None:
            if "DoseCalibrationFactor" in self._dataset:
                del self._dataset.DoseCalibrationFactor
        else:
            self._dataset.DoseCalibrationFactor = value
    ...
    @property
    def ImageIndex(self) -> Optional[int]:
        if "ImageIndex" in self._dataset:
            return self._dataset.ImageIndex
        return None

    @ImageIndex.setter
    def ImageIndex(self, value: Optional[int]):
        if value is None:
            if "ImageIndex" in self._dataset:
                del self._dataset.ImageIndex
        else:
            self._dataset.ImageIndex = value
