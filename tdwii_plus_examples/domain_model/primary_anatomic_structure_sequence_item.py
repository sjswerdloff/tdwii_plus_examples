...
from .primary_anatomic_structure_modifier_sequence_item import (
    PrimaryAnatomicStructureModifierSequenceItem,
)


class PrimaryAnatomicStructureSequenceItem:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        self._dataset = dataset if dataset is not None else pydicom.Dataset()
        self._EquivalentCodeSequence: List[CodeSequenceItem] = []
        self._PrimaryAnatomicStructureModifierSequence: List[PrimaryAnatomicStructureModifierSequenceItem] = []
    ...
    @property
    def PrimaryAnatomicStructureModifierSequence(self) -> Optional[List[PrimaryAnatomicStructureModifierSequenceItem]]:
        if "PrimaryAnatomicStructureModifierSequence" in self._dataset:
            if len(self._PrimaryAnatomicStructureModifierSequence) == len(
                self._dataset.PrimaryAnatomicStructureModifierSequence
            ):
                return self._PrimaryAnatomicStructureModifierSequence
            else:
                return [
                    PrimaryAnatomicStructureModifierSequenceItem(x)
                    for x in self._dataset.PrimaryAnatomicStructureModifierSequence
                ]
        return None

    @PrimaryAnatomicStructureModifierSequence.setter
    def PrimaryAnatomicStructureModifierSequence(self, value: Optional[List[PrimaryAnatomicStructureModifierSequenceItem]]):
        if value is None:
            self._PrimaryAnatomicStructureModifierSequence = []
            if "PrimaryAnatomicStructureModifierSequence" in self._dataset:
                del self._dataset.PrimaryAnatomicStructureModifierSequence
            ...
            self._dataset.PrimaryAnatomicStructureModifierSequence.clear()
            self._dataset.PrimaryAnatomicStructureModifierSequence.extend([item.to_dataset() for item in value])
    ...
    def add_PrimaryAnatomicStructureModifier(self, item: PrimaryAnatomicStructureModifierSequenceItem):
        ...
        if "PrimaryAnatomicStructureModifierSequence" not in self._dataset:
            self._dataset.PrimaryAnatomicStructureModifierSequence = pydicom.Sequence()
        self._dataset.PrimaryAnatomicStructureModifierSequence.append(item.to_dataset())
