...
class RadiationDoseIdentificationSequenceItem:
    ...
    @property
    def ReferenceDoseType(self) -> Optional[str]:
        if "ReferenceDoseType" in self._dataset:
            return self._dataset.ReferenceDoseType
        return None

    @ReferenceDoseType.setter
    def ReferenceDoseType(self, value: Optional[str]):
        if value is None:
            if "ReferenceDoseType" in self._dataset:
                del self._dataset.ReferenceDoseType
        else:
            self._dataset.ReferenceDoseType = value

    @property
    def ReferenceDosePointCoordinates(self) -> Optional[List[float]]:
        if "ReferenceDosePointCoordinates" in self._dataset:
            return self._dataset.ReferenceDosePointCoordinates
        return None

    @ReferenceDosePointCoordinates.setter
    def ReferenceDosePointCoordinates(self, value: Optional[List[float]]):
        if value is None:
            if "ReferenceDosePointCoordinates" in self._dataset:
                del self._dataset.ReferenceDosePointCoordinates
        else:
            self._dataset.ReferenceDosePointCoordinates = value
