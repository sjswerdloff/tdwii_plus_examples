...
from .current_frame_functional_groups_sequence_item import (
    CurrentFrameFunctionalGroupsSequenceItem,
)
...
from .real_time_bulk_data_flow_sequence_item import RealTimeBulkDataFlowSequenceItem
...
class RealTimeVideoPhotographicImage:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._RealTimeBulkDataFlowSequence: List[RealTimeBulkDataFlowSequenceItem] = []
    ...
    @property
    def CurrentFrameFunctionalGroupsSequence(self) -> Optional[List[CurrentFrameFunctionalGroupsSequenceItem]]:
        if "CurrentFrameFunctionalGroupsSequence" in self._dataset:
            if len(self._CurrentFrameFunctionalGroupsSequence) == len(self._dataset.CurrentFrameFunctionalGroupsSequence):
                return self._CurrentFrameFunctionalGroupsSequence
            else:
                return [
                    CurrentFrameFunctionalGroupsSequenceItem(x) for x in self._dataset.CurrentFrameFunctionalGroupsSequence
                ]
        return None

    @CurrentFrameFunctionalGroupsSequence.setter
    def CurrentFrameFunctionalGroupsSequence(self, value: Optional[List[CurrentFrameFunctionalGroupsSequenceItem]]):
        if value is None:
            self._CurrentFrameFunctionalGroupsSequence = []
            if "CurrentFrameFunctionalGroupsSequence" in self._dataset:
                del self._dataset.CurrentFrameFunctionalGroupsSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, CurrentFrameFunctionalGroupsSequenceItem) for item in value
        ):
            raise ValueError(
                f"CurrentFrameFunctionalGroupsSequence must be a list of CurrentFrameFunctionalGroupsSequenceItem objects"
            )
        else:
            self._CurrentFrameFunctionalGroupsSequence = value
            if "CurrentFrameFunctionalGroupsSequence" not in self._dataset:
                self._dataset.CurrentFrameFunctionalGroupsSequence = pydicom.Sequence()
            self._dataset.CurrentFrameFunctionalGroupsSequence.clear()
            self._dataset.CurrentFrameFunctionalGroupsSequence.extend([item.to_dataset() for item in value])

    def add_CurrentFrameFunctionalGroups(self, item: CurrentFrameFunctionalGroupsSequenceItem):
        if not isinstance(item, CurrentFrameFunctionalGroupsSequenceItem):
            raise ValueError(f"Item must be an instance of CurrentFrameFunctionalGroupsSequenceItem")
        self._CurrentFrameFunctionalGroupsSequence.append(item)
        if "CurrentFrameFunctionalGroupsSequence" not in self._dataset:
            self._dataset.CurrentFrameFunctionalGroupsSequence = pydicom.Sequence()
        self._dataset.CurrentFrameFunctionalGroupsSequence.append(item.to_dataset())
    ...
    @property
    def RealTimeBulkDataFlowSequence(self) -> Optional[List[RealTimeBulkDataFlowSequenceItem]]:
        if "RealTimeBulkDataFlowSequence" in self._dataset:
            if len(self._RealTimeBulkDataFlowSequence) == len(self._dataset.RealTimeBulkDataFlowSequence):
                return self._RealTimeBulkDataFlowSequence
            else:
                return [RealTimeBulkDataFlowSequenceItem(x) for x in self._dataset.RealTimeBulkDataFlowSequence]
        return None

    @RealTimeBulkDataFlowSequence.setter
    def RealTimeBulkDataFlowSequence(self, value: Optional[List[RealTimeBulkDataFlowSequenceItem]]):
        if value is None:
            self._RealTimeBulkDataFlowSequence = []
            if "RealTimeBulkDataFlowSequence" in self._dataset:
                del self._dataset.RealTimeBulkDataFlowSequence
        elif not isinstance(value, list) or not all(isinstance(item, RealTimeBulkDataFlowSequenceItem) for item in value):
            raise ValueError(f"RealTimeBulkDataFlowSequence must be a list of RealTimeBulkDataFlowSequenceItem objects")
        else:
            self._RealTimeBulkDataFlowSequence = value
            if "RealTimeBulkDataFlowSequence" not in self._dataset:
                self._dataset.RealTimeBulkDataFlowSequence = pydicom.Sequence()
            self._dataset.RealTimeBulkDataFlowSequence.clear()
            self._dataset.RealTimeBulkDataFlowSequence.extend([item.to_dataset() for item in value])

    def add_RealTimeBulkDataFlow(self, item: RealTimeBulkDataFlowSequenceItem):
        if not isinstance(item, RealTimeBulkDataFlowSequenceItem):
            raise ValueError(f"Item must be an instance of RealTimeBulkDataFlowSequenceItem")
        self._RealTimeBulkDataFlowSequence.append(item)
        if "RealTimeBulkDataFlowSequence" not in self._dataset:
            self._dataset.RealTimeBulkDataFlowSequence = pydicom.Sequence()
        self._dataset.RealTimeBulkDataFlowSequence.append(item.to_dataset())
