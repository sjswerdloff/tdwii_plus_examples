...
from .referenced_image_real_world_value_mapping_sequence_item import (
...
class RealWorldValueMapping:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._ReferencedImageRealWorldValueMappingSequence: List[ReferencedImageRealWorldValueMappingSequenceItem] = []
    ...
    @property
    def ReferencedImageRealWorldValueMappingSequence(self) -> Optional[List[ReferencedImageRealWorldValueMappingSequenceItem]]:
        if "ReferencedImageRealWorldValueMappingSequence" in self._dataset:
            if len(self._ReferencedImageRealWorldValueMappingSequence) == len(
                self._dataset.ReferencedImageRealWorldValueMappingSequence
            ):
                return self._ReferencedImageRealWorldValueMappingSequence
            else:
                return [
                    ReferencedImageRealWorldValueMappingSequenceItem(x)
                    for x in self._dataset.ReferencedImageRealWorldValueMappingSequence
                ]
        return None

    @ReferencedImageRealWorldValueMappingSequence.setter
    def ReferencedImageRealWorldValueMappingSequence(
        self, value: Optional[List[ReferencedImageRealWorldValueMappingSequenceItem]]
    ):
        if value is None:
            self._ReferencedImageRealWorldValueMappingSequence = []
            if "ReferencedImageRealWorldValueMappingSequence" in self._dataset:
                del self._dataset.ReferencedImageRealWorldValueMappingSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, ReferencedImageRealWorldValueMappingSequenceItem) for item in value
        ):
            raise ValueError(
                f"ReferencedImageRealWorldValueMappingSequence must be a list of ReferencedImageRealWorldValueMappingSequenceItem objects"
            )
        else:
            self._ReferencedImageRealWorldValueMappingSequence = value
            if "ReferencedImageRealWorldValueMappingSequence" not in self._dataset:
                self._dataset.ReferencedImageRealWorldValueMappingSequence = pydicom.Sequence()
            self._dataset.ReferencedImageRealWorldValueMappingSequence.clear()
            self._dataset.ReferencedImageRealWorldValueMappingSequence.extend([item.to_dataset() for item in value])

    def add_ReferencedImageRealWorldValueMapping(self, item: ReferencedImageRealWorldValueMappingSequenceItem):
        if not isinstance(item, ReferencedImageRealWorldValueMappingSequenceItem):
            raise ValueError(f"Item must be an instance of ReferencedImageRealWorldValueMappingSequenceItem")
        self._ReferencedImageRealWorldValueMappingSequence.append(item)
        if "ReferencedImageRealWorldValueMappingSequence" not in self._dataset:
            self._dataset.ReferencedImageRealWorldValueMappingSequence = pydicom.Sequence()
        self._dataset.ReferencedImageRealWorldValueMappingSequence.append(item.to_dataset())
