...
class ReferencedRTPlanSequenceItem:
    ...
    @property
    def RTPlanRelationship(self) -> Optional[str]:
        if "RTPlanRelationship" in self._dataset:
            return self._dataset.RTPlanRelationship
        return None

    @RTPlanRelationship.setter
    def RTPlanRelationship(self, value: Optional[str]):
        if value is None:
            if "RTPlanRelationship" in self._dataset:
                del self._dataset.RTPlanRelationship
        else:
            self._dataset.RTPlanRelationship = value
