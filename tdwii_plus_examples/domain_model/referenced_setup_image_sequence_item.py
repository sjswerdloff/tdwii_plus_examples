...
class ReferencedSetupImageSequenceItem:
    ...
    @property
    def SetupImageComment(self) -> Optional[str]:
        if "SetupImageComment" in self._dataset:
            return self._dataset.SetupImageComment
        return None

    @SetupImageComment.setter
    def SetupImageComment(self, value: Optional[str]):
        if value is None:
            if "SetupImageComment" in self._dataset:
                del self._dataset.SetupImageComment
        else:
            self._dataset.SetupImageComment = value
