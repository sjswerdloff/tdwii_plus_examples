...
from .robotic_path_control_point_sequence_item import (
    RoboticPathControlPointSequenceItem,
)
...
class RoboticArmRadiationRecord:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._RoboticPathNodeSetCodeSequence: List[CodeSequenceItem] = []
        self._RoboticPathControlPointSequence: List[RoboticPathControlPointSequenceItem] = []
    ...
    @property
    def RoboticPathNodeSetCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "RoboticPathNodeSetCodeSequence" in self._dataset:
            if len(self._RoboticPathNodeSetCodeSequence) == len(self._dataset.RoboticPathNodeSetCodeSequence):
                return self._RoboticPathNodeSetCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.RoboticPathNodeSetCodeSequence]
        return None

    @RoboticPathNodeSetCodeSequence.setter
    def RoboticPathNodeSetCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._RoboticPathNodeSetCodeSequence = []
            if "RoboticPathNodeSetCodeSequence" in self._dataset:
                del self._dataset.RoboticPathNodeSetCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"RoboticPathNodeSetCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._RoboticPathNodeSetCodeSequence = value
            if "RoboticPathNodeSetCodeSequence" not in self._dataset:
                self._dataset.RoboticPathNodeSetCodeSequence = pydicom.Sequence()
            self._dataset.RoboticPathNodeSetCodeSequence.clear()
            self._dataset.RoboticPathNodeSetCodeSequence.extend([item.to_dataset() for item in value])

    def add_RoboticPathNodeSetCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._RoboticPathNodeSetCodeSequence.append(item)
        if "RoboticPathNodeSetCodeSequence" not in self._dataset:
            self._dataset.RoboticPathNodeSetCodeSequence = pydicom.Sequence()
        self._dataset.RoboticPathNodeSetCodeSequence.append(item.to_dataset())

    @property
    def RoboticPathControlPointSequence(self) -> Optional[List[RoboticPathControlPointSequenceItem]]:
        if "RoboticPathControlPointSequence" in self._dataset:
            if len(self._RoboticPathControlPointSequence) == len(self._dataset.RoboticPathControlPointSequence):
                return self._RoboticPathControlPointSequence
            else:
                return [RoboticPathControlPointSequenceItem(x) for x in self._dataset.RoboticPathControlPointSequence]
        return None

    @RoboticPathControlPointSequence.setter
    def RoboticPathControlPointSequence(self, value: Optional[List[RoboticPathControlPointSequenceItem]]):
        if value is None:
            self._RoboticPathControlPointSequence = []
            if "RoboticPathControlPointSequence" in self._dataset:
                del self._dataset.RoboticPathControlPointSequence
        elif not isinstance(value, list) or not all(isinstance(item, RoboticPathControlPointSequenceItem) for item in value):
            raise ValueError(f"RoboticPathControlPointSequence must be a list of RoboticPathControlPointSequenceItem objects")
        else:
            self._RoboticPathControlPointSequence = value
            if "RoboticPathControlPointSequence" not in self._dataset:
                self._dataset.RoboticPathControlPointSequence = pydicom.Sequence()
            self._dataset.RoboticPathControlPointSequence.clear()
            self._dataset.RoboticPathControlPointSequence.extend([item.to_dataset() for item in value])

    def add_RoboticPathControlPoint(self, item: RoboticPathControlPointSequenceItem):
        if not isinstance(item, RoboticPathControlPointSequenceItem):
            raise ValueError(f"Item must be an instance of RoboticPathControlPointSequenceItem")
        self._RoboticPathControlPointSequence.append(item)
        if "RoboticPathControlPointSequence" not in self._dataset:
            self._dataset.RoboticPathControlPointSequence = pydicom.Sequence()
        self._dataset.RoboticPathControlPointSequence.append(item.to_dataset())
