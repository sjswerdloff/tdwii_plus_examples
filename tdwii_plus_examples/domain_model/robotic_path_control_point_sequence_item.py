...
class RoboticPathControlPointSequenceItem:
    ...
    @property
    def RoboticNodeIdentifier(self) -> Optional[int]:
        if "RoboticNodeIdentifier" in self._dataset:
            return self._dataset.RoboticNodeIdentifier
        return None

    @RoboticNodeIdentifier.setter
    def RoboticNodeIdentifier(self, value: Optional[int]):
        if value is None:
            if "RoboticNodeIdentifier" in self._dataset:
                del self._dataset.RoboticNodeIdentifier
        else:
            self._dataset.RoboticNodeIdentifier = value

    @property
    def RTTreatmentSourceCoordinates(self) -> Optional[List[float]]:
        if "RTTreatmentSourceCoordinates" in self._dataset:
            return self._dataset.RTTreatmentSourceCoordinates
        return None

    @RTTreatmentSourceCoordinates.setter
    def RTTreatmentSourceCoordinates(self, value: Optional[List[float]]):
        if value is None:
            if "RTTreatmentSourceCoordinates" in self._dataset:
                del self._dataset.RTTreatmentSourceCoordinates
        else:
            self._dataset.RTTreatmentSourceCoordinates = value

    @property
    def RadiationSourceCoordinateSystemYawAngle(self) -> Optional[float]:
        if "RadiationSourceCoordinateSystemYawAngle" in self._dataset:
            return self._dataset.RadiationSourceCoordinateSystemYawAngle
        return None

    @RadiationSourceCoordinateSystemYawAngle.setter
    def RadiationSourceCoordinateSystemYawAngle(self, value: Optional[float]):
        if value is None:
            if "RadiationSourceCoordinateSystemYawAngle" in self._dataset:
                del self._dataset.RadiationSourceCoordinateSystemYawAngle
        else:
            self._dataset.RadiationSourceCoordinateSystemYawAngle = value

    @property
    def RadiationSourceCoordinateSystemRollAngle(self) -> Optional[float]:
        if "RadiationSourceCoordinateSystemRollAngle" in self._dataset:
            return self._dataset.RadiationSourceCoordinateSystemRollAngle
        return None

    @RadiationSourceCoordinateSystemRollAngle.setter
    def RadiationSourceCoordinateSystemRollAngle(self, value: Optional[float]):
        if value is None:
            if "RadiationSourceCoordinateSystemRollAngle" in self._dataset:
                del self._dataset.RadiationSourceCoordinateSystemRollAngle
        else:
            self._dataset.RadiationSourceCoordinateSystemRollAngle = value

    @property
    def RadiationSourceCoordinateSystemPitchAngle(self) -> Optional[float]:
        if "RadiationSourceCoordinateSystemPitchAngle" in self._dataset:
            return self._dataset.RadiationSourceCoordinateSystemPitchAngle
        return None

    @RadiationSourceCoordinateSystemPitchAngle.setter
    def RadiationSourceCoordinateSystemPitchAngle(self, value: Optional[float]):
        if value is None:
            if "RadiationSourceCoordinateSystemPitchAngle" in self._dataset:
                del self._dataset.RadiationSourceCoordinateSystemPitchAngle
        else:
            self._dataset.RadiationSourceCoordinateSystemPitchAngle = value
