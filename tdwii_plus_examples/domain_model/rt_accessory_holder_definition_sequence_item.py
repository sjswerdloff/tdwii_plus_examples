...
from .rt_accessory_holder_slot_sequence_item import RTAccessoryHolderSlotSequenceItem
from .udi_sequence_item import UDISequenceItem


class RTAccessoryHolderDefinitionSequenceItem:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        self._dataset = dataset if dataset is not None else pydicom.Dataset()
        self._UDISequence: List[UDISequenceItem] = []
        self._RTAccessoryHolderSlotSequence: List[RTAccessoryHolderSlotSequenceItem] = []
    ...
    @property
    def RTAccessoryHolderWaterEquivalentThickness(self) -> Optional[float]:
        if "RTAccessoryHolderWaterEquivalentThickness" in self._dataset:
            return self._dataset.RTAccessoryHolderWaterEquivalentThickness
        return None

    @RTAccessoryHolderWaterEquivalentThickness.setter
    def RTAccessoryHolderWaterEquivalentThickness(self, value: Optional[float]):
        if value is None:
            if "RTAccessoryHolderWaterEquivalentThickness" in self._dataset:
                del self._dataset.RTAccessoryHolderWaterEquivalentThickness
        else:
            self._dataset.RTAccessoryHolderWaterEquivalentThickness = value
    ...
    @property
    def RTAccessoryHolderSlotExistenceFlag(self) -> Optional[str]:
        if "RTAccessoryHolderSlotExistenceFlag" in self._dataset:
            return self._dataset.RTAccessoryHolderSlotExistenceFlag
        return None

    @RTAccessoryHolderSlotExistenceFlag.setter
    def RTAccessoryHolderSlotExistenceFlag(self, value: Optional[str]):
        if value is None:
            if "RTAccessoryHolderSlotExistenceFlag" in self._dataset:
                del self._dataset.RTAccessoryHolderSlotExistenceFlag
        else:
            self._dataset.RTAccessoryHolderSlotExistenceFlag = value

    @property
    def RTAccessoryHolderSlotSequence(self) -> Optional[List[RTAccessoryHolderSlotSequenceItem]]:
        if "RTAccessoryHolderSlotSequence" in self._dataset:
            if len(self._RTAccessoryHolderSlotSequence) == len(self._dataset.RTAccessoryHolderSlotSequence):
                return self._RTAccessoryHolderSlotSequence
            else:
                return [RTAccessoryHolderSlotSequenceItem(x) for x in self._dataset.RTAccessoryHolderSlotSequence]
        return None

    @RTAccessoryHolderSlotSequence.setter
    def RTAccessoryHolderSlotSequence(self, value: Optional[List[RTAccessoryHolderSlotSequenceItem]]):
        if value is None:
            self._RTAccessoryHolderSlotSequence = []
            if "RTAccessoryHolderSlotSequence" in self._dataset:
            ...
            self._dataset.RTAccessoryHolderSlotSequence.clear()
            self._dataset.RTAccessoryHolderSlotSequence.extend([item.to_dataset() for item in value])
    ...
    def add_RTAccessoryHolderSlot(self, item: RTAccessoryHolderSlotSequenceItem):
        ...
        if "RTAccessoryHolderSlotSequence" not in self._dataset:
            self._dataset.RTAccessoryHolderSlotSequence = pydicom.Sequence()
        self._dataset.RTAccessoryHolderSlotSequence.append(item.to_dataset())
