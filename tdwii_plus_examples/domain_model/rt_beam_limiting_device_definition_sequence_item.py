...
from .fixed_rt_beam_delimiter_device_sequence_item import (
    FixedRTBeamDelimiterDeviceSequenceItem,
)
from .parallel_rt_beam_delimiter_device_sequence_item import (
    ParallelRTBeamDelimiterDeviceSequenceItem,
)
from .udi_sequence_item import UDISequenceItem


class RTBeamLimitingDeviceDefinitionSequenceItem:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        self._dataset = dataset if dataset is not None else pydicom.Dataset()
        self._UDISequence: List[UDISequenceItem] = []
        self._FixedRTBeamDelimiterDeviceSequence: List[FixedRTBeamDelimiterDeviceSequenceItem] = []
        self._ParallelRTBeamDelimiterDeviceSequence: List[ParallelRTBeamDelimiterDeviceSequenceItem] = []
    ...
    @property
    def RTBeamLimitingDeviceProximalDistance(self) -> Optional[float]:
        if "RTBeamLimitingDeviceProximalDistance" in self._dataset:
            return self._dataset.RTBeamLimitingDeviceProximalDistance
        return None

    @RTBeamLimitingDeviceProximalDistance.setter
    def RTBeamLimitingDeviceProximalDistance(self, value: Optional[float]):
        if value is None:
            if "RTBeamLimitingDeviceProximalDistance" in self._dataset:
                del self._dataset.RTBeamLimitingDeviceProximalDistance
        else:
            self._dataset.RTBeamLimitingDeviceProximalDistance = value

    @property
    def RTBeamLimitingDeviceDistalDistance(self) -> Optional[float]:
        if "RTBeamLimitingDeviceDistalDistance" in self._dataset:
            return self._dataset.RTBeamLimitingDeviceDistalDistance
        return None

    @RTBeamLimitingDeviceDistalDistance.setter
    def RTBeamLimitingDeviceDistalDistance(self, value: Optional[float]):
        if value is None:
            if "RTBeamLimitingDeviceDistalDistance" in self._dataset:
                del self._dataset.RTBeamLimitingDeviceDistalDistance
        else:
            self._dataset.RTBeamLimitingDeviceDistalDistance = value
    ...
    @property
    def FixedRTBeamDelimiterDeviceSequence(self) -> Optional[List[FixedRTBeamDelimiterDeviceSequenceItem]]:
        if "FixedRTBeamDelimiterDeviceSequence" in self._dataset:
            if len(self._FixedRTBeamDelimiterDeviceSequence) == len(self._dataset.FixedRTBeamDelimiterDeviceSequence):
                return self._FixedRTBeamDelimiterDeviceSequence
            else:
                return [FixedRTBeamDelimiterDeviceSequenceItem(x) for x in self._dataset.FixedRTBeamDelimiterDeviceSequence]
        return None

    @FixedRTBeamDelimiterDeviceSequence.setter
    def FixedRTBeamDelimiterDeviceSequence(self, value: Optional[List[FixedRTBeamDelimiterDeviceSequenceItem]]):
        if value is None:
            self._FixedRTBeamDelimiterDeviceSequence = []
            if "FixedRTBeamDelimiterDeviceSequence" in self._dataset:
                del self._dataset.FixedRTBeamDelimiterDeviceSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, FixedRTBeamDelimiterDeviceSequenceItem) for item in value
        ):
            raise ValueError(
                f"FixedRTBeamDelimiterDeviceSequence must be a list of FixedRTBeamDelimiterDeviceSequenceItem objects"
            )
        else:
            self._FixedRTBeamDelimiterDeviceSequence = value
            if "FixedRTBeamDelimiterDeviceSequence" not in self._dataset:
                self._dataset.FixedRTBeamDelimiterDeviceSequence = pydicom.Sequence()
            self._dataset.FixedRTBeamDelimiterDeviceSequence.clear()
            self._dataset.FixedRTBeamDelimiterDeviceSequence.extend([item.to_dataset() for item in value])

    def add_FixedRTBeamDelimiterDevice(self, item: FixedRTBeamDelimiterDeviceSequenceItem):
        if not isinstance(item, FixedRTBeamDelimiterDeviceSequenceItem):
            raise ValueError(f"Item must be an instance of FixedRTBeamDelimiterDeviceSequenceItem")
        self._FixedRTBeamDelimiterDeviceSequence.append(item)
        if "FixedRTBeamDelimiterDeviceSequence" not in self._dataset:
            self._dataset.FixedRTBeamDelimiterDeviceSequence = pydicom.Sequence()
        self._dataset.FixedRTBeamDelimiterDeviceSequence.append(item.to_dataset())

    @property
    def ParallelRTBeamDelimiterDeviceSequence(self) -> Optional[List[ParallelRTBeamDelimiterDeviceSequenceItem]]:
        if "ParallelRTBeamDelimiterDeviceSequence" in self._dataset:
            if len(self._ParallelRTBeamDelimiterDeviceSequence) == len(self._dataset.ParallelRTBeamDelimiterDeviceSequence):
                return self._ParallelRTBeamDelimiterDeviceSequence
            else:
                return [
                    ParallelRTBeamDelimiterDeviceSequenceItem(x) for x in self._dataset.ParallelRTBeamDelimiterDeviceSequence
                ]
        return None

    @ParallelRTBeamDelimiterDeviceSequence.setter
    def ParallelRTBeamDelimiterDeviceSequence(self, value: Optional[List[ParallelRTBeamDelimiterDeviceSequenceItem]]):
        if value is None:
            self._ParallelRTBeamDelimiterDeviceSequence = []
            if "ParallelRTBeamDelimiterDeviceSequence" in self._dataset:
                del self._dataset.ParallelRTBeamDelimiterDeviceSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, ParallelRTBeamDelimiterDeviceSequenceItem) for item in value
        ):
            raise ValueError(
                f"ParallelRTBeamDelimiterDeviceSequence must be a list of ParallelRTBeamDelimiterDeviceSequenceItem objects"
            )
        else:
            self._ParallelRTBeamDelimiterDeviceSequence = value
            if "ParallelRTBeamDelimiterDeviceSequence" not in self._dataset:
                self._dataset.ParallelRTBeamDelimiterDeviceSequence = pydicom.Sequence()
            self._dataset.ParallelRTBeamDelimiterDeviceSequence.clear()
            self._dataset.ParallelRTBeamDelimiterDeviceSequence.extend([item.to_dataset() for item in value])

    def add_ParallelRTBeamDelimiterDevice(self, item: ParallelRTBeamDelimiterDeviceSequenceItem):
        if not isinstance(item, ParallelRTBeamDelimiterDeviceSequenceItem):
            raise ValueError(f"Item must be an instance of ParallelRTBeamDelimiterDeviceSequenceItem")
        self._ParallelRTBeamDelimiterDeviceSequence.append(item)
        if "ParallelRTBeamDelimiterDeviceSequence" not in self._dataset:
            self._dataset.ParallelRTBeamDelimiterDeviceSequence = pydicom.Sequence()
        self._dataset.ParallelRTBeamDelimiterDeviceSequence.append(item.to_dataset())
