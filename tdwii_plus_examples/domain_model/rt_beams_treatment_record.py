...
from .treatment_session_beam_sequence_item import TreatmentSessionBeamSequenceItem
...
class RtBeamsTreatmentRecord:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._TreatmentSessionBeamSequence: List[TreatmentSessionBeamSequenceItem] = []
    ...
    @property
    def TreatmentSessionBeamSequence(self) -> Optional[List[TreatmentSessionBeamSequenceItem]]:
        if "TreatmentSessionBeamSequence" in self._dataset:
            if len(self._TreatmentSessionBeamSequence) == len(self._dataset.TreatmentSessionBeamSequence):
                return self._TreatmentSessionBeamSequence
            else:
                return [TreatmentSessionBeamSequenceItem(x) for x in self._dataset.TreatmentSessionBeamSequence]
        return None

    @TreatmentSessionBeamSequence.setter
    def TreatmentSessionBeamSequence(self, value: Optional[List[TreatmentSessionBeamSequenceItem]]):
        if value is None:
            self._TreatmentSessionBeamSequence = []
            if "TreatmentSessionBeamSequence" in self._dataset:
                del self._dataset.TreatmentSessionBeamSequence
        elif not isinstance(value, list) or not all(isinstance(item, TreatmentSessionBeamSequenceItem) for item in value):
            raise ValueError("TreatmentSessionBeamSequence must be a list of TreatmentSessionBeamSequenceItem objects")
        else:
            self._TreatmentSessionBeamSequence = value
            if "TreatmentSessionBeamSequence" not in self._dataset:
                self._dataset.TreatmentSessionBeamSequence = pydicom.Sequence()
            self._dataset.TreatmentSessionBeamSequence.clear()
            self._dataset.TreatmentSessionBeamSequence.extend([item.to_dataset() for item in value])

    def add_TreatmentSessionBeam(self, item: TreatmentSessionBeamSequenceItem):
        if not isinstance(item, TreatmentSessionBeamSequenceItem):
            raise ValueError("Item must be an instance of TreatmentSessionBeamSequenceItem")
        self._TreatmentSessionBeamSequence.append(item)
        if "TreatmentSessionBeamSequence" not in self._dataset:
            self._dataset.TreatmentSessionBeamSequence = pydicom.Sequence()
        self._dataset.TreatmentSessionBeamSequence.append(item.to_dataset())
