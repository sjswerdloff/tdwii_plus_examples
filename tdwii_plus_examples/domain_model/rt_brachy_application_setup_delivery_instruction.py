...
from .brachy_task_sequence_item import BrachyTaskSequenceItem
...
from .omitted_application_setup_sequence_item import OmittedApplicationSetupSequenceItem
...
class RtBrachyApplicationSetupDeliveryInstruction:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._BrachyTaskSequence: List[BrachyTaskSequenceItem] = []
        self._OmittedApplicationSetupSequence: List[OmittedApplicationSetupSequenceItem] = []
    ...
    @property
    def BrachyTaskSequence(self) -> Optional[List[BrachyTaskSequenceItem]]:
        if "BrachyTaskSequence" in self._dataset:
            if len(self._BrachyTaskSequence) == len(self._dataset.BrachyTaskSequence):
                return self._BrachyTaskSequence
            else:
                return [BrachyTaskSequenceItem(x) for x in self._dataset.BrachyTaskSequence]
        return None

    @BrachyTaskSequence.setter
    def BrachyTaskSequence(self, value: Optional[List[BrachyTaskSequenceItem]]):
        if value is None:
            self._BrachyTaskSequence = []
            if "BrachyTaskSequence" in self._dataset:
                del self._dataset.BrachyTaskSequence
        elif not isinstance(value, list) or not all(isinstance(item, BrachyTaskSequenceItem) for item in value):
            raise ValueError("BrachyTaskSequence must be a list of BrachyTaskSequenceItem objects")
        else:
            self._BrachyTaskSequence = value
            if "BrachyTaskSequence" not in self._dataset:
                self._dataset.BrachyTaskSequence = pydicom.Sequence()
            self._dataset.BrachyTaskSequence.clear()
            self._dataset.BrachyTaskSequence.extend([item.to_dataset() for item in value])

    def add_BrachyTask(self, item: BrachyTaskSequenceItem):
        if not isinstance(item, BrachyTaskSequenceItem):
            raise ValueError("Item must be an instance of BrachyTaskSequenceItem")
        self._BrachyTaskSequence.append(item)
        if "BrachyTaskSequence" not in self._dataset:
            self._dataset.BrachyTaskSequence = pydicom.Sequence()
        self._dataset.BrachyTaskSequence.append(item.to_dataset())

    @property
    def ContinuationPulseNumber(self) -> Optional[int]:
        if "ContinuationPulseNumber" in self._dataset:
            return self._dataset.ContinuationPulseNumber
        return None

    @ContinuationPulseNumber.setter
    def ContinuationPulseNumber(self, value: Optional[int]):
        if value is None:
            if "ContinuationPulseNumber" in self._dataset:
                del self._dataset.ContinuationPulseNumber
        else:
            self._dataset.ContinuationPulseNumber = value

    @property
    def OmittedApplicationSetupSequence(self) -> Optional[List[OmittedApplicationSetupSequenceItem]]:
        if "OmittedApplicationSetupSequence" in self._dataset:
            if len(self._OmittedApplicationSetupSequence) == len(self._dataset.OmittedApplicationSetupSequence):
                return self._OmittedApplicationSetupSequence
            else:
                return [OmittedApplicationSetupSequenceItem(x) for x in self._dataset.OmittedApplicationSetupSequence]
        return None

    @OmittedApplicationSetupSequence.setter
    def OmittedApplicationSetupSequence(self, value: Optional[List[OmittedApplicationSetupSequenceItem]]):
        if value is None:
            self._OmittedApplicationSetupSequence = []
            if "OmittedApplicationSetupSequence" in self._dataset:
                del self._dataset.OmittedApplicationSetupSequence
        elif not isinstance(value, list) or not all(isinstance(item, OmittedApplicationSetupSequenceItem) for item in value):
            raise ValueError("OmittedApplicationSetupSequence must be a list of OmittedApplicationSetupSequenceItem objects")
        else:
            self._OmittedApplicationSetupSequence = value
            if "OmittedApplicationSetupSequence" not in self._dataset:
                self._dataset.OmittedApplicationSetupSequence = pydicom.Sequence()
            self._dataset.OmittedApplicationSetupSequence.clear()
            self._dataset.OmittedApplicationSetupSequence.extend([item.to_dataset() for item in value])

    def add_OmittedApplicationSetup(self, item: OmittedApplicationSetupSequenceItem):
        if not isinstance(item, OmittedApplicationSetupSequenceItem):
            raise ValueError("Item must be an instance of OmittedApplicationSetupSequenceItem")
        self._OmittedApplicationSetupSequence.append(item)
        if "OmittedApplicationSetupSequence" not in self._dataset:
            self._dataset.OmittedApplicationSetupSequence = pydicom.Sequence()
        self._dataset.OmittedApplicationSetupSequence.append(item.to_dataset())
