...
from .calculated_dose_reference_sequence_item import CalculatedDoseReferenceSequenceItem
...
from .fraction_group_summary_sequence_item import FractionGroupSummarySequenceItem
...
from .measured_dose_reference_sequence_item import MeasuredDoseReferenceSequenceItem
...
from .patient_setup_sequence_item import PatientSetupSequenceItem
...
from .recorded_source_sequence_item import RecordedSourceSequenceItem
...
from .referenced_treatment_record_sequence_item import (
    ReferencedTreatmentRecordSequenceItem,
)
...
from .treatment_machine_sequence_item import TreatmentMachineSequenceItem
from .treatment_session_application_setup_sequence_item import (
    TreatmentSessionApplicationSetupSequenceItem,
)
from .treatment_summary_calculated_dose_reference_sequence_item import (
    TreatmentSummaryCalculatedDoseReferenceSequenceItem,
)
from .treatment_summary_measured_dose_reference_sequence_item import (
    TreatmentSummaryMeasuredDoseReferenceSequenceItem,
)
from .udi_sequence_item import UDISequenceItem


class RtBrachyTreatmentRecord:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        self._dataset = dataset if dataset is not None else pydicom.Dataset()
        self._RecordedSourceSequence: List[RecordedSourceSequenceItem] = []
        self._TreatmentSessionApplicationSetupSequence: List[TreatmentSessionApplicationSetupSequenceItem] = []
        ...
        self._PatientSetupSequence: List[PatientSetupSequenceItem] = []
        ...
        self._ReferencedTreatmentRecordSequence: List[ReferencedTreatmentRecordSequenceItem] = []
        self._ReferencedRTPlanSequence: List[ReferencedRTPlanSequenceItem] = []
        self._TreatmentMachineSequence: List[TreatmentMachineSequenceItem] = []
        self._InstitutionalDepartmentTypeCodeSequence: List[CodeSequenceItem] = []
        self._UDISequence: List[UDISequenceItem] = []
        self._MeasuredDoseReferenceSequence: List[MeasuredDoseReferenceSequenceItem] = []
        self._CalculatedDoseReferenceSequence: List[CalculatedDoseReferenceSequenceItem] = []
        ...
        self._TreatmentSummaryCalculatedDoseReferenceSequence: List[TreatmentSummaryCalculatedDoseReferenceSequenceItem] = []
        self._TreatmentSummaryMeasuredDoseReferenceSequence: List[TreatmentSummaryMeasuredDoseReferenceSequenceItem] = []
        self._FractionGroupSummarySequence: List[FractionGroupSummarySequenceItem] = []
    ...
    @property
    def RecordedSourceSequence(self) -> Optional[List[RecordedSourceSequenceItem]]:
        if "RecordedSourceSequence" in self._dataset:
            if len(self._RecordedSourceSequence) == len(self._dataset.RecordedSourceSequence):
                return self._RecordedSourceSequence
            else:
                return [RecordedSourceSequenceItem(x) for x in self._dataset.RecordedSourceSequence]
        return None

    @RecordedSourceSequence.setter
    def RecordedSourceSequence(self, value: Optional[List[RecordedSourceSequenceItem]]):
        if value is None:
            self._RecordedSourceSequence = []
            if "RecordedSourceSequence" in self._dataset:
                del self._dataset.RecordedSourceSequence
        elif not isinstance(value, list) or not all(isinstance(item, RecordedSourceSequenceItem) for item in value):
            raise ValueError("RecordedSourceSequence must be a list of RecordedSourceSequenceItem objects")
        else:
            self._RecordedSourceSequence = value
            if "RecordedSourceSequence" not in self._dataset:
                self._dataset.RecordedSourceSequence = pydicom.Sequence()
            self._dataset.RecordedSourceSequence.clear()
            self._dataset.RecordedSourceSequence.extend([item.to_dataset() for item in value])

    def add_RecordedSource(self, item: RecordedSourceSequenceItem):
        if not isinstance(item, RecordedSourceSequenceItem):
            raise ValueError("Item must be an instance of RecordedSourceSequenceItem")
        self._RecordedSourceSequence.append(item)
        if "RecordedSourceSequence" not in self._dataset:
            self._dataset.RecordedSourceSequence = pydicom.Sequence()
        self._dataset.RecordedSourceSequence.append(item.to_dataset())

    @property
    def TreatmentSessionApplicationSetupSequence(self) -> Optional[List[TreatmentSessionApplicationSetupSequenceItem]]:
        if "TreatmentSessionApplicationSetupSequence" in self._dataset:
            if len(self._TreatmentSessionApplicationSetupSequence) == len(
                self._dataset.TreatmentSessionApplicationSetupSequence
            ):
                return self._TreatmentSessionApplicationSetupSequence
            else:
                return [
                    TreatmentSessionApplicationSetupSequenceItem(x)
                    for x in self._dataset.TreatmentSessionApplicationSetupSequence
                ]
        return None

    @TreatmentSessionApplicationSetupSequence.setter
    def TreatmentSessionApplicationSetupSequence(self, value: Optional[List[TreatmentSessionApplicationSetupSequenceItem]]):
        if value is None:
            self._TreatmentSessionApplicationSetupSequence = []
            if "TreatmentSessionApplicationSetupSequence" in self._dataset:
                del self._dataset.TreatmentSessionApplicationSetupSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, TreatmentSessionApplicationSetupSequenceItem) for item in value
        ):
            raise ValueError(
                "TreatmentSessionApplicationSetupSequence must be a list of TreatmentSessionApplicationSetupSequenceItem"
                " objects"
            )
        else:
            self._TreatmentSessionApplicationSetupSequence = value
            if "TreatmentSessionApplicationSetupSequence" not in self._dataset:
                self._dataset.TreatmentSessionApplicationSetupSequence = pydicom.Sequence()
            self._dataset.TreatmentSessionApplicationSetupSequence.clear()
            self._dataset.TreatmentSessionApplicationSetupSequence.extend([item.to_dataset() for item in value])

    def add_TreatmentSessionApplicationSetup(self, item: TreatmentSessionApplicationSetupSequenceItem):
        if not isinstance(item, TreatmentSessionApplicationSetupSequenceItem):
            raise ValueError("Item must be an instance of TreatmentSessionApplicationSetupSequenceItem")
        self._TreatmentSessionApplicationSetupSequence.append(item)
        if "TreatmentSessionApplicationSetupSequence" not in self._dataset:
            self._dataset.TreatmentSessionApplicationSetupSequence = pydicom.Sequence()
        self._dataset.TreatmentSessionApplicationSetupSequence.append(item.to_dataset())
    ...
    @property
    def BrachyTreatmentTechnique(self) -> Optional[str]:
        if "BrachyTreatmentTechnique" in self._dataset:
            return self._dataset.BrachyTreatmentTechnique
        return None

    @BrachyTreatmentTechnique.setter
    def BrachyTreatmentTechnique(self, value: Optional[str]):
        if value is None:
            if "BrachyTreatmentTechnique" in self._dataset:
                del self._dataset.BrachyTreatmentTechnique
        else:
            self._dataset.BrachyTreatmentTechnique = value

    @property
    def BrachyTreatmentType(self) -> Optional[str]:
        if "BrachyTreatmentType" in self._dataset:
            return self._dataset.BrachyTreatmentType
        return None

    @BrachyTreatmentType.setter
    def BrachyTreatmentType(self, value: Optional[str]):
        if value is None:
            if "BrachyTreatmentType" in self._dataset:
                del self._dataset.BrachyTreatmentType
        else:
            self._dataset.BrachyTreatmentType = value
    ...
    @property
    def PatientSetupSequence(self) -> Optional[List[PatientSetupSequenceItem]]:
        if "PatientSetupSequence" in self._dataset:
            if len(self._PatientSetupSequence) == len(self._dataset.PatientSetupSequence):
                return self._PatientSetupSequence
            else:
                return [PatientSetupSequenceItem(x) for x in self._dataset.PatientSetupSequence]
        return None

    @PatientSetupSequence.setter
    def PatientSetupSequence(self, value: Optional[List[PatientSetupSequenceItem]]):
        if value is None:
            self._PatientSetupSequence = []
            if "PatientSetupSequence" in self._dataset:
                del self._dataset.PatientSetupSequence
        elif not isinstance(value, list) or not all(isinstance(item, PatientSetupSequenceItem) for item in value):
            raise ValueError("PatientSetupSequence must be a list of PatientSetupSequenceItem objects")
        else:
            self._PatientSetupSequence = value
            if "PatientSetupSequence" not in self._dataset:
                self._dataset.PatientSetupSequence = pydicom.Sequence()
            self._dataset.PatientSetupSequence.clear()
            self._dataset.PatientSetupSequence.extend([item.to_dataset() for item in value])

    def add_PatientSetup(self, item: PatientSetupSequenceItem):
        if not isinstance(item, PatientSetupSequenceItem):
            raise ValueError("Item must be an instance of PatientSetupSequenceItem")
        self._PatientSetupSequence.append(item)
        if "PatientSetupSequence" not in self._dataset:
            self._dataset.PatientSetupSequence = pydicom.Sequence()
        self._dataset.PatientSetupSequence.append(item.to_dataset())
    ...
    @property
    def ReferencedTreatmentRecordSequence(self) -> Optional[List[ReferencedTreatmentRecordSequenceItem]]:
        if "ReferencedTreatmentRecordSequence" in self._dataset:
            if len(self._ReferencedTreatmentRecordSequence) == len(self._dataset.ReferencedTreatmentRecordSequence):
                return self._ReferencedTreatmentRecordSequence
            else:
                return [ReferencedTreatmentRecordSequenceItem(x) for x in self._dataset.ReferencedTreatmentRecordSequence]
        return None

    @ReferencedTreatmentRecordSequence.setter
    def ReferencedTreatmentRecordSequence(self, value: Optional[List[ReferencedTreatmentRecordSequenceItem]]):
        if value is None:
            self._ReferencedTreatmentRecordSequence = []
            if "ReferencedTreatmentRecordSequence" in self._dataset:
                del self._dataset.ReferencedTreatmentRecordSequence
        elif not isinstance(value, list) or not all(isinstance(item, ReferencedTreatmentRecordSequenceItem) for item in value):
            raise ValueError(
                "ReferencedTreatmentRecordSequence must be a list of ReferencedTreatmentRecordSequenceItem objects"
            )
        else:
            self._ReferencedTreatmentRecordSequence = value
            if "ReferencedTreatmentRecordSequence" not in self._dataset:
                self._dataset.ReferencedTreatmentRecordSequence = pydicom.Sequence()
            self._dataset.ReferencedTreatmentRecordSequence.clear()
            self._dataset.ReferencedTreatmentRecordSequence.extend([item.to_dataset() for item in value])

    def add_ReferencedTreatmentRecord(self, item: ReferencedTreatmentRecordSequenceItem):
        if not isinstance(item, ReferencedTreatmentRecordSequenceItem):
            raise ValueError("Item must be an instance of ReferencedTreatmentRecordSequenceItem")
        self._ReferencedTreatmentRecordSequence.append(item)
        if "ReferencedTreatmentRecordSequence" not in self._dataset:
            self._dataset.ReferencedTreatmentRecordSequence = pydicom.Sequence()
        self._dataset.ReferencedTreatmentRecordSequence.append(item.to_dataset())
    ...
    def ReferencedRTPlanSequence(self, value: Optional[List[ReferencedRTPlanSequenceItem]]):
        if value is None:
            ...
            raise ValueError("ReferencedRTPlanSequence must be a list of ReferencedRTPlanSequenceItem objects")
        else:
    ...
    def add_ReferencedRTPlan(self, item: ReferencedRTPlanSequenceItem):
        if not isinstance(item, ReferencedRTPlanSequenceItem):
            raise ValueError("Item must be an instance of ReferencedRTPlanSequenceItem")
    ...
    @property
    def TreatmentMachineSequence(self) -> Optional[List[TreatmentMachineSequenceItem]]:
        if "TreatmentMachineSequence" in self._dataset:
            if len(self._TreatmentMachineSequence) == len(self._dataset.TreatmentMachineSequence):
                return self._TreatmentMachineSequence
            else:
                return [TreatmentMachineSequenceItem(x) for x in self._dataset.TreatmentMachineSequence]
        return None

    @TreatmentMachineSequence.setter
    def TreatmentMachineSequence(self, value: Optional[List[TreatmentMachineSequenceItem]]):
        if value is None:
            self._TreatmentMachineSequence = []
            if "TreatmentMachineSequence" in self._dataset:
                del self._dataset.TreatmentMachineSequence
        elif not isinstance(value, list) or not all(isinstance(item, TreatmentMachineSequenceItem) for item in value):
            raise ValueError("TreatmentMachineSequence must be a list of TreatmentMachineSequenceItem objects")
        else:
            self._TreatmentMachineSequence = value
            if "TreatmentMachineSequence" not in self._dataset:
                self._dataset.TreatmentMachineSequence = pydicom.Sequence()
            self._dataset.TreatmentMachineSequence.clear()
            self._dataset.TreatmentMachineSequence.extend([item.to_dataset() for item in value])

    def add_TreatmentMachine(self, item: TreatmentMachineSequenceItem):
        if not isinstance(item, TreatmentMachineSequenceItem):
            raise ValueError("Item must be an instance of TreatmentMachineSequenceItem")
        self._TreatmentMachineSequence.append(item)
        if "TreatmentMachineSequence" not in self._dataset:
            self._dataset.TreatmentMachineSequence = pydicom.Sequence()
        self._dataset.TreatmentMachineSequence.append(item.to_dataset())
    ...
    @property
    def MeasuredDoseReferenceSequence(self) -> Optional[List[MeasuredDoseReferenceSequenceItem]]:
        if "MeasuredDoseReferenceSequence" in self._dataset:
            if len(self._MeasuredDoseReferenceSequence) == len(self._dataset.MeasuredDoseReferenceSequence):
                return self._MeasuredDoseReferenceSequence
            else:
                return [MeasuredDoseReferenceSequenceItem(x) for x in self._dataset.MeasuredDoseReferenceSequence]
        return None

    @MeasuredDoseReferenceSequence.setter
    def MeasuredDoseReferenceSequence(self, value: Optional[List[MeasuredDoseReferenceSequenceItem]]):
        if value is None:
            self._MeasuredDoseReferenceSequence = []
            if "MeasuredDoseReferenceSequence" in self._dataset:
                del self._dataset.MeasuredDoseReferenceSequence
        elif not isinstance(value, list) or not all(isinstance(item, MeasuredDoseReferenceSequenceItem) for item in value):
            raise ValueError("MeasuredDoseReferenceSequence must be a list of MeasuredDoseReferenceSequenceItem objects")
        else:
            self._MeasuredDoseReferenceSequence = value
            if "MeasuredDoseReferenceSequence" not in self._dataset:
                self._dataset.MeasuredDoseReferenceSequence = pydicom.Sequence()
            self._dataset.MeasuredDoseReferenceSequence.clear()
            self._dataset.MeasuredDoseReferenceSequence.extend([item.to_dataset() for item in value])

    def add_MeasuredDoseReference(self, item: MeasuredDoseReferenceSequenceItem):
        if not isinstance(item, MeasuredDoseReferenceSequenceItem):
            raise ValueError("Item must be an instance of MeasuredDoseReferenceSequenceItem")
        self._MeasuredDoseReferenceSequence.append(item)
        if "MeasuredDoseReferenceSequence" not in self._dataset:
            self._dataset.MeasuredDoseReferenceSequence = pydicom.Sequence()
        self._dataset.MeasuredDoseReferenceSequence.append(item.to_dataset())

    @property
    def CalculatedDoseReferenceSequence(self) -> Optional[List[CalculatedDoseReferenceSequenceItem]]:
        if "CalculatedDoseReferenceSequence" in self._dataset:
            if len(self._CalculatedDoseReferenceSequence) == len(self._dataset.CalculatedDoseReferenceSequence):
                return self._CalculatedDoseReferenceSequence
            else:
                return [CalculatedDoseReferenceSequenceItem(x) for x in self._dataset.CalculatedDoseReferenceSequence]
        return None

    @CalculatedDoseReferenceSequence.setter
    def CalculatedDoseReferenceSequence(self, value: Optional[List[CalculatedDoseReferenceSequenceItem]]):
        if value is None:
            self._CalculatedDoseReferenceSequence = []
            if "CalculatedDoseReferenceSequence" in self._dataset:
                del self._dataset.CalculatedDoseReferenceSequence
        elif not isinstance(value, list) or not all(isinstance(item, CalculatedDoseReferenceSequenceItem) for item in value):
            raise ValueError("CalculatedDoseReferenceSequence must be a list of CalculatedDoseReferenceSequenceItem objects")
        else:
            self._CalculatedDoseReferenceSequence = value
            if "CalculatedDoseReferenceSequence" not in self._dataset:
                self._dataset.CalculatedDoseReferenceSequence = pydicom.Sequence()
            self._dataset.CalculatedDoseReferenceSequence.clear()
            self._dataset.CalculatedDoseReferenceSequence.extend([item.to_dataset() for item in value])

    def add_CalculatedDoseReference(self, item: CalculatedDoseReferenceSequenceItem):
        if not isinstance(item, CalculatedDoseReferenceSequenceItem):
            raise ValueError("Item must be an instance of CalculatedDoseReferenceSequenceItem")
        self._CalculatedDoseReferenceSequence.append(item)
        if "CalculatedDoseReferenceSequence" not in self._dataset:
            self._dataset.CalculatedDoseReferenceSequence = pydicom.Sequence()
        self._dataset.CalculatedDoseReferenceSequence.append(item.to_dataset())
    ...
    def TreatmentSummaryCalculatedDoseReferenceSequence(
        self,
    ) -> Optional[List[TreatmentSummaryCalculatedDoseReferenceSequenceItem]]:
        if "TreatmentSummaryCalculatedDoseReferenceSequence" in self._dataset:
            if len(self._TreatmentSummaryCalculatedDoseReferenceSequence) == len(
                self._dataset.TreatmentSummaryCalculatedDoseReferenceSequence
            ):
                return self._TreatmentSummaryCalculatedDoseReferenceSequence
            else:
                return [
                    TreatmentSummaryCalculatedDoseReferenceSequenceItem(x)
                    for x in self._dataset.TreatmentSummaryCalculatedDoseReferenceSequence
                ]
        return None

    @TreatmentSummaryCalculatedDoseReferenceSequence.setter
    def TreatmentSummaryCalculatedDoseReferenceSequence(
        self, value: Optional[List[TreatmentSummaryCalculatedDoseReferenceSequenceItem]]
    ):
        if value is None:
            self._TreatmentSummaryCalculatedDoseReferenceSequence = []
            if "TreatmentSummaryCalculatedDoseReferenceSequence" in self._dataset:
                del self._dataset.TreatmentSummaryCalculatedDoseReferenceSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, TreatmentSummaryCalculatedDoseReferenceSequenceItem) for item in value
        ):
            raise ValueError(
                "TreatmentSummaryCalculatedDoseReferenceSequence must be a list of"
                " TreatmentSummaryCalculatedDoseReferenceSequenceItem objects"
            )
        else:
            self._TreatmentSummaryCalculatedDoseReferenceSequence = value
            if "TreatmentSummaryCalculatedDoseReferenceSequence" not in self._dataset:
                self._dataset.TreatmentSummaryCalculatedDoseReferenceSequence = pydicom.Sequence()
            self._dataset.TreatmentSummaryCalculatedDoseReferenceSequence.clear()
            self._dataset.TreatmentSummaryCalculatedDoseReferenceSequence.extend([item.to_dataset() for item in value])

    def add_TreatmentSummaryCalculatedDoseReference(self, item: TreatmentSummaryCalculatedDoseReferenceSequenceItem):
        if not isinstance(item, TreatmentSummaryCalculatedDoseReferenceSequenceItem):
            raise ValueError("Item must be an instance of TreatmentSummaryCalculatedDoseReferenceSequenceItem")
        self._TreatmentSummaryCalculatedDoseReferenceSequence.append(item)
        if "TreatmentSummaryCalculatedDoseReferenceSequence" not in self._dataset:
            self._dataset.TreatmentSummaryCalculatedDoseReferenceSequence = pydicom.Sequence()
        self._dataset.TreatmentSummaryCalculatedDoseReferenceSequence.append(item.to_dataset())

    @property
    def FirstTreatmentDate(self) -> Optional[str]:
        if "FirstTreatmentDate" in self._dataset:
            return self._dataset.FirstTreatmentDate
        return None

    @FirstTreatmentDate.setter
    def FirstTreatmentDate(self, value: Optional[str]):
        if value is None:
            if "FirstTreatmentDate" in self._dataset:
                del self._dataset.FirstTreatmentDate
        else:
            self._dataset.FirstTreatmentDate = value

    @property
    def MostRecentTreatmentDate(self) -> Optional[str]:
        if "MostRecentTreatmentDate" in self._dataset:
            return self._dataset.MostRecentTreatmentDate
        return None

    @MostRecentTreatmentDate.setter
    def MostRecentTreatmentDate(self, value: Optional[str]):
        if value is None:
            if "MostRecentTreatmentDate" in self._dataset:
                del self._dataset.MostRecentTreatmentDate
        else:
            self._dataset.MostRecentTreatmentDate = value

    @property
    def TreatmentSummaryMeasuredDoseReferenceSequence(
        self,
    ) -> Optional[List[TreatmentSummaryMeasuredDoseReferenceSequenceItem]]:
        if "TreatmentSummaryMeasuredDoseReferenceSequence" in self._dataset:
            if len(self._TreatmentSummaryMeasuredDoseReferenceSequence) == len(
                self._dataset.TreatmentSummaryMeasuredDoseReferenceSequence
            ):
                return self._TreatmentSummaryMeasuredDoseReferenceSequence
            else:
                return [
                    TreatmentSummaryMeasuredDoseReferenceSequenceItem(x)
                    for x in self._dataset.TreatmentSummaryMeasuredDoseReferenceSequence
                ]
        return None

    @TreatmentSummaryMeasuredDoseReferenceSequence.setter
    def TreatmentSummaryMeasuredDoseReferenceSequence(
        self, value: Optional[List[TreatmentSummaryMeasuredDoseReferenceSequenceItem]]
    ):
        if value is None:
            self._TreatmentSummaryMeasuredDoseReferenceSequence = []
            if "TreatmentSummaryMeasuredDoseReferenceSequence" in self._dataset:
                del self._dataset.TreatmentSummaryMeasuredDoseReferenceSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, TreatmentSummaryMeasuredDoseReferenceSequenceItem) for item in value
        ):
            raise ValueError(
                "TreatmentSummaryMeasuredDoseReferenceSequence must be a list of"
                " TreatmentSummaryMeasuredDoseReferenceSequenceItem objects"
            )
        else:
            self._TreatmentSummaryMeasuredDoseReferenceSequence = value
            if "TreatmentSummaryMeasuredDoseReferenceSequence" not in self._dataset:
                self._dataset.TreatmentSummaryMeasuredDoseReferenceSequence = pydicom.Sequence()
            self._dataset.TreatmentSummaryMeasuredDoseReferenceSequence.clear()
            self._dataset.TreatmentSummaryMeasuredDoseReferenceSequence.extend([item.to_dataset() for item in value])

    def add_TreatmentSummaryMeasuredDoseReference(self, item: TreatmentSummaryMeasuredDoseReferenceSequenceItem):
        if not isinstance(item, TreatmentSummaryMeasuredDoseReferenceSequenceItem):
            raise ValueError("Item must be an instance of TreatmentSummaryMeasuredDoseReferenceSequenceItem")
        self._TreatmentSummaryMeasuredDoseReferenceSequence.append(item)
        if "TreatmentSummaryMeasuredDoseReferenceSequence" not in self._dataset:
            self._dataset.TreatmentSummaryMeasuredDoseReferenceSequence = pydicom.Sequence()
        self._dataset.TreatmentSummaryMeasuredDoseReferenceSequence.append(item.to_dataset())

    @property
    def CurrentTreatmentStatus(self) -> Optional[str]:
        if "CurrentTreatmentStatus" in self._dataset:
            return self._dataset.CurrentTreatmentStatus
        return None

    @CurrentTreatmentStatus.setter
    def CurrentTreatmentStatus(self, value: Optional[str]):
        if value is None:
            if "CurrentTreatmentStatus" in self._dataset:
                del self._dataset.CurrentTreatmentStatus
        else:
            self._dataset.CurrentTreatmentStatus = value

    @property
    def TreatmentStatusComment(self) -> Optional[str]:
        if "TreatmentStatusComment" in self._dataset:
            return self._dataset.TreatmentStatusComment
        return None

    @TreatmentStatusComment.setter
    def TreatmentStatusComment(self, value: Optional[str]):
        if value is None:
            if "TreatmentStatusComment" in self._dataset:
                del self._dataset.TreatmentStatusComment
        else:
            self._dataset.TreatmentStatusComment = value

    @property
    def FractionGroupSummarySequence(self) -> Optional[List[FractionGroupSummarySequenceItem]]:
        if "FractionGroupSummarySequence" in self._dataset:
            if len(self._FractionGroupSummarySequence) == len(self._dataset.FractionGroupSummarySequence):
                return self._FractionGroupSummarySequence
            else:
                return [FractionGroupSummarySequenceItem(x) for x in self._dataset.FractionGroupSummarySequence]
        return None

    @FractionGroupSummarySequence.setter
    def FractionGroupSummarySequence(self, value: Optional[List[FractionGroupSummarySequenceItem]]):
        if value is None:
            self._FractionGroupSummarySequence = []
            if "FractionGroupSummarySequence" in self._dataset:
                del self._dataset.FractionGroupSummarySequence
        elif not isinstance(value, list) or not all(isinstance(item, FractionGroupSummarySequenceItem) for item in value):
            raise ValueError("FractionGroupSummarySequence must be a list of FractionGroupSummarySequenceItem objects")
        else:
            self._FractionGroupSummarySequence = value
            if "FractionGroupSummarySequence" not in self._dataset:
                self._dataset.FractionGroupSummarySequence = pydicom.Sequence()
            self._dataset.FractionGroupSummarySequence.clear()
            self._dataset.FractionGroupSummarySequence.extend([item.to_dataset() for item in value])

    def add_FractionGroupSummary(self, item: FractionGroupSummarySequenceItem):
        if not isinstance(item, FractionGroupSummarySequenceItem):
            raise ValueError("Item must be an instance of FractionGroupSummarySequenceItem")
        self._FractionGroupSummarySequence.append(item)
        if "FractionGroupSummarySequence" not in self._dataset:
            self._dataset.FractionGroupSummarySequence = pydicom.Sequence()
        self._dataset.FractionGroupSummarySequence.append(item.to_dataset())
