...
from .dvh_sequence_item import DVHSequenceItem
...
from .plan_overview_sequence_item import PlanOverviewSequenceItem
...
class RtDose:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._PlanOverviewSequence: List[PlanOverviewSequenceItem] = []
        self._DVHSequence: List[DVHSequenceItem] = []
    ...
    @property
    def SpatialTransformOfDose(self) -> Optional[str]:
        if "SpatialTransformOfDose" in self._dataset:
            return self._dataset.SpatialTransformOfDose
        return None

    @SpatialTransformOfDose.setter
    def SpatialTransformOfDose(self, value: Optional[str]):
        if value is None:
            if "SpatialTransformOfDose" in self._dataset:
                del self._dataset.SpatialTransformOfDose
        else:
            self._dataset.SpatialTransformOfDose = value

    @property
    def DoseComment(self) -> Optional[str]:
        if "DoseComment" in self._dataset:
            return self._dataset.DoseComment
        return None

    @DoseComment.setter
    def DoseComment(self, value: Optional[str]):
        if value is None:
            if "DoseComment" in self._dataset:
                del self._dataset.DoseComment
        else:
            self._dataset.DoseComment = value

    @property
    def NormalizationPoint(self) -> Optional[List[Decimal]]:
        if "NormalizationPoint" in self._dataset:
            return self._dataset.NormalizationPoint
        return None

    @NormalizationPoint.setter
    def NormalizationPoint(self, value: Optional[List[Decimal]]):
        if value is None:
            if "NormalizationPoint" in self._dataset:
                del self._dataset.NormalizationPoint
        else:
            self._dataset.NormalizationPoint = value

    @property
    def DoseSummationType(self) -> Optional[str]:
        if "DoseSummationType" in self._dataset:
            return self._dataset.DoseSummationType
        return None

    @DoseSummationType.setter
    def DoseSummationType(self, value: Optional[str]):
        if value is None:
            if "DoseSummationType" in self._dataset:
                del self._dataset.DoseSummationType
        else:
            self._dataset.DoseSummationType = value

    @property
    def GridFrameOffsetVector(self) -> Optional[List[Decimal]]:
        if "GridFrameOffsetVector" in self._dataset:
            return self._dataset.GridFrameOffsetVector
        return None

    @GridFrameOffsetVector.setter
    def GridFrameOffsetVector(self, value: Optional[List[Decimal]]):
        if value is None:
            if "GridFrameOffsetVector" in self._dataset:
                del self._dataset.GridFrameOffsetVector
        else:
            self._dataset.GridFrameOffsetVector = value

    @property
    def DoseGridScaling(self) -> Optional[Decimal]:
        if "DoseGridScaling" in self._dataset:
            return self._dataset.DoseGridScaling
        return None

    @DoseGridScaling.setter
    def DoseGridScaling(self, value: Optional[Decimal]):
        if value is None:
            if "DoseGridScaling" in self._dataset:
                del self._dataset.DoseGridScaling
        else:
            self._dataset.DoseGridScaling = value

    @property
    def TissueHeterogeneityCorrection(self) -> Optional[List[str]]:
        if "TissueHeterogeneityCorrection" in self._dataset:
            return self._dataset.TissueHeterogeneityCorrection
        return None

    @TissueHeterogeneityCorrection.setter
    def TissueHeterogeneityCorrection(self, value: Optional[List[str]]):
        if value is None:
            if "TissueHeterogeneityCorrection" in self._dataset:
                del self._dataset.TissueHeterogeneityCorrection
        else:
            self._dataset.TissueHeterogeneityCorrection = value
    ...
    @property
    def PlanOverviewSequence(self) -> Optional[List[PlanOverviewSequenceItem]]:
        if "PlanOverviewSequence" in self._dataset:
            if len(self._PlanOverviewSequence) == len(self._dataset.PlanOverviewSequence):
                return self._PlanOverviewSequence
            else:
                return [PlanOverviewSequenceItem(x) for x in self._dataset.PlanOverviewSequence]
        return None

    @PlanOverviewSequence.setter
    def PlanOverviewSequence(self, value: Optional[List[PlanOverviewSequenceItem]]):
        if value is None:
            self._PlanOverviewSequence = []
            if "PlanOverviewSequence" in self._dataset:
                del self._dataset.PlanOverviewSequence
        elif not isinstance(value, list) or not all(isinstance(item, PlanOverviewSequenceItem) for item in value):
            raise ValueError("PlanOverviewSequence must be a list of PlanOverviewSequenceItem objects")
        else:
            self._PlanOverviewSequence = value
            if "PlanOverviewSequence" not in self._dataset:
                self._dataset.PlanOverviewSequence = pydicom.Sequence()
            self._dataset.PlanOverviewSequence.clear()
            self._dataset.PlanOverviewSequence.extend([item.to_dataset() for item in value])

    def add_PlanOverview(self, item: PlanOverviewSequenceItem):
        if not isinstance(item, PlanOverviewSequenceItem):
            raise ValueError("Item must be an instance of PlanOverviewSequenceItem")
        self._PlanOverviewSequence.append(item)
        if "PlanOverviewSequence" not in self._dataset:
            self._dataset.PlanOverviewSequence = pydicom.Sequence()
        self._dataset.PlanOverviewSequence.append(item.to_dataset())
    ...
    @property
    def DVHNormalizationPoint(self) -> Optional[List[Decimal]]:
        if "DVHNormalizationPoint" in self._dataset:
            return self._dataset.DVHNormalizationPoint
        return None

    @DVHNormalizationPoint.setter
    def DVHNormalizationPoint(self, value: Optional[List[Decimal]]):
        if value is None:
            if "DVHNormalizationPoint" in self._dataset:
                del self._dataset.DVHNormalizationPoint
        else:
            self._dataset.DVHNormalizationPoint = value

    @property
    def DVHNormalizationDoseValue(self) -> Optional[Decimal]:
        if "DVHNormalizationDoseValue" in self._dataset:
            return self._dataset.DVHNormalizationDoseValue
        return None

    @DVHNormalizationDoseValue.setter
    def DVHNormalizationDoseValue(self, value: Optional[Decimal]):
        if value is None:
            if "DVHNormalizationDoseValue" in self._dataset:
                del self._dataset.DVHNormalizationDoseValue
        else:
            self._dataset.DVHNormalizationDoseValue = value

    @property
    def DVHSequence(self) -> Optional[List[DVHSequenceItem]]:
        if "DVHSequence" in self._dataset:
            if len(self._DVHSequence) == len(self._dataset.DVHSequence):
                return self._DVHSequence
            else:
                return [DVHSequenceItem(x) for x in self._dataset.DVHSequence]
        return None

    @DVHSequence.setter
    def DVHSequence(self, value: Optional[List[DVHSequenceItem]]):
        if value is None:
            self._DVHSequence = []
            if "DVHSequence" in self._dataset:
                del self._dataset.DVHSequence
        elif not isinstance(value, list) or not all(isinstance(item, DVHSequenceItem) for item in value):
            raise ValueError("DVHSequence must be a list of DVHSequenceItem objects")
        else:
            self._DVHSequence = value
            if "DVHSequence" not in self._dataset:
                self._dataset.DVHSequence = pydicom.Sequence()
            self._dataset.DVHSequence.clear()
            self._dataset.DVHSequence.extend([item.to_dataset() for item in value])

    def add_DVH(self, item: DVHSequenceItem):
        if not isinstance(item, DVHSequenceItem):
            raise ValueError("Item must be an instance of DVHSequenceItem")
        self._DVHSequence.append(item)
        if "DVHSequence" not in self._dataset:
            self._dataset.DVHSequence = pydicom.Sequence()
        self._dataset.DVHSequence.append(item.to_dataset())
