...
from .fluence_map_sequence_item import FluenceMapSequenceItem
...
class RtImage:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._ExposureSequence: List[ExposureSequenceItem] = []
        self._FluenceMapSequence: List[FluenceMapSequenceItem] = []
    ...
    @property
    def RTImageLabel(self) -> Optional[str]:
        if "RTImageLabel" in self._dataset:
            return self._dataset.RTImageLabel
        return None

    @RTImageLabel.setter
    def RTImageLabel(self, value: Optional[str]):
        if value is None:
            if "RTImageLabel" in self._dataset:
                del self._dataset.RTImageLabel
        else:
            self._dataset.RTImageLabel = value

    @property
    def RTImageName(self) -> Optional[str]:
        if "RTImageName" in self._dataset:
            return self._dataset.RTImageName
        return None

    @RTImageName.setter
    def RTImageName(self, value: Optional[str]):
        if value is None:
            if "RTImageName" in self._dataset:
                del self._dataset.RTImageName
        else:
            self._dataset.RTImageName = value

    @property
    def RTImageDescription(self) -> Optional[str]:
        if "RTImageDescription" in self._dataset:
            return self._dataset.RTImageDescription
        return None

    @RTImageDescription.setter
    def RTImageDescription(self, value: Optional[str]):
        if value is None:
            if "RTImageDescription" in self._dataset:
                del self._dataset.RTImageDescription
        else:
            self._dataset.RTImageDescription = value

    @property
    def ReportedValuesOrigin(self) -> Optional[str]:
        if "ReportedValuesOrigin" in self._dataset:
            return self._dataset.ReportedValuesOrigin
        return None

    @ReportedValuesOrigin.setter
    def ReportedValuesOrigin(self, value: Optional[str]):
        if value is None:
            if "ReportedValuesOrigin" in self._dataset:
                del self._dataset.ReportedValuesOrigin
        else:
            self._dataset.ReportedValuesOrigin = value
    ...
    @property
    def ImagePlanePixelSpacing(self) -> Optional[List[Decimal]]:
        if "ImagePlanePixelSpacing" in self._dataset:
            return self._dataset.ImagePlanePixelSpacing
        return None

    @ImagePlanePixelSpacing.setter
    def ImagePlanePixelSpacing(self, value: Optional[List[Decimal]]):
        if value is None:
            if "ImagePlanePixelSpacing" in self._dataset:
                del self._dataset.ImagePlanePixelSpacing
        else:
            self._dataset.ImagePlanePixelSpacing = value
    ...
    @property
    def RadiationMachineName(self) -> Optional[str]:
        if "RadiationMachineName" in self._dataset:
            return self._dataset.RadiationMachineName
        return None

    @RadiationMachineName.setter
    def RadiationMachineName(self, value: Optional[str]):
        if value is None:
            if "RadiationMachineName" in self._dataset:
                del self._dataset.RadiationMachineName
        else:
            self._dataset.RadiationMachineName = value

    @property
    def RadiationMachineSAD(self) -> Optional[Decimal]:
        if "RadiationMachineSAD" in self._dataset:
            return self._dataset.RadiationMachineSAD
        return None

    @RadiationMachineSAD.setter
    def RadiationMachineSAD(self, value: Optional[Decimal]):
        if value is None:
            if "RadiationMachineSAD" in self._dataset:
                del self._dataset.RadiationMachineSAD
        else:
            self._dataset.RadiationMachineSAD = value

    @property
    def RadiationMachineSSD(self) -> Optional[Decimal]:
        if "RadiationMachineSSD" in self._dataset:
            return self._dataset.RadiationMachineSSD
        return None

    @RadiationMachineSSD.setter
    def RadiationMachineSSD(self, value: Optional[Decimal]):
        if value is None:
            if "RadiationMachineSSD" in self._dataset:
                del self._dataset.RadiationMachineSSD
        else:
            self._dataset.RadiationMachineSSD = value
    ...
    @property
    def SourceToReferenceObjectDistance(self) -> Optional[Decimal]:
        if "SourceToReferenceObjectDistance" in self._dataset:
            return self._dataset.SourceToReferenceObjectDistance
        return None

    @SourceToReferenceObjectDistance.setter
    def SourceToReferenceObjectDistance(self, value: Optional[Decimal]):
        if value is None:
            if "SourceToReferenceObjectDistance" in self._dataset:
                del self._dataset.SourceToReferenceObjectDistance
        else:
            self._dataset.SourceToReferenceObjectDistance = value

    @property
    def FractionNumber(self) -> Optional[int]:
        if "FractionNumber" in self._dataset:
            return self._dataset.FractionNumber
        return None

    @FractionNumber.setter
    def FractionNumber(self, value: Optional[int]):
        if value is None:
            if "FractionNumber" in self._dataset:
                del self._dataset.FractionNumber
        else:
            self._dataset.FractionNumber = value

    @property
    def ExposureSequence(self) -> Optional[List[ExposureSequenceItem]]:
        if "ExposureSequence" in self._dataset:
            if len(self._ExposureSequence) == len(self._dataset.ExposureSequence):
                return self._ExposureSequence
            else:
                return [ExposureSequenceItem(x) for x in self._dataset.ExposureSequence]
        return None

    @ExposureSequence.setter
    def ExposureSequence(self, value: Optional[List[ExposureSequenceItem]]):
        if value is None:
            self._ExposureSequence = []
            if "ExposureSequence" in self._dataset:
                del self._dataset.ExposureSequence
        elif not isinstance(value, list) or not all(isinstance(item, ExposureSequenceItem) for item in value):
            raise ValueError(f"ExposureSequence must be a list of ExposureSequenceItem objects")
        else:
            self._ExposureSequence = value
            if "ExposureSequence" not in self._dataset:
                self._dataset.ExposureSequence = pydicom.Sequence()
            self._dataset.ExposureSequence.clear()
            self._dataset.ExposureSequence.extend([item.to_dataset() for item in value])

    def add_Exposure(self, item: ExposureSequenceItem):
        if not isinstance(item, ExposureSequenceItem):
            raise ValueError(f"Item must be an instance of ExposureSequenceItem")
        self._ExposureSequence.append(item)
        if "ExposureSequence" not in self._dataset:
            self._dataset.ExposureSequence = pydicom.Sequence()
        self._dataset.ExposureSequence.append(item.to_dataset())
    ...
    @property
    def FluenceMapSequence(self) -> Optional[List[FluenceMapSequenceItem]]:
        if "FluenceMapSequence" in self._dataset:
            if len(self._FluenceMapSequence) == len(self._dataset.FluenceMapSequence):
                return self._FluenceMapSequence
            else:
                return [FluenceMapSequenceItem(x) for x in self._dataset.FluenceMapSequence]
        return None

    @FluenceMapSequence.setter
    def FluenceMapSequence(self, value: Optional[List[FluenceMapSequenceItem]]):
        if value is None:
            self._FluenceMapSequence = []
            if "FluenceMapSequence" in self._dataset:
                del self._dataset.FluenceMapSequence
        elif not isinstance(value, list) or not all(isinstance(item, FluenceMapSequenceItem) for item in value):
            raise ValueError(f"FluenceMapSequence must be a list of FluenceMapSequenceItem objects")
        else:
            self._FluenceMapSequence = value
            if "FluenceMapSequence" not in self._dataset:
                self._dataset.FluenceMapSequence = pydicom.Sequence()
            self._dataset.FluenceMapSequence.clear()
            self._dataset.FluenceMapSequence.extend([item.to_dataset() for item in value])

    def add_FluenceMap(self, item: FluenceMapSequenceItem):
        if not isinstance(item, FluenceMapSequenceItem):
            raise ValueError(f"Item must be an instance of FluenceMapSequenceItem")
        self._FluenceMapSequence.append(item)
        if "FluenceMapSequence" not in self._dataset:
            self._dataset.FluenceMapSequence = pydicom.Sequence()
        self._dataset.FluenceMapSequence.append(item.to_dataset())
