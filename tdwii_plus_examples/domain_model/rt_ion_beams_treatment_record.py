...
from .treatment_session_ion_beam_sequence_item import (
...
class RtIonBeamsTreatmentRecord:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._TreatmentSessionIonBeamSequence: List[TreatmentSessionIonBeamSequenceItem] = []
    ...
    @property
    def TreatmentSessionIonBeamSequence(self) -> Optional[List[TreatmentSessionIonBeamSequenceItem]]:
        if "TreatmentSessionIonBeamSequence" in self._dataset:
            if len(self._TreatmentSessionIonBeamSequence) == len(self._dataset.TreatmentSessionIonBeamSequence):
                return self._TreatmentSessionIonBeamSequence
            else:
                return [TreatmentSessionIonBeamSequenceItem(x) for x in self._dataset.TreatmentSessionIonBeamSequence]
        return None

    @TreatmentSessionIonBeamSequence.setter
    def TreatmentSessionIonBeamSequence(self, value: Optional[List[TreatmentSessionIonBeamSequenceItem]]):
        if value is None:
            self._TreatmentSessionIonBeamSequence = []
            if "TreatmentSessionIonBeamSequence" in self._dataset:
                del self._dataset.TreatmentSessionIonBeamSequence
        elif not isinstance(value, list) or not all(isinstance(item, TreatmentSessionIonBeamSequenceItem) for item in value):
            raise ValueError("TreatmentSessionIonBeamSequence must be a list of TreatmentSessionIonBeamSequenceItem objects")
        else:
            self._TreatmentSessionIonBeamSequence = value
            if "TreatmentSessionIonBeamSequence" not in self._dataset:
                self._dataset.TreatmentSessionIonBeamSequence = pydicom.Sequence()
            self._dataset.TreatmentSessionIonBeamSequence.clear()
            self._dataset.TreatmentSessionIonBeamSequence.extend([item.to_dataset() for item in value])

    def add_TreatmentSessionIonBeam(self, item: TreatmentSessionIonBeamSequenceItem):
        if not isinstance(item, TreatmentSessionIonBeamSequenceItem):
            raise ValueError("Item must be an instance of TreatmentSessionIonBeamSequenceItem")
        self._TreatmentSessionIonBeamSequence.append(item)
        if "TreatmentSessionIonBeamSequence" not in self._dataset:
            self._dataset.TreatmentSessionIonBeamSequence = pydicom.Sequence()
        self._dataset.TreatmentSessionIonBeamSequence.append(item.to_dataset())
