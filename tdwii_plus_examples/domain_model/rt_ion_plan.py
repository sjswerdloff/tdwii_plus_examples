...
from .ion_beam_sequence_item import IonBeamSequenceItem
from .ion_tolerance_table_sequence_item import IonToleranceTableSequenceItem
...
class RtIonPlan:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._IonBeamSequence: List[IonBeamSequenceItem] = []
        ...
        self._IonToleranceTableSequence: List[IonToleranceTableSequenceItem] = []
    ...
    @property
    def IonBeamSequence(self) -> Optional[List[IonBeamSequenceItem]]:
        if "IonBeamSequence" in self._dataset:
            if len(self._IonBeamSequence) == len(self._dataset.IonBeamSequence):
                return self._IonBeamSequence
            else:
                return [IonBeamSequenceItem(x) for x in self._dataset.IonBeamSequence]
        return None

    @IonBeamSequence.setter
    def IonBeamSequence(self, value: Optional[List[IonBeamSequenceItem]]):
        if value is None:
            self._IonBeamSequence = []
            if "IonBeamSequence" in self._dataset:
                del self._dataset.IonBeamSequence
        elif not isinstance(value, list) or not all(isinstance(item, IonBeamSequenceItem) for item in value):
            raise ValueError("IonBeamSequence must be a list of IonBeamSequenceItem objects")
        else:
            self._IonBeamSequence = value
            if "IonBeamSequence" not in self._dataset:
                self._dataset.IonBeamSequence = pydicom.Sequence()
            self._dataset.IonBeamSequence.clear()
            self._dataset.IonBeamSequence.extend([item.to_dataset() for item in value])

    def add_IonBeam(self, item: IonBeamSequenceItem):
        if not isinstance(item, IonBeamSequenceItem):
            raise ValueError("Item must be an instance of IonBeamSequenceItem")
        self._IonBeamSequence.append(item)
        if "IonBeamSequence" not in self._dataset:
            self._dataset.IonBeamSequence = pydicom.Sequence()
        self._dataset.IonBeamSequence.append(item.to_dataset())
    ...
    @property
    def IonToleranceTableSequence(self) -> Optional[List[IonToleranceTableSequenceItem]]:
        if "IonToleranceTableSequence" in self._dataset:
            if len(self._IonToleranceTableSequence) == len(self._dataset.IonToleranceTableSequence):
                return self._IonToleranceTableSequence
            else:
                return [IonToleranceTableSequenceItem(x) for x in self._dataset.IonToleranceTableSequence]
        return None

    @IonToleranceTableSequence.setter
    def IonToleranceTableSequence(self, value: Optional[List[IonToleranceTableSequenceItem]]):
        if value is None:
            self._IonToleranceTableSequence = []
            if "IonToleranceTableSequence" in self._dataset:
                del self._dataset.IonToleranceTableSequence
        elif not isinstance(value, list) or not all(isinstance(item, IonToleranceTableSequenceItem) for item in value):
            raise ValueError("IonToleranceTableSequence must be a list of IonToleranceTableSequenceItem objects")
        else:
            self._IonToleranceTableSequence = value
            if "IonToleranceTableSequence" not in self._dataset:
                self._dataset.IonToleranceTableSequence = pydicom.Sequence()
            self._dataset.IonToleranceTableSequence.clear()
            self._dataset.IonToleranceTableSequence.extend([item.to_dataset() for item in value])

    def add_IonToleranceTable(self, item: IonToleranceTableSequenceItem):
        if not isinstance(item, IonToleranceTableSequenceItem):
            raise ValueError("Item must be an instance of IonToleranceTableSequenceItem")
        self._IonToleranceTableSequence.append(item)
        if "IonToleranceTableSequence" not in self._dataset:
            self._dataset.IonToleranceTableSequence = pydicom.Sequence()
        self._dataset.IonToleranceTableSequence.append(item.to_dataset())
