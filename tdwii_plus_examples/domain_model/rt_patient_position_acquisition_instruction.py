...
from .acquisition_task_sequence_item import AcquisitionTaskSequenceItem
...
class RtPatientPositionAcquisitionInstruction:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._AcquisitionTaskSequence: List[AcquisitionTaskSequenceItem] = []
    ...
    @property
    def AcquisitionTaskSequence(self) -> Optional[List[AcquisitionTaskSequenceItem]]:
        if "AcquisitionTaskSequence" in self._dataset:
            if len(self._AcquisitionTaskSequence) == len(self._dataset.AcquisitionTaskSequence):
                return self._AcquisitionTaskSequence
            else:
                return [AcquisitionTaskSequenceItem(x) for x in self._dataset.AcquisitionTaskSequence]
        return None

    @AcquisitionTaskSequence.setter
    def AcquisitionTaskSequence(self, value: Optional[List[AcquisitionTaskSequenceItem]]):
        if value is None:
            self._AcquisitionTaskSequence = []
            if "AcquisitionTaskSequence" in self._dataset:
                del self._dataset.AcquisitionTaskSequence
        elif not isinstance(value, list) or not all(isinstance(item, AcquisitionTaskSequenceItem) for item in value):
            raise ValueError(f"AcquisitionTaskSequence must be a list of AcquisitionTaskSequenceItem objects")
        else:
            self._AcquisitionTaskSequence = value
            if "AcquisitionTaskSequence" not in self._dataset:
                self._dataset.AcquisitionTaskSequence = pydicom.Sequence()
            self._dataset.AcquisitionTaskSequence.clear()
            self._dataset.AcquisitionTaskSequence.extend([item.to_dataset() for item in value])

    def add_AcquisitionTask(self, item: AcquisitionTaskSequenceItem):
        if not isinstance(item, AcquisitionTaskSequenceItem):
            raise ValueError(f"Item must be an instance of AcquisitionTaskSequenceItem")
        self._AcquisitionTaskSequence.append(item)
        if "AcquisitionTaskSequence" not in self._dataset:
            self._dataset.AcquisitionTaskSequence = pydicom.Sequence()
        self._dataset.AcquisitionTaskSequence.append(item.to_dataset())
