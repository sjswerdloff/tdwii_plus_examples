...
from .dosimetric_objective_sequence_item import DosimetricObjectiveSequenceItem
...
from .intended_rt_treatment_phase_sequence_item import (
    IntendedRTTreatmentPhaseSequenceItem,
)
...
from .rt_physician_intent_sequence_item import RTPhysicianIntentSequenceItem
from .rt_prescription_sequence_item import RTPrescriptionSequenceItem
from .rt_treatment_phase_interval_sequence_item import (
    RTTreatmentPhaseIntervalSequenceItem,
)
...
class RtPhysicianIntent:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._RTPrescriptionSequence: List[RTPrescriptionSequenceItem] = []
        self._DosimetricObjectiveSequence: List[DosimetricObjectiveSequenceItem] = []
        ...
        self._IntendedRTTreatmentPhaseSequence: List[IntendedRTTreatmentPhaseSequenceItem] = []
        self._RTTreatmentPhaseIntervalSequence: List[RTTreatmentPhaseIntervalSequenceItem] = []
    ...
    @property
    def RTTreatmentPhaseIntentPresenceFlag(self) -> Optional[str]:
        if "RTTreatmentPhaseIntentPresenceFlag" in self._dataset:
            return self._dataset.RTTreatmentPhaseIntentPresenceFlag
        return None

    @RTTreatmentPhaseIntentPresenceFlag.setter
    def RTTreatmentPhaseIntentPresenceFlag(self, value: Optional[str]):
        if value is None:
            if "RTTreatmentPhaseIntentPresenceFlag" in self._dataset:
                del self._dataset.RTTreatmentPhaseIntentPresenceFlag
        else:
            self._dataset.RTTreatmentPhaseIntentPresenceFlag = value

    @property
    def RTPhysicianIntentSequence(self) -> Optional[List[RTPhysicianIntentSequenceItem]]:
        if "RTPhysicianIntentSequence" in self._dataset:
            if len(self._RTPhysicianIntentSequence) == len(self._dataset.RTPhysicianIntentSequence):
                return self._RTPhysicianIntentSequence
            else:
                return [RTPhysicianIntentSequenceItem(x) for x in self._dataset.RTPhysicianIntentSequence]
        return None

    @RTPhysicianIntentSequence.setter
    def RTPhysicianIntentSequence(self, value: Optional[List[RTPhysicianIntentSequenceItem]]):
        if value is None:
            self._RTPhysicianIntentSequence = []
            if "RTPhysicianIntentSequence" in self._dataset:
                del self._dataset.RTPhysicianIntentSequence
        elif not isinstance(value, list) or not all(isinstance(item, RTPhysicianIntentSequenceItem) for item in value):
            raise ValueError(f"RTPhysicianIntentSequence must be a list of RTPhysicianIntentSequenceItem objects")
        else:
            self._RTPhysicianIntentSequence = value
            if "RTPhysicianIntentSequence" not in self._dataset:
                self._dataset.RTPhysicianIntentSequence = pydicom.Sequence()
            self._dataset.RTPhysicianIntentSequence.clear()
            self._dataset.RTPhysicianIntentSequence.extend([item.to_dataset() for item in value])

    def add_RTPhysicianIntent(self, item: RTPhysicianIntentSequenceItem):
        if not isinstance(item, RTPhysicianIntentSequenceItem):
            raise ValueError(f"Item must be an instance of RTPhysicianIntentSequenceItem")
        self._RTPhysicianIntentSequence.append(item)
        if "RTPhysicianIntentSequence" not in self._dataset:
            self._dataset.RTPhysicianIntentSequence = pydicom.Sequence()
        self._dataset.RTPhysicianIntentSequence.append(item.to_dataset())
    ...
    @property
    def RTPrescriptionSequence(self) -> Optional[List[RTPrescriptionSequenceItem]]:
        if "RTPrescriptionSequence" in self._dataset:
            if len(self._RTPrescriptionSequence) == len(self._dataset.RTPrescriptionSequence):
                return self._RTPrescriptionSequence
            else:
                return [RTPrescriptionSequenceItem(x) for x in self._dataset.RTPrescriptionSequence]
        return None

    @RTPrescriptionSequence.setter
    def RTPrescriptionSequence(self, value: Optional[List[RTPrescriptionSequenceItem]]):
        if value is None:
            self._RTPrescriptionSequence = []
            if "RTPrescriptionSequence" in self._dataset:
                del self._dataset.RTPrescriptionSequence
        elif not isinstance(value, list) or not all(isinstance(item, RTPrescriptionSequenceItem) for item in value):
            raise ValueError(f"RTPrescriptionSequence must be a list of RTPrescriptionSequenceItem objects")
        else:
            self._RTPrescriptionSequence = value
            if "RTPrescriptionSequence" not in self._dataset:
                self._dataset.RTPrescriptionSequence = pydicom.Sequence()
            self._dataset.RTPrescriptionSequence.clear()
            self._dataset.RTPrescriptionSequence.extend([item.to_dataset() for item in value])

    def add_RTPrescription(self, item: RTPrescriptionSequenceItem):
        if not isinstance(item, RTPrescriptionSequenceItem):
            raise ValueError(f"Item must be an instance of RTPrescriptionSequenceItem")
        self._RTPrescriptionSequence.append(item)
        if "RTPrescriptionSequence" not in self._dataset:
            self._dataset.RTPrescriptionSequence = pydicom.Sequence()
        self._dataset.RTPrescriptionSequence.append(item.to_dataset())

    @property
    def DosimetricObjectiveSequence(self) -> Optional[List[DosimetricObjectiveSequenceItem]]:
        if "DosimetricObjectiveSequence" in self._dataset:
            if len(self._DosimetricObjectiveSequence) == len(self._dataset.DosimetricObjectiveSequence):
                return self._DosimetricObjectiveSequence
            else:
                return [DosimetricObjectiveSequenceItem(x) for x in self._dataset.DosimetricObjectiveSequence]
        return None

    @DosimetricObjectiveSequence.setter
    def DosimetricObjectiveSequence(self, value: Optional[List[DosimetricObjectiveSequenceItem]]):
        if value is None:
            self._DosimetricObjectiveSequence = []
            if "DosimetricObjectiveSequence" in self._dataset:
                del self._dataset.DosimetricObjectiveSequence
        elif not isinstance(value, list) or not all(isinstance(item, DosimetricObjectiveSequenceItem) for item in value):
            raise ValueError(f"DosimetricObjectiveSequence must be a list of DosimetricObjectiveSequenceItem objects")
        else:
            self._DosimetricObjectiveSequence = value
            if "DosimetricObjectiveSequence" not in self._dataset:
                self._dataset.DosimetricObjectiveSequence = pydicom.Sequence()
            self._dataset.DosimetricObjectiveSequence.clear()
            self._dataset.DosimetricObjectiveSequence.extend([item.to_dataset() for item in value])

    def add_DosimetricObjective(self, item: DosimetricObjectiveSequenceItem):
        if not isinstance(item, DosimetricObjectiveSequenceItem):
            raise ValueError(f"Item must be an instance of DosimetricObjectiveSequenceItem")
        self._DosimetricObjectiveSequence.append(item)
        if "DosimetricObjectiveSequence" not in self._dataset:
            self._dataset.DosimetricObjectiveSequence = pydicom.Sequence()
        self._dataset.DosimetricObjectiveSequence.append(item.to_dataset())
    ...
    @property
    def IntendedRTTreatmentPhaseSequence(self) -> Optional[List[IntendedRTTreatmentPhaseSequenceItem]]:
        if "IntendedRTTreatmentPhaseSequence" in self._dataset:
            if len(self._IntendedRTTreatmentPhaseSequence) == len(self._dataset.IntendedRTTreatmentPhaseSequence):
                return self._IntendedRTTreatmentPhaseSequence
            else:
                return [IntendedRTTreatmentPhaseSequenceItem(x) for x in self._dataset.IntendedRTTreatmentPhaseSequence]
        return None

    @IntendedRTTreatmentPhaseSequence.setter
    def IntendedRTTreatmentPhaseSequence(self, value: Optional[List[IntendedRTTreatmentPhaseSequenceItem]]):
        if value is None:
            self._IntendedRTTreatmentPhaseSequence = []
            if "IntendedRTTreatmentPhaseSequence" in self._dataset:
                del self._dataset.IntendedRTTreatmentPhaseSequence
        elif not isinstance(value, list) or not all(isinstance(item, IntendedRTTreatmentPhaseSequenceItem) for item in value):
            raise ValueError(
                f"IntendedRTTreatmentPhaseSequence must be a list of IntendedRTTreatmentPhaseSequenceItem objects"
            )
        else:
            self._IntendedRTTreatmentPhaseSequence = value
            if "IntendedRTTreatmentPhaseSequence" not in self._dataset:
                self._dataset.IntendedRTTreatmentPhaseSequence = pydicom.Sequence()
            self._dataset.IntendedRTTreatmentPhaseSequence.clear()
            self._dataset.IntendedRTTreatmentPhaseSequence.extend([item.to_dataset() for item in value])

    def add_IntendedRTTreatmentPhase(self, item: IntendedRTTreatmentPhaseSequenceItem):
        if not isinstance(item, IntendedRTTreatmentPhaseSequenceItem):
            raise ValueError(f"Item must be an instance of IntendedRTTreatmentPhaseSequenceItem")
        self._IntendedRTTreatmentPhaseSequence.append(item)
        if "IntendedRTTreatmentPhaseSequence" not in self._dataset:
            self._dataset.IntendedRTTreatmentPhaseSequence = pydicom.Sequence()
        self._dataset.IntendedRTTreatmentPhaseSequence.append(item.to_dataset())

    @property
    def RTTreatmentPhaseIntervalSequence(self) -> Optional[List[RTTreatmentPhaseIntervalSequenceItem]]:
        if "RTTreatmentPhaseIntervalSequence" in self._dataset:
            if len(self._RTTreatmentPhaseIntervalSequence) == len(self._dataset.RTTreatmentPhaseIntervalSequence):
                return self._RTTreatmentPhaseIntervalSequence
            else:
                return [RTTreatmentPhaseIntervalSequenceItem(x) for x in self._dataset.RTTreatmentPhaseIntervalSequence]
        return None

    @RTTreatmentPhaseIntervalSequence.setter
    def RTTreatmentPhaseIntervalSequence(self, value: Optional[List[RTTreatmentPhaseIntervalSequenceItem]]):
        if value is None:
            self._RTTreatmentPhaseIntervalSequence = []
            if "RTTreatmentPhaseIntervalSequence" in self._dataset:
                del self._dataset.RTTreatmentPhaseIntervalSequence
        elif not isinstance(value, list) or not all(isinstance(item, RTTreatmentPhaseIntervalSequenceItem) for item in value):
            raise ValueError(
                f"RTTreatmentPhaseIntervalSequence must be a list of RTTreatmentPhaseIntervalSequenceItem objects"
            )
        else:
            self._RTTreatmentPhaseIntervalSequence = value
            if "RTTreatmentPhaseIntervalSequence" not in self._dataset:
                self._dataset.RTTreatmentPhaseIntervalSequence = pydicom.Sequence()
            self._dataset.RTTreatmentPhaseIntervalSequence.clear()
            self._dataset.RTTreatmentPhaseIntervalSequence.extend([item.to_dataset() for item in value])

    def add_RTTreatmentPhaseInterval(self, item: RTTreatmentPhaseIntervalSequenceItem):
        if not isinstance(item, RTTreatmentPhaseIntervalSequenceItem):
            raise ValueError(f"Item must be an instance of RTTreatmentPhaseIntervalSequenceItem")
        self._RTTreatmentPhaseIntervalSequence.append(item)
        if "RTTreatmentPhaseIntervalSequence" not in self._dataset:
            self._dataset.RTTreatmentPhaseIntervalSequence = pydicom.Sequence()
        self._dataset.RTTreatmentPhaseIntervalSequence.append(item.to_dataset())
