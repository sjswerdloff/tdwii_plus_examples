...
class RTPhysicianIntentPredecessorSequenceItem:
    ...
    @property
    def ReasonForSuperseding(self) -> Optional[str]:
        if "ReasonForSuperseding" in self._dataset:
            return self._dataset.ReasonForSuperseding
        return None

    @ReasonForSuperseding.setter
    def ReasonForSuperseding(self, value: Optional[str]):
        if value is None:
            if "ReasonForSuperseding" in self._dataset:
                del self._dataset.ReasonForSuperseding
        else:
            self._dataset.ReasonForSuperseding = value
