...
from .application_setup_sequence_item import ApplicationSetupSequenceItem
from .beam_sequence_item import BeamSequenceItem
...
from .dose_reference_sequence_item import DoseReferenceSequenceItem
from .encrypted_attributes_sequence_item import EncryptedAttributesSequenceItem
from .fraction_group_sequence_item import FractionGroupSequenceItem
...
from .source_sequence_item import SourceSequenceItem
...
from .tolerance_table_sequence_item import ToleranceTableSequenceItem
...
class RtPlan:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._ToleranceTableSequence: List[ToleranceTableSequenceItem] = []
        ...
        self._SourceSequence: List[SourceSequenceItem] = []
        self._ApplicationSetupSequence: List[ApplicationSetupSequenceItem] = []
        ...
        self._BeamSequence: List[BeamSequenceItem] = []
        ...
        self._FractionGroupSequence: List[FractionGroupSequenceItem] = []
    ...
    @property
    def ToleranceTableSequence(self) -> Optional[List[ToleranceTableSequenceItem]]:
        if "ToleranceTableSequence" in self._dataset:
            if len(self._ToleranceTableSequence) == len(self._dataset.ToleranceTableSequence):
                return self._ToleranceTableSequence
            else:
                return [ToleranceTableSequenceItem(x) for x in self._dataset.ToleranceTableSequence]
        return None

    @ToleranceTableSequence.setter
    def ToleranceTableSequence(self, value: Optional[List[ToleranceTableSequenceItem]]):
        if value is None:
            self._ToleranceTableSequence = []
            if "ToleranceTableSequence" in self._dataset:
                del self._dataset.ToleranceTableSequence
        elif not isinstance(value, list) or not all(isinstance(item, ToleranceTableSequenceItem) for item in value):
            raise ValueError("ToleranceTableSequence must be a list of ToleranceTableSequenceItem objects")
        else:
            self._ToleranceTableSequence = value
            if "ToleranceTableSequence" not in self._dataset:
                self._dataset.ToleranceTableSequence = pydicom.Sequence()
            self._dataset.ToleranceTableSequence.clear()
            self._dataset.ToleranceTableSequence.extend([item.to_dataset() for item in value])

    def add_ToleranceTable(self, item: ToleranceTableSequenceItem):
        if not isinstance(item, ToleranceTableSequenceItem):
            raise ValueError("Item must be an instance of ToleranceTableSequenceItem")
        self._ToleranceTableSequence.append(item)
        if "ToleranceTableSequence" not in self._dataset:
            self._dataset.ToleranceTableSequence = pydicom.Sequence()
        self._dataset.ToleranceTableSequence.append(item.to_dataset())
    ...
    @property
    def SourceSequence(self) -> Optional[List[SourceSequenceItem]]:
        if "SourceSequence" in self._dataset:
            if len(self._SourceSequence) == len(self._dataset.SourceSequence):
                return self._SourceSequence
            else:
                return [SourceSequenceItem(x) for x in self._dataset.SourceSequence]
        return None

    @SourceSequence.setter
    def SourceSequence(self, value: Optional[List[SourceSequenceItem]]):
        if value is None:
            self._SourceSequence = []
            if "SourceSequence" in self._dataset:
                del self._dataset.SourceSequence
        elif not isinstance(value, list) or not all(isinstance(item, SourceSequenceItem) for item in value):
            raise ValueError("SourceSequence must be a list of SourceSequenceItem objects")
        else:
            self._SourceSequence = value
            if "SourceSequence" not in self._dataset:
                self._dataset.SourceSequence = pydicom.Sequence()
            self._dataset.SourceSequence.clear()
            self._dataset.SourceSequence.extend([item.to_dataset() for item in value])

    def add_Source(self, item: SourceSequenceItem):
        if not isinstance(item, SourceSequenceItem):
            raise ValueError("Item must be an instance of SourceSequenceItem")
        self._SourceSequence.append(item)
        if "SourceSequence" not in self._dataset:
            self._dataset.SourceSequence = pydicom.Sequence()
        self._dataset.SourceSequence.append(item.to_dataset())

    @property
    def ApplicationSetupSequence(self) -> Optional[List[ApplicationSetupSequenceItem]]:
        if "ApplicationSetupSequence" in self._dataset:
            if len(self._ApplicationSetupSequence) == len(self._dataset.ApplicationSetupSequence):
                return self._ApplicationSetupSequence
            else:
                return [ApplicationSetupSequenceItem(x) for x in self._dataset.ApplicationSetupSequence]
        return None

    @ApplicationSetupSequence.setter
    def ApplicationSetupSequence(self, value: Optional[List[ApplicationSetupSequenceItem]]):
        if value is None:
            self._ApplicationSetupSequence = []
            if "ApplicationSetupSequence" in self._dataset:
                del self._dataset.ApplicationSetupSequence
        elif not isinstance(value, list) or not all(isinstance(item, ApplicationSetupSequenceItem) for item in value):
            raise ValueError("ApplicationSetupSequence must be a list of ApplicationSetupSequenceItem objects")
        else:
            self._ApplicationSetupSequence = value
            if "ApplicationSetupSequence" not in self._dataset:
                self._dataset.ApplicationSetupSequence = pydicom.Sequence()
            self._dataset.ApplicationSetupSequence.clear()
            self._dataset.ApplicationSetupSequence.extend([item.to_dataset() for item in value])

    def add_ApplicationSetup(self, item: ApplicationSetupSequenceItem):
        if not isinstance(item, ApplicationSetupSequenceItem):
            raise ValueError("Item must be an instance of ApplicationSetupSequenceItem")
        self._ApplicationSetupSequence.append(item)
        if "ApplicationSetupSequence" not in self._dataset:
            self._dataset.ApplicationSetupSequence = pydicom.Sequence()
        self._dataset.ApplicationSetupSequence.append(item.to_dataset())
    ...
    @property
    def PrescriptionDescription(self) -> Optional[str]:
        if "PrescriptionDescription" in self._dataset:
            return self._dataset.PrescriptionDescription
        return None

    @PrescriptionDescription.setter
    def PrescriptionDescription(self, value: Optional[str]):
        if value is None:
            if "PrescriptionDescription" in self._dataset:
                del self._dataset.PrescriptionDescription
        else:
            self._dataset.PrescriptionDescription = value

    @property
    def DoseReferenceSequence(self) -> Optional[List[DoseReferenceSequenceItem]]:
        if "DoseReferenceSequence" in self._dataset:
            if len(self._DoseReferenceSequence) == len(self._dataset.DoseReferenceSequence):
                return self._DoseReferenceSequence
            else:
                return [DoseReferenceSequenceItem(x) for x in self._dataset.DoseReferenceSequence]
        return None

    @DoseReferenceSequence.setter
    def DoseReferenceSequence(self, value: Optional[List[DoseReferenceSequenceItem]]):
        if value is None:
            self._DoseReferenceSequence = []
            if "DoseReferenceSequence" in self._dataset:
                del self._dataset.DoseReferenceSequence
        elif not isinstance(value, list) or not all(isinstance(item, DoseReferenceSequenceItem) for item in value):
            raise ValueError("DoseReferenceSequence must be a list of DoseReferenceSequenceItem objects")
        else:
            self._DoseReferenceSequence = value
            if "DoseReferenceSequence" not in self._dataset:
                self._dataset.DoseReferenceSequence = pydicom.Sequence()
            self._dataset.DoseReferenceSequence.clear()
            self._dataset.DoseReferenceSequence.extend([item.to_dataset() for item in value])

    def add_DoseReference(self, item: DoseReferenceSequenceItem):
        if not isinstance(item, DoseReferenceSequenceItem):
            raise ValueError("Item must be an instance of DoseReferenceSequenceItem")
        self._DoseReferenceSequence.append(item)
        if "DoseReferenceSequence" not in self._dataset:
            self._dataset.DoseReferenceSequence = pydicom.Sequence()
        self._dataset.DoseReferenceSequence.append(item.to_dataset())

    @property
    def ApprovalStatus(self) -> Optional[str]:
        if "ApprovalStatus" in self._dataset:
            return self._dataset.ApprovalStatus
        return None

    @ApprovalStatus.setter
    def ApprovalStatus(self, value: Optional[str]):
        if value is None:
            if "ApprovalStatus" in self._dataset:
                del self._dataset.ApprovalStatus
        else:
            self._dataset.ApprovalStatus = value

    @property
    def ReviewDate(self) -> Optional[str]:
        if "ReviewDate" in self._dataset:
            return self._dataset.ReviewDate
        return None

    @ReviewDate.setter
    def ReviewDate(self, value: Optional[str]):
        if value is None:
            if "ReviewDate" in self._dataset:
                del self._dataset.ReviewDate
        else:
            self._dataset.ReviewDate = value

    @property
    def ReviewTime(self) -> Optional[str]:
        if "ReviewTime" in self._dataset:
            return self._dataset.ReviewTime
        return None

    @ReviewTime.setter
    def ReviewTime(self, value: Optional[str]):
        if value is None:
            if "ReviewTime" in self._dataset:
                del self._dataset.ReviewTime
        else:
            self._dataset.ReviewTime = value

    @property
    def ReviewerName(self) -> Optional[str]:
        if "ReviewerName" in self._dataset:
            return self._dataset.ReviewerName
        return None

    @ReviewerName.setter
    def ReviewerName(self, value: Optional[str]):
        if value is None:
            if "ReviewerName" in self._dataset:
                del self._dataset.ReviewerName
        else:
            self._dataset.ReviewerName = value
    ...
    @property
    def BeamSequence(self) -> Optional[List[BeamSequenceItem]]:
        if "BeamSequence" in self._dataset:
            if len(self._BeamSequence) == len(self._dataset.BeamSequence):
                return self._BeamSequence
            else:
                return [BeamSequenceItem(x) for x in self._dataset.BeamSequence]
        return None

    @BeamSequence.setter
    def BeamSequence(self, value: Optional[List[BeamSequenceItem]]):
        if value is None:
            self._BeamSequence = []
            if "BeamSequence" in self._dataset:
                del self._dataset.BeamSequence
        elif not isinstance(value, list) or not all(isinstance(item, BeamSequenceItem) for item in value):
            raise ValueError("BeamSequence must be a list of BeamSequenceItem objects")
        else:
            self._BeamSequence = value
            if "BeamSequence" not in self._dataset:
                self._dataset.BeamSequence = pydicom.Sequence()
            self._dataset.BeamSequence.clear()
            self._dataset.BeamSequence.extend([item.to_dataset() for item in value])

    def add_Beam(self, item: BeamSequenceItem):
        if not isinstance(item, BeamSequenceItem):
            raise ValueError("Item must be an instance of BeamSequenceItem")
        self._BeamSequence.append(item)
        if "BeamSequence" not in self._dataset:
            self._dataset.BeamSequence = pydicom.Sequence()
        self._dataset.BeamSequence.append(item.to_dataset())
    ...
    @property
    def FrameOfReferenceToDisplayedCoordinateSystemTransformationMatrix(self) -> Optional[List[float]]:
        if "FrameOfReferenceToDisplayedCoordinateSystemTransformationMatrix" in self._dataset:
            return self._dataset.FrameOfReferenceToDisplayedCoordinateSystemTransformationMatrix
        return None

    @FrameOfReferenceToDisplayedCoordinateSystemTransformationMatrix.setter
    def FrameOfReferenceToDisplayedCoordinateSystemTransformationMatrix(self, value: Optional[List[float]]):
        if value is None:
            if "FrameOfReferenceToDisplayedCoordinateSystemTransformationMatrix" in self._dataset:
                del self._dataset.FrameOfReferenceToDisplayedCoordinateSystemTransformationMatrix
        else:
            self._dataset.FrameOfReferenceToDisplayedCoordinateSystemTransformationMatrix = value
    ...
    @property
    def RTPlanName(self) -> Optional[str]:
        if "RTPlanName" in self._dataset:
            return self._dataset.RTPlanName
        return None

    @RTPlanName.setter
    def RTPlanName(self, value: Optional[str]):
        if value is None:
            if "RTPlanName" in self._dataset:
                del self._dataset.RTPlanName
        else:
            self._dataset.RTPlanName = value

    @property
    def RTPlanDescription(self) -> Optional[str]:
        if "RTPlanDescription" in self._dataset:
            return self._dataset.RTPlanDescription
        return None

    @RTPlanDescription.setter
    def RTPlanDescription(self, value: Optional[str]):
        if value is None:
            if "RTPlanDescription" in self._dataset:
                del self._dataset.RTPlanDescription
        else:
            self._dataset.RTPlanDescription = value

    @property
    def RTPlanDate(self) -> Optional[str]:
        if "RTPlanDate" in self._dataset:
            return self._dataset.RTPlanDate
        return None

    @RTPlanDate.setter
    def RTPlanDate(self, value: Optional[str]):
        if value is None:
            if "RTPlanDate" in self._dataset:
                del self._dataset.RTPlanDate
        else:
            self._dataset.RTPlanDate = value

    @property
    def RTPlanTime(self) -> Optional[str]:
        if "RTPlanTime" in self._dataset:
            return self._dataset.RTPlanTime
        return None

    @RTPlanTime.setter
    def RTPlanTime(self, value: Optional[str]):
        if value is None:
            if "RTPlanTime" in self._dataset:
                del self._dataset.RTPlanTime
        else:
            self._dataset.RTPlanTime = value

    @property
    def TreatmentProtocols(self) -> Optional[List[str]]:
        if "TreatmentProtocols" in self._dataset:
            return self._dataset.TreatmentProtocols
        return None

    @TreatmentProtocols.setter
    def TreatmentProtocols(self, value: Optional[List[str]]):
        if value is None:
            if "TreatmentProtocols" in self._dataset:
                del self._dataset.TreatmentProtocols
        else:
            self._dataset.TreatmentProtocols = value

    @property
    def PlanIntent(self) -> Optional[str]:
        if "PlanIntent" in self._dataset:
            return self._dataset.PlanIntent
        return None

    @PlanIntent.setter
    def PlanIntent(self, value: Optional[str]):
        if value is None:
            if "PlanIntent" in self._dataset:
                del self._dataset.PlanIntent
        else:
            self._dataset.PlanIntent = value

    @property
    def RTPlanGeometry(self) -> Optional[str]:
        if "RTPlanGeometry" in self._dataset:
            return self._dataset.RTPlanGeometry
        return None

    @RTPlanGeometry.setter
    def RTPlanGeometry(self, value: Optional[str]):
        if value is None:
            if "RTPlanGeometry" in self._dataset:
                del self._dataset.RTPlanGeometry
        else:
            self._dataset.RTPlanGeometry = value
    ...
    @property
    def FractionGroupSequence(self) -> Optional[List[FractionGroupSequenceItem]]:
        if "FractionGroupSequence" in self._dataset:
            if len(self._FractionGroupSequence) == len(self._dataset.FractionGroupSequence):
                return self._FractionGroupSequence
            else:
                return [FractionGroupSequenceItem(x) for x in self._dataset.FractionGroupSequence]
        return None

    @FractionGroupSequence.setter
    def FractionGroupSequence(self, value: Optional[List[FractionGroupSequenceItem]]):
        if value is None:
            self._FractionGroupSequence = []
            if "FractionGroupSequence" in self._dataset:
                del self._dataset.FractionGroupSequence
        elif not isinstance(value, list) or not all(isinstance(item, FractionGroupSequenceItem) for item in value):
            raise ValueError("FractionGroupSequence must be a list of FractionGroupSequenceItem objects")
        else:
            self._FractionGroupSequence = value
            if "FractionGroupSequence" not in self._dataset:
                self._dataset.FractionGroupSequence = pydicom.Sequence()
            self._dataset.FractionGroupSequence.clear()
            self._dataset.FractionGroupSequence.extend([item.to_dataset() for item in value])

    def add_FractionGroup(self, item: FractionGroupSequenceItem):
        if not isinstance(item, FractionGroupSequenceItem):
            raise ValueError("Item must be an instance of FractionGroupSequenceItem")
        self._FractionGroupSequence.append(item)
        if "FractionGroupSequence" not in self._dataset:
            self._dataset.FractionGroupSequence = pydicom.Sequence()
        self._dataset.FractionGroupSequence.append(item.to_dataset())
