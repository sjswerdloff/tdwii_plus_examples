...
from .rt_radiation_salvage_record_control_point_sequence_item import (
    RTRadiationSalvageRecordControlPointSequenceItem,
)
...
class RtRadiationSalvageRecord:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        self._dataset = dataset if dataset is not None else pydicom.Dataset()
        self._RadiationGenerationModeSequence: List[RadiationGenerationModeSequenceItem] = []
        self._RTRadiationSalvageRecordControlPointSequence: List[RTRadiationSalvageRecordControlPointSequenceItem] = []
    ...
    @property
    def RTRadiationSalvageRecordControlPointSequence(self) -> Optional[List[RTRadiationSalvageRecordControlPointSequenceItem]]:
        if "RTRadiationSalvageRecordControlPointSequence" in self._dataset:
            if len(self._RTRadiationSalvageRecordControlPointSequence) == len(
                self._dataset.RTRadiationSalvageRecordControlPointSequence
            ):
                return self._RTRadiationSalvageRecordControlPointSequence
            else:
                return [
                    RTRadiationSalvageRecordControlPointSequenceItem(x)
                    for x in self._dataset.RTRadiationSalvageRecordControlPointSequence
                ]
        return None

    @RTRadiationSalvageRecordControlPointSequence.setter
    def RTRadiationSalvageRecordControlPointSequence(
        self, value: Optional[List[RTRadiationSalvageRecordControlPointSequenceItem]]
    ):
        if value is None:
            self._RTRadiationSalvageRecordControlPointSequence = []
            if "RTRadiationSalvageRecordControlPointSequence" in self._dataset:
                del self._dataset.RTRadiationSalvageRecordControlPointSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, RTRadiationSalvageRecordControlPointSequenceItem) for item in value
        ):
            raise ValueError(
                f"RTRadiationSalvageRecordControlPointSequence must be a list of RTRadiationSalvageRecordControlPointSequenceItem objects"
            )
        else:
            self._RTRadiationSalvageRecordControlPointSequence = value
            if "RTRadiationSalvageRecordControlPointSequence" not in self._dataset:
                self._dataset.RTRadiationSalvageRecordControlPointSequence = pydicom.Sequence()
            self._dataset.RTRadiationSalvageRecordControlPointSequence.clear()
            self._dataset.RTRadiationSalvageRecordControlPointSequence.extend([item.to_dataset() for item in value])

    def add_RTRadiationSalvageRecordControlPoint(self, item: RTRadiationSalvageRecordControlPointSequenceItem):
        if not isinstance(item, RTRadiationSalvageRecordControlPointSequenceItem):
            raise ValueError(f"Item must be an instance of RTRadiationSalvageRecordControlPointSequenceItem")
        self._RTRadiationSalvageRecordControlPointSequence.append(item)
        if "RTRadiationSalvageRecordControlPointSequence" not in self._dataset:
            self._dataset.RTRadiationSalvageRecordControlPointSequence = pydicom.Sequence()
        self._dataset.RTRadiationSalvageRecordControlPointSequence.append(item.to_dataset())

    @property
    def StartingMetersetValueKnownFlag(self) -> Optional[str]:
        if "StartingMetersetValueKnownFlag" in self._dataset:
            return self._dataset.StartingMetersetValueKnownFlag
        return None

    @StartingMetersetValueKnownFlag.setter
    def StartingMetersetValueKnownFlag(self, value: Optional[str]):
        if value is None:
            if "StartingMetersetValueKnownFlag" in self._dataset:
                del self._dataset.StartingMetersetValueKnownFlag
        else:
            self._dataset.StartingMetersetValueKnownFlag = value
