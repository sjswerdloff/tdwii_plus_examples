...
from .radiation_dose_identification_sequence_item import (
    RadiationDoseIdentificationSequenceItem,
)
from .radiation_dose_sequence_item import RadiationDoseSequenceItem
...
from .referenced_rt_physician_intent_sequence_item import (
    ReferencedRTPhysicianIntentSequenceItem,
)
...
from .rt_radiation_sequence_item import RTRadiationSequenceItem
...
from .treatment_position_group_sequence_item import TreatmentPositionGroupSequenceItem
from .udi_sequence_item import UDISequenceItem


class RtRadiationSet:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._RadiationDoseSequence: List[RadiationDoseSequenceItem] = []
        self._RadiationDoseIdentificationSequence: List[RadiationDoseIdentificationSequenceItem] = []
        ...
        self._TreatmentPositionGroupSequence: List[TreatmentPositionGroupSequenceItem] = []
        self._RTRadiationSequence: List[RTRadiationSequenceItem] = []
        self._ReferencedRTPhysicianIntentSequence: List[ReferencedRTPhysicianIntentSequenceItem] = []
    ...
    @property
    def RadiationDoseSequence(self) -> Optional[List[RadiationDoseSequenceItem]]:
        if "RadiationDoseSequence" in self._dataset:
            if len(self._RadiationDoseSequence) == len(self._dataset.RadiationDoseSequence):
                return self._RadiationDoseSequence
            else:
                return [RadiationDoseSequenceItem(x) for x in self._dataset.RadiationDoseSequence]
        return None

    @RadiationDoseSequence.setter
    def RadiationDoseSequence(self, value: Optional[List[RadiationDoseSequenceItem]]):
        if value is None:
            self._RadiationDoseSequence = []
            if "RadiationDoseSequence" in self._dataset:
                del self._dataset.RadiationDoseSequence
        elif not isinstance(value, list) or not all(isinstance(item, RadiationDoseSequenceItem) for item in value):
            raise ValueError(f"RadiationDoseSequence must be a list of RadiationDoseSequenceItem objects")
        else:
            self._RadiationDoseSequence = value
            if "RadiationDoseSequence" not in self._dataset:
                self._dataset.RadiationDoseSequence = pydicom.Sequence()
            self._dataset.RadiationDoseSequence.clear()
            self._dataset.RadiationDoseSequence.extend([item.to_dataset() for item in value])

    def add_RadiationDose(self, item: RadiationDoseSequenceItem):
        if not isinstance(item, RadiationDoseSequenceItem):
            raise ValueError(f"Item must be an instance of RadiationDoseSequenceItem")
        self._RadiationDoseSequence.append(item)
        if "RadiationDoseSequence" not in self._dataset:
            self._dataset.RadiationDoseSequence = pydicom.Sequence()
        self._dataset.RadiationDoseSequence.append(item.to_dataset())

    @property
    def RadiationDoseIdentificationSequence(self) -> Optional[List[RadiationDoseIdentificationSequenceItem]]:
        if "RadiationDoseIdentificationSequence" in self._dataset:
            if len(self._RadiationDoseIdentificationSequence) == len(self._dataset.RadiationDoseIdentificationSequence):
                return self._RadiationDoseIdentificationSequence
            else:
                return [RadiationDoseIdentificationSequenceItem(x) for x in self._dataset.RadiationDoseIdentificationSequence]
        return None

    @RadiationDoseIdentificationSequence.setter
    def RadiationDoseIdentificationSequence(self, value: Optional[List[RadiationDoseIdentificationSequenceItem]]):
        if value is None:
            self._RadiationDoseIdentificationSequence = []
            if "RadiationDoseIdentificationSequence" in self._dataset:
                del self._dataset.RadiationDoseIdentificationSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, RadiationDoseIdentificationSequenceItem) for item in value
        ):
            raise ValueError(
                f"RadiationDoseIdentificationSequence must be a list of RadiationDoseIdentificationSequenceItem objects"
            )
        else:
            self._RadiationDoseIdentificationSequence = value
            if "RadiationDoseIdentificationSequence" not in self._dataset:
                self._dataset.RadiationDoseIdentificationSequence = pydicom.Sequence()
            self._dataset.RadiationDoseIdentificationSequence.clear()
            self._dataset.RadiationDoseIdentificationSequence.extend([item.to_dataset() for item in value])

    def add_RadiationDoseIdentification(self, item: RadiationDoseIdentificationSequenceItem):
        if not isinstance(item, RadiationDoseIdentificationSequenceItem):
            raise ValueError(f"Item must be an instance of RadiationDoseIdentificationSequenceItem")
        self._RadiationDoseIdentificationSequence.append(item)
        if "RadiationDoseIdentificationSequence" not in self._dataset:
            self._dataset.RadiationDoseIdentificationSequence = pydicom.Sequence()
        self._dataset.RadiationDoseIdentificationSequence.append(item.to_dataset())
    ...
    @property
    def TreatmentPositionGroupSequence(self) -> Optional[List[TreatmentPositionGroupSequenceItem]]:
        if "TreatmentPositionGroupSequence" in self._dataset:
            if len(self._TreatmentPositionGroupSequence) == len(self._dataset.TreatmentPositionGroupSequence):
                return self._TreatmentPositionGroupSequence
            else:
                return [TreatmentPositionGroupSequenceItem(x) for x in self._dataset.TreatmentPositionGroupSequence]
        return None

    @TreatmentPositionGroupSequence.setter
    def TreatmentPositionGroupSequence(self, value: Optional[List[TreatmentPositionGroupSequenceItem]]):
        if value is None:
            self._TreatmentPositionGroupSequence = []
            if "TreatmentPositionGroupSequence" in self._dataset:
                del self._dataset.TreatmentPositionGroupSequence
        elif not isinstance(value, list) or not all(isinstance(item, TreatmentPositionGroupSequenceItem) for item in value):
            raise ValueError(f"TreatmentPositionGroupSequence must be a list of TreatmentPositionGroupSequenceItem objects")
        else:
            self._TreatmentPositionGroupSequence = value
            if "TreatmentPositionGroupSequence" not in self._dataset:
                self._dataset.TreatmentPositionGroupSequence = pydicom.Sequence()
            self._dataset.TreatmentPositionGroupSequence.clear()
            self._dataset.TreatmentPositionGroupSequence.extend([item.to_dataset() for item in value])

    def add_TreatmentPositionGroup(self, item: TreatmentPositionGroupSequenceItem):
        if not isinstance(item, TreatmentPositionGroupSequenceItem):
            raise ValueError(f"Item must be an instance of TreatmentPositionGroupSequenceItem")
        self._TreatmentPositionGroupSequence.append(item)
        if "TreatmentPositionGroupSequence" not in self._dataset:
            self._dataset.TreatmentPositionGroupSequence = pydicom.Sequence()
        self._dataset.TreatmentPositionGroupSequence.append(item.to_dataset())

    @property
    def RTRadiationSequence(self) -> Optional[List[RTRadiationSequenceItem]]:
        if "RTRadiationSequence" in self._dataset:
            if len(self._RTRadiationSequence) == len(self._dataset.RTRadiationSequence):
                return self._RTRadiationSequence
            else:
                return [RTRadiationSequenceItem(x) for x in self._dataset.RTRadiationSequence]
        return None

    @RTRadiationSequence.setter
    def RTRadiationSequence(self, value: Optional[List[RTRadiationSequenceItem]]):
        if value is None:
            self._RTRadiationSequence = []
            if "RTRadiationSequence" in self._dataset:
                del self._dataset.RTRadiationSequence
        elif not isinstance(value, list) or not all(isinstance(item, RTRadiationSequenceItem) for item in value):
            raise ValueError(f"RTRadiationSequence must be a list of RTRadiationSequenceItem objects")
        else:
            self._RTRadiationSequence = value
            if "RTRadiationSequence" not in self._dataset:
                self._dataset.RTRadiationSequence = pydicom.Sequence()
            self._dataset.RTRadiationSequence.clear()
            self._dataset.RTRadiationSequence.extend([item.to_dataset() for item in value])

    def add_RTRadiation(self, item: RTRadiationSequenceItem):
        if not isinstance(item, RTRadiationSequenceItem):
            raise ValueError(f"Item must be an instance of RTRadiationSequenceItem")
        self._RTRadiationSequence.append(item)
        if "RTRadiationSequence" not in self._dataset:
            self._dataset.RTRadiationSequence = pydicom.Sequence()
        self._dataset.RTRadiationSequence.append(item.to_dataset())

    @property
    def IntendedNumberOfFractions(self) -> Optional[int]:
        if "IntendedNumberOfFractions" in self._dataset:
            return self._dataset.IntendedNumberOfFractions
        return None

    @IntendedNumberOfFractions.setter
    def IntendedNumberOfFractions(self, value: Optional[int]):
        if value is None:
            if "IntendedNumberOfFractions" in self._dataset:
                del self._dataset.IntendedNumberOfFractions
        else:
            self._dataset.IntendedNumberOfFractions = value

    @property
    def RTRadiationSetIntent(self) -> Optional[str]:
        if "RTRadiationSetIntent" in self._dataset:
            return self._dataset.RTRadiationSetIntent
        return None

    @RTRadiationSetIntent.setter
    def RTRadiationSetIntent(self, value: Optional[str]):
        if value is None:
            if "RTRadiationSetIntent" in self._dataset:
                del self._dataset.RTRadiationSetIntent
        else:
            self._dataset.RTRadiationSetIntent = value

    @property
    def ReferencedRTPhysicianIntentSequence(self) -> Optional[List[ReferencedRTPhysicianIntentSequenceItem]]:
        if "ReferencedRTPhysicianIntentSequence" in self._dataset:
            if len(self._ReferencedRTPhysicianIntentSequence) == len(self._dataset.ReferencedRTPhysicianIntentSequence):
                return self._ReferencedRTPhysicianIntentSequence
            else:
                return [ReferencedRTPhysicianIntentSequenceItem(x) for x in self._dataset.ReferencedRTPhysicianIntentSequence]
        return None

    @ReferencedRTPhysicianIntentSequence.setter
    def ReferencedRTPhysicianIntentSequence(self, value: Optional[List[ReferencedRTPhysicianIntentSequenceItem]]):
        if value is None:
            self._ReferencedRTPhysicianIntentSequence = []
            if "ReferencedRTPhysicianIntentSequence" in self._dataset:
                del self._dataset.ReferencedRTPhysicianIntentSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, ReferencedRTPhysicianIntentSequenceItem) for item in value
        ):
            raise ValueError(
                f"ReferencedRTPhysicianIntentSequence must be a list of ReferencedRTPhysicianIntentSequenceItem objects"
            )
        else:
            self._ReferencedRTPhysicianIntentSequence = value
            if "ReferencedRTPhysicianIntentSequence" not in self._dataset:
                self._dataset.ReferencedRTPhysicianIntentSequence = pydicom.Sequence()
            self._dataset.ReferencedRTPhysicianIntentSequence.clear()
            self._dataset.ReferencedRTPhysicianIntentSequence.extend([item.to_dataset() for item in value])

    def add_ReferencedRTPhysicianIntent(self, item: ReferencedRTPhysicianIntentSequenceItem):
        if not isinstance(item, ReferencedRTPhysicianIntentSequenceItem):
            raise ValueError(f"Item must be an instance of ReferencedRTPhysicianIntentSequenceItem")
        self._ReferencedRTPhysicianIntentSequence.append(item)
        if "ReferencedRTPhysicianIntentSequence" not in self._dataset:
            self._dataset.ReferencedRTPhysicianIntentSequence = pydicom.Sequence()
        self._dataset.ReferencedRTPhysicianIntentSequence.append(item.to_dataset())
    ...
    def FractionPatternSequence(self, value: Optional[List[FractionPatternSequenceItem]]):
        if value is None:
            ...
            raise ValueError(f"FractionPatternSequence must be a list of FractionPatternSequenceItem objects")
        else:
    ...
    def add_FractionPattern(self, item: FractionPatternSequenceItem):
        if not isinstance(item, FractionPatternSequenceItem):
            raise ValueError(f"Item must be an instance of FractionPatternSequenceItem")
