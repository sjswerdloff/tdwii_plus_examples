...
from .rt_radiation_task_sequence_item import RTRadiationTaskSequenceItem
...
class RtRadiationSetDeliveryInstruction:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._OmittedRadiationSequence: List[OmittedRadiationSequenceItem] = []
        self._RTRadiationTaskSequence: List[RTRadiationTaskSequenceItem] = []
    ...
    def ReferencedRTRadiationSetSequence(self, value: Optional[List[ReferencedRTRadiationSetSequenceItem]]):
        if value is None:
            ...
            raise ValueError("ReferencedRTRadiationSetSequence must be a list of ReferencedRTRadiationSetSequenceItem objects")
        else:
    ...
    def add_ReferencedRTRadiationSet(self, item: ReferencedRTRadiationSetSequenceItem):
        if not isinstance(item, ReferencedRTRadiationSetSequenceItem):
            raise ValueError("Item must be an instance of ReferencedRTRadiationSetSequenceItem")
    ...
    @property
    def OmittedRadiationSequence(self) -> Optional[List[OmittedRadiationSequenceItem]]:
        if "OmittedRadiationSequence" in self._dataset:
            if len(self._OmittedRadiationSequence) == len(self._dataset.OmittedRadiationSequence):
                return self._OmittedRadiationSequence
            else:
                return [OmittedRadiationSequenceItem(x) for x in self._dataset.OmittedRadiationSequence]
        return None

    @OmittedRadiationSequence.setter
    def OmittedRadiationSequence(self, value: Optional[List[OmittedRadiationSequenceItem]]):
        if value is None:
            self._OmittedRadiationSequence = []
            if "OmittedRadiationSequence" in self._dataset:
                del self._dataset.OmittedRadiationSequence
        elif not isinstance(value, list) or not all(isinstance(item, OmittedRadiationSequenceItem) for item in value):
            raise ValueError("OmittedRadiationSequence must be a list of OmittedRadiationSequenceItem objects")
        else:
            self._OmittedRadiationSequence = value
            if "OmittedRadiationSequence" not in self._dataset:
                self._dataset.OmittedRadiationSequence = pydicom.Sequence()
            self._dataset.OmittedRadiationSequence.clear()
            self._dataset.OmittedRadiationSequence.extend([item.to_dataset() for item in value])

    def add_OmittedRadiation(self, item: OmittedRadiationSequenceItem):
        if not isinstance(item, OmittedRadiationSequenceItem):
            raise ValueError("Item must be an instance of OmittedRadiationSequenceItem")
        self._OmittedRadiationSequence.append(item)
        if "OmittedRadiationSequence" not in self._dataset:
            self._dataset.OmittedRadiationSequence = pydicom.Sequence()
        self._dataset.OmittedRadiationSequence.append(item.to_dataset())

    @property
    def RTRadiationTaskSequence(self) -> Optional[List[RTRadiationTaskSequenceItem]]:
        if "RTRadiationTaskSequence" in self._dataset:
            if len(self._RTRadiationTaskSequence) == len(self._dataset.RTRadiationTaskSequence):
                return self._RTRadiationTaskSequence
            else:
                return [RTRadiationTaskSequenceItem(x) for x in self._dataset.RTRadiationTaskSequence]
        return None

    @RTRadiationTaskSequence.setter
    def RTRadiationTaskSequence(self, value: Optional[List[RTRadiationTaskSequenceItem]]):
        if value is None:
            self._RTRadiationTaskSequence = []
            if "RTRadiationTaskSequence" in self._dataset:
                del self._dataset.RTRadiationTaskSequence
        elif not isinstance(value, list) or not all(isinstance(item, RTRadiationTaskSequenceItem) for item in value):
            raise ValueError("RTRadiationTaskSequence must be a list of RTRadiationTaskSequenceItem objects")
        else:
            self._RTRadiationTaskSequence = value
            if "RTRadiationTaskSequence" not in self._dataset:
                self._dataset.RTRadiationTaskSequence = pydicom.Sequence()
            self._dataset.RTRadiationTaskSequence.clear()
            self._dataset.RTRadiationTaskSequence.extend([item.to_dataset() for item in value])

    def add_RTRadiationTask(self, item: RTRadiationTaskSequenceItem):
        if not isinstance(item, RTRadiationTaskSequenceItem):
            raise ValueError("Item must be an instance of RTRadiationTaskSequenceItem")
        self._RTRadiationTaskSequence.append(item)
        if "RTRadiationTaskSequence" not in self._dataset:
            self._dataset.RTRadiationTaskSequence = pydicom.Sequence()
        self._dataset.RTRadiationTaskSequence.append(item.to_dataset())

    @property
    def RTRadiationSetDeliveryUsage(self) -> Optional[str]:
        if "RTRadiationSetDeliveryUsage" in self._dataset:
            return self._dataset.RTRadiationSetDeliveryUsage
        return None

    @RTRadiationSetDeliveryUsage.setter
    def RTRadiationSetDeliveryUsage(self, value: Optional[str]):
        if value is None:
            if "RTRadiationSetDeliveryUsage" in self._dataset:
                del self._dataset.RTRadiationSetDeliveryUsage
        else:
            self._dataset.RTRadiationSetDeliveryUsage = value
