...
from .rt_segment_annotation_sequence_item import RTSegmentAnnotationSequenceItem
from .segment_reference_sequence_item import SegmentReferenceSequenceItem
...
class RtSegmentAnnotation:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._SegmentReferenceSequence: List[SegmentReferenceSequenceItem] = []
        self._ContentCreatorIdentificationCodeSequence: List[CodeSequenceItem] = []
        self._RTSegmentAnnotationSequence: List[RTSegmentAnnotationSequenceItem] = []
    ...
    @property
    def SegmentReferenceSequence(self) -> Optional[List[SegmentReferenceSequenceItem]]:
        if "SegmentReferenceSequence" in self._dataset:
            if len(self._SegmentReferenceSequence) == len(self._dataset.SegmentReferenceSequence):
                return self._SegmentReferenceSequence
            else:
                return [SegmentReferenceSequenceItem(x) for x in self._dataset.SegmentReferenceSequence]
        return None

    @SegmentReferenceSequence.setter
    def SegmentReferenceSequence(self, value: Optional[List[SegmentReferenceSequenceItem]]):
        if value is None:
            self._SegmentReferenceSequence = []
            if "SegmentReferenceSequence" in self._dataset:
                del self._dataset.SegmentReferenceSequence
        elif not isinstance(value, list) or not all(isinstance(item, SegmentReferenceSequenceItem) for item in value):
            raise ValueError(f"SegmentReferenceSequence must be a list of SegmentReferenceSequenceItem objects")
        else:
            self._SegmentReferenceSequence = value
            if "SegmentReferenceSequence" not in self._dataset:
                self._dataset.SegmentReferenceSequence = pydicom.Sequence()
            self._dataset.SegmentReferenceSequence.clear()
            self._dataset.SegmentReferenceSequence.extend([item.to_dataset() for item in value])

    def add_SegmentReference(self, item: SegmentReferenceSequenceItem):
        if not isinstance(item, SegmentReferenceSequenceItem):
            raise ValueError(f"Item must be an instance of SegmentReferenceSequenceItem")
        self._SegmentReferenceSequence.append(item)
        if "SegmentReferenceSequence" not in self._dataset:
            self._dataset.SegmentReferenceSequence = pydicom.Sequence()
        self._dataset.SegmentReferenceSequence.append(item.to_dataset())
    ...
    @property
    def RTSegmentAnnotationSequence(self) -> Optional[List[RTSegmentAnnotationSequenceItem]]:
        if "RTSegmentAnnotationSequence" in self._dataset:
            if len(self._RTSegmentAnnotationSequence) == len(self._dataset.RTSegmentAnnotationSequence):
                return self._RTSegmentAnnotationSequence
            else:
                return [RTSegmentAnnotationSequenceItem(x) for x in self._dataset.RTSegmentAnnotationSequence]
        return None

    @RTSegmentAnnotationSequence.setter
    def RTSegmentAnnotationSequence(self, value: Optional[List[RTSegmentAnnotationSequenceItem]]):
        if value is None:
            self._RTSegmentAnnotationSequence = []
            if "RTSegmentAnnotationSequence" in self._dataset:
                del self._dataset.RTSegmentAnnotationSequence
        elif not isinstance(value, list) or not all(isinstance(item, RTSegmentAnnotationSequenceItem) for item in value):
            raise ValueError(f"RTSegmentAnnotationSequence must be a list of RTSegmentAnnotationSequenceItem objects")
        else:
            self._RTSegmentAnnotationSequence = value
            if "RTSegmentAnnotationSequence" not in self._dataset:
                self._dataset.RTSegmentAnnotationSequence = pydicom.Sequence()
            self._dataset.RTSegmentAnnotationSequence.clear()
            self._dataset.RTSegmentAnnotationSequence.extend([item.to_dataset() for item in value])

    def add_RTSegmentAnnotation(self, item: RTSegmentAnnotationSequenceItem):
        if not isinstance(item, RTSegmentAnnotationSequenceItem):
            raise ValueError(f"Item must be an instance of RTSegmentAnnotationSequenceItem")
        self._RTSegmentAnnotationSequence.append(item)
        if "RTSegmentAnnotationSequence" not in self._dataset:
            self._dataset.RTSegmentAnnotationSequence = pydicom.Sequence()
        self._dataset.RTSegmentAnnotationSequence.append(item.to_dataset())
