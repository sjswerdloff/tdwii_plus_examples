...
from .referenced_frame_of_reference_sequence_item import (
...
from .roi_contour_sequence_item import ROIContourSequenceItem
from .rtroi_observations_sequence_item import RTROIObservationsSequenceItem
...
from .source_series_information_sequence_item import SourceSeriesInformationSequenceItem
from .strain_stock_sequence_item import StrainStockSequenceItem
from .structure_set_roi_sequence_item import StructureSetROISequenceItem
...
class RtStructureSet:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._PredecessorStructureSetSequence: List[PredecessorStructureSetSequenceItem] = []
        self._StructureSetROISequence: List[StructureSetROISequenceItem] = []
        self._SourceSeriesInformationSequence: List[SourceSeriesInformationSequenceItem] = []
        ...
        self._ROIContourSequence: List[ROIContourSequenceItem] = []
        ...
        self._RTROIObservationsSequence: List[RTROIObservationsSequenceItem] = []
    ...
    @property
    def StructureSetLabel(self) -> Optional[str]:
        if "StructureSetLabel" in self._dataset:
            return self._dataset.StructureSetLabel
        return None

    @StructureSetLabel.setter
    def StructureSetLabel(self, value: Optional[str]):
        if value is None:
            if "StructureSetLabel" in self._dataset:
                del self._dataset.StructureSetLabel
        else:
            self._dataset.StructureSetLabel = value

    @property
    def StructureSetName(self) -> Optional[str]:
        if "StructureSetName" in self._dataset:
            return self._dataset.StructureSetName
        return None

    @StructureSetName.setter
    def StructureSetName(self, value: Optional[str]):
        if value is None:
            if "StructureSetName" in self._dataset:
                del self._dataset.StructureSetName
        else:
            self._dataset.StructureSetName = value

    @property
    def StructureSetDescription(self) -> Optional[str]:
        if "StructureSetDescription" in self._dataset:
            return self._dataset.StructureSetDescription
        return None

    @StructureSetDescription.setter
    def StructureSetDescription(self, value: Optional[str]):
        if value is None:
            if "StructureSetDescription" in self._dataset:
                del self._dataset.StructureSetDescription
        else:
            self._dataset.StructureSetDescription = value

    @property
    def StructureSetDate(self) -> Optional[str]:
        if "StructureSetDate" in self._dataset:
            return self._dataset.StructureSetDate
        return None

    @StructureSetDate.setter
    def StructureSetDate(self, value: Optional[str]):
        if value is None:
            if "StructureSetDate" in self._dataset:
                del self._dataset.StructureSetDate
        else:
            self._dataset.StructureSetDate = value

    @property
    def StructureSetTime(self) -> Optional[str]:
        if "StructureSetTime" in self._dataset:
            return self._dataset.StructureSetTime
        return None

    @StructureSetTime.setter
    def StructureSetTime(self, value: Optional[str]):
        if value is None:
            if "StructureSetTime" in self._dataset:
                del self._dataset.StructureSetTime
        else:
            self._dataset.StructureSetTime = value

    @property
    def ReferencedFrameOfReferenceSequence(self) -> Optional[List[ReferencedFrameOfReferenceSequenceItem]]:
        if "ReferencedFrameOfReferenceSequence" in self._dataset:
            if len(self._ReferencedFrameOfReferenceSequence) == len(self._dataset.ReferencedFrameOfReferenceSequence):
                return self._ReferencedFrameOfReferenceSequence
            else:
                return [ReferencedFrameOfReferenceSequenceItem(x) for x in self._dataset.ReferencedFrameOfReferenceSequence]
        return None
    ...
    def ReferencedFrameOfReferenceSequence(self, value: Optional[List[ReferencedFrameOfReferenceSequenceItem]]):
        if value is None:
            self._ReferencedFrameOfReferenceSequence = []
            if "ReferencedFrameOfReferenceSequence" in self._dataset:
                del self._dataset.ReferencedFrameOfReferenceSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, ReferencedFrameOfReferenceSequenceItem) for item in value
        ):
            raise ValueError(
                f"ReferencedFrameOfReferenceSequence must be a list of ReferencedFrameOfReferenceSequenceItem objects"
            )
        else:
            self._ReferencedFrameOfReferenceSequence = value
            if "ReferencedFrameOfReferenceSequence" not in self._dataset:
                self._dataset.ReferencedFrameOfReferenceSequence = pydicom.Sequence()
            self._dataset.ReferencedFrameOfReferenceSequence.clear()
            self._dataset.ReferencedFrameOfReferenceSequence.extend([item.to_dataset() for item in value])

    def add_ReferencedFrameOfReference(self, item: ReferencedFrameOfReferenceSequenceItem):
        if not isinstance(item, ReferencedFrameOfReferenceSequenceItem):
            raise ValueError(f"Item must be an instance of ReferencedFrameOfReferenceSequenceItem")
        self._ReferencedFrameOfReferenceSequence.append(item)
        if "ReferencedFrameOfReferenceSequence" not in self._dataset:
            self._dataset.ReferencedFrameOfReferenceSequence = pydicom.Sequence()
        self._dataset.ReferencedFrameOfReferenceSequence.append(item.to_dataset())

    @property
    def PredecessorStructureSetSequence(self) -> Optional[List[PredecessorStructureSetSequenceItem]]:
        if "PredecessorStructureSetSequence" in self._dataset:
            if len(self._PredecessorStructureSetSequence) == len(self._dataset.PredecessorStructureSetSequence):
                return self._PredecessorStructureSetSequence
            else:
                return [PredecessorStructureSetSequenceItem(x) for x in self._dataset.PredecessorStructureSetSequence]
        return None

    @PredecessorStructureSetSequence.setter
    def PredecessorStructureSetSequence(self, value: Optional[List[PredecessorStructureSetSequenceItem]]):
        if value is None:
            self._PredecessorStructureSetSequence = []
            if "PredecessorStructureSetSequence" in self._dataset:
                del self._dataset.PredecessorStructureSetSequence
        elif not isinstance(value, list) or not all(isinstance(item, PredecessorStructureSetSequenceItem) for item in value):
            raise ValueError(f"PredecessorStructureSetSequence must be a list of PredecessorStructureSetSequenceItem objects")
        else:
            self._PredecessorStructureSetSequence = value
            if "PredecessorStructureSetSequence" not in self._dataset:
                self._dataset.PredecessorStructureSetSequence = pydicom.Sequence()
            self._dataset.PredecessorStructureSetSequence.clear()
            self._dataset.PredecessorStructureSetSequence.extend([item.to_dataset() for item in value])

    def add_PredecessorStructureSet(self, item: PredecessorStructureSetSequenceItem):
        if not isinstance(item, PredecessorStructureSetSequenceItem):
            raise ValueError(f"Item must be an instance of PredecessorStructureSetSequenceItem")
        self._PredecessorStructureSetSequence.append(item)
        if "PredecessorStructureSetSequence" not in self._dataset:
            self._dataset.PredecessorStructureSetSequence = pydicom.Sequence()
        self._dataset.PredecessorStructureSetSequence.append(item.to_dataset())

    @property
    def StructureSetROISequence(self) -> Optional[List[StructureSetROISequenceItem]]:
        if "StructureSetROISequence" in self._dataset:
            if len(self._StructureSetROISequence) == len(self._dataset.StructureSetROISequence):
                return self._StructureSetROISequence
            else:
                return [StructureSetROISequenceItem(x) for x in self._dataset.StructureSetROISequence]
        return None

    @StructureSetROISequence.setter
    def StructureSetROISequence(self, value: Optional[List[StructureSetROISequenceItem]]):
        if value is None:
            self._StructureSetROISequence = []
            if "StructureSetROISequence" in self._dataset:
                del self._dataset.StructureSetROISequence
        elif not isinstance(value, list) or not all(isinstance(item, StructureSetROISequenceItem) for item in value):
            raise ValueError(f"StructureSetROISequence must be a list of StructureSetROISequenceItem objects")
        else:
            self._StructureSetROISequence = value
            if "StructureSetROISequence" not in self._dataset:
                self._dataset.StructureSetROISequence = pydicom.Sequence()
            self._dataset.StructureSetROISequence.clear()
            self._dataset.StructureSetROISequence.extend([item.to_dataset() for item in value])

    def add_StructureSetROI(self, item: StructureSetROISequenceItem):
        if not isinstance(item, StructureSetROISequenceItem):
            raise ValueError(f"Item must be an instance of StructureSetROISequenceItem")
        self._StructureSetROISequence.append(item)
        if "StructureSetROISequence" not in self._dataset:
            self._dataset.StructureSetROISequence = pydicom.Sequence()
        self._dataset.StructureSetROISequence.append(item.to_dataset())

    @property
    def SourceSeriesInformationSequence(self) -> Optional[List[SourceSeriesInformationSequenceItem]]:
        if "SourceSeriesInformationSequence" in self._dataset:
            if len(self._SourceSeriesInformationSequence) == len(self._dataset.SourceSeriesInformationSequence):
                return self._SourceSeriesInformationSequence
            else:
                return [SourceSeriesInformationSequenceItem(x) for x in self._dataset.SourceSeriesInformationSequence]
        return None

    @SourceSeriesInformationSequence.setter
    def SourceSeriesInformationSequence(self, value: Optional[List[SourceSeriesInformationSequenceItem]]):
        if value is None:
            self._SourceSeriesInformationSequence = []
            if "SourceSeriesInformationSequence" in self._dataset:
                del self._dataset.SourceSeriesInformationSequence
        elif not isinstance(value, list) or not all(isinstance(item, SourceSeriesInformationSequenceItem) for item in value):
            raise ValueError(f"SourceSeriesInformationSequence must be a list of SourceSeriesInformationSequenceItem objects")
        else:
            self._SourceSeriesInformationSequence = value
            if "SourceSeriesInformationSequence" not in self._dataset:
                self._dataset.SourceSeriesInformationSequence = pydicom.Sequence()
            self._dataset.SourceSeriesInformationSequence.clear()
            self._dataset.SourceSeriesInformationSequence.extend([item.to_dataset() for item in value])

    def add_SourceSeriesInformation(self, item: SourceSeriesInformationSequenceItem):
        if not isinstance(item, SourceSeriesInformationSequenceItem):
            raise ValueError(f"Item must be an instance of SourceSeriesInformationSequenceItem")
        self._SourceSeriesInformationSequence.append(item)
        if "SourceSeriesInformationSequence" not in self._dataset:
            self._dataset.SourceSeriesInformationSequence = pydicom.Sequence()
        self._dataset.SourceSeriesInformationSequence.append(item.to_dataset())
    ...
    @property
    def ROIContourSequence(self) -> Optional[List[ROIContourSequenceItem]]:
        if "ROIContourSequence" in self._dataset:
            if len(self._ROIContourSequence) == len(self._dataset.ROIContourSequence):
                return self._ROIContourSequence
            else:
                return [ROIContourSequenceItem(x) for x in self._dataset.ROIContourSequence]
        return None

    @ROIContourSequence.setter
    def ROIContourSequence(self, value: Optional[List[ROIContourSequenceItem]]):
        if value is None:
            self._ROIContourSequence = []
            if "ROIContourSequence" in self._dataset:
                del self._dataset.ROIContourSequence
        elif not isinstance(value, list) or not all(isinstance(item, ROIContourSequenceItem) for item in value):
            raise ValueError(f"ROIContourSequence must be a list of ROIContourSequenceItem objects")
        else:
            self._ROIContourSequence = value
            if "ROIContourSequence" not in self._dataset:
                self._dataset.ROIContourSequence = pydicom.Sequence()
            self._dataset.ROIContourSequence.clear()
            self._dataset.ROIContourSequence.extend([item.to_dataset() for item in value])

    def add_ROIContour(self, item: ROIContourSequenceItem):
        if not isinstance(item, ROIContourSequenceItem):
            raise ValueError(f"Item must be an instance of ROIContourSequenceItem")
        self._ROIContourSequence.append(item)
        if "ROIContourSequence" not in self._dataset:
            self._dataset.ROIContourSequence = pydicom.Sequence()
        self._dataset.ROIContourSequence.append(item.to_dataset())
    ...
    @property
    def RTROIObservationsSequence(self) -> Optional[List[RTROIObservationsSequenceItem]]:
        if "RTROIObservationsSequence" in self._dataset:
            if len(self._RTROIObservationsSequence) == len(self._dataset.RTROIObservationsSequence):
                return self._RTROIObservationsSequence
            else:
                return [RTROIObservationsSequenceItem(x) for x in self._dataset.RTROIObservationsSequence]
        return None

    @RTROIObservationsSequence.setter
    def RTROIObservationsSequence(self, value: Optional[List[RTROIObservationsSequenceItem]]):
        if value is None:
            self._RTROIObservationsSequence = []
            if "RTROIObservationsSequence" in self._dataset:
                del self._dataset.RTROIObservationsSequence
        elif not isinstance(value, list) or not all(isinstance(item, RTROIObservationsSequenceItem) for item in value):
            raise ValueError(f"RTROIObservationsSequence must be a list of RTROIObservationsSequenceItem objects")
        else:
            self._RTROIObservationsSequence = value
            if "RTROIObservationsSequence" not in self._dataset:
                self._dataset.RTROIObservationsSequence = pydicom.Sequence()
            self._dataset.RTROIObservationsSequence.clear()
            self._dataset.RTROIObservationsSequence.extend([item.to_dataset() for item in value])

    def add_RTROIObservations(self, item: RTROIObservationsSequenceItem):
        if not isinstance(item, RTROIObservationsSequenceItem):
            raise ValueError(f"Item must be an instance of RTROIObservationsSequenceItem")
        self._RTROIObservationsSequence.append(item)
        if "RTROIObservationsSequence" not in self._dataset:
            self._dataset.RTROIObservationsSequence = pydicom.Sequence()
        self._dataset.RTROIObservationsSequence.append(item.to_dataset())
