...
from .patient_treatment_preparation_procedure_sequence_item import (
    PatientTreatmentPreparationProcedureSequenceItem,
)
...
from .referenced_patient_setup_photo_sequence_item import (
    ReferencedPatientSetupPhotoSequenceItem,
)
...
from .rt_patient_position_scope_sequence_item import RTPatientPositionScopeSequenceItem
from .rt_treatment_preparation_patient_position_sequence_item import (
    RTTreatmentPreparationPatientPositionSequenceItem,
)
...
class RtTreatmentPreparation:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._RTPatientPositionScopeSequence: List[RTPatientPositionScopeSequenceItem] = []
        self._RTTreatmentPreparationPatientPositionSequence: List[RTTreatmentPreparationPatientPositionSequenceItem] = []
        self._ReferencedPatientSetupPhotoSequence: List[ReferencedPatientSetupPhotoSequenceItem] = []
        self._PatientTreatmentPreparationMethodCodeSequence: List[CodeSequenceItem] = []
        self._PatientTreatmentPreparationProcedureSequence: List[PatientTreatmentPreparationProcedureSequenceItem] = []
    ...
    @property
    def RTPatientPositionScopeSequence(self) -> Optional[List[RTPatientPositionScopeSequenceItem]]:
        if "RTPatientPositionScopeSequence" in self._dataset:
            if len(self._RTPatientPositionScopeSequence) == len(self._dataset.RTPatientPositionScopeSequence):
                return self._RTPatientPositionScopeSequence
            else:
                return [RTPatientPositionScopeSequenceItem(x) for x in self._dataset.RTPatientPositionScopeSequence]
        return None

    @RTPatientPositionScopeSequence.setter
    def RTPatientPositionScopeSequence(self, value: Optional[List[RTPatientPositionScopeSequenceItem]]):
        if value is None:
            self._RTPatientPositionScopeSequence = []
            if "RTPatientPositionScopeSequence" in self._dataset:
                del self._dataset.RTPatientPositionScopeSequence
        elif not isinstance(value, list) or not all(isinstance(item, RTPatientPositionScopeSequenceItem) for item in value):
            raise ValueError(f"RTPatientPositionScopeSequence must be a list of RTPatientPositionScopeSequenceItem objects")
        else:
            self._RTPatientPositionScopeSequence = value
            if "RTPatientPositionScopeSequence" not in self._dataset:
                self._dataset.RTPatientPositionScopeSequence = pydicom.Sequence()
            self._dataset.RTPatientPositionScopeSequence.clear()
            self._dataset.RTPatientPositionScopeSequence.extend([item.to_dataset() for item in value])

    def add_RTPatientPositionScope(self, item: RTPatientPositionScopeSequenceItem):
        if not isinstance(item, RTPatientPositionScopeSequenceItem):
            raise ValueError(f"Item must be an instance of RTPatientPositionScopeSequenceItem")
        self._RTPatientPositionScopeSequence.append(item)
        if "RTPatientPositionScopeSequence" not in self._dataset:
            self._dataset.RTPatientPositionScopeSequence = pydicom.Sequence()
        self._dataset.RTPatientPositionScopeSequence.append(item.to_dataset())

    @property
    def RTTreatmentPreparationPatientPositionSequence(
        self,
    ) -> Optional[List[RTTreatmentPreparationPatientPositionSequenceItem]]:
        if "RTTreatmentPreparationPatientPositionSequence" in self._dataset:
            if len(self._RTTreatmentPreparationPatientPositionSequence) == len(
                self._dataset.RTTreatmentPreparationPatientPositionSequence
            ):
                return self._RTTreatmentPreparationPatientPositionSequence
            else:
                return [
                    RTTreatmentPreparationPatientPositionSequenceItem(x)
                    for x in self._dataset.RTTreatmentPreparationPatientPositionSequence
                ]
        return None

    @RTTreatmentPreparationPatientPositionSequence.setter
    def RTTreatmentPreparationPatientPositionSequence(
        self, value: Optional[List[RTTreatmentPreparationPatientPositionSequenceItem]]
    ):
        if value is None:
            self._RTTreatmentPreparationPatientPositionSequence = []
            if "RTTreatmentPreparationPatientPositionSequence" in self._dataset:
                del self._dataset.RTTreatmentPreparationPatientPositionSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, RTTreatmentPreparationPatientPositionSequenceItem) for item in value
        ):
            raise ValueError(
                f"RTTreatmentPreparationPatientPositionSequence must be a list of RTTreatmentPreparationPatientPositionSequenceItem objects"
            )
        else:
            self._RTTreatmentPreparationPatientPositionSequence = value
            if "RTTreatmentPreparationPatientPositionSequence" not in self._dataset:
                self._dataset.RTTreatmentPreparationPatientPositionSequence = pydicom.Sequence()
            self._dataset.RTTreatmentPreparationPatientPositionSequence.clear()
            self._dataset.RTTreatmentPreparationPatientPositionSequence.extend([item.to_dataset() for item in value])

    def add_RTTreatmentPreparationPatientPosition(self, item: RTTreatmentPreparationPatientPositionSequenceItem):
        if not isinstance(item, RTTreatmentPreparationPatientPositionSequenceItem):
            raise ValueError(f"Item must be an instance of RTTreatmentPreparationPatientPositionSequenceItem")
        self._RTTreatmentPreparationPatientPositionSequence.append(item)
        if "RTTreatmentPreparationPatientPositionSequence" not in self._dataset:
            self._dataset.RTTreatmentPreparationPatientPositionSequence = pydicom.Sequence()
        self._dataset.RTTreatmentPreparationPatientPositionSequence.append(item.to_dataset())

    @property
    def ReferencedPatientSetupPhotoSequence(self) -> Optional[List[ReferencedPatientSetupPhotoSequenceItem]]:
        if "ReferencedPatientSetupPhotoSequence" in self._dataset:
            if len(self._ReferencedPatientSetupPhotoSequence) == len(self._dataset.ReferencedPatientSetupPhotoSequence):
                return self._ReferencedPatientSetupPhotoSequence
            else:
                return [ReferencedPatientSetupPhotoSequenceItem(x) for x in self._dataset.ReferencedPatientSetupPhotoSequence]
        return None

    @ReferencedPatientSetupPhotoSequence.setter
    def ReferencedPatientSetupPhotoSequence(self, value: Optional[List[ReferencedPatientSetupPhotoSequenceItem]]):
        if value is None:
            self._ReferencedPatientSetupPhotoSequence = []
            if "ReferencedPatientSetupPhotoSequence" in self._dataset:
                del self._dataset.ReferencedPatientSetupPhotoSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, ReferencedPatientSetupPhotoSequenceItem) for item in value
        ):
            raise ValueError(
                f"ReferencedPatientSetupPhotoSequence must be a list of ReferencedPatientSetupPhotoSequenceItem objects"
            )
        else:
            self._ReferencedPatientSetupPhotoSequence = value
            if "ReferencedPatientSetupPhotoSequence" not in self._dataset:
                self._dataset.ReferencedPatientSetupPhotoSequence = pydicom.Sequence()
            self._dataset.ReferencedPatientSetupPhotoSequence.clear()
            self._dataset.ReferencedPatientSetupPhotoSequence.extend([item.to_dataset() for item in value])

    def add_ReferencedPatientSetupPhoto(self, item: ReferencedPatientSetupPhotoSequenceItem):
        if not isinstance(item, ReferencedPatientSetupPhotoSequenceItem):
            raise ValueError(f"Item must be an instance of ReferencedPatientSetupPhotoSequenceItem")
        self._ReferencedPatientSetupPhotoSequence.append(item)
        if "ReferencedPatientSetupPhotoSequence" not in self._dataset:
            self._dataset.ReferencedPatientSetupPhotoSequence = pydicom.Sequence()
        self._dataset.ReferencedPatientSetupPhotoSequence.append(item.to_dataset())

    @property
    def PatientTreatmentPreparationMethodCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "PatientTreatmentPreparationMethodCodeSequence" in self._dataset:
            if len(self._PatientTreatmentPreparationMethodCodeSequence) == len(
                self._dataset.PatientTreatmentPreparationMethodCodeSequence
            ):
                return self._PatientTreatmentPreparationMethodCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.PatientTreatmentPreparationMethodCodeSequence]
        return None

    @PatientTreatmentPreparationMethodCodeSequence.setter
    def PatientTreatmentPreparationMethodCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._PatientTreatmentPreparationMethodCodeSequence = []
            if "PatientTreatmentPreparationMethodCodeSequence" in self._dataset:
                del self._dataset.PatientTreatmentPreparationMethodCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"PatientTreatmentPreparationMethodCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._PatientTreatmentPreparationMethodCodeSequence = value
            if "PatientTreatmentPreparationMethodCodeSequence" not in self._dataset:
                self._dataset.PatientTreatmentPreparationMethodCodeSequence = pydicom.Sequence()
            self._dataset.PatientTreatmentPreparationMethodCodeSequence.clear()
            self._dataset.PatientTreatmentPreparationMethodCodeSequence.extend([item.to_dataset() for item in value])

    def add_PatientTreatmentPreparationMethodCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._PatientTreatmentPreparationMethodCodeSequence.append(item)
        if "PatientTreatmentPreparationMethodCodeSequence" not in self._dataset:
            self._dataset.PatientTreatmentPreparationMethodCodeSequence = pydicom.Sequence()
        self._dataset.PatientTreatmentPreparationMethodCodeSequence.append(item.to_dataset())

    @property
    def PatientTreatmentPreparationProcedureSequence(self) -> Optional[List[PatientTreatmentPreparationProcedureSequenceItem]]:
        if "PatientTreatmentPreparationProcedureSequence" in self._dataset:
            if len(self._PatientTreatmentPreparationProcedureSequence) == len(
                self._dataset.PatientTreatmentPreparationProcedureSequence
            ):
                return self._PatientTreatmentPreparationProcedureSequence
            else:
                return [
                    PatientTreatmentPreparationProcedureSequenceItem(x)
                    for x in self._dataset.PatientTreatmentPreparationProcedureSequence
                ]
        return None

    @PatientTreatmentPreparationProcedureSequence.setter
    def PatientTreatmentPreparationProcedureSequence(
        self, value: Optional[List[PatientTreatmentPreparationProcedureSequenceItem]]
    ):
        if value is None:
            self._PatientTreatmentPreparationProcedureSequence = []
            if "PatientTreatmentPreparationProcedureSequence" in self._dataset:
                del self._dataset.PatientTreatmentPreparationProcedureSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, PatientTreatmentPreparationProcedureSequenceItem) for item in value
        ):
            raise ValueError(
                f"PatientTreatmentPreparationProcedureSequence must be a list of PatientTreatmentPreparationProcedureSequenceItem objects"
            )
        else:
            self._PatientTreatmentPreparationProcedureSequence = value
            if "PatientTreatmentPreparationProcedureSequence" not in self._dataset:
                self._dataset.PatientTreatmentPreparationProcedureSequence = pydicom.Sequence()
            self._dataset.PatientTreatmentPreparationProcedureSequence.clear()
            self._dataset.PatientTreatmentPreparationProcedureSequence.extend([item.to_dataset() for item in value])

    def add_PatientTreatmentPreparationProcedure(self, item: PatientTreatmentPreparationProcedureSequenceItem):
        if not isinstance(item, PatientTreatmentPreparationProcedureSequenceItem):
            raise ValueError(f"Item must be an instance of PatientTreatmentPreparationProcedureSequenceItem")
        self._PatientTreatmentPreparationProcedureSequence.append(item)
        if "PatientTreatmentPreparationProcedureSequence" not in self._dataset:
            self._dataset.PatientTreatmentPreparationProcedureSequence = pydicom.Sequence()
        self._dataset.PatientTreatmentPreparationProcedureSequence.append(item.to_dataset())

    @property
    def PatientTreatmentPreparationMethodDescription(self) -> Optional[str]:
        if "PatientTreatmentPreparationMethodDescription" in self._dataset:
            return self._dataset.PatientTreatmentPreparationMethodDescription
        return None

    @PatientTreatmentPreparationMethodDescription.setter
    def PatientTreatmentPreparationMethodDescription(self, value: Optional[str]):
        if value is None:
            if "PatientTreatmentPreparationMethodDescription" in self._dataset:
                del self._dataset.PatientTreatmentPreparationMethodDescription
        else:
            self._dataset.PatientTreatmentPreparationMethodDescription = value
