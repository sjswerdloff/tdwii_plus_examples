...
from .segment_sequence_item import SegmentSequenceItem
...
from .total_pixel_matrix_origin_sequence_item import TotalPixelMatrixOriginSequenceItem
...
class Segmentation:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._TotalPixelMatrixOriginSequence: List[TotalPixelMatrixOriginSequenceItem] = []
        self._ConceptNameCodeSequence: List[CodeSequenceItem] = []
        self._SegmentSequence: List[SegmentSequenceItem] = []
    ...
    @property
    def TotalPixelMatrixColumns(self) -> Optional[int]:
        if "TotalPixelMatrixColumns" in self._dataset:
            return self._dataset.TotalPixelMatrixColumns
        return None

    @TotalPixelMatrixColumns.setter
    def TotalPixelMatrixColumns(self, value: Optional[int]):
        if value is None:
            if "TotalPixelMatrixColumns" in self._dataset:
                del self._dataset.TotalPixelMatrixColumns
        else:
            self._dataset.TotalPixelMatrixColumns = value

    @property
    def TotalPixelMatrixRows(self) -> Optional[int]:
        if "TotalPixelMatrixRows" in self._dataset:
            return self._dataset.TotalPixelMatrixRows
        return None

    @TotalPixelMatrixRows.setter
    def TotalPixelMatrixRows(self, value: Optional[int]):
        if value is None:
            if "TotalPixelMatrixRows" in self._dataset:
                del self._dataset.TotalPixelMatrixRows
        else:
            self._dataset.TotalPixelMatrixRows = value

    @property
    def TotalPixelMatrixOriginSequence(self) -> Optional[List[TotalPixelMatrixOriginSequenceItem]]:
        if "TotalPixelMatrixOriginSequence" in self._dataset:
            if len(self._TotalPixelMatrixOriginSequence) == len(self._dataset.TotalPixelMatrixOriginSequence):
                return self._TotalPixelMatrixOriginSequence
            else:
                return [TotalPixelMatrixOriginSequenceItem(x) for x in self._dataset.TotalPixelMatrixOriginSequence]
        return None

    @TotalPixelMatrixOriginSequence.setter
    def TotalPixelMatrixOriginSequence(self, value: Optional[List[TotalPixelMatrixOriginSequenceItem]]):
        if value is None:
            self._TotalPixelMatrixOriginSequence = []
            if "TotalPixelMatrixOriginSequence" in self._dataset:
                del self._dataset.TotalPixelMatrixOriginSequence
        elif not isinstance(value, list) or not all(isinstance(item, TotalPixelMatrixOriginSequenceItem) for item in value):
            raise ValueError(f"TotalPixelMatrixOriginSequence must be a list of TotalPixelMatrixOriginSequenceItem objects")
        else:
            self._TotalPixelMatrixOriginSequence = value
            if "TotalPixelMatrixOriginSequence" not in self._dataset:
                self._dataset.TotalPixelMatrixOriginSequence = pydicom.Sequence()
            self._dataset.TotalPixelMatrixOriginSequence.clear()
            self._dataset.TotalPixelMatrixOriginSequence.extend([item.to_dataset() for item in value])

    def add_TotalPixelMatrixOrigin(self, item: TotalPixelMatrixOriginSequenceItem):
        if not isinstance(item, TotalPixelMatrixOriginSequenceItem):
            raise ValueError(f"Item must be an instance of TotalPixelMatrixOriginSequenceItem")
        self._TotalPixelMatrixOriginSequence.append(item)
        if "TotalPixelMatrixOriginSequence" not in self._dataset:
            self._dataset.TotalPixelMatrixOriginSequence = pydicom.Sequence()
        self._dataset.TotalPixelMatrixOriginSequence.append(item.to_dataset())

    @property
    def ImageOrientationSlide(self) -> Optional[List[Decimal]]:
        if "ImageOrientationSlide" in self._dataset:
            return self._dataset.ImageOrientationSlide
        return None

    @ImageOrientationSlide.setter
    def ImageOrientationSlide(self, value: Optional[List[Decimal]]):
        if value is None:
            if "ImageOrientationSlide" in self._dataset:
                del self._dataset.ImageOrientationSlide
        else:
            self._dataset.ImageOrientationSlide = value

    @property
    def TotalPixelMatrixFocalPlanes(self) -> Optional[int]:
        if "TotalPixelMatrixFocalPlanes" in self._dataset:
            return self._dataset.TotalPixelMatrixFocalPlanes
        return None

    @TotalPixelMatrixFocalPlanes.setter
    def TotalPixelMatrixFocalPlanes(self, value: Optional[int]):
        if value is None:
            if "TotalPixelMatrixFocalPlanes" in self._dataset:
                del self._dataset.TotalPixelMatrixFocalPlanes
        else:
            self._dataset.TotalPixelMatrixFocalPlanes = value
    ...
    @property
    def SegmentationType(self) -> Optional[str]:
        if "SegmentationType" in self._dataset:
            return self._dataset.SegmentationType
        return None

    @SegmentationType.setter
    def SegmentationType(self, value: Optional[str]):
        if value is None:
            if "SegmentationType" in self._dataset:
                del self._dataset.SegmentationType
        else:
            self._dataset.SegmentationType = value

    @property
    def SegmentSequence(self) -> Optional[List[SegmentSequenceItem]]:
        if "SegmentSequence" in self._dataset:
            if len(self._SegmentSequence) == len(self._dataset.SegmentSequence):
                return self._SegmentSequence
            else:
                return [SegmentSequenceItem(x) for x in self._dataset.SegmentSequence]
        return None

    @SegmentSequence.setter
    def SegmentSequence(self, value: Optional[List[SegmentSequenceItem]]):
        if value is None:
            self._SegmentSequence = []
            if "SegmentSequence" in self._dataset:
                del self._dataset.SegmentSequence
        elif not isinstance(value, list) or not all(isinstance(item, SegmentSequenceItem) for item in value):
            raise ValueError(f"SegmentSequence must be a list of SegmentSequenceItem objects")
        else:
            self._SegmentSequence = value
            if "SegmentSequence" not in self._dataset:
                self._dataset.SegmentSequence = pydicom.Sequence()
            self._dataset.SegmentSequence.clear()
            self._dataset.SegmentSequence.extend([item.to_dataset() for item in value])

    def add_Segment(self, item: SegmentSequenceItem):
        if not isinstance(item, SegmentSequenceItem):
            raise ValueError(f"Item must be an instance of SegmentSequenceItem")
        self._SegmentSequence.append(item)
        if "SegmentSequence" not in self._dataset:
            self._dataset.SegmentSequence = pydicom.Sequence()
        self._dataset.SegmentSequence.append(item.to_dataset())

    @property
    def MaximumFractionalValue(self) -> Optional[int]:
        if "MaximumFractionalValue" in self._dataset:
            return self._dataset.MaximumFractionalValue
        return None

    @MaximumFractionalValue.setter
    def MaximumFractionalValue(self, value: Optional[int]):
        if value is None:
            if "MaximumFractionalValue" in self._dataset:
                del self._dataset.MaximumFractionalValue
        else:
            self._dataset.MaximumFractionalValue = value

    @property
    def SegmentationFractionalType(self) -> Optional[str]:
        if "SegmentationFractionalType" in self._dataset:
            return self._dataset.SegmentationFractionalType
        return None

    @SegmentationFractionalType.setter
    def SegmentationFractionalType(self, value: Optional[str]):
        if value is None:
            if "SegmentationFractionalType" in self._dataset:
                del self._dataset.SegmentationFractionalType
        else:
            self._dataset.SegmentationFractionalType = value

    @property
    def SegmentsOverlap(self) -> Optional[str]:
        if "SegmentsOverlap" in self._dataset:
            return self._dataset.SegmentsOverlap
        return None

    @SegmentsOverlap.setter
    def SegmentsOverlap(self, value: Optional[str]):
        if value is None:
            if "SegmentsOverlap" in self._dataset:
                del self._dataset.SegmentsOverlap
        else:
            self._dataset.SegmentsOverlap = value
    ...
    @property
    def PyramidUID(self) -> Optional[str]:
        if "PyramidUID" in self._dataset:
            return self._dataset.PyramidUID
        return None

    @PyramidUID.setter
    def PyramidUID(self, value: Optional[str]):
        if value is None:
            if "PyramidUID" in self._dataset:
                del self._dataset.PyramidUID
        else:
            self._dataset.PyramidUID = value

    @property
    def PyramidDescription(self) -> Optional[str]:
        if "PyramidDescription" in self._dataset:
            return self._dataset.PyramidDescription
        return None

    @PyramidDescription.setter
    def PyramidDescription(self, value: Optional[str]):
        if value is None:
            if "PyramidDescription" in self._dataset:
                del self._dataset.PyramidDescription
        else:
            self._dataset.PyramidDescription = value

    @property
    def PyramidLabel(self) -> Optional[str]:
        if "PyramidLabel" in self._dataset:
            return self._dataset.PyramidLabel
        return None

    @PyramidLabel.setter
    def PyramidLabel(self, value: Optional[str]):
        if value is None:
            if "PyramidLabel" in self._dataset:
                del self._dataset.PyramidLabel
        else:
            self._dataset.PyramidLabel = value
