...
from .mr_image_frame_type_sequence_item import MRImageFrameTypeSequenceItem
...
class SharedFunctionalGroupsSequenceItem:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._MRImageFrameTypeSequence: List[MRImageFrameTypeSequenceItem] = []
    ...
    @property
    def MRImageFrameTypeSequence(self) -> Optional[List[MRImageFrameTypeSequenceItem]]:
        if "MRImageFrameTypeSequence" in self._dataset:
            if len(self._MRImageFrameTypeSequence) == len(self._dataset.MRImageFrameTypeSequence):
                return self._MRImageFrameTypeSequence
            else:
                return [MRImageFrameTypeSequenceItem(x) for x in self._dataset.MRImageFrameTypeSequence]
        return None

    @MRImageFrameTypeSequence.setter
    def MRImageFrameTypeSequence(self, value: Optional[List[MRImageFrameTypeSequenceItem]]):
        if value is None:
            self._MRImageFrameTypeSequence = []
            if "MRImageFrameTypeSequence" in self._dataset:
                del self._dataset.MRImageFrameTypeSequence
        elif not isinstance(value, list) or not all(isinstance(item, MRImageFrameTypeSequenceItem) for item in value):
            raise ValueError(f"MRImageFrameTypeSequence must be a list of MRImageFrameTypeSequenceItem objects")
        else:
            self._MRImageFrameTypeSequence = value
            if "MRImageFrameTypeSequence" not in self._dataset:
                self._dataset.MRImageFrameTypeSequence = pydicom.Sequence()
            self._dataset.MRImageFrameTypeSequence.clear()
            self._dataset.MRImageFrameTypeSequence.extend([item.to_dataset() for item in value])

    def add_MRImageFrameType(self, item: MRImageFrameTypeSequenceItem):
        if not isinstance(item, MRImageFrameTypeSequenceItem):
            raise ValueError(f"Item must be an instance of MRImageFrameTypeSequenceItem")
        self._MRImageFrameTypeSequence.append(item)
        if "MRImageFrameTypeSequence" not in self._dataset:
            self._dataset.MRImageFrameTypeSequence = pydicom.Sequence()
        self._dataset.MRImageFrameTypeSequence.append(item.to_dataset())
