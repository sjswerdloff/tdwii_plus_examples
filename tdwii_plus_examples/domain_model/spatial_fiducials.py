...
from .fiducial_set_sequence_item import FiducialSetSequenceItem
...
class SpatialFiducials:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._FiducialSetSequence: List[FiducialSetSequenceItem] = []
    ...
    @property
    def FiducialSetSequence(self) -> Optional[List[FiducialSetSequenceItem]]:
        if "FiducialSetSequence" in self._dataset:
            if len(self._FiducialSetSequence) == len(self._dataset.FiducialSetSequence):
                return self._FiducialSetSequence
            else:
                return [FiducialSetSequenceItem(x) for x in self._dataset.FiducialSetSequence]
        return None

    @FiducialSetSequence.setter
    def FiducialSetSequence(self, value: Optional[List[FiducialSetSequenceItem]]):
        if value is None:
            self._FiducialSetSequence = []
            if "FiducialSetSequence" in self._dataset:
                del self._dataset.FiducialSetSequence
        elif not isinstance(value, list) or not all(isinstance(item, FiducialSetSequenceItem) for item in value):
            raise ValueError(f"FiducialSetSequence must be a list of FiducialSetSequenceItem objects")
        else:
            self._FiducialSetSequence = value
            if "FiducialSetSequence" not in self._dataset:
                self._dataset.FiducialSetSequence = pydicom.Sequence()
            self._dataset.FiducialSetSequence.clear()
            self._dataset.FiducialSetSequence.extend([item.to_dataset() for item in value])

    def add_FiducialSet(self, item: FiducialSetSequenceItem):
        if not isinstance(item, FiducialSetSequenceItem):
            raise ValueError(f"Item must be an instance of FiducialSetSequenceItem")
        self._FiducialSetSequence.append(item)
        if "FiducialSetSequence" not in self._dataset:
            self._dataset.FiducialSetSequence = pydicom.Sequence()
        self._dataset.FiducialSetSequence.append(item.to_dataset())
