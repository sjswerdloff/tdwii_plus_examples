...
class SpatialRegistration:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._RegistrationSequence: List[RegistrationSequenceItem] = []
    ...
    @property
    def RegistrationSequence(self) -> Optional[List[RegistrationSequenceItem]]:
        if "RegistrationSequence" in self._dataset:
            if len(self._RegistrationSequence) == len(self._dataset.RegistrationSequence):
                return self._RegistrationSequence
            else:
                return [RegistrationSequenceItem(x) for x in self._dataset.RegistrationSequence]
        return None

    @RegistrationSequence.setter
    def RegistrationSequence(self, value: Optional[List[RegistrationSequenceItem]]):
        if value is None:
            self._RegistrationSequence = []
            if "RegistrationSequence" in self._dataset:
                del self._dataset.RegistrationSequence
        elif not isinstance(value, list) or not all(isinstance(item, RegistrationSequenceItem) for item in value):
            raise ValueError(f"RegistrationSequence must be a list of RegistrationSequenceItem objects")
        else:
            self._RegistrationSequence = value
            if "RegistrationSequence" not in self._dataset:
                self._dataset.RegistrationSequence = pydicom.Sequence()
            self._dataset.RegistrationSequence.clear()
            self._dataset.RegistrationSequence.extend([item.to_dataset() for item in value])

    def add_Registration(self, item: RegistrationSequenceItem):
        if not isinstance(item, RegistrationSequenceItem):
            raise ValueError(f"Item must be an instance of RegistrationSequenceItem")
        self._RegistrationSequence.append(item)
        if "RegistrationSequence" not in self._dataset:
            self._dataset.RegistrationSequence = pydicom.Sequence()
        self._dataset.RegistrationSequence.append(item.to_dataset())
