...
from .stereo_pairs_sequence_item import StereoPairsSequenceItem
...
class StereometricRelationship:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        self._dataset = dataset if dataset is not None else pydicom.Dataset()
        self._StereoPairsSequence: List[StereoPairsSequenceItem] = []
    ...
    @property
    def StereoPairsSequence(self) -> Optional[List[StereoPairsSequenceItem]]:
        if "StereoPairsSequence" in self._dataset:
            if len(self._StereoPairsSequence) == len(self._dataset.StereoPairsSequence):
                return self._StereoPairsSequence
            else:
                return [StereoPairsSequenceItem(x) for x in self._dataset.StereoPairsSequence]
        return None

    @StereoPairsSequence.setter
    def StereoPairsSequence(self, value: Optional[List[StereoPairsSequenceItem]]):
        if value is None:
            self._StereoPairsSequence = []
            if "StereoPairsSequence" in self._dataset:
                del self._dataset.StereoPairsSequence
        elif not isinstance(value, list) or not all(isinstance(item, StereoPairsSequenceItem) for item in value):
            raise ValueError(f"StereoPairsSequence must be a list of StereoPairsSequenceItem objects")
        else:
            self._StereoPairsSequence = value
            if "StereoPairsSequence" not in self._dataset:
                self._dataset.StereoPairsSequence = pydicom.Sequence()
            self._dataset.StereoPairsSequence.clear()
            self._dataset.StereoPairsSequence.extend([item.to_dataset() for item in value])

    def add_StereoPairs(self, item: StereoPairsSequenceItem):
        if not isinstance(item, StereoPairsSequenceItem):
            raise ValueError(f"Item must be an instance of StereoPairsSequenceItem")
        self._StereoPairsSequence.append(item)
        if "StereoPairsSequence" not in self._dataset:
            self._dataset.StereoPairsSequence = pydicom.Sequence()
        self._dataset.StereoPairsSequence.append(item.to_dataset())
