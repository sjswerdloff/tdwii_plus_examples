...
from .assessed_attribute_value_sequence_item import AssessedAttributeValueSequenceItem
...
class StructuredConstraintObservationSequenceItem:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        self._dataset = dataset if dataset is not None else pydicom.Dataset()
        self._MeasurementUnitsCodeSequence: List[CodeSequenceItem] = []
        self._AssessedAttributeValueSequence: List[AssessedAttributeValueSequenceItem] = []
    ...
    @property
    def AssessedAttributeValueSequence(self) -> Optional[List[AssessedAttributeValueSequenceItem]]:
        if "AssessedAttributeValueSequence" in self._dataset:
            if len(self._AssessedAttributeValueSequence) == len(self._dataset.AssessedAttributeValueSequence):
                return self._AssessedAttributeValueSequence
            else:
                return [AssessedAttributeValueSequenceItem(x) for x in self._dataset.AssessedAttributeValueSequence]
        return None

    @AssessedAttributeValueSequence.setter
    def AssessedAttributeValueSequence(self, value: Optional[List[AssessedAttributeValueSequenceItem]]):
        if value is None:
            ...
            self._dataset.AssessedAttributeValueSequence.clear()
            self._dataset.AssessedAttributeValueSequence.extend([item.to_dataset() for item in value])
    ...
    def add_AssessedAttributeValue(self, item: AssessedAttributeValueSequenceItem):
        ...
        if "AssessedAttributeValueSequence" not in self._dataset:
            self._dataset.AssessedAttributeValueSequence = pydicom.Sequence()
        self._dataset.AssessedAttributeValueSequence.append(item.to_dataset())
