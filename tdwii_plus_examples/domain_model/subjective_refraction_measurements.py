...
from .subjective_refraction_left_eye_sequence_item import (
    SubjectiveRefractionLeftEyeSequenceItem,
)
from .subjective_refraction_right_eye_sequence_item import (
    SubjectiveRefractionRightEyeSequenceItem,
)
from .udi_sequence_item import UDISequenceItem


class SubjectiveRefractionMeasurements:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._SubjectiveRefractionRightEyeSequence: List[SubjectiveRefractionRightEyeSequenceItem] = []
        self._SubjectiveRefractionLeftEyeSequence: List[SubjectiveRefractionLeftEyeSequenceItem] = []
    ...
    @property
    def DistancePupillaryDistance(self) -> Optional[float]:
        if "DistancePupillaryDistance" in self._dataset:
            return self._dataset.DistancePupillaryDistance
        return None

    @DistancePupillaryDistance.setter
    def DistancePupillaryDistance(self, value: Optional[float]):
        if value is None:
            if "DistancePupillaryDistance" in self._dataset:
                del self._dataset.DistancePupillaryDistance
        else:
            self._dataset.DistancePupillaryDistance = value

    @property
    def NearPupillaryDistance(self) -> Optional[float]:
        if "NearPupillaryDistance" in self._dataset:
            return self._dataset.NearPupillaryDistance
        return None

    @NearPupillaryDistance.setter
    def NearPupillaryDistance(self, value: Optional[float]):
        if value is None:
            if "NearPupillaryDistance" in self._dataset:
                del self._dataset.NearPupillaryDistance
        else:
            self._dataset.NearPupillaryDistance = value

    @property
    def IntermediatePupillaryDistance(self) -> Optional[float]:
        if "IntermediatePupillaryDistance" in self._dataset:
            return self._dataset.IntermediatePupillaryDistance
        return None

    @IntermediatePupillaryDistance.setter
    def IntermediatePupillaryDistance(self, value: Optional[float]):
        if value is None:
            if "IntermediatePupillaryDistance" in self._dataset:
                del self._dataset.IntermediatePupillaryDistance
        else:
            self._dataset.IntermediatePupillaryDistance = value

    @property
    def OtherPupillaryDistance(self) -> Optional[float]:
        if "OtherPupillaryDistance" in self._dataset:
            return self._dataset.OtherPupillaryDistance
        return None

    @OtherPupillaryDistance.setter
    def OtherPupillaryDistance(self, value: Optional[float]):
        if value is None:
            if "OtherPupillaryDistance" in self._dataset:
                del self._dataset.OtherPupillaryDistance
        else:
            self._dataset.OtherPupillaryDistance = value

    @property
    def SubjectiveRefractionRightEyeSequence(self) -> Optional[List[SubjectiveRefractionRightEyeSequenceItem]]:
        if "SubjectiveRefractionRightEyeSequence" in self._dataset:
            if len(self._SubjectiveRefractionRightEyeSequence) == len(self._dataset.SubjectiveRefractionRightEyeSequence):
                return self._SubjectiveRefractionRightEyeSequence
            else:
                return [
                    SubjectiveRefractionRightEyeSequenceItem(x) for x in self._dataset.SubjectiveRefractionRightEyeSequence
                ]
        return None

    @SubjectiveRefractionRightEyeSequence.setter
    def SubjectiveRefractionRightEyeSequence(self, value: Optional[List[SubjectiveRefractionRightEyeSequenceItem]]):
        if value is None:
            self._SubjectiveRefractionRightEyeSequence = []
            if "SubjectiveRefractionRightEyeSequence" in self._dataset:
                del self._dataset.SubjectiveRefractionRightEyeSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, SubjectiveRefractionRightEyeSequenceItem) for item in value
        ):
            raise ValueError(
                f"SubjectiveRefractionRightEyeSequence must be a list of SubjectiveRefractionRightEyeSequenceItem objects"
            )
        else:
            self._SubjectiveRefractionRightEyeSequence = value
            if "SubjectiveRefractionRightEyeSequence" not in self._dataset:
                self._dataset.SubjectiveRefractionRightEyeSequence = pydicom.Sequence()
            self._dataset.SubjectiveRefractionRightEyeSequence.clear()
            self._dataset.SubjectiveRefractionRightEyeSequence.extend([item.to_dataset() for item in value])

    def add_SubjectiveRefractionRightEye(self, item: SubjectiveRefractionRightEyeSequenceItem):
        if not isinstance(item, SubjectiveRefractionRightEyeSequenceItem):
            raise ValueError(f"Item must be an instance of SubjectiveRefractionRightEyeSequenceItem")
        self._SubjectiveRefractionRightEyeSequence.append(item)
        if "SubjectiveRefractionRightEyeSequence" not in self._dataset:
            self._dataset.SubjectiveRefractionRightEyeSequence = pydicom.Sequence()
        self._dataset.SubjectiveRefractionRightEyeSequence.append(item.to_dataset())

    @property
    def SubjectiveRefractionLeftEyeSequence(self) -> Optional[List[SubjectiveRefractionLeftEyeSequenceItem]]:
        if "SubjectiveRefractionLeftEyeSequence" in self._dataset:
            if len(self._SubjectiveRefractionLeftEyeSequence) == len(self._dataset.SubjectiveRefractionLeftEyeSequence):
                return self._SubjectiveRefractionLeftEyeSequence
            else:
                return [SubjectiveRefractionLeftEyeSequenceItem(x) for x in self._dataset.SubjectiveRefractionLeftEyeSequence]
        return None

    @SubjectiveRefractionLeftEyeSequence.setter
    def SubjectiveRefractionLeftEyeSequence(self, value: Optional[List[SubjectiveRefractionLeftEyeSequenceItem]]):
        if value is None:
            self._SubjectiveRefractionLeftEyeSequence = []
            if "SubjectiveRefractionLeftEyeSequence" in self._dataset:
                del self._dataset.SubjectiveRefractionLeftEyeSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, SubjectiveRefractionLeftEyeSequenceItem) for item in value
        ):
            raise ValueError(
                f"SubjectiveRefractionLeftEyeSequence must be a list of SubjectiveRefractionLeftEyeSequenceItem objects"
            )
        else:
            self._SubjectiveRefractionLeftEyeSequence = value
            if "SubjectiveRefractionLeftEyeSequence" not in self._dataset:
                self._dataset.SubjectiveRefractionLeftEyeSequence = pydicom.Sequence()
            self._dataset.SubjectiveRefractionLeftEyeSequence.clear()
            self._dataset.SubjectiveRefractionLeftEyeSequence.extend([item.to_dataset() for item in value])

    def add_SubjectiveRefractionLeftEye(self, item: SubjectiveRefractionLeftEyeSequenceItem):
        if not isinstance(item, SubjectiveRefractionLeftEyeSequenceItem):
            raise ValueError(f"Item must be an instance of SubjectiveRefractionLeftEyeSequenceItem")
        self._SubjectiveRefractionLeftEyeSequence.append(item)
        if "SubjectiveRefractionLeftEyeSequence" not in self._dataset:
            self._dataset.SubjectiveRefractionLeftEyeSequence = pydicom.Sequence()
        self._dataset.SubjectiveRefractionLeftEyeSequence.append(item.to_dataset())
