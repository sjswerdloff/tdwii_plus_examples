...
from .referenced_surface_data_sequence_item import ReferencedSurfaceDataSequenceItem
...
from .surface_sequence_item import SurfaceSequenceItem
from .udi_sequence_item import UDISequenceItem
from .uv_mapping_sequence_item import UVMappingSequenceItem


class SurfaceScanMesh:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._ReferencedSurfaceDataSequence: List[ReferencedSurfaceDataSequenceItem] = []
        self._SurfaceSequence: List[SurfaceSequenceItem] = []
        self._UVMappingSequence: List[UVMappingSequenceItem] = []
        self._SurfaceScanAcquisitionTypeCodeSequence: List[CodeSequenceItem] = []
    ...
    @property
    def ReferencedSurfaceDataSequence(self) -> Optional[List[ReferencedSurfaceDataSequenceItem]]:
        if "ReferencedSurfaceDataSequence" in self._dataset:
            if len(self._ReferencedSurfaceDataSequence) == len(self._dataset.ReferencedSurfaceDataSequence):
                return self._ReferencedSurfaceDataSequence
            else:
                return [ReferencedSurfaceDataSequenceItem(x) for x in self._dataset.ReferencedSurfaceDataSequence]
        return None

    @ReferencedSurfaceDataSequence.setter
    def ReferencedSurfaceDataSequence(self, value: Optional[List[ReferencedSurfaceDataSequenceItem]]):
        if value is None:
            self._ReferencedSurfaceDataSequence = []
            if "ReferencedSurfaceDataSequence" in self._dataset:
                del self._dataset.ReferencedSurfaceDataSequence
        elif not isinstance(value, list) or not all(isinstance(item, ReferencedSurfaceDataSequenceItem) for item in value):
            raise ValueError(f"ReferencedSurfaceDataSequence must be a list of ReferencedSurfaceDataSequenceItem objects")
        else:
            self._ReferencedSurfaceDataSequence = value
            if "ReferencedSurfaceDataSequence" not in self._dataset:
                self._dataset.ReferencedSurfaceDataSequence = pydicom.Sequence()
            self._dataset.ReferencedSurfaceDataSequence.clear()
            self._dataset.ReferencedSurfaceDataSequence.extend([item.to_dataset() for item in value])

    def add_ReferencedSurfaceData(self, item: ReferencedSurfaceDataSequenceItem):
        if not isinstance(item, ReferencedSurfaceDataSequenceItem):
            raise ValueError(f"Item must be an instance of ReferencedSurfaceDataSequenceItem")
        self._ReferencedSurfaceDataSequence.append(item)
        if "ReferencedSurfaceDataSequence" not in self._dataset:
            self._dataset.ReferencedSurfaceDataSequence = pydicom.Sequence()
        self._dataset.ReferencedSurfaceDataSequence.append(item.to_dataset())

    @property
    def NumberOfSurfaces(self) -> Optional[int]:
        if "NumberOfSurfaces" in self._dataset:
            return self._dataset.NumberOfSurfaces
        return None

    @NumberOfSurfaces.setter
    def NumberOfSurfaces(self, value: Optional[int]):
        if value is None:
            if "NumberOfSurfaces" in self._dataset:
                del self._dataset.NumberOfSurfaces
        else:
            self._dataset.NumberOfSurfaces = value

    @property
    def SurfaceSequence(self) -> Optional[List[SurfaceSequenceItem]]:
        if "SurfaceSequence" in self._dataset:
            if len(self._SurfaceSequence) == len(self._dataset.SurfaceSequence):
                return self._SurfaceSequence
            else:
                return [SurfaceSequenceItem(x) for x in self._dataset.SurfaceSequence]
        return None

    @SurfaceSequence.setter
    def SurfaceSequence(self, value: Optional[List[SurfaceSequenceItem]]):
        if value is None:
            self._SurfaceSequence = []
            if "SurfaceSequence" in self._dataset:
                del self._dataset.SurfaceSequence
        elif not isinstance(value, list) or not all(isinstance(item, SurfaceSequenceItem) for item in value):
            raise ValueError(f"SurfaceSequence must be a list of SurfaceSequenceItem objects")
        else:
            self._SurfaceSequence = value
            if "SurfaceSequence" not in self._dataset:
                self._dataset.SurfaceSequence = pydicom.Sequence()
            self._dataset.SurfaceSequence.clear()
            self._dataset.SurfaceSequence.extend([item.to_dataset() for item in value])

    def add_Surface(self, item: SurfaceSequenceItem):
        if not isinstance(item, SurfaceSequenceItem):
            raise ValueError(f"Item must be an instance of SurfaceSequenceItem")
        self._SurfaceSequence.append(item)
        if "SurfaceSequence" not in self._dataset:
            self._dataset.SurfaceSequence = pydicom.Sequence()
        self._dataset.SurfaceSequence.append(item.to_dataset())

    @property
    def UVMappingSequence(self) -> Optional[List[UVMappingSequenceItem]]:
        if "UVMappingSequence" in self._dataset:
            if len(self._UVMappingSequence) == len(self._dataset.UVMappingSequence):
                return self._UVMappingSequence
            else:
                return [UVMappingSequenceItem(x) for x in self._dataset.UVMappingSequence]
        return None

    @UVMappingSequence.setter
    def UVMappingSequence(self, value: Optional[List[UVMappingSequenceItem]]):
        if value is None:
            self._UVMappingSequence = []
            if "UVMappingSequence" in self._dataset:
                del self._dataset.UVMappingSequence
        elif not isinstance(value, list) or not all(isinstance(item, UVMappingSequenceItem) for item in value):
            raise ValueError(f"UVMappingSequence must be a list of UVMappingSequenceItem objects")
        else:
            self._UVMappingSequence = value
            if "UVMappingSequence" not in self._dataset:
                self._dataset.UVMappingSequence = pydicom.Sequence()
            self._dataset.UVMappingSequence.clear()
            self._dataset.UVMappingSequence.extend([item.to_dataset() for item in value])

    def add_UVMapping(self, item: UVMappingSequenceItem):
        if not isinstance(item, UVMappingSequenceItem):
            raise ValueError(f"Item must be an instance of UVMappingSequenceItem")
        self._UVMappingSequence.append(item)
        if "UVMappingSequence" not in self._dataset:
            self._dataset.UVMappingSequence = pydicom.Sequence()
        self._dataset.UVMappingSequence.append(item.to_dataset())
    ...
    @property
    def SurfaceScanAcquisitionTypeCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "SurfaceScanAcquisitionTypeCodeSequence" in self._dataset:
            if len(self._SurfaceScanAcquisitionTypeCodeSequence) == len(self._dataset.SurfaceScanAcquisitionTypeCodeSequence):
                return self._SurfaceScanAcquisitionTypeCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.SurfaceScanAcquisitionTypeCodeSequence]
        return None

    @SurfaceScanAcquisitionTypeCodeSequence.setter
    def SurfaceScanAcquisitionTypeCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._SurfaceScanAcquisitionTypeCodeSequence = []
            if "SurfaceScanAcquisitionTypeCodeSequence" in self._dataset:
                del self._dataset.SurfaceScanAcquisitionTypeCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"SurfaceScanAcquisitionTypeCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._SurfaceScanAcquisitionTypeCodeSequence = value
            if "SurfaceScanAcquisitionTypeCodeSequence" not in self._dataset:
                self._dataset.SurfaceScanAcquisitionTypeCodeSequence = pydicom.Sequence()
            self._dataset.SurfaceScanAcquisitionTypeCodeSequence.clear()
            self._dataset.SurfaceScanAcquisitionTypeCodeSequence.extend([item.to_dataset() for item in value])

    def add_SurfaceScanAcquisitionTypeCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._SurfaceScanAcquisitionTypeCodeSequence.append(item)
        if "SurfaceScanAcquisitionTypeCodeSequence" not in self._dataset:
            self._dataset.SurfaceScanAcquisitionTypeCodeSequence = pydicom.Sequence()
        self._dataset.SurfaceScanAcquisitionTypeCodeSequence.append(item.to_dataset())

    @property
    def SurfaceScanModeCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "SurfaceScanModeCodeSequence" in self._dataset:
            if len(self._SurfaceScanModeCodeSequence) == len(self._dataset.SurfaceScanModeCodeSequence):
                return self._SurfaceScanModeCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.SurfaceScanModeCodeSequence]
        return None

    @SurfaceScanModeCodeSequence.setter
    def SurfaceScanModeCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._SurfaceScanModeCodeSequence = []
            if "SurfaceScanModeCodeSequence" in self._dataset:
                del self._dataset.SurfaceScanModeCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"SurfaceScanModeCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._SurfaceScanModeCodeSequence = value
            if "SurfaceScanModeCodeSequence" not in self._dataset:
                self._dataset.SurfaceScanModeCodeSequence = pydicom.Sequence()
            self._dataset.SurfaceScanModeCodeSequence.clear()
            self._dataset.SurfaceScanModeCodeSequence.extend([item.to_dataset() for item in value])

    def add_SurfaceScanModeCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._SurfaceScanModeCodeSequence.append(item)
        if "SurfaceScanModeCodeSequence" not in self._dataset:
            self._dataset.SurfaceScanModeCodeSequence = pydicom.Sequence()
        self._dataset.SurfaceScanModeCodeSequence.append(item.to_dataset())

    @property
    def RegistrationMethodCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "RegistrationMethodCodeSequence" in self._dataset:
            if len(self._RegistrationMethodCodeSequence) == len(self._dataset.RegistrationMethodCodeSequence):
                return self._RegistrationMethodCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.RegistrationMethodCodeSequence]
        return None

    @RegistrationMethodCodeSequence.setter
    def RegistrationMethodCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._RegistrationMethodCodeSequence = []
            if "RegistrationMethodCodeSequence" in self._dataset:
                del self._dataset.RegistrationMethodCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"RegistrationMethodCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._RegistrationMethodCodeSequence = value
            if "RegistrationMethodCodeSequence" not in self._dataset:
                self._dataset.RegistrationMethodCodeSequence = pydicom.Sequence()
            self._dataset.RegistrationMethodCodeSequence.clear()
            self._dataset.RegistrationMethodCodeSequence.extend([item.to_dataset() for item in value])

    def add_RegistrationMethodCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._RegistrationMethodCodeSequence.append(item)
        if "RegistrationMethodCodeSequence" not in self._dataset:
            self._dataset.RegistrationMethodCodeSequence = pydicom.Sequence()
        self._dataset.RegistrationMethodCodeSequence.append(item.to_dataset())

    @property
    def ShotDurationTime(self) -> Optional[float]:
        if "ShotDurationTime" in self._dataset:
            return self._dataset.ShotDurationTime
        return None

    @ShotDurationTime.setter
    def ShotDurationTime(self, value: Optional[float]):
        if value is None:
            if "ShotDurationTime" in self._dataset:
                del self._dataset.ShotDurationTime
        else:
            self._dataset.ShotDurationTime = value

    @property
    def ShotOffsetTime(self) -> Optional[float]:
        if "ShotOffsetTime" in self._dataset:
            return self._dataset.ShotOffsetTime
        return None

    @ShotOffsetTime.setter
    def ShotOffsetTime(self, value: Optional[float]):
        if value is None:
            if "ShotOffsetTime" in self._dataset:
                del self._dataset.ShotOffsetTime
        else:
            self._dataset.ShotOffsetTime = value
