...
from .track_set_sequence_item import TrackSetSequenceItem
from .udi_sequence_item import UDISequenceItem


class TractographyResults:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._TrackSetSequence: List[TrackSetSequenceItem] = []
    ...
    @property
    def TrackSetSequence(self) -> Optional[List[TrackSetSequenceItem]]:
        if "TrackSetSequence" in self._dataset:
            if len(self._TrackSetSequence) == len(self._dataset.TrackSetSequence):
                return self._TrackSetSequence
            else:
                return [TrackSetSequenceItem(x) for x in self._dataset.TrackSetSequence]
        return None

    @TrackSetSequence.setter
    def TrackSetSequence(self, value: Optional[List[TrackSetSequenceItem]]):
        if value is None:
            self._TrackSetSequence = []
            if "TrackSetSequence" in self._dataset:
                del self._dataset.TrackSetSequence
        elif not isinstance(value, list) or not all(isinstance(item, TrackSetSequenceItem) for item in value):
            raise ValueError(f"TrackSetSequence must be a list of TrackSetSequenceItem objects")
        else:
            self._TrackSetSequence = value
            if "TrackSetSequence" not in self._dataset:
                self._dataset.TrackSetSequence = pydicom.Sequence()
            self._dataset.TrackSetSequence.clear()
            self._dataset.TrackSetSequence.extend([item.to_dataset() for item in value])

    def add_TrackSet(self, item: TrackSetSequenceItem):
        if not isinstance(item, TrackSetSequenceItem):
            raise ValueError(f"Item must be an instance of TrackSetSequenceItem")
        self._TrackSetSequence.append(item)
        if "TrackSetSequence" not in self._dataset:
            self._dataset.TrackSetSequence = pydicom.Sequence()
        self._dataset.TrackSetSequence.append(item.to_dataset())
