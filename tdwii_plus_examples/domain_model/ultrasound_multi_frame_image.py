...
class UltrasoundMultiFrameImage:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._MultiplexedAudioChannelsDescriptionCodeSequence: List[CodeSequenceItem] = []
    ...
    @property
    def CineRate(self) -> Optional[int]:
        if "CineRate" in self._dataset:
            return self._dataset.CineRate
        return None

    @CineRate.setter
    def CineRate(self, value: Optional[int]):
        if value is None:
            if "CineRate" in self._dataset:
                del self._dataset.CineRate
        else:
            self._dataset.CineRate = value

    @property
    def EffectiveDuration(self) -> Optional[Decimal]:
        if "EffectiveDuration" in self._dataset:
            return self._dataset.EffectiveDuration
        return None

    @EffectiveDuration.setter
    def EffectiveDuration(self, value: Optional[Decimal]):
        if value is None:
            if "EffectiveDuration" in self._dataset:
                del self._dataset.EffectiveDuration
        else:
            self._dataset.EffectiveDuration = value
    ...
    @property
    def FrameTimeVector(self) -> Optional[List[Decimal]]:
        if "FrameTimeVector" in self._dataset:
            return self._dataset.FrameTimeVector
        return None

    @FrameTimeVector.setter
    def FrameTimeVector(self, value: Optional[List[Decimal]]):
        if value is None:
            if "FrameTimeVector" in self._dataset:
                del self._dataset.FrameTimeVector
        else:
            self._dataset.FrameTimeVector = value

    @property
    def FrameDelay(self) -> Optional[Decimal]:
        if "FrameDelay" in self._dataset:
            return self._dataset.FrameDelay
        return None

    @FrameDelay.setter
    def FrameDelay(self, value: Optional[Decimal]):
        if value is None:
            if "FrameDelay" in self._dataset:
                del self._dataset.FrameDelay
        else:
            self._dataset.FrameDelay = value

    @property
    def ImageTriggerDelay(self) -> Optional[Decimal]:
        if "ImageTriggerDelay" in self._dataset:
            return self._dataset.ImageTriggerDelay
        return None

    @ImageTriggerDelay.setter
    def ImageTriggerDelay(self, value: Optional[Decimal]):
        if value is None:
            if "ImageTriggerDelay" in self._dataset:
                del self._dataset.ImageTriggerDelay
        else:
            self._dataset.ImageTriggerDelay = value
    ...
    @property
    def MultiplexedAudioChannelsDescriptionCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "MultiplexedAudioChannelsDescriptionCodeSequence" in self._dataset:
            if len(self._MultiplexedAudioChannelsDescriptionCodeSequence) == len(
                self._dataset.MultiplexedAudioChannelsDescriptionCodeSequence
            ):
                return self._MultiplexedAudioChannelsDescriptionCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.MultiplexedAudioChannelsDescriptionCodeSequence]
        return None

    @MultiplexedAudioChannelsDescriptionCodeSequence.setter
    def MultiplexedAudioChannelsDescriptionCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._MultiplexedAudioChannelsDescriptionCodeSequence = []
            if "MultiplexedAudioChannelsDescriptionCodeSequence" in self._dataset:
            ...
            self._dataset.MultiplexedAudioChannelsDescriptionCodeSequence.clear()
            self._dataset.MultiplexedAudioChannelsDescriptionCodeSequence.extend([item.to_dataset() for item in value])
    ...
    def add_MultiplexedAudioChannelsDescriptionCode(self, item: CodeSequenceItem):
        ...
        if "MultiplexedAudioChannelsDescriptionCodeSequence" not in self._dataset:
            self._dataset.MultiplexedAudioChannelsDescriptionCodeSequence = pydicom.Sequence()
        self._dataset.MultiplexedAudioChannelsDescriptionCodeSequence.append(item.to_dataset())
    ...
    def ContrastBolusAdministrationRouteSequence(self, value: Optional[List[ContrastBolusAdministrationRouteSequenceItem]]):
        if value is None:
            ...
            raise ValueError(
                f"ContrastBolusAdministrationRouteSequence must be a list of ContrastBolusAdministrationRouteSequenceItem objects"
            )
        else:
    ...
    def add_ContrastBolusAdministrationRoute(self, item: ContrastBolusAdministrationRouteSequenceItem):
        if not isinstance(item, ContrastBolusAdministrationRouteSequenceItem):
            raise ValueError(f"Item must be an instance of ContrastBolusAdministrationRouteSequenceItem")
    ...
    @property
    def SequenceOfUltrasoundRegions(self) -> Optional[list]:
        if "SequenceOfUltrasoundRegions" in self._dataset:
            return self._dataset.SequenceOfUltrasoundRegions
        return None

    @SequenceOfUltrasoundRegions.setter
    def SequenceOfUltrasoundRegions(self, value: Optional[list]):
        if value is None:
            if "SequenceOfUltrasoundRegions" in self._dataset:
                del self._dataset.SequenceOfUltrasoundRegions
        else:
            self._dataset.SequenceOfUltrasoundRegions = value
    ...
    @property
    def StageName(self) -> Optional[str]:
        if "StageName" in self._dataset:
            return self._dataset.StageName
        return None

    @StageName.setter
    def StageName(self, value: Optional[str]):
        if value is None:
            if "StageName" in self._dataset:
                del self._dataset.StageName
        else:
            self._dataset.StageName = value
    ...
    @property
    def ViewName(self) -> Optional[str]:
        if "ViewName" in self._dataset:
            return self._dataset.ViewName
        return None

    @ViewName.setter
    def ViewName(self, value: Optional[str]):
        if value is None:
            if "ViewName" in self._dataset:
                del self._dataset.ViewName
        else:
            self._dataset.ViewName = value

    @property
    def ViewNumber(self) -> Optional[int]:
        if "ViewNumber" in self._dataset:
            return self._dataset.ViewNumber
        return None

    @ViewNumber.setter
    def ViewNumber(self, value: Optional[int]):
        if value is None:
            if "ViewNumber" in self._dataset:
                del self._dataset.ViewNumber
        else:
            self._dataset.ViewNumber = value

    @property
    def NumberOfEventTimers(self) -> Optional[int]:
        if "NumberOfEventTimers" in self._dataset:
            return self._dataset.NumberOfEventTimers
        return None

    @NumberOfEventTimers.setter
    def NumberOfEventTimers(self, value: Optional[int]):
        if value is None:
            if "NumberOfEventTimers" in self._dataset:
                del self._dataset.NumberOfEventTimers
        else:
            self._dataset.NumberOfEventTimers = value

    @property
    def NumberOfViewsInStage(self) -> Optional[int]:
        if "NumberOfViewsInStage" in self._dataset:
            return self._dataset.NumberOfViewsInStage
        return None

    @NumberOfViewsInStage.setter
    def NumberOfViewsInStage(self, value: Optional[int]):
        if value is None:
            if "NumberOfViewsInStage" in self._dataset:
                del self._dataset.NumberOfViewsInStage
        else:
            self._dataset.NumberOfViewsInStage = value

    @property
    def EventElapsedTimes(self) -> Optional[List[Decimal]]:
        if "EventElapsedTimes" in self._dataset:
            return self._dataset.EventElapsedTimes
        return None

    @EventElapsedTimes.setter
    def EventElapsedTimes(self, value: Optional[List[Decimal]]):
        if value is None:
            if "EventElapsedTimes" in self._dataset:
                del self._dataset.EventElapsedTimes
        else:
            self._dataset.EventElapsedTimes = value
    ...
    @property
    def LesionNumber(self) -> Optional[List[int]]:
        if "LesionNumber" in self._dataset:
            return self._dataset.LesionNumber
        return None

    @LesionNumber.setter
    def LesionNumber(self, value: Optional[List[int]]):
        if value is None:
            if "LesionNumber" in self._dataset:
                del self._dataset.LesionNumber
        else:
            self._dataset.LesionNumber = value

    @property
    def OutputPower(self) -> Optional[List[str]]:
        if "OutputPower" in self._dataset:
            return self._dataset.OutputPower
        return None

    @OutputPower.setter
    def OutputPower(self, value: Optional[List[str]]):
        if value is None:
            if "OutputPower" in self._dataset:
                del self._dataset.OutputPower
        else:
            self._dataset.OutputPower = value
    ...
    @property
    def FocusDepth(self) -> Optional[Decimal]:
        if "FocusDepth" in self._dataset:
            return self._dataset.FocusDepth
        return None

    @FocusDepth.setter
    def FocusDepth(self, value: Optional[Decimal]):
        if value is None:
            if "FocusDepth" in self._dataset:
                del self._dataset.FocusDepth
        else:
            self._dataset.FocusDepth = value
    ...
    @property
    def SoftTissueFocusThermalIndex(self) -> Optional[Decimal]:
        if "SoftTissueFocusThermalIndex" in self._dataset:
            return self._dataset.SoftTissueFocusThermalIndex
        return None

    @SoftTissueFocusThermalIndex.setter
    def SoftTissueFocusThermalIndex(self, value: Optional[Decimal]):
        if value is None:
            if "SoftTissueFocusThermalIndex" in self._dataset:
                del self._dataset.SoftTissueFocusThermalIndex
        else:
            self._dataset.SoftTissueFocusThermalIndex = value

    @property
    def SoftTissueSurfaceThermalIndex(self) -> Optional[Decimal]:
        if "SoftTissueSurfaceThermalIndex" in self._dataset:
            return self._dataset.SoftTissueSurfaceThermalIndex
        return None

    @SoftTissueSurfaceThermalIndex.setter
    def SoftTissueSurfaceThermalIndex(self, value: Optional[Decimal]):
        if value is None:
            if "SoftTissueSurfaceThermalIndex" in self._dataset:
                del self._dataset.SoftTissueSurfaceThermalIndex
        else:
            self._dataset.SoftTissueSurfaceThermalIndex = value
    ...
    @property
    def TransducerType(self) -> Optional[str]:
        if "TransducerType" in self._dataset:
            return self._dataset.TransducerType
        return None

    @TransducerType.setter
    def TransducerType(self, value: Optional[str]):
        if value is None:
            if "TransducerType" in self._dataset:
                del self._dataset.TransducerType
        else:
            self._dataset.TransducerType = value

    @property
    def RWaveTimeVector(self) -> Optional[List[float]]:
        if "RWaveTimeVector" in self._dataset:
            return self._dataset.RWaveTimeVector
        return None

    @RWaveTimeVector.setter
    def RWaveTimeVector(self, value: Optional[List[float]]):
        if value is None:
            if "RWaveTimeVector" in self._dataset:
                del self._dataset.RWaveTimeVector
        else:
            self._dataset.RWaveTimeVector = value
    ...
    @property
    def UltrasoundColorDataPresent(self) -> Optional[int]:
        if "UltrasoundColorDataPresent" in self._dataset:
            return self._dataset.UltrasoundColorDataPresent
        return None

    @UltrasoundColorDataPresent.setter
    def UltrasoundColorDataPresent(self, value: Optional[int]):
        if value is None:
            if "UltrasoundColorDataPresent" in self._dataset:
                del self._dataset.UltrasoundColorDataPresent
        else:
            self._dataset.UltrasoundColorDataPresent = value
    ...
    @property
    def FrameNumbersOfInterest(self) -> Optional[List[int]]:
        if "FrameNumbersOfInterest" in self._dataset:
            return self._dataset.FrameNumbersOfInterest
        return None

    @FrameNumbersOfInterest.setter
    def FrameNumbersOfInterest(self, value: Optional[List[int]]):
        if value is None:
            if "FrameNumbersOfInterest" in self._dataset:
                del self._dataset.FrameNumbersOfInterest
        else:
            self._dataset.FrameNumbersOfInterest = value

    @property
    def FrameOfInterestDescription(self) -> Optional[List[str]]:
        if "FrameOfInterestDescription" in self._dataset:
            return self._dataset.FrameOfInterestDescription
        return None

    @FrameOfInterestDescription.setter
    def FrameOfInterestDescription(self, value: Optional[List[str]]):
        if value is None:
            if "FrameOfInterestDescription" in self._dataset:
                del self._dataset.FrameOfInterestDescription
        else:
            self._dataset.FrameOfInterestDescription = value

    @property
    def FrameOfInterestType(self) -> Optional[List[str]]:
        if "FrameOfInterestType" in self._dataset:
            return self._dataset.FrameOfInterestType
        return None

    @FrameOfInterestType.setter
    def FrameOfInterestType(self, value: Optional[List[str]]):
        if value is None:
            if "FrameOfInterestType" in self._dataset:
                del self._dataset.FrameOfInterestType
        else:
            self._dataset.FrameOfInterestType = value
