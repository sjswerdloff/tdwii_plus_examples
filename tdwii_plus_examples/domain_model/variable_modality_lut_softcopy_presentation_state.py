...
from .displayed_area_selection_sequence_item import DisplayedAreaSelectionSequenceItem
...
from .mask_subtraction_sequence_item import MaskSubtractionSequenceItem
...
from .presentation_lut_sequence_item import PresentationLUTSequenceItem
...
from .variable_modality_lut_sequence_item import VariableModalityLUTSequenceItem


class VariableModalityLutSoftcopyPresentationState:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._DisplayedAreaSelectionSequence: List[DisplayedAreaSelectionSequenceItem] = []
        ...
        self._PresentationLUTSequence: List[PresentationLUTSequenceItem] = []
    ...
    @property
    def MaskSubtractionSequence(self) -> Optional[List[MaskSubtractionSequenceItem]]:
        if "MaskSubtractionSequence" in self._dataset:
            if len(self._MaskSubtractionSequence) == len(self._dataset.MaskSubtractionSequence):
                return self._MaskSubtractionSequence
            else:
                return [MaskSubtractionSequenceItem(x) for x in self._dataset.MaskSubtractionSequence]
        return None

    @MaskSubtractionSequence.setter
    def MaskSubtractionSequence(self, value: Optional[List[MaskSubtractionSequenceItem]]):
        if value is None:
            self._MaskSubtractionSequence = []
            if "MaskSubtractionSequence" in self._dataset:
                del self._dataset.MaskSubtractionSequence
        elif not isinstance(value, list) or not all(isinstance(item, MaskSubtractionSequenceItem) for item in value):
            raise ValueError(f"MaskSubtractionSequence must be a list of MaskSubtractionSequenceItem objects")
        else:
            self._MaskSubtractionSequence = value
            if "MaskSubtractionSequence" not in self._dataset:
                self._dataset.MaskSubtractionSequence = pydicom.Sequence()
            self._dataset.MaskSubtractionSequence.clear()
            self._dataset.MaskSubtractionSequence.extend([item.to_dataset() for item in value])

    def add_MaskSubtraction(self, item: MaskSubtractionSequenceItem):
        if not isinstance(item, MaskSubtractionSequenceItem):
            raise ValueError(f"Item must be an instance of MaskSubtractionSequenceItem")
        self._MaskSubtractionSequence.append(item)
        if "MaskSubtractionSequence" not in self._dataset:
            self._dataset.MaskSubtractionSequence = pydicom.Sequence()
        self._dataset.MaskSubtractionSequence.append(item.to_dataset())

    @property
    def OverlayActivationLayer(self) -> Optional[str]:
        if "OverlayActivationLayer" in self._dataset:
            return self._dataset.OverlayActivationLayer
        return None

    @OverlayActivationLayer.setter
    def OverlayActivationLayer(self, value: Optional[str]):
        if value is None:
            if "OverlayActivationLayer" in self._dataset:
                del self._dataset.OverlayActivationLayer
        else:
            self._dataset.OverlayActivationLayer = value

    @property
    def VariableModalityLUTSequence(self) -> Optional[List[VariableModalityLUTSequenceItem]]:
        if "VariableModalityLUTSequence" in self._dataset:
            if len(self._VariableModalityLUTSequence) == len(self._dataset.VariableModalityLUTSequence):
                return self._VariableModalityLUTSequence
            else:
                return [VariableModalityLUTSequenceItem(x) for x in self._dataset.VariableModalityLUTSequence]
        return None

    @VariableModalityLUTSequence.setter
    def VariableModalityLUTSequence(self, value: Optional[List[VariableModalityLUTSequenceItem]]):
        if value is None:
            self._VariableModalityLUTSequence = []
            if "VariableModalityLUTSequence" in self._dataset:
                del self._dataset.VariableModalityLUTSequence
        elif not isinstance(value, list) or not all(isinstance(item, VariableModalityLUTSequenceItem) for item in value):
            raise ValueError(f"VariableModalityLUTSequence must be a list of VariableModalityLUTSequenceItem objects")
        else:
            self._VariableModalityLUTSequence = value
            if "VariableModalityLUTSequence" not in self._dataset:
                self._dataset.VariableModalityLUTSequence = pydicom.Sequence()
            self._dataset.VariableModalityLUTSequence.clear()
            self._dataset.VariableModalityLUTSequence.extend([item.to_dataset() for item in value])

    def add_VariableModalityLUT(self, item: VariableModalityLUTSequenceItem):
        if not isinstance(item, VariableModalityLUTSequenceItem):
            raise ValueError(f"Item must be an instance of VariableModalityLUTSequenceItem")
        self._VariableModalityLUTSequence.append(item)
        if "VariableModalityLUTSequence" not in self._dataset:
            self._dataset.VariableModalityLUTSequence = pydicom.Sequence()
        self._dataset.VariableModalityLUTSequence.append(item.to_dataset())
    ...
    @property
    def DisplayedAreaSelectionSequence(self) -> Optional[List[DisplayedAreaSelectionSequenceItem]]:
        if "DisplayedAreaSelectionSequence" in self._dataset:
            if len(self._DisplayedAreaSelectionSequence) == len(self._dataset.DisplayedAreaSelectionSequence):
                return self._DisplayedAreaSelectionSequence
            else:
                return [DisplayedAreaSelectionSequenceItem(x) for x in self._dataset.DisplayedAreaSelectionSequence]
        return None

    @DisplayedAreaSelectionSequence.setter
    def DisplayedAreaSelectionSequence(self, value: Optional[List[DisplayedAreaSelectionSequenceItem]]):
        if value is None:
            self._DisplayedAreaSelectionSequence = []
            if "DisplayedAreaSelectionSequence" in self._dataset:
                del self._dataset.DisplayedAreaSelectionSequence
        elif not isinstance(value, list) or not all(isinstance(item, DisplayedAreaSelectionSequenceItem) for item in value):
            raise ValueError(f"DisplayedAreaSelectionSequence must be a list of DisplayedAreaSelectionSequenceItem objects")
        else:
            self._DisplayedAreaSelectionSequence = value
            if "DisplayedAreaSelectionSequence" not in self._dataset:
                self._dataset.DisplayedAreaSelectionSequence = pydicom.Sequence()
            self._dataset.DisplayedAreaSelectionSequence.clear()
            self._dataset.DisplayedAreaSelectionSequence.extend([item.to_dataset() for item in value])

    def add_DisplayedAreaSelection(self, item: DisplayedAreaSelectionSequenceItem):
        if not isinstance(item, DisplayedAreaSelectionSequenceItem):
            raise ValueError(f"Item must be an instance of DisplayedAreaSelectionSequenceItem")
        self._DisplayedAreaSelectionSequence.append(item)
        if "DisplayedAreaSelectionSequence" not in self._dataset:
            self._dataset.DisplayedAreaSelectionSequence = pydicom.Sequence()
        self._dataset.DisplayedAreaSelectionSequence.append(item.to_dataset())
    ...
    @property
    def PresentationLUTSequence(self) -> Optional[List[PresentationLUTSequenceItem]]:
        if "PresentationLUTSequence" in self._dataset:
            if len(self._PresentationLUTSequence) == len(self._dataset.PresentationLUTSequence):
                return self._PresentationLUTSequence
            else:
                return [PresentationLUTSequenceItem(x) for x in self._dataset.PresentationLUTSequence]
        return None

    @PresentationLUTSequence.setter
    def PresentationLUTSequence(self, value: Optional[List[PresentationLUTSequenceItem]]):
        if value is None:
            self._PresentationLUTSequence = []
            if "PresentationLUTSequence" in self._dataset:
                del self._dataset.PresentationLUTSequence
        elif not isinstance(value, list) or not all(isinstance(item, PresentationLUTSequenceItem) for item in value):
            raise ValueError(f"PresentationLUTSequence must be a list of PresentationLUTSequenceItem objects")
        else:
            self._PresentationLUTSequence = value
            if "PresentationLUTSequence" not in self._dataset:
                self._dataset.PresentationLUTSequence = pydicom.Sequence()
            self._dataset.PresentationLUTSequence.clear()
            self._dataset.PresentationLUTSequence.extend([item.to_dataset() for item in value])

    def add_PresentationLUT(self, item: PresentationLUTSequenceItem):
        if not isinstance(item, PresentationLUTSequenceItem):
            raise ValueError(f"Item must be an instance of PresentationLUTSequenceItem")
        self._PresentationLUTSequence.append(item)
        if "PresentationLUTSequence" not in self._dataset:
            self._dataset.PresentationLUTSequence = pydicom.Sequence()
        self._dataset.PresentationLUTSequence.append(item.to_dataset())
