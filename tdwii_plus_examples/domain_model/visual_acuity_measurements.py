...
from .referenced_refractive_measurements_sequence_item import (
    ReferencedRefractiveMeasurementsSequenceItem,
)
...
from .visual_acuity_both_eyes_open_sequence_item import (
    VisualAcuityBothEyesOpenSequenceItem,
)
from .visual_acuity_left_eye_sequence_item import VisualAcuityLeftEyeSequenceItem
from .visual_acuity_right_eye_sequence_item import VisualAcuityRightEyeSequenceItem


class VisualAcuityMeasurements:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        self._dataset = dataset if dataset is not None else pydicom.Dataset()
        self._VisualAcuityTypeCodeSequence: List[CodeSequenceItem] = []
        self._VisualAcuityRightEyeSequence: List[VisualAcuityRightEyeSequenceItem] = []
        self._VisualAcuityLeftEyeSequence: List[VisualAcuityLeftEyeSequenceItem] = []
        self._VisualAcuityBothEyesOpenSequence: List[VisualAcuityBothEyesOpenSequenceItem] = []
    ...
    def BackgroundColor(self) -> Optional[str]:
        if "BackgroundColor" in self._dataset:
            return self._dataset.BackgroundColor
        return None

    @BackgroundColor.setter
    def BackgroundColor(self, value: Optional[str]):
        if value is None:
            if "BackgroundColor" in self._dataset:
                del self._dataset.BackgroundColor
        else:
            self._dataset.BackgroundColor = value

    @property
    def Optotype(self) -> Optional[str]:
        if "Optotype" in self._dataset:
            return self._dataset.Optotype
        return None

    @Optotype.setter
    def Optotype(self, value: Optional[str]):
        if value is None:
            if "Optotype" in self._dataset:
                del self._dataset.Optotype
        else:
            self._dataset.Optotype = value

    @property
    def OptotypePresentation(self) -> Optional[str]:
        if "OptotypePresentation" in self._dataset:
            return self._dataset.OptotypePresentation
        return None

    @OptotypePresentation.setter
    def OptotypePresentation(self, value: Optional[str]):
        if value is None:
            if "OptotypePresentation" in self._dataset:
                del self._dataset.OptotypePresentation
        else:
            self._dataset.OptotypePresentation = value

    @property
    def VisualAcuityTypeCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "VisualAcuityTypeCodeSequence" in self._dataset:
            if len(self._VisualAcuityTypeCodeSequence) == len(self._dataset.VisualAcuityTypeCodeSequence):
                return self._VisualAcuityTypeCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.VisualAcuityTypeCodeSequence]
        return None

    @VisualAcuityTypeCodeSequence.setter
    def VisualAcuityTypeCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._VisualAcuityTypeCodeSequence = []
            if "VisualAcuityTypeCodeSequence" in self._dataset:
                del self._dataset.VisualAcuityTypeCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"VisualAcuityTypeCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._VisualAcuityTypeCodeSequence = value
            if "VisualAcuityTypeCodeSequence" not in self._dataset:
                self._dataset.VisualAcuityTypeCodeSequence = pydicom.Sequence()
            self._dataset.VisualAcuityTypeCodeSequence.clear()
            self._dataset.VisualAcuityTypeCodeSequence.extend([item.to_dataset() for item in value])

    def add_VisualAcuityTypeCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._VisualAcuityTypeCodeSequence.append(item)
        if "VisualAcuityTypeCodeSequence" not in self._dataset:
            self._dataset.VisualAcuityTypeCodeSequence = pydicom.Sequence()
        self._dataset.VisualAcuityTypeCodeSequence.append(item.to_dataset())

    @property
    def VisualAcuityRightEyeSequence(self) -> Optional[List[VisualAcuityRightEyeSequenceItem]]:
        if "VisualAcuityRightEyeSequence" in self._dataset:
            if len(self._VisualAcuityRightEyeSequence) == len(self._dataset.VisualAcuityRightEyeSequence):
                return self._VisualAcuityRightEyeSequence
            else:
                return [VisualAcuityRightEyeSequenceItem(x) for x in self._dataset.VisualAcuityRightEyeSequence]
        return None

    @VisualAcuityRightEyeSequence.setter
    def VisualAcuityRightEyeSequence(self, value: Optional[List[VisualAcuityRightEyeSequenceItem]]):
        if value is None:
            self._VisualAcuityRightEyeSequence = []
            if "VisualAcuityRightEyeSequence" in self._dataset:
                del self._dataset.VisualAcuityRightEyeSequence
        elif not isinstance(value, list) or not all(isinstance(item, VisualAcuityRightEyeSequenceItem) for item in value):
            raise ValueError(f"VisualAcuityRightEyeSequence must be a list of VisualAcuityRightEyeSequenceItem objects")
        else:
            self._VisualAcuityRightEyeSequence = value
            if "VisualAcuityRightEyeSequence" not in self._dataset:
                self._dataset.VisualAcuityRightEyeSequence = pydicom.Sequence()
            self._dataset.VisualAcuityRightEyeSequence.clear()
            self._dataset.VisualAcuityRightEyeSequence.extend([item.to_dataset() for item in value])

    def add_VisualAcuityRightEye(self, item: VisualAcuityRightEyeSequenceItem):
        if not isinstance(item, VisualAcuityRightEyeSequenceItem):
            raise ValueError(f"Item must be an instance of VisualAcuityRightEyeSequenceItem")
        self._VisualAcuityRightEyeSequence.append(item)
        if "VisualAcuityRightEyeSequence" not in self._dataset:
            self._dataset.VisualAcuityRightEyeSequence = pydicom.Sequence()
        self._dataset.VisualAcuityRightEyeSequence.append(item.to_dataset())

    @property
    def VisualAcuityLeftEyeSequence(self) -> Optional[List[VisualAcuityLeftEyeSequenceItem]]:
        if "VisualAcuityLeftEyeSequence" in self._dataset:
            if len(self._VisualAcuityLeftEyeSequence) == len(self._dataset.VisualAcuityLeftEyeSequence):
                return self._VisualAcuityLeftEyeSequence
            else:
                return [VisualAcuityLeftEyeSequenceItem(x) for x in self._dataset.VisualAcuityLeftEyeSequence]
        return None

    @VisualAcuityLeftEyeSequence.setter
    def VisualAcuityLeftEyeSequence(self, value: Optional[List[VisualAcuityLeftEyeSequenceItem]]):
        if value is None:
            self._VisualAcuityLeftEyeSequence = []
            if "VisualAcuityLeftEyeSequence" in self._dataset:
                del self._dataset.VisualAcuityLeftEyeSequence
        elif not isinstance(value, list) or not all(isinstance(item, VisualAcuityLeftEyeSequenceItem) for item in value):
            raise ValueError(f"VisualAcuityLeftEyeSequence must be a list of VisualAcuityLeftEyeSequenceItem objects")
        else:
            self._VisualAcuityLeftEyeSequence = value
            if "VisualAcuityLeftEyeSequence" not in self._dataset:
                self._dataset.VisualAcuityLeftEyeSequence = pydicom.Sequence()
            self._dataset.VisualAcuityLeftEyeSequence.clear()
            self._dataset.VisualAcuityLeftEyeSequence.extend([item.to_dataset() for item in value])

    def add_VisualAcuityLeftEye(self, item: VisualAcuityLeftEyeSequenceItem):
        if not isinstance(item, VisualAcuityLeftEyeSequenceItem):
            raise ValueError(f"Item must be an instance of VisualAcuityLeftEyeSequenceItem")
        self._VisualAcuityLeftEyeSequence.append(item)
        if "VisualAcuityLeftEyeSequence" not in self._dataset:
            self._dataset.VisualAcuityLeftEyeSequence = pydicom.Sequence()
        self._dataset.VisualAcuityLeftEyeSequence.append(item.to_dataset())

    @property
    def VisualAcuityBothEyesOpenSequence(self) -> Optional[List[VisualAcuityBothEyesOpenSequenceItem]]:
        if "VisualAcuityBothEyesOpenSequence" in self._dataset:
            if len(self._VisualAcuityBothEyesOpenSequence) == len(self._dataset.VisualAcuityBothEyesOpenSequence):
                return self._VisualAcuityBothEyesOpenSequence
            else:
                return [VisualAcuityBothEyesOpenSequenceItem(x) for x in self._dataset.VisualAcuityBothEyesOpenSequence]
        return None

    @VisualAcuityBothEyesOpenSequence.setter
    def VisualAcuityBothEyesOpenSequence(self, value: Optional[List[VisualAcuityBothEyesOpenSequenceItem]]):
        if value is None:
            self._VisualAcuityBothEyesOpenSequence = []
            if "VisualAcuityBothEyesOpenSequence" in self._dataset:
                del self._dataset.VisualAcuityBothEyesOpenSequence
        elif not isinstance(value, list) or not all(isinstance(item, VisualAcuityBothEyesOpenSequenceItem) for item in value):
            raise ValueError(
                f"VisualAcuityBothEyesOpenSequence must be a list of VisualAcuityBothEyesOpenSequenceItem objects"
            )
        else:
            self._VisualAcuityBothEyesOpenSequence = value
            if "VisualAcuityBothEyesOpenSequence" not in self._dataset:
                self._dataset.VisualAcuityBothEyesOpenSequence = pydicom.Sequence()
            self._dataset.VisualAcuityBothEyesOpenSequence.clear()
            self._dataset.VisualAcuityBothEyesOpenSequence.extend([item.to_dataset() for item in value])

    def add_VisualAcuityBothEyesOpen(self, item: VisualAcuityBothEyesOpenSequenceItem):
        if not isinstance(item, VisualAcuityBothEyesOpenSequenceItem):
            raise ValueError(f"Item must be an instance of VisualAcuityBothEyesOpenSequenceItem")
        self._VisualAcuityBothEyesOpenSequence.append(item)
        if "VisualAcuityBothEyesOpenSequence" not in self._dataset:
            self._dataset.VisualAcuityBothEyesOpenSequence = pydicom.Sequence()
        self._dataset.VisualAcuityBothEyesOpenSequence.append(item.to_dataset())

    @property
    def ViewingDistanceType(self) -> Optional[str]:
        if "ViewingDistanceType" in self._dataset:
            return self._dataset.ViewingDistanceType
        return None

    @ViewingDistanceType.setter
    def ViewingDistanceType(self, value: Optional[str]):
        if value is None:
            if "ViewingDistanceType" in self._dataset:
                del self._dataset.ViewingDistanceType
        else:
            self._dataset.ViewingDistanceType = value

    @property
    def OptotypeDetailedDefinition(self) -> Optional[str]:
        if "OptotypeDetailedDefinition" in self._dataset:
            return self._dataset.OptotypeDetailedDefinition
        return None

    @OptotypeDetailedDefinition.setter
    def OptotypeDetailedDefinition(self, value: Optional[str]):
        if value is None:
            if "OptotypeDetailedDefinition" in self._dataset:
                del self._dataset.OptotypeDetailedDefinition
        else:
            self._dataset.OptotypeDetailedDefinition = value
    ...
    @property
    def ReferencedRefractiveMeasurementsSequence(self) -> Optional[List[ReferencedRefractiveMeasurementsSequenceItem]]:
        if "ReferencedRefractiveMeasurementsSequence" in self._dataset:
            if len(self._ReferencedRefractiveMeasurementsSequence) == len(
                self._dataset.ReferencedRefractiveMeasurementsSequence
            ):
                return self._ReferencedRefractiveMeasurementsSequence
            else:
                return [
                    ReferencedRefractiveMeasurementsSequenceItem(x)
                    for x in self._dataset.ReferencedRefractiveMeasurementsSequence
                ]
        return None

    @ReferencedRefractiveMeasurementsSequence.setter
    def ReferencedRefractiveMeasurementsSequence(self, value: Optional[List[ReferencedRefractiveMeasurementsSequenceItem]]):
        if value is None:
            self._ReferencedRefractiveMeasurementsSequence = []
            if "ReferencedRefractiveMeasurementsSequence" in self._dataset:
                del self._dataset.ReferencedRefractiveMeasurementsSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, ReferencedRefractiveMeasurementsSequenceItem) for item in value
        ):
            raise ValueError(
                f"ReferencedRefractiveMeasurementsSequence must be a list of ReferencedRefractiveMeasurementsSequenceItem objects"
            )
        else:
            self._ReferencedRefractiveMeasurementsSequence = value
            if "ReferencedRefractiveMeasurementsSequence" not in self._dataset:
                self._dataset.ReferencedRefractiveMeasurementsSequence = pydicom.Sequence()
            self._dataset.ReferencedRefractiveMeasurementsSequence.clear()
            self._dataset.ReferencedRefractiveMeasurementsSequence.extend([item.to_dataset() for item in value])

    def add_ReferencedRefractiveMeasurements(self, item: ReferencedRefractiveMeasurementsSequenceItem):
        if not isinstance(item, ReferencedRefractiveMeasurementsSequenceItem):
            raise ValueError(f"Item must be an instance of ReferencedRefractiveMeasurementsSequenceItem")
        self._ReferencedRefractiveMeasurementsSequence.append(item)
        if "ReferencedRefractiveMeasurementsSequence" not in self._dataset:
            self._dataset.ReferencedRefractiveMeasurementsSequence = pydicom.Sequence()
        self._dataset.ReferencedRefractiveMeasurementsSequence.append(item.to_dataset())
