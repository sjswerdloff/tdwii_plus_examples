...
class VlPhotographicImage:
    ...
    @property
    def WhitePoint(self) -> Optional[Decimal]:
        if "WhitePoint" in self._dataset:
            return self._dataset.WhitePoint
        return None

    @WhitePoint.setter
    def WhitePoint(self, value: Optional[Decimal]):
        if value is None:
            if "WhitePoint" in self._dataset:
                del self._dataset.WhitePoint
        else:
            self._dataset.WhitePoint = value

    @property
    def PrimaryChromaticities(self) -> Optional[List[Decimal]]:
        if "PrimaryChromaticities" in self._dataset:
            return self._dataset.PrimaryChromaticities
        return None

    @PrimaryChromaticities.setter
    def PrimaryChromaticities(self, value: Optional[List[Decimal]]):
        if value is None:
            if "PrimaryChromaticities" in self._dataset:
                del self._dataset.PrimaryChromaticities
        else:
            self._dataset.PrimaryChromaticities = value

    @property
    def BatteryLevel(self) -> Optional[str]:
        if "BatteryLevel" in self._dataset:
            return self._dataset.BatteryLevel
        return None

    @BatteryLevel.setter
    def BatteryLevel(self, value: Optional[str]):
        if value is None:
            if "BatteryLevel" in self._dataset:
                del self._dataset.BatteryLevel
        else:
            self._dataset.BatteryLevel = value

    @property
    def ExposureTimeInSeconds(self) -> Optional[Decimal]:
        if "ExposureTimeInSeconds" in self._dataset:
            return self._dataset.ExposureTimeInSeconds
        return None

    @ExposureTimeInSeconds.setter
    def ExposureTimeInSeconds(self, value: Optional[Decimal]):
        if value is None:
            if "ExposureTimeInSeconds" in self._dataset:
                del self._dataset.ExposureTimeInSeconds
        else:
            self._dataset.ExposureTimeInSeconds = value

    @property
    def FNumber(self) -> Optional[Decimal]:
        if "FNumber" in self._dataset:
            return self._dataset.FNumber
        return None

    @FNumber.setter
    def FNumber(self, value: Optional[Decimal]):
        if value is None:
            if "FNumber" in self._dataset:
                del self._dataset.FNumber
        else:
            self._dataset.FNumber = value

    @property
    def OECFRows(self) -> Optional[int]:
        if "OECFRows" in self._dataset:
            return self._dataset.OECFRows
        return None

    @OECFRows.setter
    def OECFRows(self, value: Optional[int]):
        if value is None:
            if "OECFRows" in self._dataset:
                del self._dataset.OECFRows
        else:
            self._dataset.OECFRows = value

    @property
    def OECFColumns(self) -> Optional[int]:
        if "OECFColumns" in self._dataset:
            return self._dataset.OECFColumns
        return None

    @OECFColumns.setter
    def OECFColumns(self, value: Optional[int]):
        if value is None:
            if "OECFColumns" in self._dataset:
                del self._dataset.OECFColumns
        else:
            self._dataset.OECFColumns = value

    @property
    def OECFColumnNames(self) -> Optional[List[str]]:
        if "OECFColumnNames" in self._dataset:
            return self._dataset.OECFColumnNames
        return None

    @OECFColumnNames.setter
    def OECFColumnNames(self, value: Optional[List[str]]):
        if value is None:
            if "OECFColumnNames" in self._dataset:
                del self._dataset.OECFColumnNames
        else:
            self._dataset.OECFColumnNames = value

    @property
    def OECFValues(self) -> Optional[List[Decimal]]:
        if "OECFValues" in self._dataset:
            return self._dataset.OECFValues
        return None

    @OECFValues.setter
    def OECFValues(self, value: Optional[List[Decimal]]):
        if value is None:
            if "OECFValues" in self._dataset:
                del self._dataset.OECFValues
        else:
            self._dataset.OECFValues = value

    @property
    def SpatialFrequencyResponseRows(self) -> Optional[int]:
        if "SpatialFrequencyResponseRows" in self._dataset:
            return self._dataset.SpatialFrequencyResponseRows
        return None

    @SpatialFrequencyResponseRows.setter
    def SpatialFrequencyResponseRows(self, value: Optional[int]):
        if value is None:
            if "SpatialFrequencyResponseRows" in self._dataset:
                del self._dataset.SpatialFrequencyResponseRows
        else:
            self._dataset.SpatialFrequencyResponseRows = value

    @property
    def SpatialFrequencyResponseColumns(self) -> Optional[int]:
        if "SpatialFrequencyResponseColumns" in self._dataset:
            return self._dataset.SpatialFrequencyResponseColumns
        return None

    @SpatialFrequencyResponseColumns.setter
    def SpatialFrequencyResponseColumns(self, value: Optional[int]):
        if value is None:
            if "SpatialFrequencyResponseColumns" in self._dataset:
                del self._dataset.SpatialFrequencyResponseColumns
        else:
            self._dataset.SpatialFrequencyResponseColumns = value

    @property
    def SpatialFrequencyResponseColumnNames(self) -> Optional[List[str]]:
        if "SpatialFrequencyResponseColumnNames" in self._dataset:
            return self._dataset.SpatialFrequencyResponseColumnNames
        return None

    @SpatialFrequencyResponseColumnNames.setter
    def SpatialFrequencyResponseColumnNames(self, value: Optional[List[str]]):
        if value is None:
            if "SpatialFrequencyResponseColumnNames" in self._dataset:
                del self._dataset.SpatialFrequencyResponseColumnNames
        else:
            self._dataset.SpatialFrequencyResponseColumnNames = value

    @property
    def SpatialFrequencyResponseValues(self) -> Optional[List[Decimal]]:
        if "SpatialFrequencyResponseValues" in self._dataset:
            return self._dataset.SpatialFrequencyResponseValues
        return None

    @SpatialFrequencyResponseValues.setter
    def SpatialFrequencyResponseValues(self, value: Optional[List[Decimal]]):
        if value is None:
            if "SpatialFrequencyResponseValues" in self._dataset:
                del self._dataset.SpatialFrequencyResponseValues
        else:
            self._dataset.SpatialFrequencyResponseValues = value

    @property
    def ColorFilterArrayPatternRows(self) -> Optional[int]:
        if "ColorFilterArrayPatternRows" in self._dataset:
            return self._dataset.ColorFilterArrayPatternRows
        return None

    @ColorFilterArrayPatternRows.setter
    def ColorFilterArrayPatternRows(self, value: Optional[int]):
        if value is None:
            if "ColorFilterArrayPatternRows" in self._dataset:
                del self._dataset.ColorFilterArrayPatternRows
        else:
            self._dataset.ColorFilterArrayPatternRows = value

    @property
    def ColorFilterArrayPatternColumns(self) -> Optional[int]:
        if "ColorFilterArrayPatternColumns" in self._dataset:
            return self._dataset.ColorFilterArrayPatternColumns
        return None

    @ColorFilterArrayPatternColumns.setter
    def ColorFilterArrayPatternColumns(self, value: Optional[int]):
        if value is None:
            if "ColorFilterArrayPatternColumns" in self._dataset:
                del self._dataset.ColorFilterArrayPatternColumns
        else:
            self._dataset.ColorFilterArrayPatternColumns = value

    @property
    def ColorFilterArrayPatternValues(self) -> Optional[List[Decimal]]:
        if "ColorFilterArrayPatternValues" in self._dataset:
            return self._dataset.ColorFilterArrayPatternValues
        return None

    @ColorFilterArrayPatternValues.setter
    def ColorFilterArrayPatternValues(self, value: Optional[List[Decimal]]):
        if value is None:
            if "ColorFilterArrayPatternValues" in self._dataset:
                del self._dataset.ColorFilterArrayPatternValues
        else:
            self._dataset.ColorFilterArrayPatternValues = value

    @property
    def FlashFiringStatus(self) -> Optional[int]:
        if "FlashFiringStatus" in self._dataset:
            return self._dataset.FlashFiringStatus
        return None

    @FlashFiringStatus.setter
    def FlashFiringStatus(self, value: Optional[int]):
        if value is None:
            if "FlashFiringStatus" in self._dataset:
                del self._dataset.FlashFiringStatus
        else:
            self._dataset.FlashFiringStatus = value

    @property
    def FlashReturnStatus(self) -> Optional[int]:
        if "FlashReturnStatus" in self._dataset:
            return self._dataset.FlashReturnStatus
        return None

    @FlashReturnStatus.setter
    def FlashReturnStatus(self, value: Optional[int]):
        if value is None:
            if "FlashReturnStatus" in self._dataset:
                del self._dataset.FlashReturnStatus
        else:
            self._dataset.FlashReturnStatus = value

    @property
    def FlashMode(self) -> Optional[int]:
        if "FlashMode" in self._dataset:
            return self._dataset.FlashMode
        return None

    @FlashMode.setter
    def FlashMode(self, value: Optional[int]):
        if value is None:
            if "FlashMode" in self._dataset:
                del self._dataset.FlashMode
        else:
            self._dataset.FlashMode = value

    @property
    def FlashFunctionPresent(self) -> Optional[int]:
        if "FlashFunctionPresent" in self._dataset:
            return self._dataset.FlashFunctionPresent
        return None

    @FlashFunctionPresent.setter
    def FlashFunctionPresent(self, value: Optional[int]):
        if value is None:
            if "FlashFunctionPresent" in self._dataset:
                del self._dataset.FlashFunctionPresent
        else:
            self._dataset.FlashFunctionPresent = value

    @property
    def FlashRedEyeMode(self) -> Optional[int]:
        if "FlashRedEyeMode" in self._dataset:
            return self._dataset.FlashRedEyeMode
        return None

    @FlashRedEyeMode.setter
    def FlashRedEyeMode(self, value: Optional[int]):
        if value is None:
            if "FlashRedEyeMode" in self._dataset:
                del self._dataset.FlashRedEyeMode
        else:
            self._dataset.FlashRedEyeMode = value

    @property
    def ExposureProgram(self) -> Optional[int]:
        if "ExposureProgram" in self._dataset:
            return self._dataset.ExposureProgram
        return None

    @ExposureProgram.setter
    def ExposureProgram(self, value: Optional[int]):
        if value is None:
            if "ExposureProgram" in self._dataset:
                del self._dataset.ExposureProgram
        else:
            self._dataset.ExposureProgram = value

    @property
    def SpectralSensitivity(self) -> Optional[str]:
        if "SpectralSensitivity" in self._dataset:
            return self._dataset.SpectralSensitivity
        return None

    @SpectralSensitivity.setter
    def SpectralSensitivity(self, value: Optional[str]):
        if value is None:
            if "SpectralSensitivity" in self._dataset:
                del self._dataset.SpectralSensitivity
        else:
            self._dataset.SpectralSensitivity = value

    @property
    def PhotographicSensitivity(self) -> Optional[int]:
        if "PhotographicSensitivity" in self._dataset:
            return self._dataset.PhotographicSensitivity
        return None

    @PhotographicSensitivity.setter
    def PhotographicSensitivity(self, value: Optional[int]):
        if value is None:
            if "PhotographicSensitivity" in self._dataset:
                del self._dataset.PhotographicSensitivity
        else:
            self._dataset.PhotographicSensitivity = value

    @property
    def SelfTimerMode(self) -> Optional[int]:
        if "SelfTimerMode" in self._dataset:
            return self._dataset.SelfTimerMode
        return None

    @SelfTimerMode.setter
    def SelfTimerMode(self, value: Optional[int]):
        if value is None:
            if "SelfTimerMode" in self._dataset:
                del self._dataset.SelfTimerMode
        else:
            self._dataset.SelfTimerMode = value

    @property
    def SensitivityType(self) -> Optional[int]:
        if "SensitivityType" in self._dataset:
            return self._dataset.SensitivityType
        return None

    @SensitivityType.setter
    def SensitivityType(self, value: Optional[int]):
        if value is None:
            if "SensitivityType" in self._dataset:
                del self._dataset.SensitivityType
        else:
            self._dataset.SensitivityType = value

    @property
    def StandardOutputSensitivity(self) -> Optional[int]:
        if "StandardOutputSensitivity" in self._dataset:
            return self._dataset.StandardOutputSensitivity
        return None

    @StandardOutputSensitivity.setter
    def StandardOutputSensitivity(self, value: Optional[int]):
        if value is None:
            if "StandardOutputSensitivity" in self._dataset:
                del self._dataset.StandardOutputSensitivity
        else:
            self._dataset.StandardOutputSensitivity = value

    @property
    def RecommendedExposureIndex(self) -> Optional[int]:
        if "RecommendedExposureIndex" in self._dataset:
            return self._dataset.RecommendedExposureIndex
        return None

    @RecommendedExposureIndex.setter
    def RecommendedExposureIndex(self, value: Optional[int]):
        if value is None:
            if "RecommendedExposureIndex" in self._dataset:
                del self._dataset.RecommendedExposureIndex
        else:
            self._dataset.RecommendedExposureIndex = value

    @property
    def ISOSpeed(self) -> Optional[int]:
        if "ISOSpeed" in self._dataset:
            return self._dataset.ISOSpeed
        return None

    @ISOSpeed.setter
    def ISOSpeed(self, value: Optional[int]):
        if value is None:
            if "ISOSpeed" in self._dataset:
                del self._dataset.ISOSpeed
        else:
            self._dataset.ISOSpeed = value

    @property
    def ISOSpeedLatitudeyyy(self) -> Optional[int]:
        if "ISOSpeedLatitudeyyy" in self._dataset:
            return self._dataset.ISOSpeedLatitudeyyy
        return None

    @ISOSpeedLatitudeyyy.setter
    def ISOSpeedLatitudeyyy(self, value: Optional[int]):
        if value is None:
            if "ISOSpeedLatitudeyyy" in self._dataset:
                del self._dataset.ISOSpeedLatitudeyyy
        else:
            self._dataset.ISOSpeedLatitudeyyy = value

    @property
    def ISOSpeedLatitudezzz(self) -> Optional[int]:
        if "ISOSpeedLatitudezzz" in self._dataset:
            return self._dataset.ISOSpeedLatitudezzz
        return None

    @ISOSpeedLatitudezzz.setter
    def ISOSpeedLatitudezzz(self, value: Optional[int]):
        if value is None:
            if "ISOSpeedLatitudezzz" in self._dataset:
                del self._dataset.ISOSpeedLatitudezzz
        else:
            self._dataset.ISOSpeedLatitudezzz = value

    @property
    def EXIFVersion(self) -> Optional[str]:
        if "EXIFVersion" in self._dataset:
            return self._dataset.EXIFVersion
        return None

    @EXIFVersion.setter
    def EXIFVersion(self, value: Optional[str]):
        if value is None:
            if "EXIFVersion" in self._dataset:
                del self._dataset.EXIFVersion
        else:
            self._dataset.EXIFVersion = value

    @property
    def ShutterSpeedValue(self) -> Optional[Decimal]:
        if "ShutterSpeedValue" in self._dataset:
            return self._dataset.ShutterSpeedValue
        return None

    @ShutterSpeedValue.setter
    def ShutterSpeedValue(self, value: Optional[Decimal]):
        if value is None:
            if "ShutterSpeedValue" in self._dataset:
                del self._dataset.ShutterSpeedValue
        else:
            self._dataset.ShutterSpeedValue = value

    @property
    def ApertureValue(self) -> Optional[Decimal]:
        if "ApertureValue" in self._dataset:
            return self._dataset.ApertureValue
        return None

    @ApertureValue.setter
    def ApertureValue(self, value: Optional[Decimal]):
        if value is None:
            if "ApertureValue" in self._dataset:
                del self._dataset.ApertureValue
        else:
            self._dataset.ApertureValue = value

    @property
    def BrightnessValue(self) -> Optional[Decimal]:
        if "BrightnessValue" in self._dataset:
            return self._dataset.BrightnessValue
        return None

    @BrightnessValue.setter
    def BrightnessValue(self, value: Optional[Decimal]):
        if value is None:
            if "BrightnessValue" in self._dataset:
                del self._dataset.BrightnessValue
        else:
            self._dataset.BrightnessValue = value

    @property
    def ExposureBiasValue(self) -> Optional[Decimal]:
        if "ExposureBiasValue" in self._dataset:
            return self._dataset.ExposureBiasValue
        return None

    @ExposureBiasValue.setter
    def ExposureBiasValue(self, value: Optional[Decimal]):
        if value is None:
            if "ExposureBiasValue" in self._dataset:
                del self._dataset.ExposureBiasValue
        else:
            self._dataset.ExposureBiasValue = value

    @property
    def MaxApertureValue(self) -> Optional[Decimal]:
        if "MaxApertureValue" in self._dataset:
            return self._dataset.MaxApertureValue
        return None

    @MaxApertureValue.setter
    def MaxApertureValue(self, value: Optional[Decimal]):
        if value is None:
            if "MaxApertureValue" in self._dataset:
                del self._dataset.MaxApertureValue
        else:
            self._dataset.MaxApertureValue = value

    @property
    def SubjectDistance(self) -> Optional[Decimal]:
        if "SubjectDistance" in self._dataset:
            return self._dataset.SubjectDistance
        return None

    @SubjectDistance.setter
    def SubjectDistance(self, value: Optional[Decimal]):
        if value is None:
            if "SubjectDistance" in self._dataset:
                del self._dataset.SubjectDistance
        else:
            self._dataset.SubjectDistance = value

    @property
    def MeteringMode(self) -> Optional[int]:
        if "MeteringMode" in self._dataset:
            return self._dataset.MeteringMode
        return None

    @MeteringMode.setter
    def MeteringMode(self, value: Optional[int]):
        if value is None:
            if "MeteringMode" in self._dataset:
                del self._dataset.MeteringMode
        else:
            self._dataset.MeteringMode = value

    @property
    def LightSource(self) -> Optional[int]:
        if "LightSource" in self._dataset:
            return self._dataset.LightSource
        return None

    @LightSource.setter
    def LightSource(self, value: Optional[int]):
        if value is None:
            if "LightSource" in self._dataset:
                del self._dataset.LightSource
        else:
            self._dataset.LightSource = value

    @property
    def FocalLength(self) -> Optional[Decimal]:
        if "FocalLength" in self._dataset:
            return self._dataset.FocalLength
        return None

    @FocalLength.setter
    def FocalLength(self, value: Optional[Decimal]):
        if value is None:
            if "FocalLength" in self._dataset:
                del self._dataset.FocalLength
        else:
            self._dataset.FocalLength = value

    @property
    def SubjectArea(self) -> Optional[List[int]]:
        if "SubjectArea" in self._dataset:
            return self._dataset.SubjectArea
        return None

    @SubjectArea.setter
    def SubjectArea(self, value: Optional[List[int]]):
        if value is None:
            if "SubjectArea" in self._dataset:
                del self._dataset.SubjectArea
        else:
            self._dataset.SubjectArea = value

    @property
    def MakerNote(self) -> Optional[bytes]:
        if "MakerNote" in self._dataset:
            return self._dataset.MakerNote
        return None

    @MakerNote.setter
    def MakerNote(self, value: Optional[bytes]):
        if value is None:
            if "MakerNote" in self._dataset:
                del self._dataset.MakerNote
        else:
            self._dataset.MakerNote = value

    @property
    def Temperature(self) -> Optional[Decimal]:
        if "Temperature" in self._dataset:
            return self._dataset.Temperature
        return None

    @Temperature.setter
    def Temperature(self, value: Optional[Decimal]):
        if value is None:
            if "Temperature" in self._dataset:
                del self._dataset.Temperature
        else:
            self._dataset.Temperature = value

    @property
    def Humidity(self) -> Optional[Decimal]:
        if "Humidity" in self._dataset:
            return self._dataset.Humidity
        return None

    @Humidity.setter
    def Humidity(self, value: Optional[Decimal]):
        if value is None:
            if "Humidity" in self._dataset:
                del self._dataset.Humidity
        else:
            self._dataset.Humidity = value

    @property
    def Pressure(self) -> Optional[Decimal]:
        if "Pressure" in self._dataset:
            return self._dataset.Pressure
        return None

    @Pressure.setter
    def Pressure(self, value: Optional[Decimal]):
        if value is None:
            if "Pressure" in self._dataset:
                del self._dataset.Pressure
        else:
            self._dataset.Pressure = value

    @property
    def WaterDepth(self) -> Optional[Decimal]:
        if "WaterDepth" in self._dataset:
            return self._dataset.WaterDepth
        return None

    @WaterDepth.setter
    def WaterDepth(self, value: Optional[Decimal]):
        if value is None:
            if "WaterDepth" in self._dataset:
                del self._dataset.WaterDepth
        else:
            self._dataset.WaterDepth = value

    @property
    def Acceleration(self) -> Optional[Decimal]:
        if "Acceleration" in self._dataset:
            return self._dataset.Acceleration
        return None

    @Acceleration.setter
    def Acceleration(self, value: Optional[Decimal]):
        if value is None:
            if "Acceleration" in self._dataset:
                del self._dataset.Acceleration
        else:
            self._dataset.Acceleration = value

    @property
    def CameraElevationAngle(self) -> Optional[Decimal]:
        if "CameraElevationAngle" in self._dataset:
            return self._dataset.CameraElevationAngle
        return None

    @CameraElevationAngle.setter
    def CameraElevationAngle(self, value: Optional[Decimal]):
        if value is None:
            if "CameraElevationAngle" in self._dataset:
                del self._dataset.CameraElevationAngle
        else:
            self._dataset.CameraElevationAngle = value

    @property
    def FlashEnergy(self) -> Optional[List[Decimal]]:
        if "FlashEnergy" in self._dataset:
            return self._dataset.FlashEnergy
        return None

    @FlashEnergy.setter
    def FlashEnergy(self, value: Optional[List[Decimal]]):
        if value is None:
            if "FlashEnergy" in self._dataset:
                del self._dataset.FlashEnergy
        else:
            self._dataset.FlashEnergy = value

    @property
    def SubjectLocation(self) -> Optional[List[int]]:
        if "SubjectLocation" in self._dataset:
            return self._dataset.SubjectLocation
        return None

    @SubjectLocation.setter
    def SubjectLocation(self, value: Optional[List[int]]):
        if value is None:
            if "SubjectLocation" in self._dataset:
                del self._dataset.SubjectLocation
        else:
            self._dataset.SubjectLocation = value

    @property
    def PhotographicExposureIndex(self) -> Optional[Decimal]:
        if "PhotographicExposureIndex" in self._dataset:
            return self._dataset.PhotographicExposureIndex
        return None

    @PhotographicExposureIndex.setter
    def PhotographicExposureIndex(self, value: Optional[Decimal]):
        if value is None:
            if "PhotographicExposureIndex" in self._dataset:
                del self._dataset.PhotographicExposureIndex
        else:
            self._dataset.PhotographicExposureIndex = value

    @property
    def SensingMethod(self) -> Optional[int]:
        if "SensingMethod" in self._dataset:
            return self._dataset.SensingMethod
        return None

    @SensingMethod.setter
    def SensingMethod(self, value: Optional[int]):
        if value is None:
            if "SensingMethod" in self._dataset:
                del self._dataset.SensingMethod
        else:
            self._dataset.SensingMethod = value

    @property
    def FileSource(self) -> Optional[int]:
        if "FileSource" in self._dataset:
            return self._dataset.FileSource
        return None

    @FileSource.setter
    def FileSource(self, value: Optional[int]):
        if value is None:
            if "FileSource" in self._dataset:
                del self._dataset.FileSource
        else:
            self._dataset.FileSource = value

    @property
    def SceneType(self) -> Optional[int]:
        if "SceneType" in self._dataset:
            return self._dataset.SceneType
        return None

    @SceneType.setter
    def SceneType(self, value: Optional[int]):
        if value is None:
            if "SceneType" in self._dataset:
                del self._dataset.SceneType
        else:
            self._dataset.SceneType = value

    @property
    def CustomRendered(self) -> Optional[int]:
        if "CustomRendered" in self._dataset:
            return self._dataset.CustomRendered
        return None

    @CustomRendered.setter
    def CustomRendered(self, value: Optional[int]):
        if value is None:
            if "CustomRendered" in self._dataset:
                del self._dataset.CustomRendered
        else:
            self._dataset.CustomRendered = value

    @property
    def ExposureMode(self) -> Optional[int]:
        if "ExposureMode" in self._dataset:
            return self._dataset.ExposureMode
        return None

    @ExposureMode.setter
    def ExposureMode(self, value: Optional[int]):
        if value is None:
            if "ExposureMode" in self._dataset:
                del self._dataset.ExposureMode
        else:
            self._dataset.ExposureMode = value

    @property
    def WhiteBalance(self) -> Optional[int]:
        if "WhiteBalance" in self._dataset:
            return self._dataset.WhiteBalance
        return None

    @WhiteBalance.setter
    def WhiteBalance(self, value: Optional[int]):
        if value is None:
            if "WhiteBalance" in self._dataset:
                del self._dataset.WhiteBalance
        else:
            self._dataset.WhiteBalance = value

    @property
    def DigitalZoomRatio(self) -> Optional[Decimal]:
        if "DigitalZoomRatio" in self._dataset:
            return self._dataset.DigitalZoomRatio
        return None

    @DigitalZoomRatio.setter
    def DigitalZoomRatio(self, value: Optional[Decimal]):
        if value is None:
            if "DigitalZoomRatio" in self._dataset:
                del self._dataset.DigitalZoomRatio
        else:
            self._dataset.DigitalZoomRatio = value

    @property
    def FocalLengthIn35mmFilm(self) -> Optional[int]:
        if "FocalLengthIn35mmFilm" in self._dataset:
            return self._dataset.FocalLengthIn35mmFilm
        return None

    @FocalLengthIn35mmFilm.setter
    def FocalLengthIn35mmFilm(self, value: Optional[int]):
        if value is None:
            if "FocalLengthIn35mmFilm" in self._dataset:
                del self._dataset.FocalLengthIn35mmFilm
        else:
            self._dataset.FocalLengthIn35mmFilm = value

    @property
    def SceneCaptureType(self) -> Optional[int]:
        if "SceneCaptureType" in self._dataset:
            return self._dataset.SceneCaptureType
        return None

    @SceneCaptureType.setter
    def SceneCaptureType(self, value: Optional[int]):
        if value is None:
            if "SceneCaptureType" in self._dataset:
                del self._dataset.SceneCaptureType
        else:
            self._dataset.SceneCaptureType = value

    @property
    def GainControl(self) -> Optional[int]:
        if "GainControl" in self._dataset:
            return self._dataset.GainControl
        return None

    @GainControl.setter
    def GainControl(self, value: Optional[int]):
        if value is None:
            if "GainControl" in self._dataset:
                del self._dataset.GainControl
        else:
            self._dataset.GainControl = value

    @property
    def Contrast(self) -> Optional[int]:
        if "Contrast" in self._dataset:
            return self._dataset.Contrast
        return None

    @Contrast.setter
    def Contrast(self, value: Optional[int]):
        if value is None:
            if "Contrast" in self._dataset:
                del self._dataset.Contrast
        else:
            self._dataset.Contrast = value

    @property
    def Saturation(self) -> Optional[int]:
        if "Saturation" in self._dataset:
            return self._dataset.Saturation
        return None

    @Saturation.setter
    def Saturation(self, value: Optional[int]):
        if value is None:
            if "Saturation" in self._dataset:
                del self._dataset.Saturation
        else:
            self._dataset.Saturation = value

    @property
    def Sharpness(self) -> Optional[int]:
        if "Sharpness" in self._dataset:
            return self._dataset.Sharpness
        return None

    @Sharpness.setter
    def Sharpness(self, value: Optional[int]):
        if value is None:
            if "Sharpness" in self._dataset:
                del self._dataset.Sharpness
        else:
            self._dataset.Sharpness = value

    @property
    def DeviceSettingDescription(self) -> Optional[bytes]:
        if "DeviceSettingDescription" in self._dataset:
            return self._dataset.DeviceSettingDescription
        return None

    @DeviceSettingDescription.setter
    def DeviceSettingDescription(self, value: Optional[bytes]):
        if value is None:
            if "DeviceSettingDescription" in self._dataset:
                del self._dataset.DeviceSettingDescription
        else:
            self._dataset.DeviceSettingDescription = value

    @property
    def SubjectDistanceRange(self) -> Optional[int]:
        if "SubjectDistanceRange" in self._dataset:
            return self._dataset.SubjectDistanceRange
        return None

    @SubjectDistanceRange.setter
    def SubjectDistanceRange(self, value: Optional[int]):
        if value is None:
            if "SubjectDistanceRange" in self._dataset:
                del self._dataset.SubjectDistanceRange
        else:
            self._dataset.SubjectDistanceRange = value

    @property
    def InteroperabilityIndex(self) -> Optional[str]:
        if "InteroperabilityIndex" in self._dataset:
            return self._dataset.InteroperabilityIndex
        return None

    @InteroperabilityIndex.setter
    def InteroperabilityIndex(self, value: Optional[str]):
        if value is None:
            if "InteroperabilityIndex" in self._dataset:
                del self._dataset.InteroperabilityIndex
        else:
            self._dataset.InteroperabilityIndex = value

    @property
    def InteroperabilityVersion(self) -> Optional[bytes]:
        if "InteroperabilityVersion" in self._dataset:
            return self._dataset.InteroperabilityVersion
        return None

    @InteroperabilityVersion.setter
    def InteroperabilityVersion(self, value: Optional[bytes]):
        if value is None:
            if "InteroperabilityVersion" in self._dataset:
                del self._dataset.InteroperabilityVersion
        else:
            self._dataset.InteroperabilityVersion = value
    ...
    @property
    def GPSVersionID(self) -> Optional[bytes]:
        if "GPSVersionID" in self._dataset:
            return self._dataset.GPSVersionID
        return None

    @GPSVersionID.setter
    def GPSVersionID(self, value: Optional[bytes]):
        if value is None:
            if "GPSVersionID" in self._dataset:
                del self._dataset.GPSVersionID
        else:
            self._dataset.GPSVersionID = value

    @property
    def GPSLatitudeRef(self) -> Optional[str]:
        if "GPSLatitudeRef" in self._dataset:
            return self._dataset.GPSLatitudeRef
        return None

    @GPSLatitudeRef.setter
    def GPSLatitudeRef(self, value: Optional[str]):
        if value is None:
            if "GPSLatitudeRef" in self._dataset:
                del self._dataset.GPSLatitudeRef
        else:
            self._dataset.GPSLatitudeRef = value

    @property
    def GPSLatitude(self) -> Optional[List[Decimal]]:
        if "GPSLatitude" in self._dataset:
            return self._dataset.GPSLatitude
        return None

    @GPSLatitude.setter
    def GPSLatitude(self, value: Optional[List[Decimal]]):
        if value is None:
            if "GPSLatitude" in self._dataset:
                del self._dataset.GPSLatitude
        else:
            self._dataset.GPSLatitude = value

    @property
    def GPSLongitudeRef(self) -> Optional[str]:
        if "GPSLongitudeRef" in self._dataset:
            return self._dataset.GPSLongitudeRef
        return None

    @GPSLongitudeRef.setter
    def GPSLongitudeRef(self, value: Optional[str]):
        if value is None:
            if "GPSLongitudeRef" in self._dataset:
                del self._dataset.GPSLongitudeRef
        else:
            self._dataset.GPSLongitudeRef = value

    @property
    def GPSLongitude(self) -> Optional[List[Decimal]]:
        if "GPSLongitude" in self._dataset:
            return self._dataset.GPSLongitude
        return None

    @GPSLongitude.setter
    def GPSLongitude(self, value: Optional[List[Decimal]]):
        if value is None:
            if "GPSLongitude" in self._dataset:
                del self._dataset.GPSLongitude
        else:
            self._dataset.GPSLongitude = value

    @property
    def GPSAltitudeRef(self) -> Optional[int]:
        if "GPSAltitudeRef" in self._dataset:
            return self._dataset.GPSAltitudeRef
        return None

    @GPSAltitudeRef.setter
    def GPSAltitudeRef(self, value: Optional[int]):
        if value is None:
            if "GPSAltitudeRef" in self._dataset:
                del self._dataset.GPSAltitudeRef
        else:
            self._dataset.GPSAltitudeRef = value

    @property
    def GPSAltitude(self) -> Optional[Decimal]:
        if "GPSAltitude" in self._dataset:
            return self._dataset.GPSAltitude
        return None

    @GPSAltitude.setter
    def GPSAltitude(self, value: Optional[Decimal]):
        if value is None:
            if "GPSAltitude" in self._dataset:
                del self._dataset.GPSAltitude
        else:
            self._dataset.GPSAltitude = value

    @property
    def GPSTimeStamp(self) -> Optional[str]:
        if "GPSTimeStamp" in self._dataset:
            return self._dataset.GPSTimeStamp
        return None

    @GPSTimeStamp.setter
    def GPSTimeStamp(self, value: Optional[str]):
        if value is None:
            if "GPSTimeStamp" in self._dataset:
                del self._dataset.GPSTimeStamp
        else:
            self._dataset.GPSTimeStamp = value

    @property
    def GPSSatellites(self) -> Optional[str]:
        if "GPSSatellites" in self._dataset:
            return self._dataset.GPSSatellites
        return None

    @GPSSatellites.setter
    def GPSSatellites(self, value: Optional[str]):
        if value is None:
            if "GPSSatellites" in self._dataset:
                del self._dataset.GPSSatellites
        else:
            self._dataset.GPSSatellites = value

    @property
    def GPSStatus(self) -> Optional[str]:
        if "GPSStatus" in self._dataset:
            return self._dataset.GPSStatus
        return None

    @GPSStatus.setter
    def GPSStatus(self, value: Optional[str]):
        if value is None:
            if "GPSStatus" in self._dataset:
                del self._dataset.GPSStatus
        else:
            self._dataset.GPSStatus = value

    @property
    def GPSMeasureMode(self) -> Optional[str]:
        if "GPSMeasureMode" in self._dataset:
            return self._dataset.GPSMeasureMode
        return None

    @GPSMeasureMode.setter
    def GPSMeasureMode(self, value: Optional[str]):
        if value is None:
            if "GPSMeasureMode" in self._dataset:
                del self._dataset.GPSMeasureMode
        else:
            self._dataset.GPSMeasureMode = value

    @property
    def GPSDOP(self) -> Optional[Decimal]:
        if "GPSDOP" in self._dataset:
            return self._dataset.GPSDOP
        return None

    @GPSDOP.setter
    def GPSDOP(self, value: Optional[Decimal]):
        if value is None:
            if "GPSDOP" in self._dataset:
                del self._dataset.GPSDOP
        else:
            self._dataset.GPSDOP = value

    @property
    def GPSSpeedRef(self) -> Optional[str]:
        if "GPSSpeedRef" in self._dataset:
            return self._dataset.GPSSpeedRef
        return None

    @GPSSpeedRef.setter
    def GPSSpeedRef(self, value: Optional[str]):
        if value is None:
            if "GPSSpeedRef" in self._dataset:
                del self._dataset.GPSSpeedRef
        else:
            self._dataset.GPSSpeedRef = value

    @property
    def GPSSpeed(self) -> Optional[Decimal]:
        if "GPSSpeed" in self._dataset:
            return self._dataset.GPSSpeed
        return None

    @GPSSpeed.setter
    def GPSSpeed(self, value: Optional[Decimal]):
        if value is None:
            if "GPSSpeed" in self._dataset:
                del self._dataset.GPSSpeed
        else:
            self._dataset.GPSSpeed = value

    @property
    def GPSTrackRef(self) -> Optional[str]:
        if "GPSTrackRef" in self._dataset:
            return self._dataset.GPSTrackRef
        return None

    @GPSTrackRef.setter
    def GPSTrackRef(self, value: Optional[str]):
        if value is None:
            if "GPSTrackRef" in self._dataset:
                del self._dataset.GPSTrackRef
        else:
            self._dataset.GPSTrackRef = value

    @property
    def GPSTrack(self) -> Optional[Decimal]:
        if "GPSTrack" in self._dataset:
            return self._dataset.GPSTrack
        return None

    @GPSTrack.setter
    def GPSTrack(self, value: Optional[Decimal]):
        if value is None:
            if "GPSTrack" in self._dataset:
                del self._dataset.GPSTrack
        else:
            self._dataset.GPSTrack = value

    @property
    def GPSImgDirectionRef(self) -> Optional[str]:
        if "GPSImgDirectionRef" in self._dataset:
            return self._dataset.GPSImgDirectionRef
        return None

    @GPSImgDirectionRef.setter
    def GPSImgDirectionRef(self, value: Optional[str]):
        if value is None:
            if "GPSImgDirectionRef" in self._dataset:
                del self._dataset.GPSImgDirectionRef
        else:
            self._dataset.GPSImgDirectionRef = value

    @property
    def GPSImgDirection(self) -> Optional[Decimal]:
        if "GPSImgDirection" in self._dataset:
            return self._dataset.GPSImgDirection
        return None

    @GPSImgDirection.setter
    def GPSImgDirection(self, value: Optional[Decimal]):
        if value is None:
            if "GPSImgDirection" in self._dataset:
                del self._dataset.GPSImgDirection
        else:
            self._dataset.GPSImgDirection = value

    @property
    def GPSMapDatum(self) -> Optional[str]:
        if "GPSMapDatum" in self._dataset:
            return self._dataset.GPSMapDatum
        return None

    @GPSMapDatum.setter
    def GPSMapDatum(self, value: Optional[str]):
        if value is None:
            if "GPSMapDatum" in self._dataset:
                del self._dataset.GPSMapDatum
        else:
            self._dataset.GPSMapDatum = value

    @property
    def GPSDestLatitudeRef(self) -> Optional[str]:
        if "GPSDestLatitudeRef" in self._dataset:
            return self._dataset.GPSDestLatitudeRef
        return None

    @GPSDestLatitudeRef.setter
    def GPSDestLatitudeRef(self, value: Optional[str]):
        if value is None:
            if "GPSDestLatitudeRef" in self._dataset:
                del self._dataset.GPSDestLatitudeRef
        else:
            self._dataset.GPSDestLatitudeRef = value

    @property
    def GPSDestLatitude(self) -> Optional[List[Decimal]]:
        if "GPSDestLatitude" in self._dataset:
            return self._dataset.GPSDestLatitude
        return None

    @GPSDestLatitude.setter
    def GPSDestLatitude(self, value: Optional[List[Decimal]]):
        if value is None:
            if "GPSDestLatitude" in self._dataset:
                del self._dataset.GPSDestLatitude
        else:
            self._dataset.GPSDestLatitude = value

    @property
    def GPSDestLongitudeRef(self) -> Optional[str]:
        if "GPSDestLongitudeRef" in self._dataset:
            return self._dataset.GPSDestLongitudeRef
        return None

    @GPSDestLongitudeRef.setter
    def GPSDestLongitudeRef(self, value: Optional[str]):
        if value is None:
            if "GPSDestLongitudeRef" in self._dataset:
                del self._dataset.GPSDestLongitudeRef
        else:
            self._dataset.GPSDestLongitudeRef = value

    @property
    def GPSDestLongitude(self) -> Optional[List[Decimal]]:
        if "GPSDestLongitude" in self._dataset:
            return self._dataset.GPSDestLongitude
        return None

    @GPSDestLongitude.setter
    def GPSDestLongitude(self, value: Optional[List[Decimal]]):
        if value is None:
            if "GPSDestLongitude" in self._dataset:
                del self._dataset.GPSDestLongitude
        else:
            self._dataset.GPSDestLongitude = value

    @property
    def GPSDestBearingRef(self) -> Optional[str]:
        if "GPSDestBearingRef" in self._dataset:
            return self._dataset.GPSDestBearingRef
        return None

    @GPSDestBearingRef.setter
    def GPSDestBearingRef(self, value: Optional[str]):
        if value is None:
            if "GPSDestBearingRef" in self._dataset:
                del self._dataset.GPSDestBearingRef
        else:
            self._dataset.GPSDestBearingRef = value

    @property
    def GPSDestBearing(self) -> Optional[Decimal]:
        if "GPSDestBearing" in self._dataset:
            return self._dataset.GPSDestBearing
        return None

    @GPSDestBearing.setter
    def GPSDestBearing(self, value: Optional[Decimal]):
        if value is None:
            if "GPSDestBearing" in self._dataset:
                del self._dataset.GPSDestBearing
        else:
            self._dataset.GPSDestBearing = value

    @property
    def GPSDestDistanceRef(self) -> Optional[str]:
        if "GPSDestDistanceRef" in self._dataset:
            return self._dataset.GPSDestDistanceRef
        return None

    @GPSDestDistanceRef.setter
    def GPSDestDistanceRef(self, value: Optional[str]):
        if value is None:
            if "GPSDestDistanceRef" in self._dataset:
                del self._dataset.GPSDestDistanceRef
        else:
            self._dataset.GPSDestDistanceRef = value

    @property
    def GPSDestDistance(self) -> Optional[Decimal]:
        if "GPSDestDistance" in self._dataset:
            return self._dataset.GPSDestDistance
        return None

    @GPSDestDistance.setter
    def GPSDestDistance(self, value: Optional[Decimal]):
        if value is None:
            if "GPSDestDistance" in self._dataset:
                del self._dataset.GPSDestDistance
        else:
            self._dataset.GPSDestDistance = value

    @property
    def GPSProcessingMethod(self) -> Optional[bytes]:
        if "GPSProcessingMethod" in self._dataset:
            return self._dataset.GPSProcessingMethod
        return None

    @GPSProcessingMethod.setter
    def GPSProcessingMethod(self, value: Optional[bytes]):
        if value is None:
            if "GPSProcessingMethod" in self._dataset:
                del self._dataset.GPSProcessingMethod
        else:
            self._dataset.GPSProcessingMethod = value

    @property
    def GPSAreaInformation(self) -> Optional[bytes]:
        if "GPSAreaInformation" in self._dataset:
            return self._dataset.GPSAreaInformation
        return None

    @GPSAreaInformation.setter
    def GPSAreaInformation(self, value: Optional[bytes]):
        if value is None:
            if "GPSAreaInformation" in self._dataset:
                del self._dataset.GPSAreaInformation
        else:
            self._dataset.GPSAreaInformation = value

    @property
    def GPSDateStamp(self) -> Optional[str]:
        if "GPSDateStamp" in self._dataset:
            return self._dataset.GPSDateStamp
        return None

    @GPSDateStamp.setter
    def GPSDateStamp(self, value: Optional[str]):
        if value is None:
            if "GPSDateStamp" in self._dataset:
                del self._dataset.GPSDateStamp
        else:
            self._dataset.GPSDateStamp = value

    @property
    def GPSDifferential(self) -> Optional[int]:
        if "GPSDifferential" in self._dataset:
            return self._dataset.GPSDifferential
        return None

    @GPSDifferential.setter
    def GPSDifferential(self, value: Optional[int]):
        if value is None:
            if "GPSDifferential" in self._dataset:
                del self._dataset.GPSDifferential
        else:
            self._dataset.GPSDifferential = value
    ...
    @property
    def CameraOwnerName(self) -> Optional[str]:
        if "CameraOwnerName" in self._dataset:
            return self._dataset.CameraOwnerName
        return None

    @CameraOwnerName.setter
    def CameraOwnerName(self, value: Optional[str]):
        if value is None:
            if "CameraOwnerName" in self._dataset:
                del self._dataset.CameraOwnerName
        else:
            self._dataset.CameraOwnerName = value

    @property
    def LensSpecification(self) -> Optional[List[Decimal]]:
        if "LensSpecification" in self._dataset:
            return self._dataset.LensSpecification
        return None

    @LensSpecification.setter
    def LensSpecification(self, value: Optional[List[Decimal]]):
        if value is None:
            if "LensSpecification" in self._dataset:
                del self._dataset.LensSpecification
        else:
            self._dataset.LensSpecification = value

    @property
    def LensMake(self) -> Optional[str]:
        if "LensMake" in self._dataset:
            return self._dataset.LensMake
        return None

    @LensMake.setter
    def LensMake(self, value: Optional[str]):
        if value is None:
            if "LensMake" in self._dataset:
                del self._dataset.LensMake
        else:
            self._dataset.LensMake = value

    @property
    def LensModel(self) -> Optional[str]:
        if "LensModel" in self._dataset:
            return self._dataset.LensModel
        return None

    @LensModel.setter
    def LensModel(self, value: Optional[str]):
        if value is None:
            if "LensModel" in self._dataset:
                del self._dataset.LensModel
        else:
            self._dataset.LensModel = value

    @property
    def LensSerialNumber(self) -> Optional[str]:
        if "LensSerialNumber" in self._dataset:
            return self._dataset.LensSerialNumber
        return None

    @LensSerialNumber.setter
    def LensSerialNumber(self, value: Optional[str]):
        if value is None:
            if "LensSerialNumber" in self._dataset:
                del self._dataset.LensSerialNumber
        else:
            self._dataset.LensSerialNumber = value
