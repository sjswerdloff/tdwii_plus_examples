...
from .image_center_point_coordinates_sequence_item import (
...
class VlSlideCoordinatesMicroscopicImage:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._ImageCenterPointCoordinatesSequence: List[ImageCenterPointCoordinatesSequenceItem] = []
    ...
    @property
    def ImageCenterPointCoordinatesSequence(self) -> Optional[List[ImageCenterPointCoordinatesSequenceItem]]:
        if "ImageCenterPointCoordinatesSequence" in self._dataset:
            if len(self._ImageCenterPointCoordinatesSequence) == len(self._dataset.ImageCenterPointCoordinatesSequence):
                return self._ImageCenterPointCoordinatesSequence
            else:
                return [ImageCenterPointCoordinatesSequenceItem(x) for x in self._dataset.ImageCenterPointCoordinatesSequence]
        return None

    @ImageCenterPointCoordinatesSequence.setter
    def ImageCenterPointCoordinatesSequence(self, value: Optional[List[ImageCenterPointCoordinatesSequenceItem]]):
        if value is None:
            self._ImageCenterPointCoordinatesSequence = []
            if "ImageCenterPointCoordinatesSequence" in self._dataset:
                del self._dataset.ImageCenterPointCoordinatesSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, ImageCenterPointCoordinatesSequenceItem) for item in value
        ):
            raise ValueError(
                "ImageCenterPointCoordinatesSequence must be a list of ImageCenterPointCoordinatesSequenceItem objects"
            )
        else:
            self._ImageCenterPointCoordinatesSequence = value
            if "ImageCenterPointCoordinatesSequence" not in self._dataset:
                self._dataset.ImageCenterPointCoordinatesSequence = pydicom.Sequence()
            self._dataset.ImageCenterPointCoordinatesSequence.clear()
            self._dataset.ImageCenterPointCoordinatesSequence.extend([item.to_dataset() for item in value])

    def add_ImageCenterPointCoordinates(self, item: ImageCenterPointCoordinatesSequenceItem):
        if not isinstance(item, ImageCenterPointCoordinatesSequenceItem):
            raise ValueError("Item must be an instance of ImageCenterPointCoordinatesSequenceItem")
        self._ImageCenterPointCoordinatesSequence.append(item)
        if "ImageCenterPointCoordinatesSequence" not in self._dataset:
            self._dataset.ImageCenterPointCoordinatesSequence = pydicom.Sequence()
        self._dataset.ImageCenterPointCoordinatesSequence.append(item.to_dataset())
