...
class VlWholeSlideMicroscopyImage:
    ...
    def OpticalPathSequence(self, value: Optional[List[OpticalPathSequenceItem]]):
        if value is None:
            ...
            raise ValueError("OpticalPathSequence must be a list of OpticalPathSequenceItem objects")
        else:
    ...
    def add_OpticalPath(self, item: OpticalPathSequenceItem):
        if not isinstance(item, OpticalPathSequenceItem):
            raise ValueError("Item must be an instance of OpticalPathSequenceItem")
    ...
    def LabelText(self) -> Optional[str]:
        if "LabelText" in self._dataset:
            return self._dataset.LabelText
        return None

    @LabelText.setter
    def LabelText(self, value: Optional[str]):
        if value is None:
            if "LabelText" in self._dataset:
                del self._dataset.LabelText
        else:
            self._dataset.LabelText = value
    ...
    def ImagedVolumeWidth(self) -> Optional[float]:
        if "ImagedVolumeWidth" in self._dataset:
            return self._dataset.ImagedVolumeWidth
        return None

    @ImagedVolumeWidth.setter
    def ImagedVolumeWidth(self, value: Optional[float]):
        if value is None:
            if "ImagedVolumeWidth" in self._dataset:
                del self._dataset.ImagedVolumeWidth
        else:
            self._dataset.ImagedVolumeWidth = value

    @property
    def ImagedVolumeHeight(self) -> Optional[float]:
        if "ImagedVolumeHeight" in self._dataset:
            return self._dataset.ImagedVolumeHeight
        return None

    @ImagedVolumeHeight.setter
    def ImagedVolumeHeight(self, value: Optional[float]):
        if value is None:
            if "ImagedVolumeHeight" in self._dataset:
                del self._dataset.ImagedVolumeHeight
        else:
            self._dataset.ImagedVolumeHeight = value

    @property
    def ImagedVolumeDepth(self) -> Optional[float]:
        if "ImagedVolumeDepth" in self._dataset:
            return self._dataset.ImagedVolumeDepth
        return None

    @ImagedVolumeDepth.setter
    def ImagedVolumeDepth(self, value: Optional[float]):
        if value is None:
            if "ImagedVolumeDepth" in self._dataset:
                del self._dataset.ImagedVolumeDepth
        else:
            self._dataset.ImagedVolumeDepth = value

    @property
    def SpecimenLabelInImage(self) -> Optional[str]:
        if "SpecimenLabelInImage" in self._dataset:
            return self._dataset.SpecimenLabelInImage
        return None

    @SpecimenLabelInImage.setter
    def SpecimenLabelInImage(self, value: Optional[str]):
        if value is None:
            if "SpecimenLabelInImage" in self._dataset:
                del self._dataset.SpecimenLabelInImage
        else:
            self._dataset.SpecimenLabelInImage = value

    @property
    def FocusMethod(self) -> Optional[str]:
        if "FocusMethod" in self._dataset:
            return self._dataset.FocusMethod
        return None

    @FocusMethod.setter
    def FocusMethod(self, value: Optional[str]):
        if value is None:
            if "FocusMethod" in self._dataset:
                del self._dataset.FocusMethod
        else:
            self._dataset.FocusMethod = value

    @property
    def ExtendedDepthOfField(self) -> Optional[str]:
        if "ExtendedDepthOfField" in self._dataset:
            return self._dataset.ExtendedDepthOfField
        return None

    @ExtendedDepthOfField.setter
    def ExtendedDepthOfField(self, value: Optional[str]):
        if value is None:
            if "ExtendedDepthOfField" in self._dataset:
                del self._dataset.ExtendedDepthOfField
        else:
            self._dataset.ExtendedDepthOfField = value

    @property
    def NumberOfFocalPlanes(self) -> Optional[int]:
        if "NumberOfFocalPlanes" in self._dataset:
            return self._dataset.NumberOfFocalPlanes
        return None

    @NumberOfFocalPlanes.setter
    def NumberOfFocalPlanes(self, value: Optional[int]):
        if value is None:
            if "NumberOfFocalPlanes" in self._dataset:
                del self._dataset.NumberOfFocalPlanes
        else:
            self._dataset.NumberOfFocalPlanes = value

    @property
    def DistanceBetweenFocalPlanes(self) -> Optional[float]:
        if "DistanceBetweenFocalPlanes" in self._dataset:
            return self._dataset.DistanceBetweenFocalPlanes
        return None

    @DistanceBetweenFocalPlanes.setter
    def DistanceBetweenFocalPlanes(self, value: Optional[float]):
        if value is None:
            if "DistanceBetweenFocalPlanes" in self._dataset:
                del self._dataset.DistanceBetweenFocalPlanes
        else:
            self._dataset.DistanceBetweenFocalPlanes = value

    @property
    def RecommendedAbsentPixelCIELabValue(self) -> Optional[List[int]]:
        if "RecommendedAbsentPixelCIELabValue" in self._dataset:
            return self._dataset.RecommendedAbsentPixelCIELabValue
        return None

    @RecommendedAbsentPixelCIELabValue.setter
    def RecommendedAbsentPixelCIELabValue(self, value: Optional[List[int]]):
        if value is None:
            if "RecommendedAbsentPixelCIELabValue" in self._dataset:
                del self._dataset.RecommendedAbsentPixelCIELabValue
        else:
            self._dataset.RecommendedAbsentPixelCIELabValue = value
