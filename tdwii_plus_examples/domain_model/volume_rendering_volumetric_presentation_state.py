...
from .alternate_content_description_sequence_item import (
    AlternateContentDescriptionSequenceItem,
)
from .anatomic_region_sequence_item import AnatomicRegionSequenceItem
from .animation_curve_sequence_item import AnimationCurveSequenceItem
...
from .graphic_annotation_sequence_item import GraphicAnnotationSequenceItem
from .graphic_group_sequence_item import GraphicGroupSequenceItem
from .graphic_layer_sequence_item import GraphicLayerSequenceItem
...
from .presentation_state_compositor_component_sequence_item import (
    PresentationStateCompositorComponentSequenceItem,
)
...
from .rendered_image_reference_sequence_item import RenderedImageReferenceSequenceItem
...
from .volume_cropping_sequence_item import VolumeCroppingSequenceItem
from .volume_stream_sequence_item import VolumeStreamSequenceItem
...
from .volumetric_presentation_input_set_sequence_item import (
    VolumetricPresentationInputSetSequenceItem,
)
from .volumetric_presentation_state_input_sequence_item import (
    VolumetricPresentationStateInputSequenceItem,
)


class VolumeRenderingVolumetricPresentationState:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._AlternateContentDescriptionSequence: List[AlternateContentDescriptionSequenceItem] = []
        self._RenderedImageReferenceSequence: List[RenderedImageReferenceSequenceItem] = []
        self._AnimationCurveSequence: List[AnimationCurveSequenceItem] = []
        ...
        self._VolumeCroppingSequence: List[VolumeCroppingSequenceItem] = []
        ...
        self._PresentationStateCompositorComponentSequence: List[PresentationStateCompositorComponentSequenceItem] = []
        self._VolumeStreamSequence: List[VolumeStreamSequenceItem] = []
        ...
        self._VolumetricPresentationStateInputSequence: List[VolumetricPresentationStateInputSequenceItem] = []
        self._VolumetricPresentationInputSetSequence: List[VolumetricPresentationInputSetSequenceItem] = []
        ...
        self._GraphicLayerSequence: List[GraphicLayerSequenceItem] = []
        self._VolumetricAnnotationSequence: List[VolumetricAnnotationSequenceItem] = []
        self._VolumetricPresentationInputAnnotationSequence: List[VolumetricPresentationInputAnnotationSequenceItem] = []
        ...
        self._GraphicGroupSequence: List[GraphicGroupSequenceItem] = []
    ...
    @property
    def ContentLabel(self) -> Optional[str]:
        if "ContentLabel" in self._dataset:
            return self._dataset.ContentLabel
        return None

    @ContentLabel.setter
    def ContentLabel(self, value: Optional[str]):
        if value is None:
            if "ContentLabel" in self._dataset:
                del self._dataset.ContentLabel
        else:
            self._dataset.ContentLabel = value
    ...
    @property
    def PresentationCreationDate(self) -> Optional[str]:
        if "PresentationCreationDate" in self._dataset:
            return self._dataset.PresentationCreationDate
        return None

    @PresentationCreationDate.setter
    def PresentationCreationDate(self, value: Optional[str]):
        if value is None:
            if "PresentationCreationDate" in self._dataset:
                del self._dataset.PresentationCreationDate
        else:
            self._dataset.PresentationCreationDate = value

    @property
    def PresentationCreationTime(self) -> Optional[str]:
        if "PresentationCreationTime" in self._dataset:
            return self._dataset.PresentationCreationTime
        return None

    @PresentationCreationTime.setter
    def PresentationCreationTime(self, value: Optional[str]):
        if value is None:
            if "PresentationCreationTime" in self._dataset:
                del self._dataset.PresentationCreationTime
        else:
            self._dataset.PresentationCreationTime = value
    ...
    @property
    def AlternateContentDescriptionSequence(self) -> Optional[List[AlternateContentDescriptionSequenceItem]]:
        if "AlternateContentDescriptionSequence" in self._dataset:
            if len(self._AlternateContentDescriptionSequence) == len(self._dataset.AlternateContentDescriptionSequence):
                return self._AlternateContentDescriptionSequence
            else:
                return [AlternateContentDescriptionSequenceItem(x) for x in self._dataset.AlternateContentDescriptionSequence]
        return None

    @AlternateContentDescriptionSequence.setter
    def AlternateContentDescriptionSequence(self, value: Optional[List[AlternateContentDescriptionSequenceItem]]):
        if value is None:
            self._AlternateContentDescriptionSequence = []
            if "AlternateContentDescriptionSequence" in self._dataset:
                del self._dataset.AlternateContentDescriptionSequence
            ...
            self._dataset.AlternateContentDescriptionSequence.clear()
            self._dataset.AlternateContentDescriptionSequence.extend([item.to_dataset() for item in value])
    ...
    def add_AlternateContentDescription(self, item: AlternateContentDescriptionSequenceItem):
        ...
        if "AlternateContentDescriptionSequence" not in self._dataset:
            self._dataset.AlternateContentDescriptionSequence = pydicom.Sequence()
        self._dataset.AlternateContentDescriptionSequence.append(item.to_dataset())

    @property
    def PresentationDisplayCollectionUID(self) -> Optional[str]:
        if "PresentationDisplayCollectionUID" in self._dataset:
            return self._dataset.PresentationDisplayCollectionUID
        return None

    @PresentationDisplayCollectionUID.setter
    def PresentationDisplayCollectionUID(self, value: Optional[str]):
        if value is None:
            if "PresentationDisplayCollectionUID" in self._dataset:
                del self._dataset.PresentationDisplayCollectionUID
        else:
            self._dataset.PresentationDisplayCollectionUID = value

    @property
    def PresentationSequenceCollectionUID(self) -> Optional[str]:
        if "PresentationSequenceCollectionUID" in self._dataset:
            return self._dataset.PresentationSequenceCollectionUID
        return None

    @PresentationSequenceCollectionUID.setter
    def PresentationSequenceCollectionUID(self, value: Optional[str]):
        if value is None:
            if "PresentationSequenceCollectionUID" in self._dataset:
                del self._dataset.PresentationSequenceCollectionUID
        else:
            self._dataset.PresentationSequenceCollectionUID = value

    @property
    def PresentationSequencePositionIndex(self) -> Optional[int]:
        if "PresentationSequencePositionIndex" in self._dataset:
            return self._dataset.PresentationSequencePositionIndex
        return None

    @PresentationSequencePositionIndex.setter
    def PresentationSequencePositionIndex(self, value: Optional[int]):
        if value is None:
            if "PresentationSequencePositionIndex" in self._dataset:
                del self._dataset.PresentationSequencePositionIndex
        else:
            self._dataset.PresentationSequencePositionIndex = value

    @property
    def RenderedImageReferenceSequence(self) -> Optional[List[RenderedImageReferenceSequenceItem]]:
        if "RenderedImageReferenceSequence" in self._dataset:
            if len(self._RenderedImageReferenceSequence) == len(self._dataset.RenderedImageReferenceSequence):
                return self._RenderedImageReferenceSequence
            else:
                return [RenderedImageReferenceSequenceItem(x) for x in self._dataset.RenderedImageReferenceSequence]
        return None

    @RenderedImageReferenceSequence.setter
    def RenderedImageReferenceSequence(self, value: Optional[List[RenderedImageReferenceSequenceItem]]):
        if value is None:
            self._RenderedImageReferenceSequence = []
            if "RenderedImageReferenceSequence" in self._dataset:
            ...
            self._dataset.RenderedImageReferenceSequence.clear()
            self._dataset.RenderedImageReferenceSequence.extend([item.to_dataset() for item in value])
    ...
    def add_RenderedImageReference(self, item: RenderedImageReferenceSequenceItem):
        ...
        if "RenderedImageReferenceSequence" not in self._dataset:
            self._dataset.RenderedImageReferenceSequence = pydicom.Sequence()
        self._dataset.RenderedImageReferenceSequence.append(item.to_dataset())

    @property
    def PresentationAnimationStyle(self) -> Optional[str]:
        if "PresentationAnimationStyle" in self._dataset:
            return self._dataset.PresentationAnimationStyle
        return None

    @PresentationAnimationStyle.setter
    def PresentationAnimationStyle(self, value: Optional[str]):
        if value is None:
            if "PresentationAnimationStyle" in self._dataset:
                del self._dataset.PresentationAnimationStyle
        else:
            self._dataset.PresentationAnimationStyle = value

    @property
    def RecommendedAnimationRate(self) -> Optional[float]:
        if "RecommendedAnimationRate" in self._dataset:
            return self._dataset.RecommendedAnimationRate
        return None

    @RecommendedAnimationRate.setter
    def RecommendedAnimationRate(self, value: Optional[float]):
        if value is None:
            if "RecommendedAnimationRate" in self._dataset:
                del self._dataset.RecommendedAnimationRate
        else:
            self._dataset.RecommendedAnimationRate = value

    @property
    def AnimationCurveSequence(self) -> Optional[List[AnimationCurveSequenceItem]]:
        if "AnimationCurveSequence" in self._dataset:
            if len(self._AnimationCurveSequence) == len(self._dataset.AnimationCurveSequence):
                return self._AnimationCurveSequence
            else:
                return [AnimationCurveSequenceItem(x) for x in self._dataset.AnimationCurveSequence]
        return None

    @AnimationCurveSequence.setter
    def AnimationCurveSequence(self, value: Optional[List[AnimationCurveSequenceItem]]):
        if value is None:
            self._AnimationCurveSequence = []
            if "AnimationCurveSequence" in self._dataset:
            ...
            self._dataset.AnimationCurveSequence.clear()
            self._dataset.AnimationCurveSequence.extend([item.to_dataset() for item in value])
    ...
    def add_AnimationCurve(self, item: AnimationCurveSequenceItem):
        ...
        if "AnimationCurveSequence" not in self._dataset:
            self._dataset.AnimationCurveSequence = pydicom.Sequence()
        self._dataset.AnimationCurveSequence.append(item.to_dataset())

    @property
    def AnimationStepSize(self) -> Optional[float]:
        if "AnimationStepSize" in self._dataset:
            return self._dataset.AnimationStepSize
        return None

    @AnimationStepSize.setter
    def AnimationStepSize(self, value: Optional[float]):
        if value is None:
            if "AnimationStepSize" in self._dataset:
                del self._dataset.AnimationStepSize
        else:
            self._dataset.AnimationStepSize = value

    @property
    def SwivelRange(self) -> Optional[float]:
        if "SwivelRange" in self._dataset:
            return self._dataset.SwivelRange
        return None

    @SwivelRange.setter
    def SwivelRange(self, value: Optional[float]):
        if value is None:
            if "SwivelRange" in self._dataset:
                del self._dataset.SwivelRange
        else:
            self._dataset.SwivelRange = value
    ...
    @property
    def VolumeCroppingSequence(self) -> Optional[List[VolumeCroppingSequenceItem]]:
        if "VolumeCroppingSequence" in self._dataset:
            if len(self._VolumeCroppingSequence) == len(self._dataset.VolumeCroppingSequence):
                return self._VolumeCroppingSequence
            else:
                return [VolumeCroppingSequenceItem(x) for x in self._dataset.VolumeCroppingSequence]
        return None

    @VolumeCroppingSequence.setter
    def VolumeCroppingSequence(self, value: Optional[List[VolumeCroppingSequenceItem]]):
        if value is None:
            self._VolumeCroppingSequence = []
            if "VolumeCroppingSequence" in self._dataset:
            ...
            self._dataset.VolumeCroppingSequence.clear()
            self._dataset.VolumeCroppingSequence.extend([item.to_dataset() for item in value])
    ...
    def add_VolumeCropping(self, item: VolumeCroppingSequenceItem):
        ...
        if "VolumeCroppingSequence" not in self._dataset:
            self._dataset.VolumeCroppingSequence = pydicom.Sequence()
        self._dataset.VolumeCroppingSequence.append(item.to_dataset())
    ...
    @property
    def GraphicAnnotationSequence(self) -> Optional[List[GraphicAnnotationSequenceItem]]:
        if "GraphicAnnotationSequence" in self._dataset:
            if len(self._GraphicAnnotationSequence) == len(self._dataset.GraphicAnnotationSequence):
                return self._GraphicAnnotationSequence
            else:
                return [GraphicAnnotationSequenceItem(x) for x in self._dataset.GraphicAnnotationSequence]
        return None

    @GraphicAnnotationSequence.setter
    def GraphicAnnotationSequence(self, value: Optional[List[GraphicAnnotationSequenceItem]]):
        if value is None:
            self._GraphicAnnotationSequence = []
            if "GraphicAnnotationSequence" in self._dataset:
            ...
            self._dataset.GraphicAnnotationSequence.clear()
            self._dataset.GraphicAnnotationSequence.extend([item.to_dataset() for item in value])
    ...
    def add_GraphicAnnotation(self, item: GraphicAnnotationSequenceItem):
        ...
        if "GraphicAnnotationSequence" not in self._dataset:
            self._dataset.GraphicAnnotationSequence = pydicom.Sequence()
        self._dataset.GraphicAnnotationSequence.append(item.to_dataset())
    ...
    @property
    def PresentationStateCompositorComponentSequence(self) -> Optional[List[PresentationStateCompositorComponentSequenceItem]]:
        if "PresentationStateCompositorComponentSequence" in self._dataset:
            if len(self._PresentationStateCompositorComponentSequence) == len(
                self._dataset.PresentationStateCompositorComponentSequence
            ):
                return self._PresentationStateCompositorComponentSequence
            else:
                return [
                    PresentationStateCompositorComponentSequenceItem(x)
                    for x in self._dataset.PresentationStateCompositorComponentSequence
                ]
        return None

    @PresentationStateCompositorComponentSequence.setter
    def PresentationStateCompositorComponentSequence(
        self, value: Optional[List[PresentationStateCompositorComponentSequenceItem]]
    ):
        if value is None:
            self._PresentationStateCompositorComponentSequence = []
            if "PresentationStateCompositorComponentSequence" in self._dataset:
                del self._dataset.PresentationStateCompositorComponentSequence
        elif not isinstance(value, list) or not all(
            ...
            self._dataset.PresentationStateCompositorComponentSequence.clear()
            self._dataset.PresentationStateCompositorComponentSequence.extend([item.to_dataset() for item in value])
    ...
    def add_PresentationStateCompositorComponent(self, item: PresentationStateCompositorComponentSequenceItem):
        ...
        if "PresentationStateCompositorComponentSequence" not in self._dataset:
            self._dataset.PresentationStateCompositorComponentSequence = pydicom.Sequence()
        self._dataset.PresentationStateCompositorComponentSequence.append(item.to_dataset())

    @property
    def VolumeStreamSequence(self) -> Optional[List[VolumeStreamSequenceItem]]:
        if "VolumeStreamSequence" in self._dataset:
            if len(self._VolumeStreamSequence) == len(self._dataset.VolumeStreamSequence):
                return self._VolumeStreamSequence
            else:
                return [VolumeStreamSequenceItem(x) for x in self._dataset.VolumeStreamSequence]
        return None

    @VolumeStreamSequence.setter
    def VolumeStreamSequence(self, value: Optional[List[VolumeStreamSequenceItem]]):
        if value is None:
            self._VolumeStreamSequence = []
            if "VolumeStreamSequence" in self._dataset:
            ...
            self._dataset.VolumeStreamSequence.clear()
            self._dataset.VolumeStreamSequence.extend([item.to_dataset() for item in value])
    ...
    def add_VolumeStream(self, item: VolumeStreamSequenceItem):
        ...
        if "VolumeStreamSequence" not in self._dataset:
            self._dataset.VolumeStreamSequence = pydicom.Sequence()
        self._dataset.VolumeStreamSequence.append(item.to_dataset())

    @property
    def ShadingStyle(self) -> Optional[str]:
        if "ShadingStyle" in self._dataset:
            return self._dataset.ShadingStyle
        return None

    @ShadingStyle.setter
    def ShadingStyle(self, value: Optional[str]):
        if value is None:
            if "ShadingStyle" in self._dataset:
                del self._dataset.ShadingStyle
        else:
            self._dataset.ShadingStyle = value

    @property
    def AmbientReflectionIntensity(self) -> Optional[float]:
        if "AmbientReflectionIntensity" in self._dataset:
            return self._dataset.AmbientReflectionIntensity
        return None

    @AmbientReflectionIntensity.setter
    def AmbientReflectionIntensity(self, value: Optional[float]):
        if value is None:
            if "AmbientReflectionIntensity" in self._dataset:
                del self._dataset.AmbientReflectionIntensity
        else:
            self._dataset.AmbientReflectionIntensity = value

    @property
    def LightDirection(self) -> Optional[List[float]]:
        if "LightDirection" in self._dataset:
            return self._dataset.LightDirection
        return None

    @LightDirection.setter
    def LightDirection(self, value: Optional[List[float]]):
        if value is None:
            if "LightDirection" in self._dataset:
                del self._dataset.LightDirection
        else:
            self._dataset.LightDirection = value

    @property
    def DiffuseReflectionIntensity(self) -> Optional[float]:
        if "DiffuseReflectionIntensity" in self._dataset:
            return self._dataset.DiffuseReflectionIntensity
        return None

    @DiffuseReflectionIntensity.setter
    def DiffuseReflectionIntensity(self, value: Optional[float]):
        if value is None:
            if "DiffuseReflectionIntensity" in self._dataset:
                del self._dataset.DiffuseReflectionIntensity
        else:
            self._dataset.DiffuseReflectionIntensity = value

    @property
    def SpecularReflectionIntensity(self) -> Optional[float]:
        if "SpecularReflectionIntensity" in self._dataset:
            return self._dataset.SpecularReflectionIntensity
        return None

    @SpecularReflectionIntensity.setter
    def SpecularReflectionIntensity(self, value: Optional[float]):
        if value is None:
            if "SpecularReflectionIntensity" in self._dataset:
                del self._dataset.SpecularReflectionIntensity
        else:
            self._dataset.SpecularReflectionIntensity = value

    @property
    def Shininess(self) -> Optional[float]:
        if "Shininess" in self._dataset:
            return self._dataset.Shininess
        return None

    @Shininess.setter
    def Shininess(self, value: Optional[float]):
        if value is None:
            if "Shininess" in self._dataset:
                del self._dataset.Shininess
        else:
            self._dataset.Shininess = value
    ...
    @property
    def SamplingStepSize(self) -> Optional[float]:
        if "SamplingStepSize" in self._dataset:
            return self._dataset.SamplingStepSize
        return None

    @SamplingStepSize.setter
    def SamplingStepSize(self, value: Optional[float]):
        if value is None:
            if "SamplingStepSize" in self._dataset:
                del self._dataset.SamplingStepSize
        else:
            self._dataset.SamplingStepSize = value
    ...
    @property
    def VolumetricPresentationStateInputSequence(self) -> Optional[List[VolumetricPresentationStateInputSequenceItem]]:
        if "VolumetricPresentationStateInputSequence" in self._dataset:
            if len(self._VolumetricPresentationStateInputSequence) == len(
                self._dataset.VolumetricPresentationStateInputSequence
            ):
                return self._VolumetricPresentationStateInputSequence
            else:
                return [
                    VolumetricPresentationStateInputSequenceItem(x)
                    for x in self._dataset.VolumetricPresentationStateInputSequence
                ]
        return None

    @VolumetricPresentationStateInputSequence.setter
    def VolumetricPresentationStateInputSequence(self, value: Optional[List[VolumetricPresentationStateInputSequenceItem]]):
        if value is None:
            self._VolumetricPresentationStateInputSequence = []
            if "VolumetricPresentationStateInputSequence" in self._dataset:
                del self._dataset.VolumetricPresentationStateInputSequence
        elif not isinstance(value, list) or not all(
            ...
            self._dataset.VolumetricPresentationStateInputSequence.clear()
            self._dataset.VolumetricPresentationStateInputSequence.extend([item.to_dataset() for item in value])
    ...
    def add_VolumetricPresentationStateInput(self, item: VolumetricPresentationStateInputSequenceItem):
        ...
        if "VolumetricPresentationStateInputSequence" not in self._dataset:
            self._dataset.VolumetricPresentationStateInputSequence = pydicom.Sequence()
        self._dataset.VolumetricPresentationStateInputSequence.append(item.to_dataset())

    @property
    def VolumetricPresentationInputSetSequence(self) -> Optional[List[VolumetricPresentationInputSetSequenceItem]]:
        if "VolumetricPresentationInputSetSequence" in self._dataset:
            if len(self._VolumetricPresentationInputSetSequence) == len(self._dataset.VolumetricPresentationInputSetSequence):
                return self._VolumetricPresentationInputSetSequence
            else:
                return [
                    VolumetricPresentationInputSetSequenceItem(x) for x in self._dataset.VolumetricPresentationInputSetSequence
                ]
        return None

    @VolumetricPresentationInputSetSequence.setter
    def VolumetricPresentationInputSetSequence(self, value: Optional[List[VolumetricPresentationInputSetSequenceItem]]):
        if value is None:
            self._VolumetricPresentationInputSetSequence = []
            if "VolumetricPresentationInputSetSequence" in self._dataset:
                del self._dataset.VolumetricPresentationInputSetSequence
        elif not isinstance(value, list) or not all(
            ...
            self._dataset.VolumetricPresentationInputSetSequence.clear()
            self._dataset.VolumetricPresentationInputSetSequence.extend([item.to_dataset() for item in value])
    ...
    def add_VolumetricPresentationInputSet(self, item: VolumetricPresentationInputSetSequenceItem):
        ...
        if "VolumetricPresentationInputSetSequence" not in self._dataset:
            self._dataset.VolumetricPresentationInputSetSequence = pydicom.Sequence()
        self._dataset.VolumetricPresentationInputSetSequence.append(item.to_dataset())

    @property
    def GlobalCrop(self) -> Optional[str]:
        if "GlobalCrop" in self._dataset:
            return self._dataset.GlobalCrop
        return None

    @GlobalCrop.setter
    def GlobalCrop(self, value: Optional[str]):
        if value is None:
            if "GlobalCrop" in self._dataset:
                del self._dataset.GlobalCrop
        else:
            self._dataset.GlobalCrop = value

    @property
    def GlobalCroppingSpecificationIndex(self) -> Optional[List[int]]:
        if "GlobalCroppingSpecificationIndex" in self._dataset:
            return self._dataset.GlobalCroppingSpecificationIndex
        return None

    @GlobalCroppingSpecificationIndex.setter
    def GlobalCroppingSpecificationIndex(self, value: Optional[List[int]]):
        if value is None:
            if "GlobalCroppingSpecificationIndex" in self._dataset:
                del self._dataset.GlobalCroppingSpecificationIndex
        else:
            self._dataset.GlobalCroppingSpecificationIndex = value
    ...
    @property
    def GraphicLayerSequence(self) -> Optional[List[GraphicLayerSequenceItem]]:
        if "GraphicLayerSequence" in self._dataset:
            if len(self._GraphicLayerSequence) == len(self._dataset.GraphicLayerSequence):
                return self._GraphicLayerSequence
            else:
                return [GraphicLayerSequenceItem(x) for x in self._dataset.GraphicLayerSequence]
        return None

    @GraphicLayerSequence.setter
    def GraphicLayerSequence(self, value: Optional[List[GraphicLayerSequenceItem]]):
        if value is None:
            ...
            self._dataset.GraphicLayerSequence.clear()
            self._dataset.GraphicLayerSequence.extend([item.to_dataset() for item in value])
    ...
    def add_GraphicLayer(self, item: GraphicLayerSequenceItem):
        ...
        if "GraphicLayerSequence" not in self._dataset:
            self._dataset.GraphicLayerSequence = pydicom.Sequence()
        self._dataset.GraphicLayerSequence.append(item.to_dataset())

    @property
    def VolumetricAnnotationSequence(self) -> Optional[List[VolumetricAnnotationSequenceItem]]:
        if "VolumetricAnnotationSequence" in self._dataset:
            if len(self._VolumetricAnnotationSequence) == len(self._dataset.VolumetricAnnotationSequence):
                return self._VolumetricAnnotationSequence
            else:
                return [VolumetricAnnotationSequenceItem(x) for x in self._dataset.VolumetricAnnotationSequence]
        return None

    @VolumetricAnnotationSequence.setter
    def VolumetricAnnotationSequence(self, value: Optional[List[VolumetricAnnotationSequenceItem]]):
        if value is None:
            self._VolumetricAnnotationSequence = []
            if "VolumetricAnnotationSequence" in self._dataset:
            ...
            self._dataset.VolumetricAnnotationSequence.clear()
            self._dataset.VolumetricAnnotationSequence.extend([item.to_dataset() for item in value])
    ...
    def add_VolumetricAnnotation(self, item: VolumetricAnnotationSequenceItem):
        ...
        if "VolumetricAnnotationSequence" not in self._dataset:
            self._dataset.VolumetricAnnotationSequence = pydicom.Sequence()
        self._dataset.VolumetricAnnotationSequence.append(item.to_dataset())

    @property
    def VolumetricPresentationInputAnnotationSequence(
        self,
    ) -> Optional[List[VolumetricPresentationInputAnnotationSequenceItem]]:
        if "VolumetricPresentationInputAnnotationSequence" in self._dataset:
            if len(self._VolumetricPresentationInputAnnotationSequence) == len(
                self._dataset.VolumetricPresentationInputAnnotationSequence
            ):
                return self._VolumetricPresentationInputAnnotationSequence
            else:
                return [
                    VolumetricPresentationInputAnnotationSequenceItem(x)
                    for x in self._dataset.VolumetricPresentationInputAnnotationSequence
                ]
        return None

    @VolumetricPresentationInputAnnotationSequence.setter
    def VolumetricPresentationInputAnnotationSequence(
        self, value: Optional[List[VolumetricPresentationInputAnnotationSequenceItem]]
    ):
        if value is None:
            self._VolumetricPresentationInputAnnotationSequence = []
            if "VolumetricPresentationInputAnnotationSequence" in self._dataset:
                del self._dataset.VolumetricPresentationInputAnnotationSequence
        elif not isinstance(value, list) or not all(
            ...
            self._dataset.VolumetricPresentationInputAnnotationSequence.clear()
            self._dataset.VolumetricPresentationInputAnnotationSequence.extend([item.to_dataset() for item in value])
    ...
    def add_VolumetricPresentationInputAnnotation(self, item: VolumetricPresentationInputAnnotationSequenceItem):
        ...
        if "VolumetricPresentationInputAnnotationSequence" not in self._dataset:
            self._dataset.VolumetricPresentationInputAnnotationSequence = pydicom.Sequence()
        self._dataset.VolumetricPresentationInputAnnotationSequence.append(item.to_dataset())
    ...
    @property
    def GraphicGroupSequence(self) -> Optional[List[GraphicGroupSequenceItem]]:
        if "GraphicGroupSequence" in self._dataset:
            if len(self._GraphicGroupSequence) == len(self._dataset.GraphicGroupSequence):
                return self._GraphicGroupSequence
            else:
                return [GraphicGroupSequenceItem(x) for x in self._dataset.GraphicGroupSequence]
        return None

    @GraphicGroupSequence.setter
    def GraphicGroupSequence(self, value: Optional[List[GraphicGroupSequenceItem]]):
        if value is None:
            self._GraphicGroupSequence = []
            if "GraphicGroupSequence" in self._dataset:
            ...
            self._dataset.GraphicGroupSequence.clear()
            self._dataset.GraphicGroupSequence.extend([item.to_dataset() for item in value])
    ...
    def add_GraphicGroup(self, item: GraphicGroupSequenceItem):
        ...
        if "GraphicGroupSequence" not in self._dataset:
            self._dataset.GraphicGroupSequence = pydicom.Sequence()
        self._dataset.GraphicGroupSequence.append(item.to_dataset())
