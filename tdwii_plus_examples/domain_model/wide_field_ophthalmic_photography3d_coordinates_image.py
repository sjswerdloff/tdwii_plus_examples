...
from .transformation_algorithm_sequence_item import TransformationAlgorithmSequenceItem
from .two_dimensional_to_three_dimensional_map_sequence_item import (
    TwoDimensionalToThreeDimensionalMapSequenceItem,
)
from .udi_sequence_item import UDISequenceItem
from .wide_field_ophthalmic_photography_quality_rating_sequence_item import (
...
class WideFieldOphthalmicPhotography3dCoordinatesImage:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._WideFieldOphthalmicPhotographyQualityRatingSequence: List[
            WideFieldOphthalmicPhotographyQualityRatingSequenceItem
        ] = []
        ...
        self._TransformationMethodCodeSequence: List[CodeSequenceItem] = []
    ...
    @property
    def WideFieldOphthalmicPhotographyQualityRatingSequence(
        self,
    ) -> Optional[List[WideFieldOphthalmicPhotographyQualityRatingSequenceItem]]:
        if "WideFieldOphthalmicPhotographyQualityRatingSequence" in self._dataset:
            if len(self._WideFieldOphthalmicPhotographyQualityRatingSequence) == len(
                self._dataset.WideFieldOphthalmicPhotographyQualityRatingSequence
            ):
                return self._WideFieldOphthalmicPhotographyQualityRatingSequence
            else:
                return [
                    WideFieldOphthalmicPhotographyQualityRatingSequenceItem(x)
                    for x in self._dataset.WideFieldOphthalmicPhotographyQualityRatingSequence
                ]
        return None

    @WideFieldOphthalmicPhotographyQualityRatingSequence.setter
    def WideFieldOphthalmicPhotographyQualityRatingSequence(
        self, value: Optional[List[WideFieldOphthalmicPhotographyQualityRatingSequenceItem]]
    ):
        if value is None:
            self._WideFieldOphthalmicPhotographyQualityRatingSequence = []
            if "WideFieldOphthalmicPhotographyQualityRatingSequence" in self._dataset:
                del self._dataset.WideFieldOphthalmicPhotographyQualityRatingSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, WideFieldOphthalmicPhotographyQualityRatingSequenceItem) for item in value
        ):
            raise ValueError(
                f"WideFieldOphthalmicPhotographyQualityRatingSequence must be a list of WideFieldOphthalmicPhotographyQualityRatingSequenceItem objects"
            )
        else:
            self._WideFieldOphthalmicPhotographyQualityRatingSequence = value
            if "WideFieldOphthalmicPhotographyQualityRatingSequence" not in self._dataset:
                self._dataset.WideFieldOphthalmicPhotographyQualityRatingSequence = pydicom.Sequence()
            self._dataset.WideFieldOphthalmicPhotographyQualityRatingSequence.clear()
            self._dataset.WideFieldOphthalmicPhotographyQualityRatingSequence.extend([item.to_dataset() for item in value])

    def add_WideFieldOphthalmicPhotographyQualityRating(self, item: WideFieldOphthalmicPhotographyQualityRatingSequenceItem):
        if not isinstance(item, WideFieldOphthalmicPhotographyQualityRatingSequenceItem):
            raise ValueError(f"Item must be an instance of WideFieldOphthalmicPhotographyQualityRatingSequenceItem")
        self._WideFieldOphthalmicPhotographyQualityRatingSequence.append(item)
        if "WideFieldOphthalmicPhotographyQualityRatingSequence" not in self._dataset:
            self._dataset.WideFieldOphthalmicPhotographyQualityRatingSequence = pydicom.Sequence()
        self._dataset.WideFieldOphthalmicPhotographyQualityRatingSequence.append(item.to_dataset())
    ...
    @property
    def TransformationMethodCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "TransformationMethodCodeSequence" in self._dataset:
            if len(self._TransformationMethodCodeSequence) == len(self._dataset.TransformationMethodCodeSequence):
                return self._TransformationMethodCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.TransformationMethodCodeSequence]
        return None

    @TransformationMethodCodeSequence.setter
    def TransformationMethodCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._TransformationMethodCodeSequence = []
            if "TransformationMethodCodeSequence" in self._dataset:
                del self._dataset.TransformationMethodCodeSequence
        elif not isinstance(value, list) or not all(isinstance(item, CodeSequenceItem) for item in value):
            raise ValueError(f"TransformationMethodCodeSequence must be a list of CodeSequenceItem objects")
        else:
            self._TransformationMethodCodeSequence = value
            if "TransformationMethodCodeSequence" not in self._dataset:
                self._dataset.TransformationMethodCodeSequence = pydicom.Sequence()
            self._dataset.TransformationMethodCodeSequence.clear()
            self._dataset.TransformationMethodCodeSequence.extend([item.to_dataset() for item in value])

    def add_TransformationMethodCode(self, item: CodeSequenceItem):
        if not isinstance(item, CodeSequenceItem):
            raise ValueError(f"Item must be an instance of CodeSequenceItem")
        self._TransformationMethodCodeSequence.append(item)
        if "TransformationMethodCodeSequence" not in self._dataset:
            self._dataset.TransformationMethodCodeSequence = pydicom.Sequence()
        self._dataset.TransformationMethodCodeSequence.append(item.to_dataset())

    @property
    def TransformationAlgorithmSequence(self) -> Optional[List[TransformationAlgorithmSequenceItem]]:
        if "TransformationAlgorithmSequence" in self._dataset:
            if len(self._TransformationAlgorithmSequence) == len(self._dataset.TransformationAlgorithmSequence):
                return self._TransformationAlgorithmSequence
            else:
                return [TransformationAlgorithmSequenceItem(x) for x in self._dataset.TransformationAlgorithmSequence]
        return None

    @TransformationAlgorithmSequence.setter
    def TransformationAlgorithmSequence(self, value: Optional[List[TransformationAlgorithmSequenceItem]]):
        if value is None:
            self._TransformationAlgorithmSequence = []
            if "TransformationAlgorithmSequence" in self._dataset:
                del self._dataset.TransformationAlgorithmSequence
        elif not isinstance(value, list) or not all(isinstance(item, TransformationAlgorithmSequenceItem) for item in value):
            raise ValueError(f"TransformationAlgorithmSequence must be a list of TransformationAlgorithmSequenceItem objects")
        else:
            self._TransformationAlgorithmSequence = value
            if "TransformationAlgorithmSequence" not in self._dataset:
                self._dataset.TransformationAlgorithmSequence = pydicom.Sequence()
            self._dataset.TransformationAlgorithmSequence.clear()
            self._dataset.TransformationAlgorithmSequence.extend([item.to_dataset() for item in value])

    def add_TransformationAlgorithm(self, item: TransformationAlgorithmSequenceItem):
        if not isinstance(item, TransformationAlgorithmSequenceItem):
            raise ValueError(f"Item must be an instance of TransformationAlgorithmSequenceItem")
        self._TransformationAlgorithmSequence.append(item)
        if "TransformationAlgorithmSequence" not in self._dataset:
            self._dataset.TransformationAlgorithmSequence = pydicom.Sequence()
        self._dataset.TransformationAlgorithmSequence.append(item.to_dataset())

    @property
    def OphthalmicAxialLengthMethod(self) -> Optional[str]:
        if "OphthalmicAxialLengthMethod" in self._dataset:
            return self._dataset.OphthalmicAxialLengthMethod
        return None

    @OphthalmicAxialLengthMethod.setter
    def OphthalmicAxialLengthMethod(self, value: Optional[str]):
        if value is None:
            if "OphthalmicAxialLengthMethod" in self._dataset:
                del self._dataset.OphthalmicAxialLengthMethod
        else:
            self._dataset.OphthalmicAxialLengthMethod = value

    @property
    def OphthalmicFOV(self) -> Optional[float]:
        if "OphthalmicFOV" in self._dataset:
            return self._dataset.OphthalmicFOV
        return None

    @OphthalmicFOV.setter
    def OphthalmicFOV(self, value: Optional[float]):
        if value is None:
            if "OphthalmicFOV" in self._dataset:
                del self._dataset.OphthalmicFOV
        else:
            self._dataset.OphthalmicFOV = value

    @property
    def TwoDimensionalToThreeDimensionalMapSequence(self) -> Optional[List[TwoDimensionalToThreeDimensionalMapSequenceItem]]:
        if "TwoDimensionalToThreeDimensionalMapSequence" in self._dataset:
            if len(self._TwoDimensionalToThreeDimensionalMapSequence) == len(
                self._dataset.TwoDimensionalToThreeDimensionalMapSequence
            ):
                return self._TwoDimensionalToThreeDimensionalMapSequence
            else:
                return [
                    TwoDimensionalToThreeDimensionalMapSequenceItem(x)
                    for x in self._dataset.TwoDimensionalToThreeDimensionalMapSequence
                ]
        return None

    @TwoDimensionalToThreeDimensionalMapSequence.setter
    def TwoDimensionalToThreeDimensionalMapSequence(
        self, value: Optional[List[TwoDimensionalToThreeDimensionalMapSequenceItem]]
    ):
        if value is None:
            self._TwoDimensionalToThreeDimensionalMapSequence = []
            if "TwoDimensionalToThreeDimensionalMapSequence" in self._dataset:
                del self._dataset.TwoDimensionalToThreeDimensionalMapSequence
        elif not isinstance(value, list) or not all(
            isinstance(item, TwoDimensionalToThreeDimensionalMapSequenceItem) for item in value
        ):
            raise ValueError(
                f"TwoDimensionalToThreeDimensionalMapSequence must be a list of TwoDimensionalToThreeDimensionalMapSequenceItem objects"
            )
        else:
            self._TwoDimensionalToThreeDimensionalMapSequence = value
            if "TwoDimensionalToThreeDimensionalMapSequence" not in self._dataset:
                self._dataset.TwoDimensionalToThreeDimensionalMapSequence = pydicom.Sequence()
            self._dataset.TwoDimensionalToThreeDimensionalMapSequence.clear()
            self._dataset.TwoDimensionalToThreeDimensionalMapSequence.extend([item.to_dataset() for item in value])

    def add_TwoDimensionalToThreeDimensionalMap(self, item: TwoDimensionalToThreeDimensionalMapSequenceItem):
        if not isinstance(item, TwoDimensionalToThreeDimensionalMapSequenceItem):
            raise ValueError(f"Item must be an instance of TwoDimensionalToThreeDimensionalMapSequenceItem")
        self._TwoDimensionalToThreeDimensionalMapSequence.append(item)
        if "TwoDimensionalToThreeDimensionalMapSequence" not in self._dataset:
            self._dataset.TwoDimensionalToThreeDimensionalMapSequence = pydicom.Sequence()
        self._dataset.TwoDimensionalToThreeDimensionalMapSequence.append(item.to_dataset())
    ...
    @property
    def OphthalmicAnatomicReferencePointXCoordinate(self) -> Optional[float]:
        if "OphthalmicAnatomicReferencePointXCoordinate" in self._dataset:
            return self._dataset.OphthalmicAnatomicReferencePointXCoordinate
        return None

    @OphthalmicAnatomicReferencePointXCoordinate.setter
    def OphthalmicAnatomicReferencePointXCoordinate(self, value: Optional[float]):
        if value is None:
            if "OphthalmicAnatomicReferencePointXCoordinate" in self._dataset:
                del self._dataset.OphthalmicAnatomicReferencePointXCoordinate
        else:
            self._dataset.OphthalmicAnatomicReferencePointXCoordinate = value

    @property
    def OphthalmicAnatomicReferencePointYCoordinate(self) -> Optional[float]:
        if "OphthalmicAnatomicReferencePointYCoordinate" in self._dataset:
            return self._dataset.OphthalmicAnatomicReferencePointYCoordinate
        return None

    @OphthalmicAnatomicReferencePointYCoordinate.setter
    def OphthalmicAnatomicReferencePointYCoordinate(self, value: Optional[float]):
        if value is None:
            if "OphthalmicAnatomicReferencePointYCoordinate" in self._dataset:
                del self._dataset.OphthalmicAnatomicReferencePointYCoordinate
        else:
            self._dataset.OphthalmicAnatomicReferencePointYCoordinate = value
    ...
    @property
    def AcquisitionDeviceTypeCodeSequence(self) -> Optional[List[CodeSequenceItem]]:
        if "AcquisitionDeviceTypeCodeSequence" in self._dataset:
            if len(self._AcquisitionDeviceTypeCodeSequence) == len(self._dataset.AcquisitionDeviceTypeCodeSequence):
                return self._AcquisitionDeviceTypeCodeSequence
            else:
                return [CodeSequenceItem(x) for x in self._dataset.AcquisitionDeviceTypeCodeSequence]
        return None

    @AcquisitionDeviceTypeCodeSequence.setter
    def AcquisitionDeviceTypeCodeSequence(self, value: Optional[List[CodeSequenceItem]]):
        if value is None:
            self._AcquisitionDeviceTypeCodeSequence = []
            if "AcquisitionDeviceTypeCodeSequence" in self._dataset:
            ...
            self._dataset.AcquisitionDeviceTypeCodeSequence.clear()
            self._dataset.AcquisitionDeviceTypeCodeSequence.extend([item.to_dataset() for item in value])
    ...
    def add_AcquisitionDeviceTypeCode(self, item: CodeSequenceItem):
        ...
        if "AcquisitionDeviceTypeCodeSequence" not in self._dataset:
            self._dataset.AcquisitionDeviceTypeCodeSequence = pydicom.Sequence()
        self._dataset.AcquisitionDeviceTypeCodeSequence.append(item.to_dataset())
    ...
    @property
    def CameraAngleOfView(self) -> Optional[float]:
        if "CameraAngleOfView" in self._dataset:
            return self._dataset.CameraAngleOfView
        return None

    @CameraAngleOfView.setter
    def CameraAngleOfView(self, value: Optional[float]):
        if value is None:
            if "CameraAngleOfView" in self._dataset:
                del self._dataset.CameraAngleOfView
        else:
            self._dataset.CameraAngleOfView = value
    ...
    @property
    def SamplesPerPixelUsed(self) -> Optional[int]:
        if "SamplesPerPixelUsed" in self._dataset:
            return self._dataset.SamplesPerPixelUsed
        return None

    @SamplesPerPixelUsed.setter
    def SamplesPerPixelUsed(self, value: Optional[int]):
        if value is None:
            if "SamplesPerPixelUsed" in self._dataset:
                del self._dataset.SamplesPerPixelUsed
        else:
            self._dataset.SamplesPerPixelUsed = value
