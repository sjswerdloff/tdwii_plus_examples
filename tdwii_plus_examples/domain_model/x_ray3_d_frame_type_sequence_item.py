...
class XRay3DFrameTypeSequenceItem:
    ...
    @property
    def ReconstructionIndex(self) -> Optional[int]:
        if "ReconstructionIndex" in self._dataset:
            return self._dataset.ReconstructionIndex
        return None

    @ReconstructionIndex.setter
    def ReconstructionIndex(self, value: Optional[int]):
        if value is None:
            if "ReconstructionIndex" in self._dataset:
                del self._dataset.ReconstructionIndex
        else:
            self._dataset.ReconstructionIndex = value
