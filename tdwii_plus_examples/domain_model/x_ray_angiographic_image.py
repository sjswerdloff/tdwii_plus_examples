...
class XRayAngiographicImage:
    ...
    @property
    def PositionerMotion(self) -> Optional[str]:
        if "PositionerMotion" in self._dataset:
            return self._dataset.PositionerMotion
        return None

    @PositionerMotion.setter
    def PositionerMotion(self, value: Optional[str]):
        if value is None:
            if "PositionerMotion" in self._dataset:
                del self._dataset.PositionerMotion
        else:
            self._dataset.PositionerMotion = value
    ...
    @property
    def PositionerPrimaryAngleIncrement(self) -> Optional[List[Decimal]]:
        if "PositionerPrimaryAngleIncrement" in self._dataset:
            return self._dataset.PositionerPrimaryAngleIncrement
        return None

    @PositionerPrimaryAngleIncrement.setter
    def PositionerPrimaryAngleIncrement(self, value: Optional[List[Decimal]]):
        if value is None:
            if "PositionerPrimaryAngleIncrement" in self._dataset:
                del self._dataset.PositionerPrimaryAngleIncrement
        else:
            self._dataset.PositionerPrimaryAngleIncrement = value

    @property
    def PositionerSecondaryAngleIncrement(self) -> Optional[List[Decimal]]:
        if "PositionerSecondaryAngleIncrement" in self._dataset:
            return self._dataset.PositionerSecondaryAngleIncrement
        return None

    @PositionerSecondaryAngleIncrement.setter
    def PositionerSecondaryAngleIncrement(self, value: Optional[List[Decimal]]):
        if value is None:
            if "PositionerSecondaryAngleIncrement" in self._dataset:
                del self._dataset.PositionerSecondaryAngleIncrement
        else:
            self._dataset.PositionerSecondaryAngleIncrement = value
    ...
    @property
    def TypeOfFilters(self) -> Optional[List[str]]:
        if "TypeOfFilters" in self._dataset:
            return self._dataset.TypeOfFilters
        return None

    @TypeOfFilters.setter
    def TypeOfFilters(self, value: Optional[List[str]]):
        if value is None:
            if "TypeOfFilters" in self._dataset:
                del self._dataset.TypeOfFilters
        else:
            self._dataset.TypeOfFilters = value

    @property
    def IntensifierSize(self) -> Optional[Decimal]:
        if "IntensifierSize" in self._dataset:
            return self._dataset.IntensifierSize
        return None

    @IntensifierSize.setter
    def IntensifierSize(self, value: Optional[Decimal]):
        if value is None:
            if "IntensifierSize" in self._dataset:
                del self._dataset.IntensifierSize
        else:
            self._dataset.IntensifierSize = value
    ...
    @property
    def TableVerticalIncrement(self) -> Optional[List[Decimal]]:
        if "TableVerticalIncrement" in self._dataset:
            return self._dataset.TableVerticalIncrement
        return None

    @TableVerticalIncrement.setter
    def TableVerticalIncrement(self, value: Optional[List[Decimal]]):
        if value is None:
            if "TableVerticalIncrement" in self._dataset:
                del self._dataset.TableVerticalIncrement
        else:
            self._dataset.TableVerticalIncrement = value

    @property
    def TableLateralIncrement(self) -> Optional[List[Decimal]]:
        if "TableLateralIncrement" in self._dataset:
            return self._dataset.TableLateralIncrement
        return None

    @TableLateralIncrement.setter
    def TableLateralIncrement(self, value: Optional[List[Decimal]]):
        if value is None:
            if "TableLateralIncrement" in self._dataset:
                del self._dataset.TableLateralIncrement
        else:
            self._dataset.TableLateralIncrement = value

    @property
    def TableLongitudinalIncrement(self) -> Optional[List[Decimal]]:
        if "TableLongitudinalIncrement" in self._dataset:
            return self._dataset.TableLongitudinalIncrement
        return None

    @TableLongitudinalIncrement.setter
    def TableLongitudinalIncrement(self, value: Optional[List[Decimal]]):
        if value is None:
            if "TableLongitudinalIncrement" in self._dataset:
                del self._dataset.TableLongitudinalIncrement
        else:
            self._dataset.TableLongitudinalIncrement = value
    ...
    @property
    def FrameDimensionPointer(self) -> Optional[List[int]]:
        if "FrameDimensionPointer" in self._dataset:
            return self._dataset.FrameDimensionPointer
        return None

    @FrameDimensionPointer.setter
    def FrameDimensionPointer(self, value: Optional[List[int]]):
        if value is None:
            if "FrameDimensionPointer" in self._dataset:
                del self._dataset.FrameDimensionPointer
        else:
            self._dataset.FrameDimensionPointer = value
    ...
    @property
    def RWavePointer(self) -> Optional[List[int]]:
        if "RWavePointer" in self._dataset:
            return self._dataset.RWavePointer
        return None

    @RWavePointer.setter
    def RWavePointer(self, value: Optional[List[int]]):
        if value is None:
            if "RWavePointer" in self._dataset:
                del self._dataset.RWavePointer
        else:
            self._dataset.RWavePointer = value
