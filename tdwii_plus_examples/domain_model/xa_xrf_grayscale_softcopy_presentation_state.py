...
from .multi_frame_presentation_sequence_item import MultiFramePresentationSequenceItem
...
class XaXrfGrayscaleSoftcopyPresentationState:
    def __init__(self, dataset: Optional[pydicom.Dataset] = None):
        ...
        self._MultiFramePresentationSequence: List[MultiFramePresentationSequenceItem] = []
    ...
    @property
    def MultiFramePresentationSequence(self) -> Optional[List[MultiFramePresentationSequenceItem]]:
        if "MultiFramePresentationSequence" in self._dataset:
            if len(self._MultiFramePresentationSequence) == len(self._dataset.MultiFramePresentationSequence):
                return self._MultiFramePresentationSequence
            else:
                return [MultiFramePresentationSequenceItem(x) for x in self._dataset.MultiFramePresentationSequence]
        return None

    @MultiFramePresentationSequence.setter
    def MultiFramePresentationSequence(self, value: Optional[List[MultiFramePresentationSequenceItem]]):
        if value is None:
            self._MultiFramePresentationSequence = []
            if "MultiFramePresentationSequence" in self._dataset:
                del self._dataset.MultiFramePresentationSequence
        elif not isinstance(value, list) or not all(isinstance(item, MultiFramePresentationSequenceItem) for item in value):
            raise ValueError(f"MultiFramePresentationSequence must be a list of MultiFramePresentationSequenceItem objects")
        else:
            self._MultiFramePresentationSequence = value
            if "MultiFramePresentationSequence" not in self._dataset:
                self._dataset.MultiFramePresentationSequence = pydicom.Sequence()
            self._dataset.MultiFramePresentationSequence.clear()
            self._dataset.MultiFramePresentationSequence.extend([item.to_dataset() for item in value])

    def add_MultiFramePresentation(self, item: MultiFramePresentationSequenceItem):
        if not isinstance(item, MultiFramePresentationSequenceItem):
            raise ValueError(f"Item must be an instance of MultiFramePresentationSequenceItem")
        self._MultiFramePresentationSequence.append(item)
        if "MultiFramePresentationSequence" not in self._dataset:
            self._dataset.MultiFramePresentationSequence = pydicom.Sequence()
        self._dataset.MultiFramePresentationSequence.append(item.to_dataset())
