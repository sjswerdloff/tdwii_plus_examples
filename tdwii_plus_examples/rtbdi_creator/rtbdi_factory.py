...
def create_rtbdi_from_rtion_plan(plan: Dataset,
                                 fraction_number:int=1,
                                 treatment_record_list:List[Dataset]=None,
                                 bdi_uid_prefix:str=None) -> Dataset:
    """_summary_
...
def  _delete_excess_plan_elements_from_ups(ups_ds:Dataset):
    excess_elements = [ "PositionReferenceIndicator",
                       "FrameOfReferenceUID",
                       "Modality"]
    for excess in excess_elements:
        if excess in ups_ds:
...
def create_ups_from_plan_and_bdi(plan:Dataset,
                                 bdi:Dataset,
                                 retrieve_ae_title:str,
                                 scheduled_datetime: datetime,
                                 treatment_records:List[Dataset]) -> Dataset:
    """Build up the UPS
    ...
    treatment_delivery_concept = _create_code_seq_item('121740',
                                                    'DCM',
                                                    'Treatment Delivery Type'
                                                    )
    treatment_param_item = _create_ups_content_item('TEXT',
                                                    'TREATMENT',
                                                    treatment_delivery_concept
                                                    )
    
    scheduled_processing_parameters_list.append(treatment_param_item)

    plan_label_concept = _create_code_seq_item('2018001',
                                               '99IHERO2018',
                                               'Plan Label')
    ...
    plan_label_item = _create_ups_content_item('TEXT',
                                               plan_label_value,
                                               plan_label_concept)
    ...
    current_fraction_item = _create_ups_content_item('NUMERIC',
                                                     int(bdi.BeamTaskSequence[0].CurrentFractionNumber),
                                                     current_fraction_concept
                                                     )
    
    scheduled_processing_parameters_list.append(current_fraction_item)

    fractions_planned_concept = _create_code_seq_item('2018003',
                                                      '99IHERO2018',
                                                      'Number of Fractions Planned'
                                                      )
    fractions_planned_item = _create_ups_content_item('NUMERIC',
                                                      int(plan.FractionGroupSequence[0].NumberOfFractionsPlanned),
                                                      fractions_planned_concept
                                                      )
...
if __name__ == "__main__":
    if len(argv) < 2 :
        print("Usage: python {argv[0]} plan_file <fraction number> <retrieve_ae_title> <ScheduleDateTime \"YYYYmmddHHMM\"> <treatment_record1 treatment_record2 ...>")
        print("Defaults to fraction_number=1, retrieve_ae_title=TDWII_MOVE_SCP,"
